...
def git_deep_cleaning():
  proc = subprocess.Popen("git clean -xdf", shell=True, cwd="/tmp/ambari")
  return proc.wait()
...
def install_ambari_server():
  proc = subprocess.Popen(
    "sudo yum install -y ambari-server-*.x86_64.rpm",
    shell=True,
...
def install_ambari_agent():
  proc = subprocess.Popen(
    "sudo yum install -y ambari-agent-*.x86_64.rpm",
    shell=True,
...
def setup_ambari_server():
  proc = subprocess.Popen("echo -e '\n\n\n\n' | sudo ambari-server setup", shell=True)
  return proc.wait()


def start_ambari_server(debug=False):
  proc = subprocess.Popen(
    "sudo ambari-server start" + (" --debug" if debug else ""), shell=True
  )
  return proc.wait()
...
def wait_until_ambari_agent_registered():
  ...
  while count < RETRY_MAX:
    count += 1
    proc = subprocess.Popen(
      "curl " + "http://localhost:8080/api/v1/hosts " + AMBARI_AUTH_HEADERS,
