...
import { find, set, cloneDeep, isEmpty, map } from "lodash";
...
const Register = ({ readOnly }: { readOnly: boolean }): JSX.Element => {
  ...
  function getColumns() {
    return [
      ...
      },
      {
        ...
        id: "versions",
        cell: (info: any) => {
          const allVersions = isEmpty(savedRepositoryVersionDetails)
            ? info.row.original.versions
            : map(info.row.original.versions, "version");
  ...
  async function saveVersion() {
    ...
    if (!readOnly)
      try {
        const versionInfoPayload = selectedStack.fileContent
          ? selectedStack.fileContent
          : selectedStack.version_url
          ? { VersionDefinition: { version_url: selectedStack.version_url } }
          : {
              VersionDefinition: {
        ...
        const headers = selectedStack.fileContent
    ...
    payload = {
      ...
      operating_systems: addedOs?.map((os: TransformedOperatingSystem) => {
        return {
          OperatingSystems: {
            os_type: os.os,
            ambari_managed_repositories: !redhatSatellite,
  ...
  const readVersionCallback = async (
    ...
    try {
      ...
      setSavedRepositoryVersionDetails(
        versionResources?.resources?.[0]?.VersionDefinition
      );
      ...
      setOperatingSystems(operatingSystemsCopy);
      if (belongingStack) {
        belongingStack.VersionDefinition.fileContent = fileContent;
        belongingStack.VersionDefinition.version_url = version_file_url;
      }
        ...
        >
          ...
          >
            <Table
              ...
              data={
                (isEmpty(savedRepositoryVersionDetails)
                  ...
                  : savedRepositoryVersionDetails?.services) || []
              }
            />
          </div>
        </Col>
