 ...
 */
export default function bytesToSize (bytes: number | undefined, precision = 0, parseType = 'parseInt', multiplyBy = 1) {
    if (bytes === undefined) {
      return 'n/a';
    } else {
  ...
  export function getCardinalityValue(cardinality:string,isMax:boolean) {
    if (cardinality) {
      ...
      if (isOptional) {
        ...
        if (isMax) return /^\d+\+/.test(cardinality as string) || cardinality == 'ALL' ? Infinity : parseInt(cardinality);
