...
export const EnableHighAvailibilityProvider: React.FC<{
  ...
  const persistence = useClusterWorkflowPersistence(RANGER_ADMIN_HA_PERSIST_KEY, {
  ...
  async function syncUserPersistedData(generation: number) {
    ...
    try {
      ...
      const persistedData = (values?.[RANGER_ADMIN_HA_PERSIST_KEY] || initialState) as State;
      ...
      if (activeStepName) {
        ...
        const activeStepNumber = Object.keys(
          ...
          (stepNumber) =>
            currentWizardUtilities.wizardSteps[stepNumber]?.name === activeStepName,
        );
  ...
  async function flushCurrentData(
    ...
    }, activeStep || ClusterProgressStatus.ENABLING_RANGER_ADMIN_HA);
