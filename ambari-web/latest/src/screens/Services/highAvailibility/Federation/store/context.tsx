...
export const EnableNamenodeFederationProvider: React.FC<{
  ...
  }, [state.enableNamenodeFederationSteps, currStepData]);
  ...
  async function syncUserPersistedData() {
    try {
      ...
      if (!isEmpty(get(persistedData, "enableNamenodeFederationSteps", {}))) {
        dispatch({
  ...
  async function flushCurrentData() {
    await ClusterApi.postPersistData(
      JSON.stringify({
        NAMENODE_FEDERATION: JSON.stringify({
          ...state,
  ...
  async function flushOnCancel() {
    await ClusterApi.postPersistData(
      JSON.stringify({
        NAMENODE_FEDERATION: JSON.stringify(initialState),
    ...
    if (nextStep >= 0) {
      ...
      if (nextStepDetails?.keysToRemove) {
        nextStepDetails.keysToRemove.forEach((key: string) => {
          if (state?.enableNamenodeFederationSteps?.[key]) {
            dispatch({
