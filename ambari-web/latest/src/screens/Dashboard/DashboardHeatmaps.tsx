...
function DashboardHeatmaps() {
  const { clusterName } = useContext(AppContext);
  const [heatmapCategories, setHeatmapCategories] = useState<HeatmapCategory[]>([]);
  const [_allHeatmaps, setAllHeatmaps] = useState<WidgetInfo[]>([]);
  const [metricsLoading, setMetricsLoading] = useState<boolean>(false);
  const [selectedCategory, setSelectedCategory] = useState<HeatmapCategory | null>(null);
  const [selectedMetric, setSelectedMetric] = useState<WidgetInfo | null>(null);
  ...
  const categorizeByServiceName = useCallback((allHeatmaps: WidgetInfo[]): HeatmapCategory[] => {
    const categories: HeatmapCategory[] = [];

    allHeatmaps.forEach((heatmap) => {
      const metrics = heatmap.metrics;
      const serviceNamesSet = new Set(metrics.map((metric: any) => metric.service_name));
      const serviceNames = Array.from(serviceNamesSet);
      
      serviceNames.forEach((serviceName) => {
        const serviceNameStr = serviceName as string;
        let category = categories.find(cat => cat.serviceName === serviceNameStr);
        if (!category) {
          category = {
            serviceName: serviceNameStr,
            displayName: serviceNameStr === 'STACK' ? 'Host' : startCase(serviceNameStr.toLowerCase()),
            heatmaps: []
          };
          categories.push(category);
        }
        if (category) {
          category.heatmaps.push(heatmap);
        }
      });
    });
      ...
      if (racksLoaded) {
        ...
        const processedHeatmaps = map(data?.items, (item: any) => {
          ...
          if (widgetInfo.properties) {
            widgetInfo.properties = JSON.parse(widgetInfo.properties);
          }
          if (widgetInfo.values) {
            widgetInfo.values = JSON.parse(widgetInfo.values);
          }
          return widgetInfo;
        });
        ...
        setAllHeatmaps(processedHeatmaps);
        const categories = categorizeByServiceName(processedHeatmaps);
        setHeatmapCategories(categories);
        
        // Set default selections
        if (categories.length > 0) {
          setSelectedCategory(categories[0]);
          if (categories[0].heatmaps.length > 0) {
            setSelectedMetric(categories[0].heatmaps[0]);
          }
        }
  ...
  }, [clusterName, loadRacks, categorizeByServiceName]);
  ...
  const handleCategoryChange = useCallback((category: HeatmapCategory) => {
    setSelectedCategory(category);
    if (category.heatmaps.length > 0) {
      setSelectedMetric(category.heatmaps[0]);
    }
  }, []);

  const handleMetricChange = useCallback((metric: WidgetInfo) => {
                ...
                <CardBody>
                  ...
                  <div className="mb-3">
                    ...
                    <Dropdown>
                      ...
                      >
                        {selectedCategory ? selectedCategory.displayName : "SELECT CATEGORY"}
                      </Dropdown.Toggle>
                  ...
                  <div className="mb-3">
                    ...
                    <Dropdown>
                      ...
                      <Dropdown.Menu className="w-100">
                        {selectedCategory?.heatmaps.map((metric) => (
                          <Dropdown.Item
                            key={metric.id}
              ...
              <div className="text-center py-5">
                <div className="text-muted">
                  {selectedMetric
                    ...
                    : "Please select a category and metric to view the heatmap"}
                </div>
              </div>
            )}
          </Col>
...
export default DashboardHeatmaps;
