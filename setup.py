...
def create_package_dir_map():
    ...
    for ambari_server_package in ambari_server_packages:
        package_dirs[ambari_server_package] = (
            AMBARI_SERVER_TEST_PYTHON_FOLDER
            + "/"
    ...
    package_dirs["urlinfo_processor"] = (
        AMBARI_COMMON_PYTHON_FOLDER + "/urlinfo_processor"
    )
...
def get_version():
    ...
    if "AMBARI_VERSION" in os.environ:
        ...
        return (
            parent_version_tag.text if parent_version_tag is not None else __version__
        )
