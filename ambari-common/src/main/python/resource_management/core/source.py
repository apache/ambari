...
class DownloadSource(Source):
    ...
    def get_content(self):
        if self.download_path and not os.path.exists(self.download_path):
            raise Fail(
                "Directory {0} doesn't exist, please provide valid download path".format(
                    self.download_path
                )
            )
        ...
        if not self.cache or not os.path.exists(filepath):
            ...
            try:
                ...
                raise Fail(
                    "Failed to download file from {0} due to HTTP error: {1}".format(
                        self.url, str(ex)
                    )
                )
            ...
            Logger.info(
                "Not downloading the file from {0}, because {1} already exists".format(
                    self.url, filepath
                )
            )
