...
def log_function_call(function):
    def inner(command, **kwargs):
        ...
        if quiet == False or (quiet == None and not is_internal_call):
            ...
            log_msg = Logger.get_function_repr(
                "{0}[{1}]".format(function.__name__, command_repr), kwargs
            )
            Logger.info(log_msg)
    ...
    for i in range(0, tries):
        ...
        try:
            try:
                ...
                if on_timeout:
                    Logger.info(
                        "Executing '{0}'. Reason: {1}".format(on_timeout, str(ex))
                    )
            ...
            if is_last_try:  # last try
                ...
                Logger.info(
                    "Retrying after {0} seconds. Reason: {1}".format(try_sleep, str(ex))
                )
...
def _call(
    ...
    if timeout:
        ...
        # timeout occurred
            ...
            err_msg = (
                "Execution of '{0}' was killed due timeout after {1} seconds".format(
                    command, timeout
                )
            )
...
def as_sudo(command, env=None, auto_escape=True):
    ...
    if isinstance(command, (list, tuple)):
        ...
        err_msg = Logger.filter_text(
            "String command '{0}' cannot be run as sudo. Please supply the command as a tuple of arguments".format(
                command
            )
        )
        raise Fail(err_msg)
    ...
    env = (
        _get_environment_str(_add_current_path_to_env(env)) if env else ENV_PLACEHOLDER
    )
...
def as_user(command, user, env=None, auto_escape=True):
    ...
    export_env = (
        "export {0} ; ".format(_get_environment_str(_add_current_path_to_env(env)))
        if env
        else EXPORT_PLACEHOLDER
    )
    return "{0} su {1} -l -s /bin/bash -c {2}".format(
        _get_sudo_binary(), user, quote_bash_args(export_env + command)
    )
...
def _get_environment_str(env):
    return reduce(
        lambda str, x: "{0} {1}={2}".format(str, x, quote_bash_args(env[x])), env, ""
    )
