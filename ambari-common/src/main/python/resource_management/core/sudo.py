...
if os.geteuid() == 0:
    ...
    def chmod_recursive(path, recursive_mode_flags, recursion_follow_links=False):
        ...
        files_attrib = (
            recursive_mode_flags["f"] if "d" in recursive_mode_flags else None
        )
            ...
            if dir_attrib is not None:
                for dir_name in dirs:
                    ...
                    chmod(
                        full_dir_path,
                        attr_to_bitmask(
                            dir_attrib, initial_bitmask=os.stat(full_dir_path).st_mode
                        ),
                    )
        ...
        try:
            ...
            if ex.errno == errno.ENOENT:
                ...
                if os.path.islink(dirname) and not os.path.exists(dirname):
                    raise Fail(
                        "Cannot create directory '{0}' as '{1}' is a broken symlink".format(
                            path, dirname
                        )
                    )
                ...
                if os.path.isfile(dirname):
                    raise Fail(
                        "Cannot create directory '{0}' as '{1}' is a file".format(
                            path, dirname
                        )
                    )
                ...
                if os.path.islink(dirname) and not os.path.exists(dirname):
                    raise Fail(
                        "Cannot create directory '{0}' as '{1}' is a looped symlink".format(
                            path, dirname
                        )
                    )
    ...
    def chown_recursive(path, owner, group, follow_links=False):
        ...
        if owner or group:
            ...
            if follow_links:
                flags.append("-L")
            shell.checked_call(
                ["chown"] + flags + [owner + ":" + group, path], sudo=True
            )
        ...
        class Stat:
            def __init__(self, path):
                ...
                code, out, err = shell.checked_call(
                    cmd, sudo=True, stderr=subprocess.PIPE
                )
                values = out.split(" ")
                if len(values) != 3:
                    raise Fail(
                        "Execution of '{0}' returned unexpected output. {2}\n{3}".format(
                            cmd, code, err, out
                        )
                    )
    ...
    def listdir(path):
        ...
        code, out, err = shell.checked_call(
            ["ls", path], sudo=True, stderr=subprocess.PIPE
        )
    ...
    def chmod_recursive(path, recursive_mode_flags, recursion_follow_links):
        ...
        for key, flags in recursive_mode_flags.items():
            shell.checked_call(
                ["find"]
                + find_flags
                + [path, "-type", key, "-exec", "chmod", flags, "{}", ";"]
            )
