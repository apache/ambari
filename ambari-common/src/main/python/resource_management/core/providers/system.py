        ...
        if not sudo.path_exists(path):
            ...
            if self.resource.follow:
                ...
                if path != self.resource.path:
                    Logger.info(
                        "Following the link {0} to {1} to create the directory".format(
...
class LinkProvider(Provider):
    def action_create(self):
        ...
        if self.resource.hard:
            ...
            if not sudo.path_exists(self.resource.to):
                Logger.info(
                    "Warning: linking to nonexistent location %s" % self.resource.to
                )

            Logger.info(
                "Creating symbolic %s to %s" % (self.resource, self.resource.to)
            )
