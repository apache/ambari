...
def QueryPrivilegeState(hToken, priv):
    ...
    for id, attr in privList:
        if id == privId:
            privState = attr
    Logger.debug(
        "Privilege state: {0}={1} ({2}) Enabled={3}".format(
            privId, priv, LookupPrivilegeDisplayName(None, priv), privState
        )
    )
    return privState
...
def _call_command(
    ...
    if user:
        ...
        Logger.debug(
            "Redirecting stdout to '{0}', stderr to '{1}'".format(
...
def _set_file_acl(file, user, rights):
    ...
    if code != 0:
        raise Fail(
            "Can not set rights {0} for path {1} and user {2}".format(file, user)
        )
    else:
        return
...
class FileProvider(Provider):
    def action_create(self):
        ...
        if os.path.isdir(path):
            raise Fail(
                "Applying %s failed, directory with name %s exists"
                % (self.resource, path)
            )
        ...
        for i in range(0, self.resource.tries):
            try:
                ...
                if i == self.resource.tries - 1:  # last try
                    ...
                    Logger.info(
                        "Retrying after %d seconds. Reason: %s"
...
class DirectoryProvider(Provider):
    def action_create(self):
        ...
        if not os.path.isdir(path):
            raise Fail(
                "Applying %s failed, file %s already exists" % (self.resource, path)
            )
    ...
    def action_delete(self):
        ...
        if os.path.exists(path):
            if not os.path.isdir(path):
                raise Fail(
                    "Applying %s failed, %s is not a directory" % (self.resource, path)
                )
