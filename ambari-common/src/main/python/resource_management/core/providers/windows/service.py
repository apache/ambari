...
def safe_open_scmanager():
    try:
        ...
        raise Fail(
            "Error opening Service Control Manager on the local machine: {0}".format(
                details.winerror
            )
        )
...
def safe_open_service(hSCM, service_name):
    try:
        ...
        if details.winerror == winerror.ERROR_SERVICE_DOES_NOT_EXIST:
            ...
            err_msg = "Error configuring service {0}: {1}".format(
                service_name, details.winerror
            )
        raise Fail(err_msg)
...
class ServiceProvider(Provider):
    ...
    def enable(self):
        ...
        try:
            ...
            if (
                win32service.QueryServiceConfig(hSvc)[1]
            ...
            raise Fail(
                "Error enabling service {0}: {1}".format(
...
class ServiceConfigProvider(Provider):
    ...
    def action_install(self):
        ...
        try:
            ...
            raise Fail(
                "Error creating service {0}: {1}".format(
                    self.resource.service_name, details.winerror
                )
            )
        finally:
    ...
    def action_configure(self):
        ...
        try:
            ...
            try:
                ...
                raise Fail(
                    "Error configuring service {0}: {1}".format(
                        self.resource.service_name, details.winerror
                    )
                )
            finally:
    ...
    def action_change_user(self):
        ...
        try:
            ...
            try:
                ...
                raise Fail(
                    "Error changing user for service {0}: {1}".format(
                        self.resource.service_name, details.winerror
                    )
                )
            finally:
        ...
        try:
            try:
                ...
                if details.winerror == winerror.ERROR_SERVICE_DOES_NOT_EXIST:
                    ...
                    raise Fail(
                        "Error removing service {0}: {1}".format(
                            self.resource.service_name, details.winerror
                        )
                    )
    ...
    def _fix_start_type(self):
        if self.resource.start_type in ServiceConfigProvider.str_start_types.keys():
            ...
            Logger.warning(
                "Invalid service start type specified: service='{0}', start type='{1}'. Ignoring.".format(
