...
def terminate_process(proc, terminate_strategy):
    if terminate_strategy == TerminateStrategy.TERMINATE_PARENT:
        ...
        raise Fail(
            "Invalid timeout_kill_strategy = '{0}'. Use TerminateStrategy class constants as a value.".format(
                terminate_strategy
            )
        )
    ...
    if proc.poll() == None:
        try:
            ...
            for i in range(10 * timeout):
                ...
                Logger.info(
                    "Cannot gracefully kill process group {0}. Resorting to SIGKILL.".format(
                        pgid
                    )
                )
...
def kill_process_tree(proc):
    ...
    if proc.poll() == None:
        shell.checked_call(
            ["bash", kill_tree_script, str(proc.pid), str(signal.SIGKILL)]
        )
