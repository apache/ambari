...
class Script(object):
  ...
  """
            ...
            if os.path.getsize(self.stroutfile) > 0:
                with open(self.stroutfile, "r") as fp:
                    try:
                        ...
                        errMsg = (
                            "Unable to read structured output from " + self.stroutfile
                        )
    ...
    def put_structured_out(self, sout):
        ...
        try:
            ...
            Script.structuredOut.update(
                {"errMsg": "Unable to write to " + self.stroutfile}
            )
    ...
    def save_component_version_to_structured_out(self, command_name):
        ...
        is_install_command = (
            command_name is not None and command_name.lower() == "install"
        )
        ...
        if (
            ...
            and repository_version is not None
        ):
            Logger.info(
                "The repository with version {0} for this command has been marked as resolved."
                " It will be used to report the version of the component which was installed".format(
                    repository_version
                )
            )
        ...
        if stack_select_package_name and stack_name:
            ...
            if component_version is None:
                component_version = (
                    version_select_util.get_component_version_from_symlink(
            ...
            if component_version is None:
                ...
                if len(versions) == 1:
                    ...
                    Logger.error(
                        "The '{0}' component did not advertise a version. This may indicate a problem with the component packaging. "
                        "However, the stack-select tool was able to report a single version installed ({1}). "
                        "This is the version that will be reported.".format(
                            stack_select_package_name, component_version
                        )
                    )
            ...
            if component_version:
                ...
                if not self.is_hook():
                    Logger.error(
                        "The '{0}' component did not advertise a version. This may indicate a problem with the component packaging.".format(
    ...
    def should_expose_component_version(self, command_name):
        ...
        stack_version_unformatted = str(
            default("/clusterLevelParams/stack_version", "")
        )
    ...
    def execute(self):
        ...
        try:
            with open(self.command_data_file) as f:
                ...
                Script.cluster_settings = (
                    Script.execution_command.get_cluster_settings()
                )
                ...
                for k, v in _PASSWORD_MAP.items():
                    if get_path_from_configuration(
                        ...
                        Script.passwords[
                            get_path_from_configuration(k, Script.config)
    ...
    def post_start(self, env=None):
        ...
        for pid_file in pid_files:
            if not sudo.path_exists(pid_file):
                raise Fail(
                    "Pid file {0} doesn't exist after starting of the component.".format(
                        pid_file
                    )
                )
        ...
        Logger.info(
            "Component has started with pid(s): {0}".format(
                ", ".join([x.decode("utf-8") for x in pids])
            )
        )
    ...
    def choose_method_to_execute(self, command_name):
        ...
        if not command_name in self_methods:
            raise Fail(
                "Script '{0}' has no method '{1}'".format(sys.argv[0], command_name)
            )
    ...
    def get_package_from_available(self, name, available_packages_in_repos=None):
        ...
        package_regex = (
            name.replace(
                STACK_VERSION_PLACEHOLDER, "(\d|{0})+".format(package_delimiter)
            )
            + "$"
        )
        ...
        if repo:
            ...
            version_str = command_repo.version_string.replace(
                ".", package_delimiter
            ).replace("-", package_delimiter)
        ...
        if name_with_version:
            raise Fail(
                "No package found for {0}(expected name: {1})".format(
                    name, name_with_version
                )
            )
        else:
            raise Fail(
                "Cannot match package for regexp name {0}. Available packages: {1}".format(
                    name, self.available_packages_in_repos
                )
            )
    ...
    """
        ...
        if (
            ...
            Logger.info(
                "Version {0} was provided as effective cluster version.  Using package version {1}".format(
                    effective_version, package_version
                )
            )
        ...
        if not package_version or "*" in package_version:
            ...
            stack_version_package_formatted = (
                repo_version.replace(".", package_delimiter).replace(
                    "-", package_delimiter
                )
    ...
    def get_stack_root():
        ...
        if stack_name not in stack_root:
            Logger.warning(
                "Cannot determine stack root for stack named {0}".format(stack_name)
            )
            return "/usr/{0}".format(stack_name.lower())
    ...
    def load_available_packages(self):
        ...
        Logger.info(
            "Applicable repositories: {0}".format(", ".join(applicable_repo_ids))
        )
    ...
    def should_install_package(self, package):
        ...
        try:
            ...
            name = package["name"]
            raise Fail(
                "Condition with name '{0}', when installing package {1}. Please check package_conditions.py.".format(
                    condition, name
                )
            )
    ...
    def restart(self, env):
        ...
        if config is not None:
            command_params = (
                config["commandParams"] if "commandParams" in config else None
            )
            if command_params is not None:
                upgrade_type_command_param = (
                    command_params["upgrade_type"]
                    if "upgrade_type" in command_params
                    else ""
                )
                direction = (
        ...
        if componentCategory and componentCategory.strip().lower() == "CLIENT".lower():
            ...
            if "upgrade_type" in inspect.getargspec(self.stop).args:
                ...
                if is_stack_upgrade:
                    self.stop(
                        env, rolling_restart=(upgrade_type == UPGRADE_TYPE_ROLLING)
                    )
                else:
                    self.stop(env)
            ...
            try:
                ...
                if service_name in services_to_skip:
                    Logger.info(
                        "Temporarily skipping status check for {0} service only.".format(
                            service_name
                        )
                    )
                elif is_stack_upgrade:
                    Logger.info(
                        "Skipping status check for {0} service during upgrade".format(
                            service_name
                        )
                    )
                else:
    ...
    def get_instance():
        if Script.instance is None:
            ...
            use_proxy = default(
                "/agentLevelParams/agentConfigParams/agent/use_system_proxy_settings",
                True,
            )
            if not use_proxy:
