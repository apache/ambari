...
class HdfsResourceJar:
  ...
  def action_delayed(self, action_name, main_resource):
    ...
    if (
      main_resource.resource.nameservices is None and main_resource.has_core_configs
    ):  # all nameservices
...
class HdfsResourceWebHDFS:
  ...
  def _assert_download_valid(self):
    ...
    if os.path.exists(target):
      if type == "directory" and os.path.isfile(target):
        raise Fail(
          format("Trying to download directory but file exists locally {target}")
        )
      elif type == "file" and os.path.isdir(target):
        raise Fail(
          format("Trying to download file but directory exists locally {target}")
        )
  ...
  def _download_file(self, target, source, file_status):
    ...
    if file_status and os.path.exists(target):
      ...
      if local_file_size == length:
        Logger.info(
          format("DFS file {source} is identical to {target}, skipping the download")
        )
        return
  ...
  def _create_file(self, target, source=None, mode=""):
    ...
    if file_status:
      if source:
        ...
        if local_file_size == length:
          Logger.info(
            format("DFS file {target} is identical to {source}, skipping the copying")
          )
          return
        elif not self.main_resource.resource.replace_existing_files:
          Logger.info(
  ...
  def _delete_resource(self):
    if not self.target_status:
      return
    self.util.run_command(
      self.main_resource.resource.target, "DELETE", method="DELETE", recursive=True
    )

  def _set_owner(self, file_status=None):
    owner = (
      "" if not self.main_resource.resource.owner else self.main_resource.resource.owner
    )
    group = (
      "" if not self.main_resource.resource.group else self.main_resource.resource.group
    )
...
class HdfsResourceProvider(Provider):
  ...
  def action_delayed(self, action_name):
    ...
    if self.has_core_configs:
      ...
      self.create_as_root = (
        path_protocol == "file" or self.default_protocol == "file" and not path_protocol
      )
      ...
      if path_protocol and path_protocol != self.default_protocol:
        ...
        Logger.info(
          f"Cannot use webhdfs for {self.resource.target} defaultFs = {self.resource.default_fs} has different protocol"
        )
    else:
      self.can_use_webhdfs = False
    ...
    parsed_not_managed_paths = [
      HdfsResourceProvider.parse_path(path) for path in self.resource.immutable_paths
    ]
    self.manage_if_exists = not parsed_path in parsed_not_managed_paths

    if parsed_path in self.ignored_resources_list:
      Logger.info(
        f"Skipping '{self.resource}' because it is in ignore file {self.resource.hdfs_resource_ignore_file}."
      )
      return
