...
class HdfsResourceJar:
    ...
    def action_delayed(self, action_name, main_resource):
        ...
        if (
            main_resource.resource.nameservices is None
            and main_resource.has_core_configs
        ):  # all nameservices
            ...
            for nameservice in nameservices:
                try:
                    ...
                    if len(nameservices) > 1:
                        Logger.exception(
                            "Cannot run HdfsResource for nameservice {0}. Due to no active namenode present".format(
                                nameservice
                            )
                        )
                    else:
    ...
    def action_delayed_for_nameservice(self, nameservice, action_name, main_resource):
        ...
        env_dict_key = (
            "hdfs_files_sudo" if main_resource.create_as_root else "hdfs_files"
        )

        if main_resource.create_as_root:
            Logger.info(
                "Will create {0} as root user".format(main_resource.resource.target)
            )
...
class WebHDFSCallException(Fail):
    ...
    def get_exception_name(self):
        if (
            isinstance(self.result_message, dict)
            and "RemoteException" in self.result_message
            and "exception" in self.result_message["RemoteException"]
        ):
    ...
    def get_exception_text(self):
        if (
            isinstance(self.result_message, dict)
            and "RemoteException" in self.result_message
            and "message" in self.result_message["RemoteException"]
        ):
...
class WebHDFSUtil:
    def __init__(
        ...
        address = namenode_ha_utils.get_property_for_active_namenode(
            hdfs_site, nameservice, address_property, security_enabled, run_user
        )
    ...
    def run_command(self, *args, **kwargs):
        ...
        try:
            ...
            if exception_name in EXCEPTIONS_TO_RETRY:
                required_text, try_count, try_sleep = EXCEPTIONS_TO_RETRY[
                    exception_name
                ]

                if not required_text or (
                    exception_text and required_text in exception_text
                ):
                    last_exception = ex
                else:
                    raise
        ...
        while True:
            Logger.info(
                "Retrying after {0} seconds. Reason: {1}".format(
                    try_sleep, str(last_exception)
                )
            )
            try_count -= 1
    ...
    def _run_command(
        ...
        **kwargs,
        ...
        url = format(
            "{address}/webhdfs/v1{target}?op={operation}", address=self.address
        )
        request_args = kwargs
        ...
        if (
            status_code not in WebHDFSUtil.valid_status_codes + ignore_status_codes
            or assertable_result
            and result_dict
            and not result_dict["boolean"]
        ):
            formatted_output = (
  ...
  """
    ...
    def action_delayed_for_nameservice(self, nameservice, action_name, main_resource):
        ...
        self.mode = (
            ...
            if main_resource.resource.mode
            else main_resource.resource.mode
        )
        self.mode_set = False
        ...
        if self.main_resource.manage_if_exists == False and self.target_status:
            Logger.info(
                "Skipping the operation for not managed DFS directory "
                + str(self.main_resource.resource.target)
    ...
    def _download_resource(self):
        ...
        if self.main_resource.resource.type == "file":
            ...
            self._download_directory(
                self.main_resource.resource.target, self.main_resource.resource.source
            )
    ...
    def _create_local_directory(self, target):
        if not os.path.exists(target):
            ...
            owner_name = (
                ""
                if not self.main_resource.resource.owner
                else self.main_resource.resource.owner
            )
            group_name = (
                ""
                if not self.main_resource.resource.group
                else self.main_resource.resource.group
            )
    ...
    def _download_file(self, target, source, file_status):
        ...
        if file_status and os.path.exists(target):
            ...
            if local_file_size == length:
                Logger.info(
                    format(
                        "DFS file {source} is identical to {target}, skipping the download"
                    )
                )
                return
                ...
                Logger.info(
                    format(
                        "Not replacing existing local file {target} which is different from DFS file {source}, due to replace_existing_files=False"
                    )
                )
                return

        kwargs = {}
        self.util.run_command(
            source,
            "OPEN",
            method="GET",
            overwrite=True,
    ...
    def _get_file_status(self, target):
        list_status = self.util.run_command(
            target,
            "GETFILESTATUS",
            method="GET",
    ...
    def _create_file(self, target, source=None, mode=""):
        ...
        """
        file_status = (
            self._get_file_status(target)
            if target != self.main_resource.resource.target
            else self.target_status
        )
        ...
        if file_status:
            if source:
                ...
                if local_file_size == length:
                    Logger.info(
                        format(
                            "DFS file {target} is identical to {source}, skipping the copying"
                        )
                    )
                    return
                elif not self.main_resource.resource.replace_existing_files:
                    Logger.info(
                        format(
                            "Not replacing existing DFS file {target} which is different from {source}, due to replace_existing_files=False"
                        )
                    )
                    return
            else:
                Logger.info(
                    format("File {target} already exists in DFS, skipping the creation")
                )
                return
    ...
    def _set_owner(self, file_status=None):
        ...
        if (
            not self.main_resource.resource.recursive_chown
            and (not owner or file_status and file_status["owner"] == owner)
            and (not group or file_status and file_status["group"] == group)
        ):
            return
        ...
        if self.main_resource.resource.recursive_chown:
            content_summary = self.util.run_command(
                self.main_resource.resource.target,
                "GETCONTENTSUMMARY",
                method="GET",
                assertable_result=False,
            ...
            if (
                content_summary["ContentSummary"]["fileCount"]
                <= HdfsResourceWebHDFS.MAX_FILES_FOR_RECURSIVE_ACTION_VIA_WEBHDFS
                and content_summary["ContentSummary"]["directoryCount"]
        ...
        if self.main_resource.resource.change_permissions_for_parents:
            self._fill_in_parent_directories(
                self.main_resource.resource.target, results
            )
    ...
    def _fill_in_parent_directories(self, target, results):
        path_parts = HdfsResourceProvider.parse_path(target).split("/")[
            1:
        ]  # [1:] remove '' from parts
        path = "/"
    ...
    def _fill_directories_list(self, target, results):
        list_status = self.util.run_command(
            target, "LISTSTATUS", method="GET", assertable_result=False
...
class HdfsResourceProvider(Provider):
    def __init__(self, resource):
        ...
        self.ignored_resources_list = HdfsResourceProvider.get_ignored_resources_list(
        ...
        self.default_protocol = WebHDFSUtil.get_default_protocol(
            resource.default_fs, self.fsType
        )
    ...
    def get_ignored_resources_list(hdfs_resource_ignore_file):
        if not hdfs_resource_ignore_file or not os.path.exists(
    ...
    def action_delayed(self, action_name):
        ...
        if self.has_core_configs:
            ...
            self.create_as_root = (
                path_protocol == "file"
                or self.default_protocol == "file"
                and not path_protocol
            )
            ...
            if path_protocol and path_protocol != self.default_protocol:
                ...
                Logger.info(
                    "Cannot use webhdfs for {0} defaultFs = {1} has different protocol".format(
                        self.resource.target, self.resource.default_fs
                    )
                )
        else:
            self.can_use_webhdfs = False
        ...
        parsed_not_managed_paths = [
            HdfsResourceProvider.parse_path(path)
            for path in self.resource.immutable_paths
        ]
        self.manage_if_exists = not parsed_path in parsed_not_managed_paths

        if parsed_path in self.ignored_resources_list:
            Logger.info(
                "Skipping '{0}' because it is in ignore file {1}.".format(
                    self.resource, self.resource.hdfs_resource_ignore_file
                )
            )
            return
    ...
    def get_hdfs_resource_executor(self):
        if self.can_use_webhdfs and WebHDFSUtil.is_webhdfs_available(
            self.webhdfs_enabled, self.default_protocol
        ):
