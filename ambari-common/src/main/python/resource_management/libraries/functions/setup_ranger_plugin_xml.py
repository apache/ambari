...
def setup_ranger_plugin(
    ...
    if not plugin_home:
        ...
        plugin_home = format(
            "{stack_root}/{stack_version}/ranger-{service_name}-plugin/"
        )
...
def setup_ranger_plugin_keystore(
    ...
    if audit_db_is_enabled:
        ...
        Execute(
            cred_setup, environment={"JAVA_HOME": java_home}, logoutput=True, sudo=True
        )
        ...
        for cache_service in cache_service_list:
            ...
            if (
                ...
                with open(policycache_json_file) as json_file:
                    ...
                    if (
                        "serviceName" in json_data
                        and json_data["serviceName"] == repo_name
                    ):
                        Logger.info(
                            "Skipping Ranger API calls, as policy cache file exists for {0}".format(
                                service_name
                            )
                        )
                        Logger.warning(
