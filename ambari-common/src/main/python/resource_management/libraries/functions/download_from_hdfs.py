...
def download_from_hdfs(
    ...
    Logger.info(
        "Called download_from_hdfs source in HDFS: {0} , local destination path: {1}".format(
    ...
    if not os.path.exists(dest_path):
        Logger.error(
            "Cannot copy {0} because destination directory {1} does not exist.".format(
                source_file, dest_path
            )
        )
        return False
    ...
    Logger.info(
        "Will attempt to copy from DFS at {0} to local file system {1}.".format(
