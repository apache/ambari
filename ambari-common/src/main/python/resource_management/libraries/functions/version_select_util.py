...
def get_component_version_from_symlink(stack_name, component_name):
    ...
    if not stack_name or not component_name:
        Logger.error(
            ...
            "stack_name: %s, component_name: %s"
            % (str(stack_name), str(component_name))
        )
        return version
    ...
    if (
        stack_selector_name
        and stack_selector_path
        and os.path.exists(stack_selector_path)
    ):
        ...
        try:
            ...
            Logger.debug(
                "Version for component %s: %s" % (component_name, str(version))
            )
    ...
    if not stack_selector_path:
        ...
        try:
            ...
            with open(tmpfile.name, "r") as file:
                get_stack_comp_version_cmd = "{0} status {1} > {2}".format(
...
def get_versions_from_stack_root(stack_root):
    ...
    installed_stack_versions = [
        ...
        if os.path.isdir(os.path.join(stack_root, f))
        and re.match("([\d\.]+(-\d+)?)", f)
    ]
