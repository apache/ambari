...
def get_dir_to_mount_from_file(history_filename):
    ...
    if history_filename is not None and os.path.exists(str(history_filename)):
        try:
            ...
            Logger.error(
                "Encountered error while attempting to read dir mount mount values from file %s"
...
def handle_mounted_dirs(func, dirs_string, history_filename, update_cache=True):
    ...
    dirs_string = ",".join(
        [re.sub(r"^\[.+\]", "", dfs_dir.strip()) for dfs_dir in dirs_string.split(",")]
    )
    ...
    for dir_ in valid_dirs:
        last_mount_point_for_dir = (
            prev_dir_to_mount_point.get(dir_, None) if history_file_exists else None
        )
        ...
        if not folder_exists and ignore_bad_mounts:
            ...
            Logger.warning(
                "Not creating {0} as cluster-env/ignore_bad_mounts is enabled.".format(
                    dir_
                )
            )
            ...
            if (
                may_manage_this_dir
                and dir_ not in valid_existing_dirs
                and curr_mount_point in used_mounts
            ):
        ...
        if may_manage_this_dir:
            Logger.info(
                "Forcefully ensuring existence and permissions of the directory: {0}".format(
                    dir_
                )
            )
            # Call the function
    ...
    if error_messages and len(error_messages) > 0:
        ...
        msg = (
            " ".join(error_messages)
            + " Please ensure that mounts are healthy. If the mount change was intentional, you can update the contents of {0}.".format(
                history_filename
            )
        )
    ...
    if last_mount_point_for_dir is None:
        if is_non_root_dir:
            ...
            if manage_dirs_on_root:
                ...
                Logger.warning(
                    "Will not manage the directory {0} since it's on root mount and cluster-env/manage_dirs_on_root == {1}".format(
                        dir_, str(manage_dirs_on_root)
                    )
                )
                may_manage_this_dir = False
        ...
        Logger.debug(
            "Last mount for {0} in the history file is {1}".format(
                dir_, str(last_mount_point_for_dir)
            )
        )
        if last_mount_point_for_dir == curr_mount_point:
            if is_non_root_dir or manage_dirs_on_root:
                Logger.debug(
                    "Will manage {0} since it's on the same mount point: {1}".format(
                        dir_, str(last_mount_point_for_dir)
                    )
                )
                may_manage_this_dir = True
                ...
                Logger.warning(
                    "Will not manage {0} since it's on the root mount point and cluster-env/manage_dirs_on_root == {1}".format(
                        dir_, str(manage_dirs_on_root)
                    )
                )
                may_manage_this_dir = False
            ...
            msg = "Directory {0} became unmounted from {1} . Current mount point: {2} .".format(
                dir_, last_mount_point_for_dir, curr_mount_point
            )
