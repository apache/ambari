...
def get_namenode_states(
  ...
  @retry(
    times=times, sleep_time=sleep_time, backoff_factor=backoff_factor, err_class=Fail
  )
  def doRetries(hdfs_site, security_enabled, run_user):
    ...
    if active_namenodes:
      ...
      Logger.warning(
        f"No active NameNode was found after {times} retries. Will return current NameNode HA states"
      )
      return active_namenodes, standby_namenodes, unknown_namenodes
...
def get_property_for_active_namenode(
  ...
  if _is_ha_config(hdfs_site):
    ...
    if name_service not in name_services:
      raise Fail(
        f'Trying to get property for non-existing ns=\'{name_service}\'. Valid nameservices are {",".join(name_services)}'
      )
