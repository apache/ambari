...
__all__ = [
    "get_namenode_states",
    "get_active_namenode",
    "get_property_for_active_namenodes",
...
def get_namenode_states(
    ...
    def doRetries(hdfs_site, security_enabled, run_user):
        ...
        if active_namenodes:
            ...
            Logger.warning(
                "No active NameNode was found after {0} retries. Will return current NameNode HA states".format(
                    times
                )
            )
            return active_namenodes, standby_namenodes, unknown_namenodes
...
def get_property_for_active_namenodes(
    hdfs_site, property_name, security_enabled, run_user
):
    """
    ...
    name_services = get_nameservices(hdfs_site)
    for name_service in name_services:
        result[name_service] = get_property_for_active_namenode(
            hdfs_site, name_service, property_name, security_enabled, run_user
        )
...
def get_property_for_active_namenode(
    ...
    if _is_ha_config(hdfs_site):
        ...
        if name_service not in name_services:
            raise Fail(
                "Trying to get property for non-existing ns='{1}'. Valid nameservices are {2}".format(
                    property_name, name_service, ",".join(name_services)
                )
            )

        active_namenodes = get_namenode_states(
            hdfs_site, security_enabled, run_user, name_service=name_service
        )[0]
...
def get_nameservices(hdfs_site):
    ...
    name_services_param = hdfs_site.get(
        "dfs.internal.nameservices", None
    )  # in Federated mode this can be a list
    ...
    if name_services_string:
        ...
        for ns in name_services_string.split(","):
            if "dfs.namenode.shared.edits.dir" in hdfs_site and re.match(
                r".*%s$" % ns, hdfs_site["dfs.namenode.shared.edits.dir"]
            ):  # better would be core_site['fs.defaultFS'] but it's not available
                return [ns]
        return [
            name_services_string.split(",")[0]
        ]  # default to return the first nameservice
    return []
...
def get_namespace_mapping_for_hostname(hostname, hdfs_site, security_enabled, run_user):
    namenode_address_map_list = get_namenode_states(
        hdfs_site, security_enabled, run_user
    )
    ...
    if namenode_nameservices and len(namenode_nameservices) > 0:
        for name_service in namenode_nameservices:
            namenode_logical_names_list = hdfs_site.get(
                "dfs.ha.namenodes." + str(name_service), None
            )
