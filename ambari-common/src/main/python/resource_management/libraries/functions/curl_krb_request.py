...
def curl_krb_request(
    ...
    ccache_file_name = HASH_ALGORITHM(
        "{0}|{1}".format(principal, keytab).encode("utf-8")
    ).hexdigest()
    ...
    ccache_file_path = "{0}{1}{2}_{3}_cc_{4}".format(
        curl_krb_cache_path, os.sep, cache_file_prefix, user, ccache_file_name
    )
