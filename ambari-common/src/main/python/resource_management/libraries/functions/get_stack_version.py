...
def get_stack_version(package_name):
    ...
    try:
        ...
        Logger.info(
            "Skipping get_stack_version since the component {0} is not yet available".format(
                package_name
            )
        )
    ...
    if not match:
        Logger.info(
            "Failed to get extracted version for component {0}. Home dir not in expected format.".format(
                package_name
            )
        )
        return None  # lazy fail
...
def get_stack_version(package_name):
    ...
    try:
        command = (
            "ambari-python-wrap {stack_selector_path} status {package_name}".format(
    ...
    if return_code != 0:
        raise Fail(
            "Unable to determine the current version because of a non-zero return code of {0}".format(
                str(return_code)
            )
        )
