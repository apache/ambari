...
def __create_solr_cloud_cli_prefix(
  ...
  infra_solr_cli_opts = (
    format(' INFRA_SOLR_CLI_OPTS="{java_opts}"') if java_opts is not None else ""
  )
  solr_cli_prefix = format(
    "{sudo} JAVA_HOME={java64_home}{infra_solr_cli_opts} /usr/lib/ambari-infra-solr-client/solrCloudCli.sh "
...
def upload_configuration_to_zk(
  ...
  if solrconfig_content is not None:
    File(
      format("{tmp_config_set_dir}/solrconfig.xml"),
      content=solrconfig_content,
      only_if=format("test -d {tmp_config_set_dir}"),
    )
    upload_tmp_config_cmd = format(
...
def create_collection(
  ...
  create_collection_cmd = format(
    "{solr_cli_prefix} --create-collection --collection {collection} --config-set {config_set} "
  ...
  appendableDict["--key-store-password"] = (
    None if key_store_password is None else "{key_store_password_param!p}"
  )
  ...
  appendableDict["--trust-store-password"] = (
    None if trust_store_password is None else "{trust_store_password_param!p}"
  )
  ...
  create_collection_cmd = format(
    create_collection_cmd,
    key_store_password_param=key_store_password,
    trust_store_password_param=trust_store_password,
  )
...
def setup_kerberos(
  ...
  setup_kerberos_cmd = format(
    "{solr_cli_prefix} --setup-kerberos --copy-from-znode {copy_from_znode}"
  )
...
def check_znode(
  ...
  check_znode_cmd = format(
    "{solr_cli_prefix} --check-znode --retry {retry} --interval {interval}"
  )
...
def create_znode(
  ...
  create_znode_cmd = format(
    "{solr_cli_prefix} --create-znode --retry {retry} --interval {interval}"
  )
...
def setup_kerberos_plugin(
  ...
  if secure and jaas_file is not None and security_json_location is not None:
    setup_kerberos_plugin_cmd += format(
      " --jaas-file {jaas_file} --secure --security-json-location {security_json_location}"
    )
...
def set_cluster_prop(
  ...
  set_cluster_prop_cmd = format(
    "{solr_cli_prefix} --cluster-prop --property-name {prop_name} --property-value {prop_value}"
  )
  Execute(set_cluster_prop_cmd)


def set_autoscaling_props(
  zookeeper_quorum,
  solr_znode,
  autoscaling_json_file_location,
  java64_home,
  jaas_file=None,
  java_opts=None,
):
  ...
  set_cluster_prop_cmd = format(
    "{solr_cli_prefix} --set-autoscaling --autoscaling-json-location {autoscaling_json_file_location}"
  )
  Execute(set_cluster_prop_cmd)
...
def secure_znode(
  ...
  secure_znode_cmd = format(
    "{solr_cli_prefix} --secure-znode --sasl-users {sasl_users_str} --retry {retry} --interval {interval}"
  )
...
def secure_solr_znode(
  ...
  secure_solr_znode_cmd = format(
    "{solr_cli_prefix} --secure-solr-znode --sasl-users {sasl_users_str}"
  )
  Execute(secure_solr_znode_cmd)


def remove_admin_handlers(
  zookeeper_quorum,
  solr_znode,
  java64_home,
  collection,
  ...
  remove_admin_handlers_cmd = format(
    "{solr_cli_prefix} --remove-admin-handlers --collection {collection} --retry {retry} --interval {interval}"
  )
...
def copy_solr_znode(
  ...
  copy_znode_cmd = format(
    "{solr_cli_prefix} --transfer-znode --copy-src {src_znode} --copy-dest {target_znode} --retry {retry} --interval {interval}"
  )
  Execute(copy_znode_cmd)


def copy_solr_znode_to_locat(
  zookeeper_quorum,
  solr_znode,
  java64_home,
  jaas_file,
  ...
  copy_znode_cmd = format(
    "{solr_cli_prefix} --transfer-znode --transfer-mode copyToLocal --copy-src {src_znode} --copy-dest {target} --retry {retry} --interval {interval}"
  )
  Execute(copy_znode_cmd)


def copy_solr_znode_from_local(
  zookeeper_quorum,
  solr_znode,
  java64_home,
  jaas_file,
  ...
  copy_znode_cmd = format(
    "{solr_cli_prefix} --transfer-znode --transfer-mode copyFromLocal --copy-src {src} --copy-dest {target_znode} --retry {retry} --interval {interval}"
  )
  Execute(copy_znode_cmd)
...
def setup_solr_client(
  config, custom_log4j=True, custom_log_location=None, log4jcontent=None
):
  ...
  solr_client_log_maxfilesize = default_config(
    config, "configurations/infra-solr-client-log4j/infra_client_log_maxfilesize", 80
  )
  solr_client_log_maxbackupindex = default_config(
    config, "configurations/infra-solr-client-log4j/infra_client_log_maxbackupindex", 60
  )

  Directory(solr_client_log_dir, mode=0o755, cd_access="a", create_parents=True)
  Directory(
    solr_client_dir,
    mode=0o755,
    cd_access="a",
  ...
  File(solrCliFilename, mode=0o755, content=StaticFile(solrCliFilename))
  if custom_log4j:
    ...
    File(
      format("{solr_client_dir}/log4j.properties"),
      content=template.render(context),
      mode=0o644,
    )
  else:
    File(format("{solr_client_dir}/log4j.properties"), mode=0o644)

  File(solr_client_log, mode=0o664, content="")
...
def add_solr_roles(config, roles=[], new_service_principals=[], tries=30, try_sleep=10):
  ...
  solr_ssl_enabled = default_config(
    config, "configurations/infra-solr-env/infra_solr_ssl_enabled", False
  )
  solr_port = default_config(
    config, "configurations/infra-solr-env/infra_solr_port", "8886"
  )
  ...
  if "infra-solr-security-json" in config["configurations"]:
    ...
    infra_solr_security_manually_managed = config["configurations"][
      "infra-solr-security-json"
  ...
  if infra_solr_security_manually_managed:
    ...
    solr_url = format(
      "{solr_protocol}://{solr_host}:{solr_port}/solr/admin/authorization"
    )
    solr_user = config["configurations"]["infra-solr-env"]["infra_solr_user"]
    solr_user_keytab = config["configurations"]["infra-solr-env"][
      "infra_solr_kerberos_keytab"
    ]
    solr_user_principal = config["configurations"]["infra-solr-env"][
      "infra_solr_kerberos_principal"
    ].replace("_HOST", hostname)
    solr_user_kinit_cmd = format(
      "{kinit_path_local} -kt {solr_user_keytab} {solr_user_principal};"
    )
    solr_authorization_enabled_cmd = format(
      "{solr_user_kinit_cmd} curl -k -s --negotiate -u : {solr_protocol}://{solr_host}:{solr_port}/solr/admin/authorization | grep authorization.enabled"
    )
    ...
    if len(new_service_principals) > 0:
      ...
      for new_service_user in new_service_principals:
        new_service_users.append(
          __remove_host_from_principal(new_service_user, kerberos_realm)
        )
      user_role_map = {}
      ...
      add_solr_role_cmd = format(
        "{solr_user_kinit_cmd} curl -H 'Content-type:application/json' -d '{set_user_role_json}' -s -o /dev/null -w'%{{http_code}}' --negotiate -u: -k {solr_url} | grep 200"
      )

      Logger.info(
        format(
          "Check authorization enabled command: {solr_authorization_enabled_cmd} \nSet user-role settings command: {add_solr_role_cmd}"
        )
      )
      Execute(
        solr_authorization_enabled_cmd + " && " + add_solr_role_cmd,
        tries=tries,
        try_sleep=try_sleep,
        user=solr_user,
        logoutput=True,
