...
def get_package_dirs():
    ...
    if conf_select_key not in data:
        raise Fail(
            "There are no conf-select packages defined for this command for the {0} stack".format(
                stack_name
            )
        )
...
def create(stack_name, package, version, dry_run=False):
    ...
    if dry_run:
        Logger.info(
            "Checking to see which directories will be created for {0} on version {1}".format(
                package, version
            )
        )
    else:
        Logger.info(
            "Creating /etc/{0}/{1}/0 if it does not exist".format(package, version)
        )
...
def select(stack_name, package, version, ignore_errors=False):
    ...
    try:
        ...
        if ignore_errors is True:
            Logger.warning(
                "Could not select the directory for package {0}. Error: {1}".format(
                    package, str(exception)
                )
            )
        else:
            raise
...
def convert_conf_directories_to_symlinks(package, version, dirs):
    ...
    for directory_struct in dirs:
        if not os.path.exists(directory_struct["conf_dir"]):
            Logger.info(
                "Skipping the conf-select tool on {0} since {1} does not exist.".format(
    ...
    if len(dry_run_directory) == 0:
        Logger.info(
            "The conf-select tool reported an error for the package {0}. The configuration linking will be skipped.".format(
                package
            )
        )
        return
    ...
    if len(need_dirs) > 0:
        Logger.info(
            "Package {0} will have the following new configuration directories created: {1}".format(
        ...
        try:
            ...
            if os.path.islink(old_conf):
                ...
                if os.readlink(old_conf) != current_dir:
                    ...
                    Logger.info(
                        "Re-linking symlink {0} to {1}".format(old_conf, current_dir)
                    )
                    Link(old_conf, action="delete")
                    Link(old_conf, to=current_dir)
                else:
                    Logger.info(
                        "{0} is already linked to {1}".format(old_conf, current_dir)
                    )
            elif os.path.isdir(old_conf):
                # the /etc/<component>/conf directory is not a link, so turn it into one
                Logger.info(
                    "{0} is a directory - it must be converted into a symlink".format(
                        old_conf
                    )
                )
                ...
                Logger.info(
                    "Backing up {0} to {1} if destination doesn't exist already.".format(
                        old_conf, backup_dir
                    )
                )
                Execute(
          ...
          """
                    ...
                    if (
                        "ATLAS_SERVER" in component_list
                        or "ATLAS_CLIENT" in component_list
                    ):
            ...
            Logger.warning(
                "Could not change symlink for package {0} to point to current directory. Error: {1}".format(
                    package, e
                )
            )
...
def get_restricted_packages():
    ...
    if conf_select_key not in data[stack_name]:
        Logger.info(
            "There are no conf-select-patching elements defined for this command for the {0} stack".format(
                stack_name
            )
        )
        return package_names
...
def _seed_new_configuration_directories(package, created_directories):
    ...
    if package not in package_dirs:
        Logger.warning(
            "Unable to seed newly created configuration directories for {0} because it is an unknown component".format(
                package
            )
        )
        return
    ...
    try:
        ...
        if len(created_directories) != len(expected_directories):
            Logger.warning(
                "The known configuration directories for {0} do not match those created by conf-select: {1}".format(
            ...
            for created_directory in created_directories:
                for expected_directory_structure in expected_directories:
                    ...
                    if prefix is not None and created_directory.startswith(prefix):
                        source_seed_directory = expected_directory_structure[
                            "current_dir"
                        ]
                        target_seed_directory = created_directory
                        _copy_configurations(
                            source_seed_directory, target_seed_directory
                        )

    except Exception as e:
        Logger.warning(
            "Unable to seed new configuration directories for {0}. {1}".format(
                package, str(e)
            )
        )
...
def _copy_configurations(source_directory, target_directory):
    ...
    Execute(
        as_sudo(
            ["cp", "-R", "-p", "-v", source_directory, target_directory],
            auto_escape=False,
        ),
        logoutput=True,
    )
