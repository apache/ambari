...
def get_supported_packages():
    ...
    if code != 0 or stdout is None:
        raise Fail(
            "Unable to query for supported packages using {0}".format(
                stack_selector_path
            )
        )
...
def get_packages(scope, service_name=None, component_name=None):
    ...
    if stack_select_key not in data:
        raise Fail(
            "There are no stack-select packages defined for this command for the {0} stack".format(
                stack_name
            )
        )
    ...
    if service_name not in data:
        Logger.info(
            "Skipping stack-select on {0} because it does not exist in the stack-select package structure.".format(
                service_name
            )
        )
        return None
    ...
    for index, package in enumerate(packages):
        if not is_package_supported(package, supported_packages=supported_packages):
            if _PACKAGE_SCOPE_LEGACY in data[component_name]:
                ...
                Logger.info(
                    "The package {0} is not supported by this version of the stack-select tool, defaulting to the legacy package of {1}".format(
                ...
                raise Fail(
                    "The package {0} is not supported by this version of the stack-select tool.".format(
                        package
                    )
                )
...
def select_all(version_to_select):
    ...
    Logger.info(
        "Executing {0} set all on {1}".format(stack_selector_name, version_to_select)
    )
...
def select_packages(version):
    ...
    Logger.info(
        "The following packages will be stack-selected to version {0} using a {1} orchestration and {2} scope: {3}".format(
            version,
            orchestration.upper(),
            package_scope,
...
def get_role_component_current_stack_version():
    ...
    if stack_select_component is None:
        if not role:
            ...
            Logger.error(
                "Mapping unavailable for role {0}. Skip checking its version.".format(
                    role
                )
            )
        return None
    ...
    if current_stack_version is None:
        Logger.warning(
            "Unable to determine {0} version for {1}".format(
                stack_selector_name, stack_select_component
            )
        )
    else:
        Logger.info(
            "{0} is currently at version {1}".format(
...
def get_stack_version_before_install(component_name):
    ...
    """
    stack_root = Script.get_stack_root()
    component_dir = HADOOP_HOME_DIR_TEMPLATE.format(
        stack_root, "current", component_name
    )
    stack_selector_name = stack_tools.get_stack_tool_name(
        stack_tools.STACK_SELECTOR_NAME
    )
    if os.path.islink(component_dir):
        stack_version = os.path.basename(
            os.path.dirname(
                os.path.dirname(os.path.dirname(os.readlink(component_dir)))
            )
        )
        match = re.match("[0-9]+.[0-9]+.[0-9]+", stack_version)
        if match is None:
            Logger.info(
                "Failed to get extracted version with {0} in method get_stack_version_before_install".format(
                    stack_selector_name
                )
            )
