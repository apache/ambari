...
class Rangeradmin:
            ...
            if isinstance(e, urllib.error.HTTPError):
                raise Fail(
                    "Error getting {0} repository for component {1}. Http status code - {2}. \n {3}".format(
                        name, component, e.code, e.read()
                    )
                )
            else:
                raise Fail(
                    "Error getting {0} repository for component {1}. Reason - {2}.".format(
                        name, component, e.reason
                    )
                )
                ...
                while retryCount <= 5:
                    ...
                    if repo is not None:
                        ...
                        response = self.create_repository_urllib2(
                            repo_data, ambari_username_password_for_ranger, policy_user
                        )
                        if response is not None:
                            Logger.info(
                                "{0} Repository created in Ranger admin".format(
                                    component.title()
                                )
                            )
                            break
                            ...
                            if retryCount < 5:
                                ...
                                Logger.error(
                                    "{0} Repository creation failed in Ranger admin".format(
                                        component.title()
                                    )
                                )
                                break
    ...
    def create_repository_urllib2(self, data, usernamepassword, policy_user):
        ...
        try:
            ...
            base64string = (
                base64.b64encode("{0}".format(usernamepassword).encode())
                .decode()
                .replace("\n", "")
            )
    ...
    def check_ranger_login_urllib2(self, url):
        ...
        try:
            ...
            if isinstance(e, urllib.error.HTTPError):
                raise Fail(
                    "Connection to Ranger Admin failed. Http status code - {0}. \n {1}".format(
                        e.code, e.read()
                    )
                )
            else:
                raise Fail(
                    "Connection to Ranger Admin failed. Reason - {0}.".format(e.reason)
                )
    ...
    def get_policy_by_repo_name(self, name, component, status, usernamepassword):
        ...
        try:
            ...
            if isinstance(e, urllib.error.HTTPError):
                raise Fail(
                    "Error getting policy from repository {0} for component {1}. Http status code - {2}. \n {3}".format(
                        name, component, e.code, e.read()
                    )
                )
            else:
                raise Fail(
                    "Error getting policy from repository {0} for component {1}. Reason - {2}.".format(
                        name, component, e.reason
                    )
                )
        except http.client.BadStatusLine:
    ...
    def create_ambari_admin_user(
        self, ambari_admin_username, ambari_admin_password, usernamepassword
    ):
        """
        ...
        try:
            ...
            if isinstance(e, urllib.error.HTTPError):
                ...
                raise Fail(
                    "Error creating ambari admin user. Reason - {0}.".format(e.reason)
                )
        except http.client.BadStatusLine:
            raise Fail(
                "Ranger Admin service is not reachable, please restart the service and then try again"
