    ...
    def create_repo_files(self):
        ...
        for repository in self.command_repository.items:
            if repository.repo_id is None:
                raise Fail(
                    "Repository with url {0} has no id".format(repository.base_url)
                )
...
class CommandRepository(object):
    ...
    def __init__(self, repo_object):
        ...
        if isinstance(repo_object, dict):
            ...
            raise Fail(
                "Cannot deserialize command repository {0}".format(str(repo_object))
            )
        ...
        self.feat = CommandRepositoryFeature(
            _find_value(json_dict, "feature", default={})
        )
        self.all_items = []
        self.items = []
        ...
        for r in self.all_items:
            if self.repo_tags_to_skip & r.tags:
                Logger.info(
                    "Repository with url {0} is not created due to its tags: {1}".format(
                        r.base_url, r.tags
                    )
                )
            else:
