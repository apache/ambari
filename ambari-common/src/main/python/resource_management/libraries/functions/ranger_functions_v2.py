...
class RangeradminV2:
    ...
    def check_ranger_login_urllib2(self, url):
        ...
        try:
            ...
            if isinstance(e, urllib.error.HTTPError):
                ...
                raise Fail(
                    "Connection failed to Ranger Admin. Reason - {0}.".format(e.reason)
                )
        ...
        try:
            ...
            if response_stripped and len(response_stripped) > 0:
                ...
                if (
                    ...
                    and response_json["name"].lower() == name.lower()
                ):
    ...
    def update_repository_urllib2(
        ...
        try:
            ...
            if response_code == 200:
                Logger.info(
                    "Service name {0} updated successfully on Ranger Admin for service {1}".format(
                        repo_name, component
                    )
                )
                return response
                ...
                raise Fail(
                    "Service name {0} updation failed on Ranger Admin for service {1}".format(
                        repo_name, component
                    )
                )
        except urllib.error.URLError as e:
            if isinstance(e, urllib.error.HTTPError):
                raise Fail(
                    "Error updating service name {0} on Ranger Admin for service {1}. Http status code - {2} \n {3}".format(
                        repo_name, component, e.code, e.read()
                    )
                )
            else:
                raise Fail(
                    "Error updating service name {0} on Ranger Admin for service {1}. Reason - {2}".format(
                        repo_name, component, e.reason
                    )
                )
        except http.client.BadStatusLine:
            raise Fail(
                "Ranger Admin is not reachable for updating service name {0} for service {1}".format(
                    repo_name, component
                )
            )
        except TimeoutError:
