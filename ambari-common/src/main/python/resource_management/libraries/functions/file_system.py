...
def get_and_cache_mount_points(refresh=False):
    ...
    if mounts is not None and not refresh:
        ...
        Logger.info(
            "Host contains mounts: %s." % str([m["mount_point"] for m in mounts])
        )
        return mounts
...
def get_mount_point_for_dir(dir, mount_points=None):
    ...
    if dir:
        ...
        cached_mounts = (
            [m["mount_point"] for m in get_and_cache_mount_points()]
        ...
        for m in cached_mounts:
            ...
            if os.path.join(dir, "").startswith(os.path.join(m, "")):
                if best_mount_found is None:
                    best_mount_found = m
                elif os.path.join(best_mount_found, "").count(
                    os.path.sep
                ) < os.path.join(m, "").count(os.path.sep):
                    best_mount_found = m

    Logger.info(
        "Mount point for directory %s is %s" % (str(dir), str(best_mount_found))
    )
