...
def _prepare_tez_tarball():
    ...
    Logger.info(
        "Extracting {0} to {1}".format(mapreduce_source_file, mapreduce_temp_dir)
    )
    ...
    if not os.path.exists(hadoop_lib_native_dir):
        raise Fail(
            "Unable to seed the Tez tarball with native libraries since the source Hadoop native lib directory {0} does not exist".format(
                hadoop_lib_native_dir
            )
        )

    if not os.path.exists(tez_lib_dir):
        raise Fail(
            "Unable to seed the Tez tarball with native libraries since the target Tez lib directory {0} does not exist".format(
                tez_lib_dir
            )
        )
    ...
    if lzo_utils.should_install_lzo():
        ...
        if not sudo.path_isdir(hadoop_lib_native_lzo_dir):
            Logger.warning(
                "Unable to located native LZO libraries at {0}, falling back to hadoop home".format(
                    hadoop_lib_native_lzo_dir
                )
            )
            hadoop_lib_native_lzo_dir = os.path.join(
        ...
        if not sudo.path_isdir(hadoop_lib_native_lzo_dir):
            raise Fail(
                "Unable to seed the Tez tarball with native libraries since LZO is enabled but the native LZO libraries could not be found at {0}".format(
                    hadoop_lib_native_lzo_dir
                )
            )
...
def _prepare_mapreduce_tarball():
    ...
    if not sudo.path_exists(hadoop_lib_native_source_dir):
        raise Fail(
            "Unable to seed the mapreduce tarball with native LZO libraries since the source Hadoop native lib directory {0} does not exist".format(
                hadoop_lib_native_source_dir
            )
        )
    ...
    Logger.info(
        "Creating a new mapreduce tarball at {0}".format(
...
TARBALL_MAP = {
    "yarn": {
        "dirs": (
            "{0}/{1}/{2}/hadoop-yarn/lib/service-dep.tar.gz".format(
                STACK_ROOT_PATTERN, STACK_VERSION_PATTERN, LIB_DIR
            ),
            "/{0}/apps/{1}/yarn/service-dep.tar.gz".format(
                STACK_NAME_PATTERN, STACK_VERSION_PATTERN
            ),
        ),
        "service": "YARN",
    },
    "tez": {
        "dirs": (
            "{0}/{1}/{2}/tez/lib/tez.tar.gz".format(
                STACK_ROOT_PATTERN, STACK_VERSION_PATTERN, LIB_DIR
            ),
            "/{0}/apps/{1}/tez/tez.tar.gz".format(
                STACK_NAME_PATTERN, STACK_VERSION_PATTERN
            ),
        ),
        "service": "TEZ",
    ...
    "tez_hive2": {
        "dirs": (
            "{0}/{1}/{2}/tez_hive2/lib/tez.tar.gz".format(
                STACK_ROOT_PATTERN, STACK_VERSION_PATTERN, LIB_DIR
            ),
            "/{0}/apps/{1}/tez_hive2/tez.tar.gz".format(
                STACK_NAME_PATTERN, STACK_VERSION_PATTERN
            ),
        ),
        "service": "HIVE",
    ...
    "hive": {
        "dirs": (
            "{0}/{1}/{2}/hive/hive.tar.gz".format(
                STACK_ROOT_PATTERN, STACK_VERSION_PATTERN, LIB_DIR
            ),
            "/{0}/apps/{1}/hive/hive.tar.gz".format(
                STACK_NAME_PATTERN, STACK_VERSION_PATTERN
            ),
        ),
        "service": "HIVE",
    ...
    "hadoop_streaming": {
        "dirs": (
            "{0}/{1}/{2}/hadoop-mapreduce/hadoop-streaming.jar".format(
                STACK_ROOT_PATTERN, STACK_VERSION_PATTERN, LIB_DIR
            ),
            "/{0}/apps/{1}/mapreduce/hadoop-streaming.jar".format(
                STACK_NAME_PATTERN, STACK_VERSION_PATTERN
            ),
        ),
        "service": "MAPREDUCE2",
    ...
    "mapreduce": {
        "dirs": (
            "{0}/{1}/{2}/hadoop/mapreduce.tar.gz".format(
                STACK_ROOT_PATTERN, STACK_VERSION_PATTERN, LIB_DIR
            ),
            "/{0}/apps/{1}/mapreduce/mapreduce.tar.gz".format(
                STACK_NAME_PATTERN, STACK_VERSION_PATTERN
            ),
        ),
        "service": "MAPREDUCE2",
    ...
    "spark": {
        "dirs": (
            "{0}/{1}/{3}/spark/lib/spark-{2}-assembly.jar".format(
                STACK_ROOT_PATTERN, STACK_VERSION_PATTERN, STACK_NAME_PATTERN, LIB_DIR
            ),
            "/{0}/apps/{1}/spark/spark-{0}-assembly.jar".format(
                STACK_NAME_PATTERN, STACK_VERSION_PATTERN
            ),
        ),
        "service": "SPARK",
    ...
    "spark2": {
        "dirs": (
            ...
            "/{0}/apps/{1}/spark2/spark2-{0}-yarn-archive.tar.gz".format(
                STACK_NAME_PATTERN, STACK_VERSION_PATTERN
            ),
        ),
        "service": "SPARK2",
    ...
    "spark2hive": {
        "dirs": (
            ...
            "/{0}/apps/{1}/spark2/spark2-{0}-hive-archive.tar.gz".format(
                STACK_NAME_PATTERN, STACK_VERSION_PATTERN
            ),
        ),
        "service": "SPARK2",
...
def get_tarball_paths(
    ...
    if not stack_name:
        Logger.error(
            "Cannot copy {0} tarball to HDFS because stack name could not be determined.".format(
                str(name)
            )
        )
    ...
    if name is None or name.lower() not in TARBALL_MAP:
        Logger.error(
            "Cannot copy tarball to HDFS because {0} is not supported in stack {1} for this operation.".format(
    ...
    if not stack_version:
        Logger.error(
            "Cannot copy {0} tarball to HDFS because stack version could be be determined.".format(
                str(name)
            )
        )
        return False, None, None
    ...
    if not stack_root:
        Logger.error(
            "Cannot copy {0} tarball to HDFS because stack root could be be determined.".format(
                str(name)
            )
        )
        return False, None, None
...
def get_current_version(service=None, use_upgrading_version_during_upgrade=True):
  ...
  """
    ...
    if not Script.in_stack_upgrade() or use_upgrading_version_during_upgrade:
        Logger.info(
            "Tarball version was calcuated as {0}. Use Command Version: {1}".format(
...
def _get_single_version_from_stack_select():
    ...
    try:
        ...
        Logger.logger.exception(
            "Could not parse output of {0}. Error: {1}".format(str(tmp_file), str(e))
        )
    finally:
        try:
            ...
            Logger.logger.exception(
                "Could not remove file {0}. Error: {1}".format(str(tmp_file), str(e))
            )

    if code != 0 or out is None or out == "":
        Logger.error(
            "Could not verify stack version by calling '{0}'. Return Code: {1}, Output: {2}.".format(
                get_stack_versions_cmd, str(code), str(out)
            )
        )
        return None
    ...
    if matches and len(matches) == 1:
        ...
        Logger.error(
            "Found multiple matches for stack version, cannot identify the correct one from: {0}".format(
                ", ".join(matches)
            )
        )
...
def copy_to_hdfs(
    ...
    if not success:
        Logger.error(
            "Could not copy tarball {0} due to a missing or incorrect parameter.".format(
                str(name)
            )
        )
        return False
    ...
    if skip:
        Logger.warning(
            "Skipping copying {0} to {1} for {2} as it is a sys prepped host.".format(
    ...
    if not skip_component_check:
        ...
        if config is None:
            Logger.info(
                "{0} is not present on the cluster. Skip copying {1}".format(
                    config_name, source_file
                )
            )
            return False

    Logger.info(
        "Source file: {0} , Dest file in HDFS: {1}".format(source_file, dest_file)
    )

    if not os.path.exists(source_file):
        Logger.error(
            "WARNING. Cannot copy {0} tarball because file does not exist: {1} . "
    ...
    Logger.info(
        "Will attempt to copy {0} tarball from {1} to DFS at {2}.".format(
            name, source_file, dest_file
        )
    )
