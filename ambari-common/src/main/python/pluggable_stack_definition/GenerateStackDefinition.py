...
def process_replacements(file_path, config_data, stack_version_changes):
    ...
    if (
        "performCommonReplacements" in config_data
        and config_data.performCommonReplacements
    ):
        for from_version, to_version in stack_version_changes.items():
            file_data = file_data.replace(
                "HDP-" + from_version, config_data.stackName + "-" + to_version
            )
            file_data = file_data.replace(
                "HDP " + from_version, config_data.stackName + " " + to_version
            )
                            ...
                            if service.name == name:
                                ...
                                if "version" in service:
                                    ...
                                    if service_version_tag is None:
                                        service_version_tag = ET.SubElement(
                                            service_tag, "version"
                                        )
                                    ...
                                    if service_version_tag is not None:
                                        service_version_split = (
                                            service_version_tag.text.split(".")
                                        )
                                        if len(stack.baseVersion) < len(
                                            service_version_tag.text
                                        ):
                                            version_suffix = service_version_tag.text[
                                                -len(stack.baseVersion) :
                                            ]
                                            if version_suffix == stack.baseVersion:
                                                version_prefix = (
                                                    service_version_tag.text[
                                                        0 : -len(stack.baseVersion)
                                                    ]
                                                )
                                                service_version_tag.text = (
                                                    version_prefix + stack.version
                                                )
                                ######################################################################################################
                                # Update service version
                                ######################################################################################################
                                ...
                                if "packages" in service:
                                    ...
                                    osSpecifics_tag = ET.SubElement(
                                        service_tag, "osSpecifics"
                                    )
                                    for item in service["packages"]:
                                        osSpecific_tag = ET.SubElement(
                                            osSpecifics_tag, "osSpecific"
                                        )
                                        family = item["family"]
                                        osFamily_tag = ET.SubElement(
                                            osSpecific_tag, "osFamily"
                                        )
                                        osFamily_tag.text = family
                                        packages_tag = ET.SubElement(
                                            osSpecific_tag, "packages"
                                        )
                                        for package in item["packages"]:
                                            package_tag = ET.SubElement(
                                                packages_tag, "package"
                                            )
                                            name_tag = ET.SubElement(
                                                package_tag, "name"
                                            )
                                            ...
                                            if "skipUpgrade" in package:
                                                skipUpgrade_tag = ET.SubElement(
                                                    package_tag, "skipUpgrade"
                                                )
                                                skipUpgrade_tag.text = package[
                                                    "skipUpgrade"
                                                ]
                                else:
                                    ####################################################################################################
                                    ...
                                    ####################################################################################################
                                    for packages_tag in service_tag.getiterator(
                                        "packages"
                                    ):
                                        for package_tag in packages_tag.getiterator(
                                            "package"
                                        ):
                                            ...
                                            for base_version in stack_version_changes:
                                                ...
                                                name_tag.text = process_version_replace(
                                                    name_tag.text, base_version, version
                                                )
        tree.write(file_path)
    return file_path
...
def process_upgrade_xml(file_path, target_version, config_data, stack_version_changes):
    ...
    for target_tag in root.findall("target"):
        ...
        if version in stack_version_changes:
            target_tag.text = target_tag.text.replace(
                version, stack_version_changes[version]
            )
            tree.write(file_path)

    for target_tag in root.findall("target-stack"):
        base_stack_name, base_stack_version = target_tag.text.split("-")
        new_target_stack_text = target_tag.text.replace(
            base_stack_name, config_data.stackName
        )
        if base_stack_version in stack_version_changes:
            new_target_stack_text = new_target_stack_text.replace(
                base_stack_version, stack_version_changes[base_stack_version]
            )
...
def process_stack_advisor(file_path, config_data, stack_version_changes):
    ...
    for stack_name, stack_version in re.findall(
        CLASS_NAME_REGEXP, stack_advisor_content
    ):
        ...
        if stack_version_dotted in stack_version_changes:
            to = (
                ...
                + stack_version_changes[stack_version_dotted].replace(".", "")
                + "StackAdvisor"
            )
        else:
...
def process_repoinfo_xml(file_path, config_data, stack_version_changes, stack):
    if "repoinfo" in stack:
        ...
        open(file_path, "w").write(
            minidom.parseString(ET.tostring(root, "utf-8")).toprettyxml(indent="  ")
        )
    else:
        #########################################################################################
...
def process_py_files(file_path, config_data, stack_version_changes):
    ...
    if config_data.baseStackName.lower() in file_path:
        new_file_path = file_path.replace(
            config_data.baseStackName.lower(), config_data.stackName.lower()
        )
        os.rename(file_path, new_file_path)
    return new_file_path
...
class GeneratorHelper(object):
    ...
    def copy_stacks(self):
        original_folder = os.path.join(
            self.resources_folder, "stacks", self.config_data.baseStackName
        )
        partial_target_folder = os.path.join(
            self.resources_folder, "stacks", self.config_data.stackName
        )
        target_folder = os.path.join(
            self.output_folder, "stacks", self.config_data.stackName
        )
        ...
        for stack in self.config_data.versions:
            ...
            desired_services.append(
                "stack_advisor.py"
            )  # stack_advisor.py placed in stacks folder
            base_stack_services = os.listdir(os.path.join(original_stack, "services"))
            ignored_files = [
                service
                for service in base_stack_services
            ...
            def post_copy(src, target):
                if target.endswith(".xml"):
                    ...
                    if target.endswith("metainfo.xml"):
                        target = process_metainfo(
                            target,
                            self.config_data,
                            self.stack_version_changes,
                            self.common_services,
                        )
                    # process repoinfo.xml
                    if target.endswith("repoinfo.xml"):
                        target = process_repoinfo_xml(
                            target, self.config_data, self.stack_version_changes, stack
                        )
                    ...
                    ###################################################################
                    process_xml_files(
                        target, self.config_data, self.stack_version_changes
                    )
                    return
                if target.endswith(".py"):
                    ...
                    if target.endswith("stack_advisor.py"):
                        target = process_stack_advisor(
                            target, self.config_data, self.stack_version_changes
                        )
                    ####################################################################
                    # Generic processing for py files
                    ###################################################################
                    target = process_py_files(
                        target, self.config_data, self.stack_version_changes
                    )
                    return
                ####################################################################
        ...
        shutil.copy(
            os.path.join(self.resources_folder, "stacks", "stack_advisor.py"),
    ...
    def copy_remaining_common_services(self, common_services=[]):
        ignored_files = [".pyc"]
        source_common_services_path = os.path.join(
            self.resources_folder, "common-services"
        )
    ...
    def copy_resource_management(self):
        source_folder = join(
            os.path.abspath(join(self.resources_folder, "..", "..", "..", "..")),
            "ambari-common",
            "src",
            "main",
            "python",
    ...
    def copy_ambari_properties(self):
        ...
        target_ambari_properties = join(
            self.output_folder, "conf", "unix", "ambari.properties"
        )
        ...
        with open(source_ambari_properties, "r") as in_file:
            with open(target_ambari_properties, "w") as out_file:
                ...
                if len(propertyMap) - len(replaced_properties) > 0:
                    out_file.write(
                        os.linesep
                    )  # make sure we don't break last entry from original properties
