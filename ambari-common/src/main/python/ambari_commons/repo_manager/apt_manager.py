...
class AptManager(GenericManager):
    ...
    def get_available_packages_in_repos(self, repos):
        ...
        if repos.feat.scoped:
            Logger.info(
                "Looking for matching packages in the following repositories: {0}".format(
                    ", ".join(repo_ids)
                )
            )
        ...
        if not name:
            ...
            if context.use_repos:
                if "base" in context.use_repos:
                    ...
                    cmd = cmd + [
                        "-o",
                        "Dir::Etc::SourceList={0}".format(self.properties.empty_file),
                    ]
                ...
                if use_repos:
                    ...
                    Logger.info(
                        "Temporary sources directory was created: {}".format(
                    ...
                    for repo in use_repos:
                        ...
                        Logger.info(
                            "Temporary sources file will be copied: {0}".format(
                                new_sources_file
                            )
                        )
                        sudo.copy(
                            os.path.join(
                                self.properties.repo_definition_location, repo + ".list"
                            ),
                            new_sources_file,
                        )
                    ...
                    cmd = cmd + [
                        "-o",
                        "Dir::Etc::SourceParts=".format(apt_sources_list_tmp_dir),
                    ]

            cmd = cmd + [name]
            Logger.info(
                "Installing package {0} ('{1}')".format(
            ...
            if is_tmp_dir_created:
                for temporary_sources_file in copied_sources_files:
                    Logger.info(
                        "Removing temporary sources file: {0}".format(
    ...
    def remove_package(self, name, context, ignore_dependencies=False):
        ...
        if not name:
            raise ValueError(
                "Installation command were executed with no package name passed"
            )
        elif self._check_existence(name):
            cmd = self.properties.remove_cmd[context.log_output] + [name]
            Logger.info(
                "Removing package {0} ('{1}')".format(
                    name, shell.string_cmd_from_args_list(cmd)
                )
            )
