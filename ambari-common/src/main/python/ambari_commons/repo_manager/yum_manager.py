...
class YumManager(GenericManager):
    ...
    def get_available_packages_in_repos(self, repos):
        ...
        if repos.feat.scoped:
            ...
            if fallback_repo_ids:
                Logger.info(
                    "Adding fallback repositories: {0}".format(
    ...
    def uncomplete_transactions(self):
        ...
        for item in sudo.listdir(self.properties.yum_lib_dir):
            if self.properties.yum_tr_prefix == item[:prefix_len]:
                ...
                f = StringIO(
                    sudo.read_file(os.path.join(self.properties.yum_lib_dir, item))
                )
    ...
    def check_uncompleted_transactions(self):
        ...
        if len(transactions) > 0:
            Logger.info(
                "Yum non-completed transactions check failed, found {0} non-completed transaction(s):".format(
                    len(transactions)
                )
            )
            for tr in transactions:
                Logger.info(
                    "[{0}] Packages broken: {1}; Packages not-installed {2}".format(
                        tr.transaction_id,
                        ", ".join(tr.pkgs_done),
