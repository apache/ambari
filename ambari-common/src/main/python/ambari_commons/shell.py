...
def quote_bash_args(command):
    ...
    if not isinstance(command, str):
        raise ValueError(
            "Command should be a list of strings, found '{0}' in command list elements".format(
                str(command)
            )
        )
...
def launch_subprocess(command, term_geometry=(42, 255), env=None):
    ...
    if not is_under_root() and not isinstance(command, str):
        command = "{0} -H -E {1}".format(
            AMBARI_SUDO_BINARY, string_cmd_from_args_list(command)
        )  # core.shell.as_sudo
    elif not is_under_root() and isinstance(command, str):
        _logger.debug(
            'Warning, command  "{0}" doesn\'t support sudo appending'.format(command)
        )
...
def subprocess_executor(
    ...
    with process_executor(
        command, timeout, _error_handler, strategy, env=env
    ) as output:
...
def process_executor(
    ...
    try:
        ...
        if not isinstance(
            cmd, PopenEx
        ):  # ToDo: make better solution to avoid issues in UT
            yield []
            return
        ...
        if not silent:
            _logger.error(
                "Exception during command '{0}' execution: {1}".format(command, str(e))
            )
        if error_callback:
...
def kill_process_with_children(base_pid):
    ...
    if (
        ...
        for error_item in error_log:
            _logger.warn(
                "PID: {0}, Process: {1}, Exception message: {2}".format(*error_item)
            )
...
def __handle_retries(
    ...
    if (
        ...
        err_log_msg = __PACKAGE_MANAGER_LOCK_ACQUIRED_MSG.format(
...
class shellRunnerLinux(shellRunner):
    ...
    def _change_uid(self):
        try:
            ...
            _logger.warn(
                f"Unable to switch user for running command. Error details: {e}"
            )
