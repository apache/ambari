        ...
        try:
            ...
            for family in json_mapping_data:
                ...
                cls.OS_FAMILY_COLLECTION += [
                    {"name": family, "os_list": json_mapping_data[family][JSON_OS_TYPE]}
                ]

                if JSON_EXTENDS in json_mapping_data[family]:
                    cls.OS_FAMILY_COLLECTION[-1][JSON_EXTENDS] = json_mapping_data[
                        family
                    ][JSON_EXTENDS]
    ...
    def is_in_family(current_family, family):
        try:
            if (
                ...
                or OSCheck.get_os_family_parent(current_family)
                and OSCheck.is_in_family(
                    OSCheck.get_os_family_parent(current_family), family
                )
            ):
                return True
        except Exception:
