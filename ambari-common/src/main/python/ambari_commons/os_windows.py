...
def readlink(fpath):
    ...
    try:
        ...
        buffer = win32file.DeviceIoControl(
            handle, winioctlcon.FSCTL_GET_REPARSE_POINT, None, 16 * 1024
        )
...
def run_os_command_impersonated(cmd, user, password, domain="."):
    ...
    primary_token = win32security.DuplicateTokenEx(
        user_token, win32security.SecurityImpersonation, 0, win32security.TokenPrimary
    )
    info = win32process.CreateProcessAsUser(
        primary_token, None, cmd, None, None, 1, 0, None, None, si
    )
...
def run_powershell_script(script_content):
    ...
    script_file.close()
    result = os_run_os_command(
        "powershell  -ExecutionPolicy unrestricted -File {0}".format(script_file.name)
    )
...
def os_is_root():
    ...
    if retcode != 0:
        err_msg = (
            "Unable to check the current user's group memberships. "
            "Command {0} returned exit code {1} with message: {2}".format(
                WHOAMI_GROUPS, retcode, err
            )
        )
...
def wait_for_pid_wmi(processName, parentPid, pattern, timeout):
    ...
    qry = 'select * from Win32_Process where Name="%s" and ParentProcessId=%d' % (
        processName,
        parentPid,
    )
...
class WinServiceController:
    ...
    def Stop(serviceName, waitSecs=30):
        ...
        try:
            ...
            if waitSecs:
                win32serviceutil.WaitForServiceStatus(
                    serviceName, win32service.SERVICE_STOPPED, waitSecs
                )
        except win32service.error as exc:
...
class WinService(win32serviceutil.ServiceFramework):
    ...
    _hmtxOut = win32event.CreateMutex(
        None, False, None
    )  # [fbarca] Python doesn't support critical sections
    ...
    def CheckForStop(self):
        ...
        return win32event.WAIT_OBJECT_0 == win32event.WaitForSingleObject(
            WinService._heventSvcStop, 1
        )
    ...
    def _StopOrWaitForChildProcessToFinish(self, childProcess):
        ...
        if win32event.WAIT_OBJECT_0 == win32event.WaitForMultipleObjects(
            [WinService._heventSvcStop, childProcess._handle],
            False,
            win32event.INFINITE,
        ):
...
class UserHelper(object):
    ...
    def __init__(self, userName):
        ...
        if self.domainName:
            ...
            self.dcName = None
        self._policy = win32security.LsaOpenPolicy(
            self.dcName,
    ...
    def parse_user_name(userName, defDomain=None):
        ...
        if domainSepIndex != -1:
            ...
            if (
                not domainName
                or domainName == "."
                or domainName == win32api.GetComputerName()
            ):
                domainName = defDomain
