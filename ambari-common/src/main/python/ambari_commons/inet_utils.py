...
def download_file(link, destination, chunk_size=16 * 1024, progress_func=None):
    ...
    if os.path.exists(destination):
        print_warning_msg(
            "File {0} already exists, assuming it was downloaded before".format(
                destination
            )
        )
        return
...
def download_file_anyway(link, destination, chunk_size=16 * 1024, progress_func=None):
    print_info_msg(
        "Trying to download {0} to {1} with python lib [urllib2].".format(
            link, destination
        )
    )
    if os.path.exists(destination):
    ...
    try:
        ...
        print_error_msg(
            "Download {0} with python lib [urllib2] failed with error: {1}".format(
                link, str(sys.exc_info())
            )
        )

    if not os.path.exists(destination):
        print(
            "Trying to download {0} to {1} with [curl] command.".format(
                link, destination
            )
        )
        ...
        if retcode != 0:
            print_error_msg(
                "Download file {0} with [curl] command failed with error: {1}".format(
                    link, out + err
                )
            )
...
def force_download_file(link, destination, chunk_size=16 * 1024, progress_func=None):
    ...
    if downloaded_size != file_size:
        err = "Size of downloaded file {0} is {1} bytes, it is probably damaged or incomplete".format(
            destination, downloaded_size
        )
        raise FatalException(1, err)
