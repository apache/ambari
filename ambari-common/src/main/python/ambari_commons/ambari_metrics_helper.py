...
def get_metric_collectors_from_properties_file(sink_name):
    ...
    if not os.path.exists(properties_filepath):
        raise Exception(
            "Properties file doesn't exist : {0}. Can't define metric collector hosts".format(
                properties_filepath
            )
        )
    ...
    if property_key in props:
        ...
        raise Exception(
            "Properties file doesn't contain {0}. Can't define metric collector hosts".format(
                property_key
            )
        )
...
def create_ams_client(alert_id, ams_app_id, configurations, parameters):
    if (
        ...
        if not ams_collector_hosts:
            raise Exception(
                "Ambari metrics is not available: ams_collector_hosts is None"
            )
...
class AmsClient:
    ...
    def load_metric(self, ams_metric, host_filter):
        ...
        for ams_collector_host in self.ams_collector_hosts:
            try:
                ...
                if logger.isEnabledFor(logging.DEBUG):
                    logger.exception(
                        "[Alert][{0}] Unable to retrieve metrics from AMS ({1}:{2}): {3}".format(
                            self.alert_id,
                            ams_collector_host,
                            self.ams_collector_port,
                            str(exception),
                        )
                    )
        ...
        if http_code not in [200, 299]:
            raise Exception(
                "Ambari metrics is not available: http status code = " + str(http_code)
            )
        if not metric_dict:
