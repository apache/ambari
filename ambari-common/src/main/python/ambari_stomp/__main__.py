...
from ambari_stomp.connect import (
  ambari_stompConnection10,
  StompConnection11,
  StompConnection12,
)
...
class StompCLI(Cmd, ConnectionListener):
  ...
  def __error(self, msg, end="\n"):
    self.stdout.write(
      stomp.colors.BOLD + stomp.colors.RED + str(msg) + stomp.colors.NO_COLOR + end
    )
...
\t{m['description']}
  ...
  def do_subscribe(self, args):
    ...
    self.__sysout(
      'Subscribing to "%s" with acknowledge set to "%s", id set to "%s"'
      % (name, ack_mode, sid)
    )
...
\tauto.""",
  ...
  def do_sendrec(self, args):
    ...
    if len(args) < 2:
      ...
      self.conn.send(
        args[0], " ".join(args[1:]), transaction=self.transaction_id, receipt=receipt_id
      )
  ...
  def help_sendfile(self):
    self.help(
      "sendfile <destination> <filename> [headers.json]",
