...
class Utils:
    ...
    def install_package(name):
        ...
        try:
            ...
            logger.info(
                "Package {0} is already installed. Skipping installation.".format(name)
            )
...
class Installer:
    ...
    def check_dependencies(self):
        ...
        is_rpm = not OSCheck.is_ubuntu_family()
        property_prefix = (
            RPM_DEPENDENCIES_PROPERTY if is_rpm else DEB_DEPENDENCIES_PROPERTY
        )
        ...
        if self.skip_dependencies:
            var = input(
                "Please confirm you have the following packages installed {0} (y/n): ".format(
                    packages_string
                )
            )
