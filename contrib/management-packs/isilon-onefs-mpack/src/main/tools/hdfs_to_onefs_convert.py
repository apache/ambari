...
"""
import urllib.request, urllib.error, urllib.parse, base64, json, ssl, time, random, sys
...
class Url:
  ...
  def query_params(self, a_dict):
    return Url(self.base + '?' + '&'.join('%s=%s' % (name, value) for name, value in list(a_dict.items())))
...
class RestClient:
  ...
  def _request(self, suffix_str, http_method, data=""):
    ...
    request = urllib.request.Request(url, data=self.request_transformer(data))
    request.get_method = lambda: http_method
    self.authenticator.authenticate(request)
    list(map(lambda each: each.add_to(request), self.headers))
    return request, self.ssl_context.build(url)

  def _response(self, request, ssl_context):
    with closing(urllib.request.urlopen(request, context=ssl_context)) as response:
