...
public class AddComponentActionTest extends EasyMockSupport {
  ...
  private static final String SERVICE_GROUP_NAME = "CORE";
  ...
  public void testAddComponentDuringUpgrade() throws Exception {
    ...
    expect(m_mockCandidateService.getServiceComponent(NEW_COMPONENT)).andThrow(new ServiceComponentNotFoundException(
      CLUSTER_NAME, NEW_SERVICE, NEW_SERVICE, SERVICE_GROUP_NAME, NEW_COMPONENT));
    expect(m_mockCandidateService.addServiceComponent(NEW_COMPONENT, NEW_COMPONENT)).andReturn(m_mockCandidateServiceComponent).once();
