            ...
            with patch.object(
                ...
                with patch("distro.linux_distribution", return_value=os_distro_value):
                    with patch("os.symlink"):
                        with patch.object(
                            os_utils, "is_service_exist", return_value=True
                        ):
                            with patch(
                                "glob.glob", return_value=["/etc/init.d/postgresql-9.3"]
                            ):
...
from ambari_server.setupMpacks import (
    ...
    EXTENSION_DEFINITIONS_RESOURCE_NAME,
...
class TestMpacks(TestCase):
    ...
    def test_validate_purge(
        self,
        get_ambari_properties_mock,
        run_mpack_install_checker_mock,
        get_YN_input_mock,
        os_path_exists_mock,
    ):
        options = self._create_empty_options_mock()
        ...
        get_YN_input_mock.return_value = True
        fail = False
        run_mpack_install_checker_mock.return_value = (
            1,
            "",
    ...
    @patch("os.path.exists")
    @patch("ambari_server.setupMpacks.get_ambari_properties")
    def test_purge_stacks_and_mpacks(
        self, get_ambari_version_mock, os_path_exists_mock
    ):
        options = self._create_empty_options_mock()
    ...
    @patch("ambari_server.setupMpacks.download_mpack")
    @patch("ambari_server.setupMpacks.get_ambari_properties")
    def test_install_mpack_with_malformed_mpack(
        self,
        get_ambari_properties_mock,
        download_mpack_mock,
        ...
        try:
            ...
            self.assertEqual(
                "Malformed management pack. Failed to expand management pack!", e.reason
            )
            fail = True
        self.assertTrue(fail)
    ...
    def test_install_stack_mpack(
        ...
        create_symlink_calls = [
            call(
                os.path.join(mpacks_staging_directory, "common-services/SERVICEA"),
            ...
            call(
                os.path.join(mpacks_staging_directory, "common-services/SERVICEB"),
            ...
            call(
                os.path.join(mpacks_staging_directory, "stacks/MYSTACK/1.0"),
                os.path.join(stacks_directory, "MYSTACK/1.0"),
                "metainfo.xml",
                None,
            ),
            call(
                os.path.join(mpacks_staging_directory, "stacks/MYSTACK/1.0/services"),
                os.path.join(stacks_directory, "MYSTACK/1.0/services"),
                "SERVICEA",
                None,
            ),
            call(
                os.path.join(mpacks_staging_directory, "stacks/MYSTACK/1.1"),
                os.path.join(stacks_directory, "MYSTACK/1.1"),
                "metainfo.xml",
                None,
            ),
            call(
                os.path.join(mpacks_staging_directory, "stacks/MYSTACK/1.1/services"),
                os.path.join(stacks_directory, "MYSTACK/1.1/services"),
                "SERVICEA",
                None,
            ),
            call(
                os.path.join(mpacks_staging_directory, "stacks/MYSTACK/2.0"),
                os.path.join(stacks_directory, "MYSTACK/2.0"),
                "metainfo.xml",
                None,
            ),
            call(
                os.path.join(mpacks_staging_directory, "stacks/MYSTACK/2.0/services"),
    ...
    def test_install_extension_mpack(
        ...
        create_symlink_calls = [
            call(
                os.path.join(mpacks_staging_directory, "extensions/MYEXTENSION"),
    ...
    def test_install_addon_service_mpack(
        ...
        create_symlink_calls = [
            call(
                os.path.join(mpacks_staging_directory, "common-services/MYSERVICE"),
                os.path.join(common_services_directory, "MYSERVICE"),
                "1.0.0",
                None,
            )
        ]
        os_symlink_calls = [
            call(
                os.path.join(
                    mpacks_staging_directory, "custom-services/MYSERVICE/1.0.0"
                ),
                os.path.join(stacks_directory, "MYSTACK/1.0/services/MYSERVICE"),
            ),
            call(
                os.path.join(
                    mpacks_staging_directory, "custom-services/MYSERVICE/2.0.0"
                ),
    ...
    def test_upgrade_stack_mpack(
        ...
        create_symlink_calls = [
            ...
            call(
                os.path.join(mpacks_staging_directory, "common-services/SERVICEC"),
            ...
            call(
                os.path.join(mpacks_staging_directory, "stacks/MYSTACK/3.0"),
                os.path.join(stacks_directory, "MYSTACK/3.0"),
                "metainfo.xml",
                True,
            ),
            call(
                os.path.join(mpacks_staging_directory, "stacks/MYSTACK/3.0/services"),
    ...
    def test_replay_mpack_logs(
        ...
        get_replay_log_file_mock.return_value = os.path.join(
            resources_directory, "mpacks_replay.log"
        )
        os_path_exists_mock.return_value = True
        ...
        install_mpack_mock.assert_has_calls(
            [call(install_replay_options, replay_mode=True)]
        )
        upgrade_mpack_mock.assert_has_calls(
            [call(upgrade_replay_options, replay_mode=True)]
        )
    ...
    def test_uninstall_mpack(
        ...
        fake_configs = {
            serverConfiguration.STACK_LOCATION_KEY: os.path.join(
                uninstall_directory, "stacks"
            ),
            serverConfiguration.COMMON_SERVICES_PATH_PROPERTY: os.path.join(
                uninstall_directory, "common-services"
            ),
            serverConfiguration.EXTENSION_PATH_PROPERTY: os.path.join(
                uninstall_directory, "extensions"
            ),
            serverConfiguration.MPACKS_STAGING_PATH_PROPERTY: mpacks_directory,
        ...
        common_services_directory = fake_configs[
            serverConfiguration.COMMON_SERVICES_PATH_PROPERTY
        ]
        ...
        sudo_unlink_mock_calls = [
            call(os.path.join(stacks_directory, "2.0/SERVICEB")),
