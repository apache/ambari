...
class TestMpacks(TestCase):
  ...
  def test_install_mpack_with_malformed_mpack(
    ...
    try:
      ...
      self.assertEqual(
        f"Malformed management pack {options.mpack_path}. Metadata file missing!",
        e.reason,
      )
      fail = True
    self.assertTrue(fail)
  ...
  def test_install_addon_service_mpack(
    ...
    os_symlink_calls = [
      call(
        os.path.join(mpacks_staging_directory, "custom-services/MYSERVICE/1.0.0"),
        os.path.join(stacks_directory, "MYSTACK/1.0/services/MYSERVICE"),
      ),
      call(
        os.path.join(mpacks_staging_directory, "custom-services/MYSERVICE/2.0.0"),
