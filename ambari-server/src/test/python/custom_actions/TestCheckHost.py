...
@patch.object(
    HostCheckReportFileHandler, "writeHostChecksCustomActionsFile", new=MagicMock()
)
...
class TestCheckHost(TestCase):
    ...
    def testDBConnectionCheck(
        ...
        self.assertEqual(
            structured_out_mock.call_args[0][0]["db_connection_check"]["exit_code"], 1
        )
        ...
        mock_config.return_value = {
            ...
            "agentLevelParams": {"agentCacheDir": "/nonexistent_tmp"},
            "ambariLevelParams": {
                "custom_postgres_jdbc_name": "oracle-jdbc-driver.jar"
            },
        }
        format_mock.reset_mock()
        download_file_mock.reset_mock()
    ...
    @patch.object(Script, "get_tmp_dir")
    @patch("resource_management.libraries.script.Script.put_structured_out")
    def testHostResolution(
        self, structured_out_mock, get_tmp_dir_mock, mock_config, mock_socket
    ):
    ...
    @patch("time.time")
    def testLastAgentEnv(
        self,
        time_mock,
        checkReverseLookup_mock,
        checkFirewall_mock,
        getTransparentHugePage_mock,
        getUMask_mock,
        checkLiveServices_mock,
        javaProcs_mock,
        ...
        self.assertTrue(
            "hostHealth" in last_agent_env_check_result["last_agent_env_check"]
        )
        self.assertTrue(
            "firewallRunning" in last_agent_env_check_result["last_agent_env_check"]
        )
        self.assertTrue(
            "firewallName" in last_agent_env_check_result["last_agent_env_check"]
        )
        self.assertTrue(
            "reverseLookup" in last_agent_env_check_result["last_agent_env_check"]
        )
        self.assertTrue(
            "alternatives" in last_agent_env_check_result["last_agent_env_check"]
        )
        self.assertTrue("umask" in last_agent_env_check_result["last_agent_env_check"])
        self.assertTrue(
            "stackFoldersAndFiles"
            in last_agent_env_check_result["last_agent_env_check"]
        )
        self.assertTrue(
            "existingUsers" in last_agent_env_check_result["last_agent_env_check"]
        )
        ...
        put_structured_out_mock.assert_called_with(
            {"last_agent_env_check": {"message": "test exception", "exit_code": 1}}
        )
        pass
    ...
    @patch.object(
        OSCheck, "os_distribution", new=MagicMock(return_value=("debian", "7", "Final"))
    )
    ...
    def testTransparentHugePage(
        ...
        get_tmp_dir_mock.return_value = "/tmp"
        mock_config.return_value = {
            "commandParams": {"check_execute_list": "transparentHugePage"}
        }
        ...
        self.assertEqual(
            structured_out_mock.call_args[0][0],
            {"transparentHugePage": {"message": "never", "exit_code": 0}},
        )
        ...
        self.assertEqual(
            structured_out_mock.call_args[0][0],
            {"transparentHugePage": {"message": "", "exit_code": 0}},
        )
    ...
    @patch("os.path.isfile")
    @patch("resource_management.core.shell.call")
    def testUnsupportedDatabaseType(
        self,
        isfile_mock,
        format_mock,
        structured_out_mock,
        ...
        self.assertEqual(
            ...
            {
                "db_connection_check": {
                    "message": "'unsupported_db' database type not supported.",
                    "exit_code": 1,
                }
            },
        )
