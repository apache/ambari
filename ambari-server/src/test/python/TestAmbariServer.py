                        ...
                        with patch.object(
                            ...
                            with patch(
                                ...
                                with patch("builtins.open"):
                                    ...
                                    from ambari_server.hostUpdate import (
                                        update_host_names,
                                    )
                                    from ambari_server.checkDatabase import (
                                        check_database,
                                    )
...
class TestAmbariServer(TestCase):
    ...
    def test_get_validated_string_input(
        ...
        result = get_validated_string_input(
            prompt, default, pattern, description, is_pass
        )
    ...
    def test_main_test_setup_security(
        ...
        try:
            sys.argv = [
                "ambari-server",
                "setup-security",
                "--security-option=setup-security",
            ]
    ...
    def test_configure_postgresql_conf(self):
        ...
        expected = self.get_file_string(
            self.get_samples_dir("configure_postgresql_conf1")
        )
        ...
        try:
            ...
            for args_entry in set_file_permissions_mock.call_args_list:
                if args_entry[0][0] == ambari_repo_file_entry[0]:  # File name
                    ...
                    self.assertEqual(
                        args_entry[0][1], ambari_repo_file_entry[1]
                    )  # Permissions
                    self.assertEqual(
                        args_entry[0][2], ambari_repo_file_entry[2]
                    )  # File owner
                    self.assertEqual(
                        args_entry[0][3], ambari_repo_file_entry[3]
                    )  # Non-recursive
                    entry_found = True
                    break
    ...
    def test_create_custom_user(
        ...
        result = userChecks._create_custom_user()
        self.assertTrue(
            "User dummy-user already exists"
            in str(print_info_msg_mock.call_args_list[1][0])
        )
        self.assertEqual(result, 0)
        self.assertEqual(userChecks.user, user)
    ...
    def test_ambariServerSetupWithCustomDbName(
        ...
        self.assertEqual(
            str(import_file_to_keystore_mock.call_args_list),
            expect_import_file_to_keystore,
        )
        pass
    ...
    def test_get_cert_info(self, run_os_command_mock):
        ...
        out = out_pattern.format(
            notAfter=notAfter, notBefore=notBefore, subject=subject
        )
    ...
    def test_download_jdk(
        ...
        try:
            ...
            self.assertTrue(
                "Path to java home somewhere or java binary file does not exists"
                in fe.reason
            )
            pass
        pass
    ...
    def test_configure_os_settings(
        ...
        self.assertEqual(
            write_property_mock.call_args_list[0][0][0], "server.os_family"
        )
    ...
    def test_prompt_db_properties_for_each_database_type(
        ...
        for i in range(0, 5):
            ...
            if i == 0:
                ...
                self.assertEqual(
                    properties[JDBC_POSTGRES_SCHEMA_PROPERTY], postgres_schema
                )
                ...
                self.assertFalse(
                    JDBC_POSTGRES_SCHEMA_PROPERTY in properties.propertyNames()
                )
    ...
    def test_setup_jce_policy(
        ...
        shutil_copy_mock.assert_called_with(
            args[1], configDefaults.SERVER_RESOURCES_DIR
        )
        ...
        shutil_copy_mock.reset_mock()
        shutil_copy_mock.side_effect = FatalException(
            1, "Error trying to copy the file."
        )
        try:
            setup_jce_policy(args)
        except FatalException:
        ...
        unpack_jce_policy_mock.side_effect = FatalException(
            1, "Can not install JCE policy"
        )
        try:
            setup_jce_policy(args)
        except FatalException:
    ...
    def test_setup_linux(
        ...
        try:
            ...
            self.fail(
                "Should not throw exception, only print warning"
            )  # see AMBARI-15245
        ...
        self.assertTrue(
            "-U {0}".format(db_username) in run_os_command_1_mock.call_args[0][0][3]
        )
    ...
    def test_reset_default(
        self, logger_mock, is_root_mock, input_mock, get_YN_inputMock
    ):
        is_root_mock.return_value = True
    ...
    def test_start(
        ...
        try:
            ...
            # Expected
            self.assertTrue(
                "Unable to detect a system user for Ambari Server." in e.reason
            )
        ...
        try:
            _ambari_server_.start(args)
            self.fail(
                "Should fail with 'This is not an active instance. Shutting down...'"
            )
        except FatalException as e:
            # Expected
    ...
    def test_move_user_custom_actions(
        ...
        properties.process_pair(
            RESOURCES_DIR_PROPERTY, "some/test/fake/resources/dir/path"
        )
        get_ambari_properties_mock.return_value = properties
        ...
        shutil_move_mock.assert_has_calls(
            [
                call(
                    os.path.join(
                        "some/test/fake/resources/dir/path",
                        "custom_actions",
                        "sometestfile.py",
                    ),
    ...
    def test_upgrade(
        ...
        get_ambari_version_2_mock.return_value = (
            get_ambari_version_mock.return_value
        ) = CURR_AMBARI_VERSION
    ...
    def _test_update_ambari_properties(self, get_conf_dir_mock):
        ...
        for line in properties:
            if line == "agent.fqdn.service.url=URL\n":
                if (
                    not GET_FQDN_SERVICE_URL + "=URL\n" in ambari_properties_content
                ) and (line in ambari_properties_content):
                    self.fail()
    ...
    def test_ensure_jdbc_drivers_installed(
        ...
        props.process_pair(
            JDBC_DRIVER_PATH_PROPERTY, "/some/test/path/to/oracle.-jdbc.jar"
        )
        get_ambari_properties_2_mock.return_value = (
            get_ambari_properties_mock.return_value
        ) = props
    ...
    def test_check_jdbc_drivers(
        ...
        self.assertEqual(
            os_symlink_mock.call_args_list[0][0][0],
            os.path.join(os.sep, "tmp", "ojdbc6.jar"),
        )
        self.assertEqual(
            os_symlink_mock.call_args_list[0][0][1],
    ...
    def test_configure_database_password_silent(self):
        ...
        set_silent(True)
        result = LinuxDBMSConfig._configure_database_password(
            True, "CustomDefaultPasswd"
        )
    ...
    def test_ldap_sync_ssl(
        ...
        url = "{0}://{1}:{2!s}{3}".format(
            "https", socket.getfqdn(), "8443", "/api/v1/ldap_sync_events"
        )
        request = urlopen_mock.call_args_list[0][0][0]
    ...
    @patch("ambari_server.dbConfiguration_windows.store_password_file")
    @patch(
        "ambari_server.dbConfiguration_windows.MSSQLConfig._is_jdbc_driver_installed"
    )
    @patch("ambari_server.serverSetup.verify_setup_allowed")
    @patch("ambari_server.serverSetup.get_YN_input")
    @patch("ambari_server.serverSetup.configure_os_settings")
