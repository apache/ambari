...
def get_configs():
    ...
    configs_path = os.path.join(
        test_directory, "../../main/resources/scripts/configs.py"
    )
    with open(configs_path, "rb") as fp:
        return imp.load_module(
            "configs", fp, configs_path, (".py", "rb", imp.PY_SOURCE)
        )
...
class TestConfigs(TestCase):
    ...
    def test_get_config_to_file(self, urlopen_method, to_file_method):
        ...
        def config_assertion(config):
            self.assertEqual(
                config["properties"], {"config1": "value1", "config2": "value2"}
            )
    ...
    @patch.object(configs, "output_to_file")
    @patch("urllib.request.urlopen")
    def test_update_specific_config_with_attributes(
        self, urlopen_method, to_file_method
    ):
    ...
    def test_set_properties_from_xml(self, urlopen_method, to_file_method):
        ...
        test_directory = os.path.dirname(os.path.abspath(__file__))
        configs_path = os.path.join(
            test_directory, "../resources/TestConfigs-content.xml"
        )
