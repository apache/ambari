...
class TestResourceFilesKeeper(TestCase):
    ...
    DUMMY_UNCHANGEABLE_STACK = (
        ".."
        + os.sep
        + "resources"
        + os.sep
        + "TestAmbaryServer.samples"
        + os.sep
        + "dummy_stack"
        + os.sep
        + "HIVE"
    ...
    DUMMY_UNCHANGEABLE_PACKAGE = os.path.join(
        DUMMY_UNCHANGEABLE_STACK, ResourceFilesKeeper.PACKAGE_DIR
    )

    if get_platform() != PLATFORM_WINDOWS:
        DUMMY_UNCHANGEABLE_PACKAGE_HASH = "ccb9e0a255678f0b5ed1d15c8050ae01a42adcc8"
    else:
    ...
    DUMMY_UNCHANGEABLE_COMMON_SERVICES = (
        ".."
        + os.sep
        + "resources"
        + os.sep
    ...
    def test_update_directory_archives(
        self,
        abspath_mock,
        ...
        resource_files_keeper = ResourceFilesKeeper(
            self.TEST_RESOURCES_DIR, self.TEST_STACKS_DIR
        )
    ...
    def test_list_stacks(self, exists_mock, glob_mock):
        resource_files_keeper = ResourceFilesKeeper(
            self.TEST_RESOURCES_DIR, self.SOME_PATH
        )
    ...
    def test_update_directory_archive(
        self,
        write_hash_sum_mock,
        zip_directory_mock,
        read_hash_sum_mock,
        ...
        os_path_exists_mock.return_value = True
        resource_files_keeper = ResourceFilesKeeper(
            self.TEST_RESOURCES_DIR, self.SOME_PATH, nozip=True
        )
        resource_files_keeper.update_directory_archive(self.SOME_PATH)
        self.assertTrue(read_hash_sum_mock.called)
        self.assertTrue(count_hash_sum_mock.called)
    ...
    def test_write_hash_sum(self):
        ...
        with patch("builtins.open") as open_mock:
            ...
            try:
                resource_files_keeper.write_hash_sum(
                    "path-to-directory", self.DUMMY_HASH
                )
                self.fail("KeeperException not thrown")
            except KeeperException:
                pass  # Expected
    ...
    def test_zip_directory(self, os_path_exists_mock):
        ...
        arc_file = os.path.join(
            self.DUMMY_UNCHANGEABLE_PACKAGE, ResourceFilesKeeper.ARCHIVE_NAME
        )
        ...
        with patch("os.listdir") as os_listdir_mock:
            ...
            try:
                ...
                resource_files_keeper.zip_directory(
                    "empty-to-directory", skip_empty_directory
                )
