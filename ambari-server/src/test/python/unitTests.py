...
def resolve_paths_to_import_from_common_services(
    ...
    if extract is not None:
        if "common-services" in extract:
            ...
            inherited_from_older_version_path = os.path.join(
                base_stack_folder, "..", extract
            )
        ...
        print(
            "Service %s. Could not get extract <extends></extends> from metainfo file: %s. This may prevent modules from being imported."
            % (service, str(metainfo_file))
        )
        ...
        executor_result.put(
            {
                ...
                "errors": [
                    (
                        "Failed to load test files {0}".format(str(modules)),
...
def main():
    ...
    sys.path.append(
        os.path.join(ambari_server_folder, "src/main/resources/host_scripts")
    )
    ...
    for stack in os.listdir(stacks_folder):
        ...
        if os.path.isdir(current_stack_dir) and stack not in STACK_EXCLUDE:
            for service in os.listdir(current_stack_dir):
                ...
                if (
                    os.path.isdir(current_service_dir)
                    and service not in SERVICE_EXCLUDE
                ):
                    if service == "hooks":
