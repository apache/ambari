...
class TestSetupTrustedProxy(unittest.TestCase):
    ...
    def test_silent_mode_is_not_allowed(self, is_server_runing_mock, get_silent_mock):
        ...
        try:
            ...
            self.assertTrue(
                "setup-trusted-proxy is not enabled in silent mode." in e.reason
            )
            pass
    ...
    def test_enable_tproxy_support_using_configuration_file_path_from_command_line(
        ...
        self.assertEqual(
            tproxyProperties[PROXYUSER_GROUPS.format(user_name1)], "group1"
        )
        ...
        self.assertEqual(
            tproxyProperties[PROXYUSER_GROUPS.format(user_name2)], "group2"
        )
