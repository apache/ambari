...
class TestHBaseClient(RMFTestCase):
  ...
  def test_upgrade_23(self, call_mock):
    ...
    self.assertResourceCalledIgnoreEarlier('Execute', ('hdp-select', 'set', 'hbase-client', version), sudo=True)
    self.assertResourceCalledIgnoreEarlier('Execute', ('hdp-select', 'set', 'phoenix-client', version), sudo=True)
