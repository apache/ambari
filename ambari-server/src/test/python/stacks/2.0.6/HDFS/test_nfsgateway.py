...
class TestNFSGateway(RMFTestCase):
  ...
  def test_start_default(self, prepare_rpcbind_mock):
    ...
    self.assertResourceCalled('File', '/var/run/hadoop/root/hadoop_privileged_nfs3.pid',
        action = ['delete'],
        not_if = "ambari-sudo.sh [RMF_ENV_PLACEHOLDER] -H -E test -f /var/run/hadoop/root/hadoop_privileged_nfs3.pid && ambari-sudo.sh [RMF_ENV_PLACEHOLDER] -H -E pgrep --pidfile /var/run/hadoop/root/hadoop_privileged_nfs3.pid",
    )
