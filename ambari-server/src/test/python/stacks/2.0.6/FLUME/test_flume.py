...
class TestFlumeHandler(RMFTestCase):
  ...
  def test_start_default(self, set_desired_mock, cmd_target_names_mock, os_path_isfile_mock):
    ...
    self.assertResourceCalled('Execute', "ambari-sudo.sh [RMF_ENV_PLACEHOLDER] -H -E ps -f -u flume | grep '/usr/jdk64/jdk1.7.0_45.*org.apache.flume.node.Application.*--name a1' | sed -e 's/^ *flume *\([0-9*\]).*$/\1/' | head -n 1 | ambari-sudo.sh [RMF_ENV_PLACEHOLDER] -H -E tee /var/run/flume/a1.pid  && test ${PIPESTATUS[0]} -eq 0",
        logoutput = True,
        tries = 20,
        try_sleep = 10,
    )
