...
class TestHDP206StackAdvisor(TestCase):
  ...
  def test_handleComponentDependencies(self):
    ...
    self.assertEqual(recommendations['blueprint']['host_groups'][3]['components'][0]['name'], 'NAMENODE')
    self.assertEqual(len(recommendations['blueprint']['host_groups'][3]['components']), 1)
    ...
    self.assertEqual(len(recommendations['blueprint']['host_groups'][2]['components']), 2)
    ...
    recommendations = self.stackAdvisor.createComponentLayoutRecommendations(services, hosts)
    # Assert that dependencies are ignored when there are conditions and cluster scope
    self.assertEqual(recommendations['blueprint']['host_groups'][0]['components'][0]['name'], 'NAMENODE')
    self.assertEqual(len(recommendations['blueprint']['host_groups'][2]['components']), 1)
    self.assertEqual(recommendations['blueprint']['host_groups'][3]['components'][0]['name'], 'NAMENODE')
    self.assertEqual(len(recommendations['blueprint']['host_groups'][3]['components']), 1)
    ...
    self.assertEqual(recommendations['blueprint']['host_groups'][1]['components'][1]['name'], 'NAMENODE')
    self.assertEqual(len(recommendations['blueprint']['host_groups'][1]['components']), 2)
  ...
  def assertHostLayout(self, componentsHostsMap, recommendation):
    ...
    for componentName in list(componentsHostsMap.keys()):
  ...
  def checkEqual(self, l1, l2):
    isLenEq = len(l1) == len(l2)
    isSortEq = True
    #l1 and l2 are list,but the elements of list is variable
    if(len(l1)>0):
      #l1 elements is dict
      if(type(l1[0]).__name__=='dict'):
        sortedL1=sorted(l1, key=lambda one: one["message"])
        sortedL2=sorted(l2, key=lambda one: one["message"])
        isSortEq= sortedL1==sortedL2
      # l1 elements is not dict
      else:
        sortedL1=sorted(l1)
        sortedL2=sorted(l2)
        isSortEq= sortedL1==sortedL2
    if not isLenEq or not isSortEq:
