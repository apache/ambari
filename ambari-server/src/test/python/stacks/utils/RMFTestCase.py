...
class RMFTestCase(TestCase):
  ...
  def executeScript(
    ...
    with Environment(basedir, test_mode=True) as RMFTestCase.env:
      with patch(
        "resource_management.core.shell.checked_call", side_effect=checked_call_mocks
      ) as mocks_dict["checked_call"]:
        with patch(
          "resource_management.core.shell.call", side_effect=call_mocks
        ) as mocks_dict["call"]:
          with patch.object(
            Script, "get_config", return_value=self.config_dict
          ) as mocks_dict["get_config"]:
            with patch.object(Script, "get_tmp_dir", return_value="/tmp") as mocks_dict[
              "get_tmp_dir"
            ]:
                  ...
                  with patch.object(
                    ...
                    with patch(
                      ...
                      with patch(
                        ...
                        with patch.object(os, "environ", new=os_env) as mocks_dict[
                          "environ"
                        ]:
                          with patch(
  ...
  def _get_test_paths(self, target, stack_version):
    ...
    if target == self.TARGET_STACKS:
      ...
      configs_path = (
        os.path.join(src_dir, PATH_TO_STACK_TESTS, "configs")
      ...
      configs_path = os.path.join(
        src_dir, PATH_TO_STACK_TESTS, stack_version, "configs"
      )
      return base_path, configs_path
  ...
  def assertResourceCalledIgnoreEarlier(self, resource_type, name, **kwargs):
    ...
    """
    with patch.object(
      UnknownConfiguration, "__getattr__", return_value=lambda: "UnknownConfiguration()"
    ):
      while len(RMFTestCase.env.resource_list) >= 0:
        # no more items means exit the loop
        self.assertNotEqual(
          len(RMFTestCase.env.resource_list),
          0,
          "The specified resource was not found in the call stack.",
        )
  ...
  def assertResourceCalled(self, resource_type, name, **kwargs):
    with patch.object(
      ...
      self.assertNotEqual(
        len(RMFTestCase.env.resource_list), 0, "There were no more resources executed!"
      )
      resource = RMFTestCase.env.resource_list.pop(0)
    ...
    with patch.object(
      ...
      for key in set(resource.arguments.keys()) | set(kwargs.keys()):
        ...
        if self.isstring(resource_value):
          ...
          self.assertEqual(
            resource_value.__class__.__name__, actual_value.__class__.__name__
          )
  ...
  def assertNoMoreResources(self):
    self.assertEqual(
      len(RMFTestCase.env.resource_list), 0, "There were other resources executed!"
    )
...
class FunctionMock:
  ...
  def __eq__(self, other):
    return (
      hasattr(other, "__call__")
      and hasattr(other, "__name__")
      and self.name == other.__name__
    )
