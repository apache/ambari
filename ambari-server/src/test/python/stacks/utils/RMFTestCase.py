...
class RMFTestCase(TestCase):
    ...
    def executeScript(
        ...
        try:
            ...
            raise RuntimeError(
                "Cannot load class %s from %s: %s" % (classname, norm_path, err.message)
            )
        ...
        with Environment(basedir, test_mode=True) as RMFTestCase.env:
            with patch(
                "resource_management.core.shell.checked_call",
                ...
                with patch(
                    ...
                    with patch.object(
                        ...
                        with patch.object(
                            Script, "get_tmp_dir", return_value="/tmp"
                        ) as mocks_dict["get_tmp_dir"]:
                            with patch.object(Script, "post_start") as mocks_dict[
                                "post_start"
                            ]:
                                with patch(
    ...
    def assertResourceCalled(self, resource_type, name, **kwargs):
        with patch.object(
            ...
            self.assertNotEqual(
                len(RMFTestCase.env.resource_list),
                0,
                "There were no more resources executed!",
            )
            resource = RMFTestCase.env.resource_list.pop(0)
            ...
            for key in set(resource.arguments.keys()) | set(kwargs.keys()):
                ...
                if self.isstring(resource_value):
                    self.assertRegex(
                        resource_value,
                        actual_value,
                        msg="Key '%s': '%s' does not match with '%s'"
                        % (key, resource_value, actual_value),
                    )
                else:  # check only the type of a custom object
                    self.assertEqual(
                        resource_value.__class__.__name__,
    ...
    def assertNoMoreResources(self):
        self.assertEqual(
            len(RMFTestCase.env.resource_list),
            0,
            "There were other resources executed!",
        )
