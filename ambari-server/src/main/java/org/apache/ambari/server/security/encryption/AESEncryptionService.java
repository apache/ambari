...
public class AESEncryptionService implements EncryptionService {
  ...
  private void initEnvironmentMasterKeyService() {
    if (environmentMasterKeyService == null) {
      ...
      try {
        environmentMasterKeyService = MasterKeyServiceImpl.getMasterKeyService(null, configuration.getMasterKeyLocation(), configuration.isMasterKeyPersisted());
      } catch (AmbariException e) {
        throw new SecurityException("You are trying to use a persisted master key but its initialization has been failed!", e);
      }
    }
  }
