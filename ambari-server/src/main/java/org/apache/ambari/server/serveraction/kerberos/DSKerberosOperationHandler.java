...
/**
 * Implementation of <code>KerberosOperationHandler</code> to created principal in Apache Directory Server
 */
public class DSKerberosOperationHandler extends KerberosOperationHandler {

    private static final Logger LOG = LoggerFactory.getLogger(DSKerberosOperationHandler.class);

    private static final String LDAP_CONTEXT_FACTORY_CLASS = "com.sun.jndi.ldap.LdapCtxFactory";

    /**
     * A String containing the URL for the LDAP interface for the relevant Apache Directory Server
     */
    private String ldapUrl = null;

    /**
     * A String containing the DN of the container for managing Apache Directory Server accounts
     */
    private String principalContainerDn = null;

    /**
     * The LdapName of the container for managing Apache Directory Server accounts
     */
     ...
     * attributes to use to create new Apache Directory Server accounts.
     * <p/>
    ...
    public void open(PrincipalKeyCredential administratorCredential, String realm,
        ...
        this.createTemplate = kerberosConfiguration.get(KERBEROS_ENV_DS_CREATE_ATTRIBUTES_TEMPLATE);
    ...
    public Integer createPrincipal(String principal, String password, boolean service)
        ...
        if (uid == null) {
            uid = deconstructedPrincipal.getNormalizedPrincipal();
        }

        try {
            Rdn rdn = new Rdn("uid", uid);
    ...
    /**
     * Helper method to create the LDAP context needed to interact with the Apache Directory Server.
     *
    ...
    protected LdapContext createLdapContext() throws KerberosOperationException {
        ...
        try {
            ...
            String message = String.format("Failed to communicate with the Apache Directory Server at %s: %s", ldapUrl, e.getMessage());
            LOG.warn(message, e);
            ...
            String message = String.format("Failed to authenticate with the Apache Directory Server at %s: %s", ldapUrl, e.getMessage());
            LOG.warn(message, e);
     ...
     * Apache Directory Server accounts.
     * <p/>
     ...
     * @return a Map of attribute names and values to use for creating an Apache Directory Server account
    ...
    protected Map<String, Object> processCreateTemplate(Map<String, Object> context)
        ...
        try {
            if (Velocity.evaluate(new VelocityContext(context), stringWriter, "Apache Directory Server principal create template", template)) {
            ...
            LOG.warn("Failed to parse Apache Directory Server create principal template", e);
            throw new KerberosOperationException("Failed to parse Apache Directory Server create principal template", e);
        } catch (MethodInvocationException | ResourceNotFoundException e) {
            LOG.warn("Failed to process Apache Directory Server create principal template", e);
            throw new KerberosOperationException("Failed to process Apache Directory Server create principal template", e);
        }
        ...
        if (normalizedPrincipal != null) {
            ...
            try {
                results = ldapContext.search(
                        ...
                        String.format("(krb5PrincipalName=%s)", normalizedPrincipal),
                        searchControls
                );
