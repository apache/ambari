...
public class ServiceComponentSupport {
  ...
  public Set<String> unsupportedServices(Cluster cluster, String stackName, String stackVersion) {
    return cluster.getServices().entrySet().stream()
      .filter(entry -> !isServiceSupported(entry.getValue().getServiceType(), stackName, stackVersion))
      .map(Map.Entry::getKey)
      .collect(toSet());
  }

  private boolean isServiceSupported(String serviceType, String stackName, String stackVersion) {
    try {
      ServiceInfo service = metaInfo.get().getServices(stackName, stackVersion).get(serviceType);
    ...
    for (Service service : cluster.getServices().values()) {
      for (ServiceComponent component : service.getServiceComponents().values()) {
        if (!component.isVersionAdvertised())
          continue;
        if (!isComponentSupported(service.getServiceType(), component.getName(), stackName, stackVersion)) {
  ...
  private boolean isComponentSupported(String serviceType, String componentName, String stackName, String stackVersion) throws AmbariException {
    try {
      ComponentInfo component = metaInfo.get().getComponent(stackName, stackVersion, serviceType, componentName);
