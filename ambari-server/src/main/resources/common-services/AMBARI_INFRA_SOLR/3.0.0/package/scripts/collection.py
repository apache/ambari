...
def restore_collection(env):
    ...
    Logger.info(
        "New core / host map: " + str(command_commons.solr_restore_core_host_map)
    )
    ...
    Logger.info(
        format("Restore Solr Collection {collection} from {index_location} ...")
    )
    ...
    for core_pair in core_pairs:
        ...
        core_properties["collection.configName"] = (
            command_commons.solr_restore_config_set
        )
        ...
        if command_commons.solr_hdfs_path:
            if target_core in hdfs_cores_on_host:
                ...
                if command_commons.check_hdfs_folder_exists(backup_collection_core_dir):
                    collection_backup_core_dir = (
                        command_commons.solr_hdfs_path
                        + format("/{collection}/backup_{core_node}")
                    )
