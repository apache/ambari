...
class InfraSolr(Script):
    ...
    def start(self, env, upgrade_type=None):
        ...
        start_cmd = (
            format(
                "{solr_bindir}/solr start -cloud -noprompt -s {infra_solr_datadir} -Dsolr.kerberos.name.rules='{infra_solr_kerberos_name_rules}' 2>&1"
            )
            if params.security_enabled
            else format(
                "{solr_bindir}/solr start -cloud -noprompt -s {infra_solr_datadir} 2>&1"
            )
        )
        ...
        piped_start_cmd = (
            format("{start_cmd} | tee {infra_solr_log}") + '; (exit "${PIPESTATUS[0]}")'
        )
        Execute(
            piped_start_cmd,
    ...
    def stop(self, env, upgrade_type=None):
        ...
        try:
            ...
            piped_stop_cmd = (
                format("{stop_cmd} | tee {infra_solr_log}")
                + '; (exit "${PIPESTATUS[0]}")'
            )
            Execute(
                piped_stop_cmd,
                environment={
                    "SOLR_INCLUDE": format("{infra_solr_conf}/infra-solr-env.sh")
                },
                user=params.infra_solr_user,
                logoutput=True,
            )

            File(params.prev_infra_solr_pidfile, action="delete")
        except:
            Logger.warning(
                "Could not stop solr:" + str(sys.exc_info()[1]) + "\n Trying to kill it"
            )
            self.kill_process(
                params.prev_infra_solr_pidfile,
    ...
    def upgrade_solr_instance(self, env):
        ...
        pkg_provider.remove_package(
            "ambari-infra-solr", context, ignore_dependencies=True
        )
