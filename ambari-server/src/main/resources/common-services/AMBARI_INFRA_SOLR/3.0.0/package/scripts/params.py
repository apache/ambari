...
infra_solr_user_nofile_limit = default(
    "/configurations/infra-solr-env/infra_solr_user_nofile_limit", "128000"
)
infra_solr_user_nproc_limit = default(
...
if "infra-solr-env" in config["configurations"]:
    ...
    infra_solr_min_mem = format(
        config["configurations"]["infra-solr-env"]["infra_solr_minmem"]
    )
    infra_solr_max_mem = format(
        config["configurations"]["infra-solr-env"]["infra_solr_maxmem"]
    )
    infra_solr_java_stack_size = format(
        config["configurations"]["infra-solr-env"]["infra_solr_java_stack_size"]
    )
    infra_solr_instance_count = len(config["clusterHostInfo"]["infra_solr_hosts"])
    infra_solr_datadir = format(
        config["configurations"]["infra-solr-env"]["infra_solr_datadir"]
    )
    infra_solr_data_resources_dir = os.path.join(infra_solr_datadir, "resources")
    infra_solr_jmx_port = config["configurations"]["infra-solr-env"][
        "infra_solr_jmx_port"
    ]
    infra_solr_ssl_enabled = default(
        "configurations/infra-solr-env/infra_solr_ssl_enabled", False
    )
    infra_solr_keystore_location = config["configurations"]["infra-solr-env"][
        "infra_solr_keystore_location"
    ]
    infra_solr_keystore_password = config["configurations"]["infra-solr-env"][
        "infra_solr_keystore_password"
    ]
    infra_solr_keystore_type = config["configurations"]["infra-solr-env"][
        "infra_solr_keystore_type"
    ]
    infra_solr_truststore_location = config["configurations"]["infra-solr-env"][
        "infra_solr_truststore_location"
    ]
    infra_solr_truststore_password = config["configurations"]["infra-solr-env"][
        "infra_solr_truststore_password"
    ]
    infra_solr_truststore_type = config["configurations"]["infra-solr-env"][
        "infra_solr_truststore_type"
    ]
    infra_solr_user = config["configurations"]["infra-solr-env"]["infra_solr_user"]
    infra_solr_log_dir = config["configurations"]["infra-solr-env"][
        "infra_solr_log_dir"
    ]
    infra_solr_log = format("{infra_solr_log_dir}/solr-install.log")
    solr_env_content = config["configurations"]["infra-solr-env"]["content"]
    infra_solr_gc_log_opts = format(
        config["configurations"]["infra-solr-env"]["infra_solr_gc_log_opts"]
    )
    infra_solr_gc_tune = format(
        config["configurations"]["infra-solr-env"]["infra_solr_gc_tune"]
    )
    infra_solr_extra_java_opts = format(
        default("configurations/infra-solr-env/infra_solr_extra_java_opts", "")
    )
...
if "infra-solr-security-json" in config["configurations"]:
    infra_solr_security_manually_managed = default(
...
if security_enabled:
    ...
    infra_solr_kerberos_keytab = config["configurations"]["infra-solr-env"][
        "infra_solr_kerberos_keytab"
    ]
    infra_solr_kerberos_principal = config["configurations"]["infra-solr-env"][
        "infra_solr_kerberos_principal"
    ].replace("_HOST", _hostname_lowercase)
    infra_solr_web_kerberos_keytab = config["configurations"]["infra-solr-env"][
        "infra_solr_web_kerberos_keytab"
    ]
    infra_solr_web_kerberos_principal = config["configurations"]["infra-solr-env"][
        "infra_solr_web_kerberos_principal"
    ].replace("_HOST", _hostname_lowercase)
    infra_solr_kerberos_name_rules = config["configurations"]["infra-solr-env"][
    ...
    external_zk_principal_enabled = default(
        "/configurations/infra-solr-env/infra_solr_zookeeper_external_enabled", False
    )
    ...
    ranger_audit_principals.append(
        default(
            "configurations/ranger-hdfs-audit/" + ranger_audit_principal_conf_key, "nn"
        )
    )
    ranger_audit_principals.append(
        default(
            "configurations/ranger-hbase-audit/" + ranger_audit_principal_conf_key,
            "hbase",
        )
    )
    ranger_audit_principals.append(
        default(
            "configurations/ranger-hive-audit/" + ranger_audit_principal_conf_key,
            "hive",
        )
    )
    ranger_audit_principals.append(
        default(
            "configurations/ranger-knox-audit/" + ranger_audit_principal_conf_key,
            "knox",
        )
    )
    ranger_audit_principals.append(
        default(
            "configurations/ranger-kafka-audit/" + ranger_audit_principal_conf_key,
            "kafka",
        )
    )
    ranger_audit_principals.append(
        default(
            "configurations/ranger-kms-audit/" + ranger_audit_principal_conf_key,
            "rangerkms",
        )
    )
    ranger_audit_principals.append(
        default(
            "configurations/ranger-storm-audit/" + ranger_audit_principal_conf_key,
            "storm",
        )
    )
    ranger_audit_principals.append(
        default(
            "configurations/ranger-yarn-audit/" + ranger_audit_principal_conf_key,
            "yarn",
        )
    )
    ranger_audit_principals.append(
        default(
            "configurations/ranger-nifi-audit/" + ranger_audit_principal_conf_key,
            "nifi",
        )
    )
    ...
    logsearch_kerberos_service_user = get_name_from_principal(
        default(
            "/configurations/logsearch-env/logsearch_kerberos_principal", "logsearch"
        )
    )
...
infra_solr_ranger_audit_service_users = format(
    config["configurations"]["infra-solr-security-json"][
        "infra_solr_ranger_audit_service_users"
    ]
).split(",")
infra_solr_security_json_content = config["configurations"]["infra-solr-security-json"][
    "content"
]

infra_solr_jmx_enabled = str(
    default("/configurations/infra-solr-env/infra_solr_jmx_enabled", False)
).lower()

# Solr log4j
infra_log_maxfilesize = default(
    "configurations/infra-solr-log4j/infra_log_maxfilesize", 10
)
infra_log_maxbackupindex = default(
    "configurations/infra-solr-log4j/infra_log_maxbackupindex", 9
)
...
logsearch_service_logs_collection = default(
    "configurations/logsearch-properties/logsearch.solr.collection.service.logs",
    "hadoop_logs",
)
logsearch_audit_logs_collection = default(
    "configurations/logsearch-properties/logsearch.solr.collection.audit.logs",
    "audit_logs",
)
...
logfeeder_kerberos_service_user = get_name_from_principal(
    default("configurations/logfeeder-env/logfeeder_kerberos_principal", "logfeeder")
)
infra_solr_kerberos_service_user = get_name_from_principal(
    default("configurations/infra-solr-env/infra_solr_kerberos_principal", "infra-solr")
)
...
infra_solr_role_atlas = default(
    "configurations/infra-solr-security-json/infra_solr_role_atlas", "atlas_user"
)
infra_solr_role_logsearch = default(
    "configurations/infra-solr-security-json/infra_solr_role_logsearch",
    "logsearch_user",
)
infra_solr_role_logfeeder = default(
    "configurations/infra-solr-security-json/infra_solr_role_logfeeder",
    "logfeeder_user",
)
