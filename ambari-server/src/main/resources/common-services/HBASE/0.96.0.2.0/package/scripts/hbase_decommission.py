  ...
  for host in hosts:
    if host:
      if params.hbase_drain_only == True:
        ...
        regionmover_cmd = format("cmd /c {hbase_executable} org.jruby.Main {region_mover} -m 24 unload {host}")
        Execute(regiondrainer_cmd, user=params.hbase_user, logoutput=True)
  ...
  if params.hbase_drain_only:
      ...
      if host:
        try:
          regiondrainer_cmd = format(
            "{kinit_cmd} {hbase_cmd} --config {hbase_conf_dir} {client_security_config} org.jruby.Main {region_drainer} remove {host}")
          ...
          regiondrainer_cmd = format(
            "{kinit_cmd} {hbase_cmd} --config {hbase_conf_dir} {client_security_config} org.jruby.Main {region_drainer2} remove {host}")
          Execute(regiondrainer_cmd,
                  user=params.hbase_user,
                  logoutput=True
                  )
    ...
    for host in hosts:
      if host:
        try:
          regiondrainer_cmd = format(
            "{kinit_cmd} {hbase_cmd} --config {hbase_conf_dir} {client_security_config} org.jruby.Main {region_drainer} add {host}")
          regionmover_cmd = format(
            "{kinit_cmd} {hbase_cmd} --config {hbase_conf_dir} {client_security_config} org.jruby.Main {region_mover} -m 24 unload {host}")
          ...
          regiondrainer_cmd = format(
            "{kinit_cmd} {hbase_cmd} --config {hbase_conf_dir} {client_security_config} org.jruby.Main {region_drainer2} add {host}")
          regionmover_cmd = format(
            "{kinit_cmd} {hbase_cmd} --config {hbase_conf_dir} {client_security_config} org.jruby.Main {region_mover} -m 24 -o unload -r {host}")
