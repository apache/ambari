...
def get_combined_memory_mb(value1, value2):
  try:
    part1 = (
      int(value1.strip()[:-1]) if value1.lower().strip()[-1:] == "m" else int(value1)
    )
    part2 = (
      int(value2.strip()[:-1]) if value2.lower().strip()[-1:] == "m" else int(value2)
    )
...
failover_strategy_blacklisted_interval_seconds = default(
  "/configurations/ams-env/failover_strategy_blacklisted_interval", "600"
)
failover_strategy = default("/configurations/ams-site/failover.strategy", "round-robin")
if (
  default("/configurations/ams-site/timeline.metrics.service.http.policy", "HTTP_ONLY")
  == "HTTPS_ONLY"
):
...
for host in ams_collector_hosts.split(","):
  metric_truststore_alias = default(
    "/configurations/ams-ssl-client/{host}.ssl.client.truststore.alias", None
  )
...
ams_grafana_data_dir = default(
  "/configurations/ams-grafana-env/metrics_grafana_data_dir",
  "/var/lib/ambari-metrics-grafana",
)
ams_grafana_admin_user = config["configurations"]["ams-grafana-env"][
...
ams_grafana_cert_file = default(
  "/configurations/ams-grafana-ini/cert_file",
  "/etc/ambari-metrics/conf/ams-grafana.crt",
)
ams_grafana_cert_key = default(
  "/configurations/ams-grafana-ini/cert_key", "/etc/ambari-metrics/conf/ams-grafana.key"
)
ams_grafana_ca_cert = default("/configurations/ams-grafana-ini/ca_cert", None)

grafana_connect_attempts = max(
  int(default("/configurations/ams-grafana-env/metrics_grafana_connect_attempts", 15)),
  1,
)
grafana_connect_retry_delay = max(
  int(
...
ams_hbase_init_check_enabled = default(
  "/configurations/ams-site/timeline.metrics.hbase.init.check.enabled", True
)
...
is_hbase_distributed = config["configurations"]["ams-hbase-site"][
...
security_enabled = (
  ...
  else config["configurations"]["cluster-env"]["security_enabled"]
)
...
skip_disk_metrics_patterns = default(
  "/configurations/ams-env/timeline.metrics.skip.disk.metrics.patterns", None
)
skip_network_interfaces_patterns = default(
  "/configurations/ams-env/timeline.metrics.skip.network.interfaces.patterns", None
)
skip_virtual_interfaces = default(
  "/configurations/ams-env/timeline.metrics.skip.virtual.interfaces", False
)

hbase_log_dir = config["configurations"]["ams-hbase-env"]["hbase_log_dir"]
hbase_classpath_additional = default(
  "/configurations/ams-hbase-env/hbase_classpath_additional", None
)
...
host_in_memory_aggregation = default(
  "/configurations/ams-site/timeline.metrics.host.inmemory.aggregation", False
)
...
host_in_memory_aggregation_jvm_arguments = default(
...
if regionserver_xmn_max:
  ...
  regionserver_xmn_percent = expect(
    "/configurations/ams-hbase-env/hbase_regionserver_xmn_ratio", float
  )
...
hbase_master_maxperm_size = config["configurations"]["ams-hbase-env"][
  "hbase_master_maxperm_size"
]

# Check if hbase java options already have appended "m". If Yes, remove the trailing m.
hbase_master_maxperm_size = check_append_heap_property(
  str(hbase_master_maxperm_size), "m"
)
...
max_open_files_limit = default(
  "/configurations/ams-hbase-env/max_open_files_limit", "32768"
)
...
if security_enabled:
  ...
  ams_collector_keytab_path = config["configurations"]["ams-hbase-security-site"][
    "hbase.myclient.keytab"
  ]
  ams_collector_jaas_princ = config["configurations"]["ams-hbase-security-site"][
  ...
  ams_zookeeper_keytab = config["configurations"]["ams-hbase-security-site"][
    "ams.zookeeper.keytab"
  ]
  ams_zookeeper_principal_name = config["configurations"]["ams-hbase-security-site"][
    "ams.zookeeper.principal"
  ].replace("_HOST", _hostname_lowercase)

  master_jaas_config_file = format("{hbase_conf_dir}/hbase_master_jaas.conf")
  master_keytab_path = config["configurations"]["ams-hbase-security-site"][
    "hbase.master.keytab.file"
  ]
  master_jaas_princ = config["configurations"]["ams-hbase-security-site"][
  ...
  regionserver_keytab_path = config["configurations"]["ams-hbase-security-site"][
    "hbase.regionserver.keytab.file"
  ]
  regionserver_jaas_princ = config["configurations"]["ams-hbase-security-site"][
...
skip_create_hbase_root_dir = default(
  "/configurations/ams-site/timeline.metrics.skip.create.hbase.root.dir", False
)
hbase_wal_dir = default("/configurations/ams-hbase-site/hbase.wal.dir", None)
if hbase_wal_dir and re.search(
  "^file://|/", hbase_wal_dir
):  # If wal dir is on local file system, create it.
