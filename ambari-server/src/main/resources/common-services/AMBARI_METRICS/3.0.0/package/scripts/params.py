...
def get_combined_memory_mb(value1, value2):
    try:
        part1 = (
            int(value1.strip()[:-1])
            if value1.lower().strip()[-1:] == "m"
            else int(value1)
        )
        part2 = (
            int(value2.strip()[:-1])
            if value2.lower().strip()[-1:] == "m"
            else int(value2)
        )
...
grafana_connect_retry_delay = max(
    int(
        default(
            "/configurations/ams-grafana-env/metrics_grafana_connect_retry_delay", 20
        )
    ),
    1,
)
...
if security_enabled:
    ...
    smoke_user_princ = config["configurations"]["cluster-env"][
