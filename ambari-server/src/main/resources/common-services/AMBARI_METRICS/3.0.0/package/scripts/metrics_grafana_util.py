    ...
    for i in range(0, params.grafana_connect_attempts):
        try:
            ...
            userAndPass = b64encode(
                "{0}:{1}".format(server.user, server.password).encode()
            ).decode()
...
def perform_grafana_post_call(url, payload, server):
    ...
    for i in range(0, params.grafana_connect_attempts):
        try:
            Logger.info(
                "Connecting (POST) to %s:%s%s" % (server.host, server.port, url)
            )
            conn = network.get_http_connection(
                server.host,
                int(server.port),
...
def perform_grafana_delete_call(url, server):
    ...
    for i in range(0, params.grafana_connect_attempts):
        try:
            ...
            Logger.info(
                "Connecting (DELETE) to %s:%s%s" % (server.host, server.port, url)
            )
  ...
  """
        ...
        for i in range(0, len(datasources_json)):
            ...
            if datasource_name == METRICS_GRAFANA_DATASOURCE_NAME:
                ...
                if datasource_type == new_datasource_type:
                    ...
                    Logger.info(
                        "Grafana datasource type validation failed. Old type = %s, New type = %s"
                        % (datasource_type, new_datasource_type)
                    )
                    update_datasource = True
    ...
    if not create_datasource:
        ...
        if response.status == 200:
            ...
            Logger.info(
                "Ambari Metrics Grafana data source creation failed. Not retrying."
            )
            raise Fail(
            ...
            if dashboard_def:
                ...
                if update_def:
                    ...
                    if response and response.status == 200:
                        ...
                        Logger.error(
                            "Failed creating dashboard: %s" % dashboard_def["title"]
                        )
                    pass
                else:
                    Logger.info(
                        "No update needed for dashboard = %s" % dashboard_def["title"]
                    )
            pass
        pass
