...
def ams(name=None):
    ...
    if name == "collector":
        ...
        Directory(
            params.ams_collector_conf_dir, owner=params.ams_user, create_parents=True
        )
        ...
        Directory(
            params.ams_monitor_conf_dir, owner=params.ams_user, create_parents=True
        )
...
def ams(name=None, action=None):
    ...
    if name == "collector":
        ...
        if params.clusterHostInfoDict:
            ...
            if slave_components:
                new_ams_site["timeline.metrics.initial.configured.slave.components"] = (
                    ",".join(slave_components)
                )
        ...
        hbase_total_heapsize = (
            int(hbase_total_heapsize_with_trailing_m[:-1]) * 1024 * 1024
        )
        ...
        merged_ams_hbase_site["phoenix.spool.directory"] = (
        ...
        Directory(
            params.limits_conf_dir, create_parents=True, owner="root", group="root"
        )
...
def is_spnego_enabled(params):
    if (
        ...
        and params.config["configurations"]["core-site"][
