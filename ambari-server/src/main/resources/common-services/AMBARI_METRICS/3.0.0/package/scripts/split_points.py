...
class FindSplitPointsForAMSRegions:
    ...
    def initialize_region_counts(self):
        try:
            ...
            memstore_max_mem = (
                float(
                    self.ams_hbase_site["hbase.regionserver.global.memstore.upperLimit"]
                )
                * xmx_bytes
            )
    ...
    def gatherMetrics(self, metrics, dir):
        ...
        for file in files:
            ...
            if (
                ...
                or file.rstrip(metric_filename_ext) in self.services
            ):
                ...
                pass
                metrics.update(
                    self.find_equidistant_metrics(list(sorted(service_metrics)))
                )
            pass
        pass
...
def main(argv=None):
    ...
    if os.path.exists(serviceMetricsDir):
        ...
        mode = (
            "distributed"
            if "hbase.rootdir" in ams_hbase_site
            and "hdfs" in ams_hbase_site["hbase.rootdir"]
            else "embedded"
        )
