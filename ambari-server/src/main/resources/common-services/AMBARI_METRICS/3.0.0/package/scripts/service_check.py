...
class AMSServiceCheck(Script):
    ...
    def service_check_for_single_host(self, metric_collector_host, params):
        ...
        try:
            ...
            encoded_get_metrics_parameters = urllib.parse.urlencode(
                get_metrics_parameters
            )
            ...
            if is_spnego_enabled(params):
                ...
                for i in range(0, self.AMS_READ_TRIES):
                    ...
                    if response.status == 200:
                        Logger.info(
                            "Metrics were retrieved from host %s"
                            % metric_collector_host
                        )
                    else:
                        raise Fail(
                    ...
                    for metrics_data in data_json["metrics"]:
                        if (
                            ...
                            Logger.info(
                                ...
                                % (metric_collector_host, random_value1, current_time)
                            )
    ...
    def service_check(self, env):
        ...
        results = execute_in_parallel(
            ...
            params.ams_collector_hosts.split(","),
            params,
        )
    ...
    for i in range(0, tries):
        try:
            ...
            if not response:
                ...
                try:
                    ...
                    Logger.debug(
                        "Unable to close {0} connection to {1}".format(method, uri)
                    )
        ...
        if method == "GET":
            ...
            if not values_are_present:
                if i < tries - 1:  # range/xrange returns items from start to end-1
                    Logger.info(
                        "Values weren't stored yet. Retrying in %s seconds." % (tries)
                    )
                    time.sleep(connection_timeout)
                else:
                    raise Fail(
