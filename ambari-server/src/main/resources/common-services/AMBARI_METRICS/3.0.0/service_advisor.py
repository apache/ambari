...
class AMBARI_METRICSRecommender(service_advisor.ServiceAdvisor):
    ...
    def recommendAmsConfigurationsFromHDP206(
        ...
        putHbaseEnvProperty = self.putProperty(
            configurations, "ams-hbase-env", services
        )
        putGrafanaProperty = self.putProperty(
            configurations, "ams-grafana-env", services
        )
            ...
            if "components" in service:
                for component in service["components"]:
                    if "StackServiceComponents" in component:
                        ...
                        if (
                            "METRICS_GRAFANA"
                            in component["StackServiceComponents"]["component_name"]
                            and len(component["StackServiceComponents"]["hostnames"])
                            != 0
                        ):
                ...
                if host["Hosts"]["host_name"] == collectorHostName:
                    ...
                    if is_local_root_dir:
                        ...
                        if (
                            ...
                            validationItems.extend(
                                [{"config-name": "hbase.tmp.dir", "item": item}]
                            )
                    ...
                    # If no local DN in distributed mode
                        ...
                        collectorHostName not in dn_hosts
                        and distributed.lower() == "true"
                    ):
                        item = self.getWarnItem(
