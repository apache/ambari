...
class OozieServiceCheckDefault(OozieServiceCheck):
  ...
  def oozie_smoke_shell_file(file_name, prepare_hdfs_file_name):
    ...
    Execute(format("{tmp_dir}/{prepare_hdfs_file_name} {conf_dir} {oozie_examples_dir} {hadoop_conf_dir} {yarn_resourcemanager_address} {fs_root} {service_check_queue_name}"),
            tries=3,
            try_sleep=5,
            logoutput=True
    )
