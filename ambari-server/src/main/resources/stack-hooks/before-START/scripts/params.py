...
unlimited_key_jce_required = default(
...
if has_namenode or dfs_type == "HCFS":
  ...
  task_log4j_properties_location = os.path.join(
    hadoop_conf_dir, "task-log4j.properties"
  )
...
# db params
oracle_driver_symlink_url = format(
  "{ambari_server_resources_url}/oracle-jdbc-driver.jar"
)
...
yarn_log_dir_prefix = default(
  "/configurations/yarn-env/yarn_log_dir_prefix", "/var/log/hadoop-yarn"
)

dfs_hosts = default("/configurations/hdfs-site/dfs.hosts", None)

# Hdfs log4j settings
hadoop_log_max_backup_size = default(
  "configurations/hdfs-log4j/hadoop_log_max_backup_size", 256
)
hadoop_log_number_of_backup_files = default(
  "configurations/hdfs-log4j/hadoop_log_number_of_backup_files", 10
)
hadoop_security_log_max_backup_size = default(
  "configurations/hdfs-log4j/hadoop_security_log_max_backup_size", 256
)
hadoop_security_log_number_of_backup_files = default(
  "configurations/hdfs-log4j/hadoop_security_log_number_of_backup_files", 20
)

# Yarn log4j settings
yarn_rm_summary_log_max_backup_size = default(
  "configurations/yarn-log4j/yarn_rm_summary_log_max_backup_size", 256
)
yarn_rm_summary_log_number_of_backup_files = default(
  "configurations/yarn-log4j/yarn_rm_summary_log_number_of_backup_files", 20
)

# log4j.properties
if ("hdfs-log4j" in config["configurations"]) and (
  "content" in config["configurations"]["hdfs-log4j"]
):
  log4j_props = config["configurations"]["hdfs-log4j"]["content"]
  if ("yarn-log4j" in config["configurations"]) and (
    "content" in config["configurations"]["yarn-log4j"]
  ):
...
if command_params is not None:
  refresh_topology = (
    bool(command_params["refresh_topology"])
    if "refresh_topology" in command_params
    else False
  )
