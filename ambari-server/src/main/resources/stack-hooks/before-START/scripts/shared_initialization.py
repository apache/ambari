...
def setup_hadoop():
    ...
    if params.has_namenode or params.dfs_type == "HCFS":
        ...
        if params.has_namenode:
            ...
            Directory(
                format("{hadoop_pid_dir_prefix}/{hdfs_user}"),
                owner=params.hdfs_user,
                cd_access="a",
            )

        Directory(
            params.hadoop_tmp_dir,
            create_parents=True,
            owner=params.hdfs_user,
            cd_access="a",
        )
        ...
        if os.path.exists(params.hadoop_conf_dir):
            File(
                os.path.join(params.hadoop_conf_dir, "commons-logging.properties"),
                owner=tc_owner,
            ...
            File(
                os.path.join(params.hadoop_conf_dir, health_check_template_name),
                owner=tc_owner,
    ...
    if params.has_hdfs or params.dfs_type == "HCFS":
        ...
        if (
            ...
            and "ECS_CLIENT" in params.component_list
        ):
            create_dirs()
...
def create_javahome_symlink():
    if os.path.exists("/usr/jdk/jdk1.6.0_31") and not os.path.exists(
...
def create_microsoft_r_dir():
    ...
    if "MICROSOFT_R_NODE_CLIENT" in params.component_list and params.default_fs:
        ...
        try:
            ...
            Logger.warning(
                "Could not check the existence of {0} on DFS while starting {1}, exception: {2}".format(
                    directory, params.current_service, str(exception)
                )
            )
...
def setup_unlimited_key_jce_policy():
    ...
    __setup_unlimited_key_jce_policy(
        custom_java_home=params.java_home,
        custom_jdk_name=params.jdk_name,
        custom_jce_name=params.jce_policy_zip,
    )
    if params.ambari_jce_name and params.ambari_jce_name != params.jce_policy_zip:
        __setup_unlimited_key_jce_policy(
            custom_java_home=params.ambari_java_home,
            custom_jdk_name=params.ambari_jdk_name,
            custom_jce_name=params.ambari_jce_name,
        )


def __setup_unlimited_key_jce_policy(
    custom_java_home, custom_jdk_name, custom_jce_name
):
    """
    ...
    if params.sysprep_skip_setup_jce:
        Logger.info(
            "Skipping unlimited key JCE policy check and setup since the host is sys prepped"
        )

    elif not custom_jdk_name:
        Logger.info(
            "Skipping unlimited key JCE policy check and setup since the Java VM is not managed by Ambari"
        )

    elif not params.unlimited_key_jce_required:
        Logger.info(
            "Skipping unlimited key JCE policy check and setup since it is not required"
        )
        ...
        if jcePolicyInfo.is_unlimited_key_jce_policy():
            ...
            raise Fail(
                "The unlimited key JCE policy needs to be installed; however the JCE policy zip is not specified."
            )

        else:
            Logger.info(
                "The unlimited key JCE policy is required, and needs to be installed."
            )
            ...
            Logger.debug(
                "Downloading the unlimited key JCE policy files from {0} to {1}.".format(
                    jce_zip_source, jce_zip_target
                )
            )
            ...
            Logger.debug(
                "Removing existing JCE policy JAR files: {0}.".format(java_security_dir)
            )
            ...
            Logger.debug(
                "Unzipping the unlimited key JCE policy files from {0} into {1}.".format(
                    jce_zip_target, java_security_dir
                )
            )
            extract_cmd = (
                "unzip",
