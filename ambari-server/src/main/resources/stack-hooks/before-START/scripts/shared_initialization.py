...
def create_microsoft_r_dir():
  ...
  if "MICROSOFT_R_NODE_CLIENT" in params.component_list and params.default_fs:
    ...
    try:
      ...
      Logger.warning(
        f"Could not check the existence of {directory} on DFS while starting {params.current_service}, exception: {str(exception)}"
      )
...
def __setup_unlimited_key_jce_policy(
  ...
  if params.sysprep_skip_setup_jce:
    ...
    if jcePolicyInfo.is_unlimited_key_jce_policy():
      ...
      Logger.debug(
        f"Downloading the unlimited key JCE policy files from {jce_zip_source} to {jce_zip_target}."
      )
      ...
      Logger.debug(
        f"Unzipping the unlimited key JCE policy files from {jce_zip_target} into {java_security_dir}."
      )
