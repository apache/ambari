...
def setup_stack_symlinks(struct_out_file):
  ...
  if not json_version:
    Logger.info(
      f"There is no advertised version for this component stored in {struct_out_file}"
    )
    return
...
def setup_config():
  ...
  if (
    hasattr(params, "hadoop_conf_dir")
    and params.hadoop_conf_dir is not None
  ...
  if is_hadoop_conf_dir_present and (
    params.has_namenode
    ...
    if params.mount_table_content:
      File(
        os.path.join(params.hadoop_conf_dir, params.xml_inclusion_file_name),
        owner=params.hdfs_user,
        group=params.user_group,
  ...
  Directory(
    params.logsearch_logfeeder_conf, mode=0o755, cd_access="a", create_parents=True
  )
  ...
  if params.logsearch_config_file_exists:
    ...
    Logger.warning(
      "No logsearch configuration exists at " + params.logsearch_config_file_path
    )
