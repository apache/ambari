...
def _get_warnings_for_partition(parameters, disk_usage):
    ...
    label = "Capacity Used: [{0:.2f}%, {1}], Capacity Total: [{2}]".format(
        percent,
        _get_formatted_size(disk_usage.used),
    ...
    if result_code == "OK":
        ...
        if disk_usage.free < min_free_space:
            result_code = "WARNING"
            label += ". Total free space is less than {0}".format(
                _get_formatted_size(min_free_space)
            )
...
def _get_disk_usage(path="/"):
    ...
    if "statvfs" in dir(os):
        ...
        raise NotImplementedError(
            "{0} is not a supported platform for this alert".format(platform.platform())
        )
