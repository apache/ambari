...
def _get_warnings_for_partition(parameters, soft_ulimit):
    ...
    if soft_ulimit >= critical_count:
        label = "Ulimit for open files (-n) is {0} which is higher or equal than critical value of {1}".format(
            soft_ulimit, critical_count
        )
        return_code = "CRITICAL"
    elif soft_ulimit >= warning_count:
        label = "Ulimit for open files (-n) is {0} which is higher or equal than warning value of {1}".format(
            soft_ulimit, warning_count
        )
        return_code = "WARNING"
