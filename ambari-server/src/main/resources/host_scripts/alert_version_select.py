...
def execute(configurations={}, parameters={}, host_name=None):
    ...
    try:
        ...
        if STACK_TOOLS not in configurations:
            return (
                RESULT_STATE_UNKNOWN,
                ["{0} is a required parameter for the script".format(STACK_TOOLS)],
            )
        ...
        if stack_tools_str is None:
            return (
                RESULT_STATE_UNKNOWN,
                [
                    "{0} is a required parameter for the script and the value is null".format(
                        STACK_TOOLS
                    )
                ],
            )
        ...
        if os.path.isdir(stack_root_dir):
            ...
            if code == 0:
                ...
                if (
                    ...
                    msg.append(
                        "reported the following versions: {0}".format(
                            ", ".join(versions)
                        )
                    )
                ...
                msg.append(
                    "{0} could not properly read {1}. Check this directory for unexpected contents.".format(
                        distro_select, stack_root_dir
                    )
                )
                if out is not None:
                    msg.append(out)
