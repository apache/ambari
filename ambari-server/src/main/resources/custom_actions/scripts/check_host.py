...
BEFORE_CLEANUP_HOST_CHECKS = ",".join(
    [
        CHECK_LAST_AGENT_ENV,
        CHECK_INSTALLED_PACKAGES,
        CHECK_EXISTING_REPOS,
        CHECK_TRANSPARENT_HUGE_PAGE,
    ]
)
...
class CheckHost(Script):
    ...
    def actionexecute(self, env):
        ...
        if CHECK_JAVA_HOME in check_execute_list:
            try:
                java_home_check_structured_output = (
                    self.execute_java_home_available_check(config)
                )
        ...
        if (
            CHECK_INSTALLED_PACKAGES in check_execute_list
            and CHECK_EXISTING_REPOS in check_execute_list
        ):
            try:
                ...
                Logger.exception(
                    "There was an unknown error while checking installed packages and existing repositories: "
                    + str(exception)
                )
        ...
        if CHECK_TRANSPARENT_HUGE_PAGE in check_execute_list:
            try:
                transparent_huge_page_structured_output = (
                    self.execute_transparent_huge_page_check(config)
                )
                ...
                Logger.exception(
                    "There was an unknown error while getting transparent huge page data: "
                    + str(exception)
                )
                structured_output[CHECK_TRANSPARENT_HUGE_PAGE] = {
                    "exit_code": 1,
        ...
        for check_name in FALLIBLE_CHECKS:
            if (
                check_name in structured_output
                and "exit_code" in structured_output[check_name]
                and structured_output[check_name]["exit_code"] != 0
            ):
                error_message += "Check {0} was unsuccessful. Exit code: {1}.".format(
                    check_name, structured_output[check_name]["exit_code"]
                )
        ...
        if file_name and os.path.isfile(file_name):
            with open(file_name) as f:
                ...
                transparent_huge_page_check_structured_output = {
                    "exit_code": 0,
                    "message": str(re.search(thp_regex, file_content).groups()[0]),
                }
        else:
            transparent_huge_page_check_structured_output = {
                "exit_code": 0,
    ...
    def execute_existing_repos_and_installed_packages_check(self, config):
        ...
        repos = self.pkg_provider.get_installed_repos(
            self.PACKAGES,
            installedPackages + availablePackages,
            self.IGNORE_PACKAGES_FROM_REPOS,
        )

        packagesInstalled = self.pkg_provider.get_installed_pkgs_by_repo(
            repos, self.IGNORE_PACKAGES, installedPackages
        )
        additionalPkgsInstalled = self.pkg_provider.get_installed_pkgs_by_names(
            self.ADDITIONAL_PACKAGES, installedPackages
        )
        allPackages = list(set(packagesInstalled + additionalPkgsInstalled))

        installedPackages = self.pkg_provider.get_package_details(
    ...
    def execute_db_connection_check(self, config, tmp_dir):
        ...
        if not os.path.isfile(java_exec):
            ...
            if jdk_name.endswith(".exe"):
                ...
                try:
                    ...
                    Directory(
                        java_home, owner=getpass.getuser(), recursive_ownership=True
                    )
        ...
        try:
            ...
            if db_name == DB_MSSQL and OSCheck.is_windows_family():
                ...
                jdbc_auth_url = CheckHost.build_url(
                    jdk_location, JDBC_AUTH_SYMLINK_MSSQL
                )
    ...
    def execute_host_resolution_check(self, config):
        ...
        if failedCount > 0:
            message = (
                "There were "
                + str(failedCount)
                + " host(s) that could not resolve to an IP address."
            )
        else:
