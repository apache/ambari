...
class InstallPackages(Script):
  ...
  def actionexecute(self, env):
    ...
    try:
      ...
      Logger.warning(
        "Failed to check for uncompleted package manager transactions: " + str(e)
      )
  ...
  def _fix_default_links_for_current(self):
    ...
    for package_name, directories in conf_select.get_package_dirs().items():
      ...
      Logger.info(
        f"The following directories will be fixed for {package_name}: {str(directories)}"
      )
      ...
      if component_name:
        ...
        Logger.warning(
          f"Unable to fix {package_name} since stack using outdated stack_packages.json"
        )
        return
  ...
  def _relink_configurations_with_conf_select(self, stack_id, stack_version):
    ...
    if not (
      ...
      and check_stack_feature(StackFeature.CONFIG_VERSIONING, target_stack_version)
    ):
  ...
  def compute_actual_version(self):
    ...
    if 1 == len(deltas):
      ...
      if self.actual_version is not None:
        ...
        self.put_structured_out(self.structured_output)
        Logger.info(
          f"Found actual version {self.actual_version} by searching for best possible match"
        )
      else:
  ...
  def find_best_fit_version(self, versions, repo_version):
    ...
    if build_num_match and repo_version in versions:
      ...
      Logger.info(
        f"Best Fit Version: Resolved from repo version with valid build number: {repo_version}"
      )
      return repo_version
    ...
    if match_versions:
      if len(match_versions) == 1:
        ...
        Logger.info(
          f"Best Fit Version: Resolved from normalized repo version without conflicts: {match_versions[0]}"
        )
      ...
      if history_version in match_versions:
        Logger.info(
          f"Best Fit Version: Resolved from normalized repo version using {REPO_VERSION_HISTORY_FILE}: {history_version}"
        )
  ...
  def install_packages(self, package_list):
    ...
    try:
      ...
      Logger.logger.exception(
        f"Failure while computing actual version. Error: {str(err)}"
      )
    return ret_code

  def abort_handler(self, signum, frame):
    Logger.error(
      f"Caught signal {signum}, will handle it gracefully. Compute the actual version if possible before exiting."
    )
    self.check_partial_install()
