...
from resource_management.libraries.functions.repo_version_history import (
    read_actual_version_from_history_file,
    write_actual_version_to_history_file,
...
class InstallPackages(Script):
    ...
    def actionexecute(self, env):
        ...
        try:
            ...
            num_errors += 1
            Logger.logger.exception(
                "Could not install LZO packages. Error: {0}".format(str(err))
            )
    ...
    def _fix_default_links_for_current(self):
        ...
        if 0 == len(restricted_packages):
            ...
            Logger.info(
                "Restricting conf-select packages to {0}".format(restricted_packages)
            )

        for package_name, directories in conf_select.get_package_dirs().items():
            Logger.info(
                "Attempting to fix the default conf links for {0}".format(package_name)
            )
            Logger.info(
            ...
            if component_name:
                ...
                Logger.warning(
                    "Unable to fix {0} since stack using outdated stack_packages.json".format(
                        package_name
                    )
                )
                return
            ...
            if 0 == len(restricted_packages) or package_name in restricted_packages:
                if stack_version:
                    ...
                    Logger.warning(
                        "Unable to fix {0} since there is no known installed version for this component".format(
                            package_name
                        )
                    )
    ...
    def _relink_configurations_with_conf_select(self, stack_id, stack_version):
        ...
        if len(args) != 2:
            Logger.info(
                "Unrecognized stack id {0}, cannot create config links".format(stack_id)
            )
            return
        ...
        if not (
            ...
            Logger.info(
                "Configuration symlinks are not needed for {0}".format(stack_version)
            )
            return

        # After upgrading hdf-select package from HDF-2.X to HDF-3.Y, we need to create this symlink
        if (
            self.stack_name.upper() == "HDF"
            and not sudo.path_exists("/usr/bin/conf-select")
            and sudo.path_exists("/usr/bin/hdfconf-select")
        ):
    ...
    def compute_actual_version(self):
        ...
        if self.repository_version:
            ...
            if m:
                ...
                self.structured_output["actual_version"] = (
                    self.repo_version_with_build_number
        ...
        if 1 == len(deltas):
            ...
            if self.actual_version is not None:
                ...
                self.put_structured_out(self.structured_output)
                Logger.info(
                    "Found actual version {0} by searching for best possible match".format(
                        self.actual_version
                    )
                )
            else:
    ...
    def check_partial_install(self):
        ...
        if 1 == len(deltas):
            ...
            write_actual_version_to_history_file(
                normalized_repo_version, partial_install_version
            )
            Logger.info(
                "Version {0} was partially installed. ".format(partial_install_version)
            )
    ...
    def find_best_fit_version(self, versions, repo_version):
        ...
        if build_num_match and repo_version in versions:
            ...
            Logger.info(
                "Best Fit Version: Resolved from repo version with valid build number: {0}".format(
                    repo_version
                )
            )
            return repo_version
        ...
        if match_versions:
            if len(match_versions) == 1:
                ...
                Logger.info(
                    "Best Fit Version: Resolved from normalized repo version without conflicts: {0}".format(
                        match_versions[0]
                    )
                )
            ...
            history_version = read_actual_version_from_history_file(
            ...
            if history_version in match_versions:
                Logger.info(
                    "Best Fit Version: Resolved from normalized repo version using {0}: {1}".format(
                        REPO_VERSION_HISTORY_FILE, history_version
                    )
                )
    ...
    def install_packages(self, package_list):
        ...
        try:
            ...
            ret_code = 1
            Logger.logger.error(
                "Package Manager failed to install packages: {0}".format(str(err))
            )
            ...
            if packages_were_checked and packages_installed_before:
                ...
                new_packages_installed = [
                    package
                    for package in packages_installed_after
        ...
        try:
            ...
            ret_code = 1
            Logger.logger.exception(
                "Failure while computing actual version. Error: {0}".format(str(err))
            )
        return ret_code

    def abort_handler(self, signum, frame):
        Logger.error(
            "Caught signal {0}, will handle it gracefully. Compute the actual version if possible before exiting.".format(
                signum
            )
        )
        self.check_partial_install()
