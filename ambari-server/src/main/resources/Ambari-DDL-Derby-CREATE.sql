...
CREATE TABLE service_dependency_binding (
  ...
  consumer_cluster_id BIGINT NOT NULL,
  consumer_service_name VARCHAR(255) NOT NULL,
  provider_cluster_id BIGINT NOT NULL,
  provider_service_name VARCHAR(255) NOT NULL,
  dependency_type VARCHAR(32) NOT NULL,
  state VARCHAR(32) NOT NULL,
  provisioning_phase VARCHAR(64),
  row_version BIGINT NOT NULL,
  operation_epoch BIGINT NOT NULL,
  desired_snapshot_version BIGINT NOT NULL,
  snapshot_approval VARCHAR(32) NOT NULL,
  provider_preparation_hash VARCHAR(71),
  applied_snapshot_version BIGINT,
  provider_fingerprint VARCHAR(71) NOT NULL,
  applied_provider_fingerprint VARCHAR(71),
  namespace_root VARCHAR(2048),
  namespace_wal VARCHAR(2048),
  namespace_znode VARCHAR(1024),
  action_host_id BIGINT,
  active_operation_id VARCHAR(36) NOT NULL,
  active_request_id BIGINT,
  failure_code VARCHAR(128),
  failure_phase VARCHAR(64),
  failure_message VARCHAR(1024),
  failure_retryable SMALLINT NOT NULL,
...
CREATE TABLE service_dependency_snapshot (
  ...
  consumer_fingerprint VARCHAR(71) NOT NULL,
  provider_fingerprint VARCHAR(71) NOT NULL,
  ...
  client_features_hash VARCHAR(71) NOT NULL,
  security_policy_hash VARCHAR(71) NOT NULL,
...
CREATE TABLE service_dependency_host_result (
  ...
  operation_id VARCHAR(36) NOT NULL,
  component_name VARCHAR(255),
  command_request_hash VARCHAR(71) NOT NULL,
  command_json CLOB NOT NULL,
  ambari_request_id BIGINT,
  ambari_stage_id BIGINT,
  ambari_task_id BIGINT,
  required_package_hash VARCHAR(71) NOT NULL,
  observed_package_hash VARCHAR(71),
  rendered_config_hash VARCHAR(71),
  identity_fingerprint VARCHAR(71),
  result_json CLOB,
  result_hash VARCHAR(71),
  preparation_observation_id VARCHAR(36),
  preparation_request_hash VARCHAR(71),
  preparation_observation_fingerprint VARCHAR(71),
  package_name VARCHAR(128),
  package_version VARCHAR(512),
  client_software_version VARCHAR(128),
  state VARCHAR(32) NOT NULL,
...
CREATE TABLE service_dependency_fence (
  ...
  immutable_spec_hash VARCHAR(71) NOT NULL,
  dependency_type VARCHAR(32) NOT NULL,
  consumer_cluster_id BIGINT NOT NULL,
  consumer_service_name VARCHAR(255) NOT NULL,
  provider_cluster_id BIGINT NOT NULL,
  provider_service_name VARCHAR(255) NOT NULL,
  namespace_hash VARCHAR(71) NOT NULL,
  detach_operation_id VARCHAR(36) NOT NULL,
  detach_request_hash VARCHAR(71) NOT NULL,
  detached_by_user_id INTEGER NOT NULL,
...
INSERT INTO ambari_sequences (sequence_name, sequence_value)
  ...
  union all
  select 'datasource_id_seq', 0 FROM SYSIBM.SYSDUMMY1
  union all
  select 'board_id_seq', 0 FROM SYSIBM.SYSDUMMY1
  union all
  select 'chart_share_id_seq', 0 FROM SYSIBM.SYSDUMMY1
  union all
