...
class TezRecommender(service_advisor.ServiceAdvisor):
    ...
    def recommendTezConfigurationsFromHDP22(
        ...
        putTezProperty(
            "tez.runtime.io.sort.mb", min(int(taskResourceMemory * 0.4), 2047)
        )
        putTezProperty(
            ...
            if os.path.exists(views_work_dir) and os.path.isdir(views_work_dir):
                for file in os.listdir(views_work_dir):
                    if fnmatch.fnmatch(file, "TEZ{*}"):
                        has_tez_view = (
                            True  # now used just to verify if the tez view exists
                        )
                    pass
                pass
            pass
    ...
    def recommendTezConfigurationsFromHDP30(
        ...
        if (
            ...
            hive_metastore_warehouse_external_dir = services["configurations"][
                "hive-site"
            ]["properties"]["hive.metastore.warehouse.external.dir"]
            putTezProperty(
                ...
                validationItems.append(
                    {
                        ...
                        "item": self.getWarnItem(
                            "{0} should be less than YARN max allocation size ({1})".format(
                                prop_name1, yarnMaxAllocationSize
                            )
                        ),
                    }
                )
                ...
                validationItems.append(
                    {
                        ...
                        "item": self.getWarnItem(
                            "{0} should be less than YARN max allocation size ({1})".format(
                                prop_name2, yarnMaxAllocationSize
                            )
                        ),
                    }
                )
