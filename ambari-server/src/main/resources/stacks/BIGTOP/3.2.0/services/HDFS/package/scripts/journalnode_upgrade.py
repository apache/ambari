...
def ensure_jns_have_new_txn(nodelist, last_txn_id):
    ...
    if jn_uri is None:
        raise Fail(
            "No JournalNode URI found at hdfs-site/dfs.namenode.shared.edits.dir"
        )
        ...
        for node in nodes:
            ...
            if data:
                ...
                if actual_txn_ids[node] >= last_txn_id:
                    Logger.info(
                        "JournalNode %s has a higher transaction id: %s"
                        % (node, str(data))
                    )
                    jns_updated += 1
                else:
                    Logger.info(
                        "JournalNode %s is still on transaction id: %s"
                        % (node, str(data))
                    )
