...
class LivyServer(Script):
    ...
    def wait_for_dfs_directories_created(self, dirs):
        ...
        if params.security_enabled:
            Execute(
                format(
                    "{kinit_path_local} -kt {livy_kerberos_keytab} {livy_principal}"
                ),
    ...
    def wait_for_dfs_directory_created(self, dir_path, ignored_dfs_dirs):
        ...
        if not is_empty(dir_path):
            ...
            if dir_path in ignored_dfs_dirs:
                Logger.info(
                    "Skipping DFS directory '"
                    + dir_path
                    + "' as it's marked to be ignored."
                )
                return
            ...
            if WebHDFSUtil.is_webhdfs_available(
                params.is_webhdfs_enabled, params.dfs_type
            ):
                ...
                dir_exists = (
                    not dfs_ret_code
                )  # dfs -test -d returns 0 in case the dir exists
