...
class HBASERecommender(service_advisor.ServiceAdvisor):
    ...
    def recommendHbaseConfigurationsFromHDP206(
        ...
        putHbaseProperty(
            "hbase_regionserver_heapsize", int(clusterData["hbaseRam"]) * 1024
        )
    ...
    def recommendHBASEConfigurationsFromHDP22(
        ...
        hmaster_host = self.getHostWithComponent(
            "HBASE", "HBASE_MASTER", services, hosts
        )
        ...
        if int(clusterData["hbaseRam"]) > threshold:
            ...
            block_cache_heap = (
                8192  # int(regionserver_heap_size * hfile_block_cache_size)
            )
            ...
            hbase_bucketcache_percentage_in_combinedcache_str = "{0:.4f}".format(
                math.ceil(hbase_bucketcache_percentage_in_combinedcache * 10000)
                / 10000.0
            )
            ...
            # Enable in hbase-env
            putHbaseEnvProperty = self.putProperty(
                configurations, "hbase-env", services
            )
            putHbaseEnvProperty(
            ...
            if (
                ...
                putHbaseSitePropertyAttributes(
                    "hbase.bucketcache.ioengine", "delete", "true"
                )
            if (
                "hbase.bucketcache.size" in configurations["hbase-site"]["properties"]
            ) or (
                "hbase-site" in services["configurations"]
                ...
                putHbaseSitePropertyAttributes(
                    "hbase.bucketcache.size", "delete", "true"
                )
            if (
                "hbase.bucketcache.percentage.in.combinedcache"
                in configurations["hbase-site"]["properties"]
            ...
            if (
                ...
                in configurations["hbase-env"]["properties"]
            ) or (
                "hbase-env" in services["configurations"]
    ...
    def recommendHBASEConfigurationsFromHDP26(
        ...
        if (
            ...
            hbase_user = services["configurations"]["hbase-env"]["properties"][
                "hbase_user"
            ]
        else:
            hbase_user = "hbase"

        ranger_hbase_plugin_enabled = self.isRangerPluginEnabled(
        ...
        if (
            ...
            in services["configurations"]["ranger-hbase-plugin-properties"][
                "properties"
            ]
        ):
    ...
    def recommendHBASEConfigurationsFromHDP30(
        ...
        if (
            ...
            in services["configurations"]["hbase-atlas-application-properties"][
                "properties"
            ]
        ):
            enable_external_hook_for_hbase = (
                services["configurations"]["hbase-atlas-application-properties"][
                    "properties"
                ]["enable.external.atlas.for.hbase"].lower()
                == "true"
            )
        else:
        ...
        if enable_hbase_atlas_hook:
            ...
            if not is_hbase_atlas_hook_in_config:
                ...
                self.logger.info(
                    "hbase-atlas hook is already present in configuration."
                )
        else:
            hbase_master_coprocessor_list = [
    ...
    def recommendHBASEConfigurationsForKerberos(
        ...
        if is_kerberos_enabled:
            ...
            phoenix_query_server_hosts = self.getPhoenixQueryServerHosts(
                services, hosts
            )
            self.logger.debug(
                "Calculated Phoenix Query Server hosts: %s"
                % str(phoenix_query_server_hosts)
            )
            if phoenix_query_server_hosts:
                self.logger.debug(
                    "Attempting to update hadoop.proxyuser.HTTP.hosts with %s"
                    % str(phoenix_query_server_hosts)
                )
    ...
    def calculateCoprocessorConfigurations(
        ...
        hbase_site_configurations_properties = (
            configurations["hbase-site"]["properties"]
            if "hbase-site" in configurations
            and "properties" in configurations["hbase-site"]
            else {}
        )
        ...
        for key in hbaseCoProcessorConfigs:
            ...
            if key in hbase_site_configurations_properties:
                hbase_coprocessor_classes = hbase_site_configurations_properties[
                    key
                ].strip()
        ...
        if hbase_security_authorization:
            if "true" == hbase_security_authorization.lower():
                ...
                hbaseCoProcessorConfigs[
                    "hbase.coprocessor.regionserver.classes"
                ].append("org.apache.hadoop.hbase.security.access.AccessController")
                ...
                if (
                    ...
                    "hbase.coprocessor.regionserver.classes"
                    in hbase_site_services_properties
                ):
                    hbaseCoProcessorConfigAttributes[
                        "hbase.coprocessor.regionserver.classes"
                    ].append(["delete", "true"])
    ...
    def isRangerPluginEnabled(self, configurations, services):
        if (
            ...
            ranger_hbase_plugin_enabled = (
                services["configurations"]["ranger-hbase-plugin-properties"][
                    "properties"
                ]["ranger-hbase-plugin-enabled"].lower()
                == "Yes".lower()
            )
        else:
...
class HBASEValidator(service_advisor.ServiceAdvisor):
    ...
    def validateHBASEConfigurationsFromHDP22(
        ...
        if prop_name1 in hbase_site and not self.is_number(hbase_site[prop_name1]):
            ...
            and float(hbase_site[prop_name1]) + float(hbase_site[prop_name2])
            > props_max_sum
        ):
            validationItems.append(
                {
                    "config-name": prop_name1,
                    "item": self.getWarnItem(
                        "{0} and {1} sum should not exceed {2}".format(
                            prop_name1, prop_name2, props_max_sum
                        )
                    ),
                }
            )
            ...
            validationItems.append(
                {
                    "config-name": prop_name2,
                    "item": self.getWarnItem(
                        "If bucketcache ioengine is enabled, {0} should be set".format(
                            prop_name2
                        )
                    ),
                }
    ...
    def validateHBASEConfigurationsFromHDP23(
        ...
        if ("RANGER" in servicesList) and (
            ...
            if (
                prop_val in hbase_site[prop_name]
                and exclude_val not in hbase_site[prop_name]
            ):
                pass
            else:
                validationItems.append(
