...
def hive(name=None):
    ...
    Logger.info(
        "Directories to fill with configs: %s" % str(params.hive_conf_dirs_list)
    )
    ...
    if params.enable_atlas_hook:
        atlas_hook_filepath = os.path.join(
            params.hive_conf_dir, params.atlas_hook_filename
        )
        setup_atlas_hook(
            SERVICE.HIVE,
...
def setup_hiveserver2():
    ...
    if (
        params.hive_repl_cmrootdir is not None
        and params.hive_repl_cmrootdir.strip() != ""
    ):
        params.HdfsResource(
