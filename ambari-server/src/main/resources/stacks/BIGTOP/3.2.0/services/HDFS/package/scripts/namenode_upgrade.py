...
def prepare_upgrade_enter_safe_mode(hdfs_binary):
    ...
    try:
        ...
        Logger.info(
            "Transition successful: {0}, original state: {1}".format(
...
def prepare_upgrade_save_namespace(hdfs_binary):
    ...
    try:
        ...
        as_user(
            save_namespace_cmd, params.hdfs_user, env={"PATH": params.hadoop_bin_dir}
        )
    except Exception as e:
        message = format(
    ...
    try:
        ...
        if out:
            ...
            if expected_substring not in out.lower():
                Logger.warning(
                    "Finalize command did not contain substring: %s"
                    % expected_substring
                )
        else:
    ...
    if code == 0 and out is not None:
        ...
        if m and len(m.groups()) >= 1:
            ...
            if original_state == safemode_state:
                ...
                # Make a transition
                command = (
                    safemode_base_command + safemode_to_instruction[safemode_state]
                )
                Execute(
                    command, user=user, logoutput=True, path=[params.hadoop_bin_dir]
                )
...
def prepare_rolling_upgrade(hdfs_binary):
    ...
    if not params.upgrade_direction or params.upgrade_direction not in [
        ...
        raise Fail(
            "Could not retrieve upgrade direction: %s" % str(params.upgrade_direction)
        )
...
def create_upgrade_marker():
    ...
    try:
        ...
        Logger.warning(
            "Unable to create NameNode upgrade marker file {0}".format(
