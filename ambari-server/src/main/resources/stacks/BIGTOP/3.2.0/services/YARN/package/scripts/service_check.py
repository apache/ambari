    ...
    def get_active_rm_webapp_address(self):
        ...
        if rm_webapp_addresses is not None and len(rm_webapp_addresses) > 0:
            for rm_webapp_address in rm_webapp_addresses:
                rm_state_url = (
                    params.scheme + "://" + rm_webapp_address + "/ws/v1/cluster/info"
                )
        ...
        if active_rm_webapp_address is None:
            raise Fail(
                "Resource Manager state is not available. Failed to determine the active Resource Manager web application address from {0}".format(
                    ",".join(rm_webapp_addresses)
                )
            )
