...
def setup_ranger_hive(upgrade_type=None):
  ...
  if params.enable_ranger_hive:
    ...
    if params.retryAble:
      Logger.info(
        "Hive: Setup ranger: command retry enables thus retrying if ranger admin is down !"
      )
    else:
      Logger.info(
        "Hive: Setup ranger: command retry not enabled thus skipping if ranger admin is down !"
      )
    ...
    setup_ranger_plugin(
      ...
      conf_dict=params.hive_conf_dir,
...
def setup_ranger_hive_metastore_service():
  ...
  if params.has_ranger_admin and params.ranger_hive_metastore_lookup:
    ...
    repo_name_value = params.config["configurations"]["ranger-hive-security"][
    ...
    hive_ranger_plugin_config = {
      "username": params.config["configurations"]["ranger-hive-plugin-properties"][
        "REPOSITORY_CONFIG_USERNAME"
      ],
      "password": params.config["configurations"]["ranger-hive-plugin-properties"][
        "REPOSITORY_CONFIG_PASSWORD"
      ],
      "jdbc.driverClassName": params.config["configurations"][
        "ranger-hive-plugin-properties"
      ]["jdbc.driverClassName"],
      "jdbc.url": "none",
      "commonNameForCertificate": params.config["configurations"][
        "ranger-hive-plugin-properties"
      ]["common.name.for.certificate"],
      "ambari.service.check.user": params.config["configurations"][
        "ranger-hive-plugin-properties"
      ]["policy_user"],
    }
    ...
    ranger_admin_obj.create_ranger_repository(
      ...
      ambari_ranger_password=params.config["configurations"]["ranger-env"][
      ...
      policy_user=params.config["configurations"]["ranger-hive-plugin-properties"][
        "policy_user"
      ],
      is_security_enabled=params.security_enabled,
      is_stack_supports_ranger_kerberos=params.stack_supports_ranger_kerberos,
