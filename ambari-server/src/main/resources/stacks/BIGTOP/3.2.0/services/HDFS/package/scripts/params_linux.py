...
rolling_restart_safemode_exit_timeout = default(
    "/configurations/cluster-env/namenode_rolling_restart_safemode_exit_timeout", None
)
...
namenode_backup_dir = default(
    "/configurations/hadoop-env/namenode_backup_dir", "/tmp/upgrades"
)
...
if not security_enabled:
    ...
    if dfs_http_policy == "HTTPS_ONLY":
        secure_dn_ports_are_in_use = utils.is_secure_port(
            dfs_dn_port
        ) or utils.is_secure_port(dfs_dn_https_port)
    elif dfs_http_policy == "HTTP_AND_HTTPS":
        secure_dn_ports_are_in_use = (
            utils.is_secure_port(dfs_dn_port)
            or utils.is_secure_port(dfs_dn_http_port)
            or utils.is_secure_port(dfs_dn_https_port)
        )
...
skip_namenode_save_namespace_express = default(
    "/configurations/cluster-env/stack_upgrade_express_skip_namenode_save_namespace",
    False,
)
skip_namenode_namedir_backup_express = default(
    "/configurations/cluster-env/stack_upgrade_express_skip_backup_namenode_dir", False
)
...
create_lib_snappy_symlinks = check_stack_feature(
    StackFeature.SNAPPY, stack_version_formatted
)
...
if namenode_federation_enabled(hdfs_site):
    jn_edits_dirs = list(
        get_properties_for_all_nameservices(
            hdfs_site, "dfs.journalnode.edits.dir"
        ).values()
    )
else:
...
fs_checkpoint_dirs = default(
    "/configurations/hdfs-site/dfs.namenode.checkpoint.dir", ""
).split(",")

dfs_data_dirs = config["configurations"]["hdfs-site"]["dfs.datanode.data.dir"]
dfs_data_dirs_perm = default(
    "/configurations/hdfs-site/dfs.datanode.data.dir.perm", "755"
)
dfs_data_dirs_perm = int(
    dfs_data_dirs_perm, base=8
)  # convert int from octal representation
...
dfs_ha_namenode_standby = default(
    "/configurations/cluster-env/dfs_ha_initial_namenode_standby", None
)
...
zk_namespace = default(
    "/configurations/hdfs-site/ha.zookeeper.parent-znode", "/hadoop-ha"
)
    ...
    if dfs_ha_enabled:
        for nn_id in dfs_ha_namemodes_ids_list:
            nn_host = config["configurations"]["hdfs-site"][
                format("dfs.namenode.rpc-address.{ns}.{nn_id}")
            ]
            if (
                hostname.lower() in nn_host.lower()
                or public_hostname.lower() in nn_host.lower()
            ):
                namenode_id = nn_id
        ...
        if namenode_id is not None and len(dfs_ha_namemodes_ids_list) == 2:
            other_namenode_id = list(
                set(dfs_ha_namemodes_ids_list) - set([namenode_id])
            )[0]
...
mapred_log_dir_prefix = default(
    "/configurations/mapred-env/mapred_log_dir_prefix", "/var/log/hadoop-mapreduce"
)

hadoop_security_authentication = config["configurations"]["core-site"][
    "hadoop.security.authentication"
]
hadoop_security_authorization = config["configurations"]["core-site"][
...
enable_ranger_hdfs = default(
    "/configurations/ranger-hdfs-plugin-properties/ranger-hdfs-plugin-enabled", "No"
)
...
if enable_ranger_hdfs:
    ...
    if xml_configurations_supported:
        policymgr_mgr_url = config["configurations"]["ranger-hdfs-security"][
    ...
    repo_name_value = config["configurations"]["ranger-hdfs-security"][
        "ranger.plugin.hdfs.service.name"
    ]
    if not is_empty(repo_name_value) and repo_name_value != "{{repo_name}}":
        repo_name = repo_name_value

    hadoop_rpc_protection = config["configurations"]["ranger-hdfs-plugin-properties"][
        "hadoop.rpc.protection"
    ]
    common_name_for_certificate = config["configurations"][
        "ranger-hdfs-plugin-properties"
    ]["common.name.for.certificate"]
    repo_config_username = config["configurations"]["ranger-hdfs-plugin-properties"][
    ...
    if not has_ranger_admin and enable_ranger_hdfs:
        external_admin_username = default(
            "/configurations/ranger-hdfs-plugin-properties/external_admin_username",
            "admin",
        )
        external_admin_password = default(
            "/configurations/ranger-hdfs-plugin-properties/external_admin_password",
            "admin",
        )
    ...
    policy_user = config["configurations"]["ranger-hdfs-plugin-properties"][
        "policy_user"
    ]
    ...
    if has_ranger_admin and stack_supports_ranger_audit_db:
        ...
        driver_curl_target = (
            format("{hadoop_lib_home}/{jdbc_jar_name}")
            if stack_supports_ranger_audit_db
            else None
        )
        previous_jdbc_jar = (
            format("{hadoop_lib_home}/{previous_jdbc_jar_name}")
            if stack_supports_ranger_audit_db
            else None
        )
        sql_connector_jar = ""
    ...
    hdfs_ranger_plugin_config = {
        ...
        "dfs.datanode.kerberos.principal": dn_principal_name
        if security_enabled
        else "",
        "dfs.namenode.kerberos.principal": nn_principal_name
        if security_enabled
        else "",
        "dfs.secondary.namenode.kerberos.principal": sn_principal_name
        if security_enabled
        else "",
    }
    ...
    if xml_configurations_supported and stack_supports_ranger_audit_db:
        xa_audit_db_is_enabled = config["configurations"]["ranger-hdfs-audit"][
    ...
    ssl_keystore_password = (
        config["configurations"]["ranger-hdfs-policymgr-ssl"][
