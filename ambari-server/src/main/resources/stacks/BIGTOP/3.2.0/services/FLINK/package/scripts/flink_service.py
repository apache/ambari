  ...
  if action == "start":
    if name == "historyserver":
      ...
      historyserver_no_op_test = (
        as_sudo(["test", "-f", params.flink_history_server_pid_file])
        + " && "
        + as_sudo(["pgrep", "-F", params.flink_history_server_pid_file])
      )
      try:
        Execute(
          params.flink_history_server_start,
    ...
    if name == "historyserver":
      try:
        Execute(
          format("{flink_history_server_stop}"),
          user=params.flink_user,
          environment={"JAVA_HOME": params.java_home},
        )
      except:
      ...
      File(params.flink_history_server_pid_file, action="delete")
