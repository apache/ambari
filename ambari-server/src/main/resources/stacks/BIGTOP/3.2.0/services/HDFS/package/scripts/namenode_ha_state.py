        ...
        for name_service in self.name_services:
            ...
            for nn_unique_id in self.nn_unique_ids:
                ...
                try:
                    if not hostname:
                        raise Exception(
                            "Could not retrieve hostname from address " + actual_value
                        )
                    ...
                    if not state:
                        run_user = default(
                            "/configurations/hadoop-env/hdfs_user", "hdfs"
                        )
                        check_service_cmd = "hdfs haadmin -ns {dfs_ha_nameservices} -getServiceState {0}".format(
                            nn_unique_id
                        )
                        code, out = shell.call(
                            check_service_cmd, logoutput=True, user=run_user
                        )
                        if code == 0 and out:
                    ...
                    if not state:
                        raise Exception(
                            "Could not retrieve Namenode state from URL " + jmx_uri
                        )
