...
def pre_rolling_upgrade_shutdown(hdfs_binary):
    ...
    if code != 0:
        ...
        if output is not None and re.search("Shutdown already in progress", output):
            Logger.error(
                "Due to a known issue in DataNode, the command {0} did not work, so will need to shutdown the datanode forcefully.".format(
                    command
                )
            )
            return False
...
def _check_datanode_startup(hdfs_binary):
    ...
    if return_code == 0:
        ...
        if hostname in hdfs_output.lower() or hostname_ip in hdfs_output.lower():
            Logger.info(
                "DataNode {0} reports that it has rejoined the cluster.".format(
                    params.hostname
                )
            )
            return
            ...
            raise Fail(
                "DataNode {0} was not found in the list of live DataNodes".format(
                    params.hostname
                )
            )
    ...
    raise Fail(
        "Unable to determine if the DataNode has started after upgrade (result code {0})".format(
            str(return_code)
        )
    )
