...
def create_hbase_package():
    ...
    try:
        File(
            file_path, mode=0o755, content=Template("yarn_hbase_package_preparation.j2")
        )
        Execute(file_path, timeout=300, logoutput=True)
    except:
        Logger.error(
            "Error occured while executing hbase package creation file '"
            + file_path
            + "'."
        )
        raise
...
def copy_hbase_package_to_hdfs():
    ...
    try:
        Logger.info(
            "Copying hbase tarball into hdfs path'"
