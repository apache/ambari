...
class HiveServiceAdvisor(service_advisor.ServiceAdvisor):
  ...
  def isKerberosEnabled(services, configurations):
    ...
    if (
      ...
      and "hive.server2.authentication" in configurations["hive-site"]["properties"]
    ):
      return (
        configurations["hive-site"]["properties"]["hive.server2.authentication"].lower()
        == "kerberos"
      )
    elif (
      services
  ...
  def recommendHiveConfigurationsFromHDP30(
    ...
    if "DRUID" in servicesList:
      ...
      if druid_broker_host_port is None:
        druid_broker_host_port = self.druid_host(
          "DRUID_BROKER", "druid-broker", services, hosts, default_host="localhost:8083"
        )
      ...
      if "druid-common" in services["configurations"]:
        druid_metadata_uri = services["configurations"]["druid-common"]["properties"][
          "druid.metadata.storage.connector.connectURI"
        ]
        druid_metadata_type = services["configurations"]["druid-common"]["properties"][
    ...
    if hiveEnvProperties and self.checkSiteProperties(
      ...
      putHiveEnvProperty(
        "hive_database_type", self.getDBTypeAlias(hiveEnvProperties["hive_database"])
      )
    if (
      hiveEnvProperties
    ...
    if (
      ...
      and "hive.cbo.enable" in services["configurations"]["hive-site"]["properties"]
    ):
    ...
    if (
      ...
      pass
    putHiveSiteProperty(
      "hive.security.metastore.authorization.manager", ",".join(auth_manager_values)
    )
    ...
    if hive_server_hosts is not None and len(hive_server_hosts):
      ...
      putHiveEnvProperty(
        "hive.metastore.heapsize", max(512, int(hs_host_ram * hm_heapsize_multiplier))
      )
      putHiveEnvProperty(
    ...
    if (
      ...
      in services["configurations"]["hive-atlas-application.properties"]["properties"]
    ):
    ...
    if (
      ...
      "hive-env" in services["configurations"]
      and "hive.atlas.hook" in services["configurations"]["hive-env"]["properties"]
    ):
      enable_atlas_hook = (
    ...
    if is_atlas_present_in_cluster and atlas_server_host_info:
      ...
      if "application-properties" in services["configurations"]:
        ...
        if (
          ...
          metadata_port = services["configurations"]["application-properties"][
            "properties"
          ]["atlas.server.http.port"]
      ...
      putHiveSiteProperty(
        "atlas.rest.address", f"{scheme}://{atlas_rest_host}:{metadata_port}"
      )
    else:
    ...
    if len(hsi_hosts) > 0:
      ...
      if hsi_properties and "hive.llap.daemon.queue.name" in hsi_properties:
        ...
        if hive_tez_default_queue:
          ...
          self.logger.debug(
            f"Updated 'hive.server2.tez.default.queues' config : '{hive_tez_default_queue}'"
          )
    else:
      self.logger.info(
    ...
    if (
      "hive-env" in configurations
      and "hive_security_authorization" in configurations["hive-env"]["properties"]
    ):
      ranger_hive_plugin_enabled = (
        configurations["hive-env"]["properties"]["hive_security_authorization"].lower()
        == "ranger"
      )
    elif (
      "hive-env" in services["configurations"]
...
class HiveValidator(service_advisor.ServiceAdvisor):
        ...
        validationItems.append(
          {
            "config-name": prop_name,
            "item": self.getWarnItem(
              "If Hive using SQL Anywhere db." " {0} needs to be set to {1}".format(
  ...
  def validateHiveConfigurationsEnvFromHDP30(
    ...
    if (
      ...
      validationItems.append(
        {"config-name": "hive_security_authorization", "item": authorization_item}
      )
    if (
      "hive_security_authorization" in hive_env
  ...
  def validateHiveInteractiveEnvConfigurationsFromHDP30(
    ...
    if len(hsi_hosts) > 0:
      ...
      if "enable_hive_interactive" not in hive_site_env_properties or (
        ...
        and hive_site_env_properties["enable_hive_interactive"].lower() != "true"
      ):
        validationItems.append(
          {
      ...
      if (
        "enable_hive_interactive" in hive_site_env_properties
        and hive_site_env_properties["enable_hive_interactive"].lower() != "false"
      ):
        validationItems.append(
          {
            "config-name": "enable_hive_interactive",
    ...
    if hsi_site:
      if (
        ...
        if llap_queue_cap:
          ...
          min_reqd_queue_cap_perc = self.min_queue_perc_reqd_for_llap_and_hive_app(
          ...
          if llap_queue_cap_perc < min_reqd_queue_cap_perc:
            errMsg1 = (
              ...
              "app to run".format(
                llap_queue_name, llap_queue_cap_perc, min_reqd_queue_cap_perc
              )
            )
            validationItems.append(
              {
        ...
        validationItems.append(
          {
            ...
            "item": self.getErrorItem("Value should be set to 'false' for Hive2."),
          }
        )
  ...
  def get_yarn_nm_mem_in_mb(self, services, configurations):
    ...
    if (
      not ("changed-configurations" in services and services["changed-configurations"])
