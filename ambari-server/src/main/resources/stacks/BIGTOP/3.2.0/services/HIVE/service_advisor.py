...
class HiveServiceAdvisor(service_advisor.ServiceAdvisor):
    ...
    def getServiceConfigurationRecommendations(
        ...
        recommender.recommendHiveConfigurationsFromHDP30(
    ...
    def isKerberosEnabled(services, configurations):
        ...
        if (
            configurations
            and "hive-site" in configurations
            and "hive.server2.authentication"
            in configurations["hive-site"]["properties"]
        ):
            return (
...
class HiveRecommender(service_advisor.ServiceAdvisor):
    ...
    def recommendHiveConfigurationsFromHDP30(
        self, configurations, clusterData, services, hosts
    ):
        hiveSiteProperties = self.getSiteProperties(
            services["configurations"], "hive-site"
        )
        hiveEnvProperties = self.getSiteProperties(
            services["configurations"], "hive-env"
        )

        putHiveEnvProperty = self.putProperty(configurations, "hive-env", services)
        putHiveSiteProperty = self.putProperty(configurations, "hive-site", services)
        putHiveProperty = self.putProperty(configurations, "hive-site", services)
        putHiveServerProperty = self.putProperty(
        ...
        putHiveInteractiveSiteProperty = self.putProperty(
            configurations, self.HIVE_INTERACTIVE_SITE, services
        )
        putRangerHivePluginProperty = self.putProperty(
            configurations, "ranger-hive-plugin-properties", services
        )
        putHiveAtlasHookProperty = self.putProperty(
            configurations, "hive-atlas-application.properties", services
        )

        putHiveSitePropertyAttribute = self.putPropertyAttribute(
            configurations, "hive-site"
        )
        putHiveEnvPropertyAttribute = self.putPropertyAttribute(
        ...
        putHiveInteractiveSitePropertyAttribute = self.putPropertyAttribute(
            configurations, "hive-interactive-site"
        )
        putHiveAtlasHookPropertyAttribute = self.putPropertyAttribute(
        ...
        hive_client_hosts = self.getHostsWithComponent(
            "HIVE", "HIVE_CLIENT", services, hosts
        )
        ...
        if "DRUID" in servicesList:
            putHiveInteractiveSiteProperty = self.putProperty(
                configurations, "hive-interactive-site", services
            )

            druid_coordinator_host_port = self.druid_host(
                "DRUID_COORDINATOR",
                "druid-coordinator",
                services,
                hosts,
                default_host="localhost:8081",
            )
            druid_overlord_host_port = self.druid_host(
                "DRUID_OVERLORD",
                "druid-overlord",
                services,
                hosts,
                default_host="localhost:8090",
            )
            druid_broker_host_port = self.druid_host(
                "DRUID_ROUTER", "druid-router", services, hosts
            )
            ...
            if "druid-common" in services["configurations"]:
                druid_metadata_uri = services["configurations"]["druid-common"][
                    "properties"
                ]["druid.metadata.storage.connector.connectURI"]
                druid_metadata_type = services["configurations"]["druid-common"][
                    "properties"
                ]["druid.metadata.storage.type"]
                if (
                    "druid.metadata.storage.connector.user"
                    in services["configurations"]["druid-common"]["properties"]
                ):
                    druid_metadata_user = services["configurations"]["druid-common"][
                        "properties"
                    ]["druid.metadata.storage.connector.user"]
                else:
                    druid_metadata_user = ""
            ...
            putHiveInteractiveSiteProperty(
                "hive.druid.broker.address.default", druid_broker_host_port
            )
            putHiveInteractiveSiteProperty(
                "hive.druid.coordinator.address.default", druid_coordinator_host_port
            )
            putHiveInteractiveSiteProperty(
                "hive.druid.overlord.address.default", druid_overlord_host_port
            )
            putHiveInteractiveSiteProperty(
                "hive.druid.metadata.uri", druid_metadata_uri
            )
            putHiveInteractiveSiteProperty(
                "hive.druid.metadata.username", druid_metadata_user
            )
            putHiveInteractiveSiteProperty(
                "hive.druid.metadata.db.type", druid_metadata_type
            )

        # javax.jdo.option.ConnectionURL recommendations
        if hiveEnvProperties and self.checkSiteProperties(
            hiveEnvProperties, "hive_database", "hive_database_type"
        ):
            putHiveEnvProperty(
                "hive_database_type",
        ...
        if (
            hiveEnvProperties
            and hiveSiteProperties
            and self.checkSiteProperties(
                hiveSiteProperties, "javax.jdo.option.ConnectionDriverName"
            )
            and self.checkSiteProperties(hiveEnvProperties, "hive_database")
        ):
            putHiveProperty(
        ...
        if (
            ...
            hiveServerHost = self.getHostWithComponent(
                "HIVE", "HIVE_SERVER", services, hosts
            )
            ...
            oldSchemaName = self.getOldValue(
                services, "hive-site", "ambari.hive.db.schema.name"
            )
            ...
            if hiveServerHost is not None:
                if (
                    (hiveDBConnectionURL and "//localhost" in hiveDBConnectionURL)
                    or oldSchemaName
                    or oldDBType
                    or (
                        protocol
                        and hiveDBConnectionURL
                        and not hiveDBConnectionURL.startswith(protocol)
                    )
                ):
                    dbConnection = self.getDBConnectionString(
                        hiveEnvProperties["hive_database"]
                    ).format(
                        hiveServerHost["Hosts"]["host_name"],
        ...
        putHiveSiteProperty(
            "hive.compactor.worker.threads", str(round(max(cpu_count / 8, 1)))
        )

        hiveMetastoreHost = self.getHostWithComponent(
            "HIVE", "HIVE_METASTORE", services, hosts
        )
        if hiveMetastoreHost is not None and len(hiveMetastoreHost) > 0:
            putHiveSiteProperty(
                "hive.metastore.uris",
                "thrift://" + hiveMetastoreHost["Hosts"]["host_name"] + ":9083",
            )
        ...
        containerSize = min(
            clusterData["containers"] * clusterData["ramPerContainer"], containerSize
        )
        ...
        if "yarn-site" in configurations:
            if (
                "yarn.scheduler.minimum-allocation-mb"
                in configurations["yarn-site"]["properties"]
            ):
                putHiveSitePropertyAttribute(
                    "hive.tez.container.size",
                    "minimum",
        ...
        tez_container_size_bytes = int(
            int(tez_container_size) * 0.8 * 1024 * 1024
        )  # Xmx == 80% of container

        # Memory
        putHiveSiteProperty(
            "hive.auto.convert.join.noconditionaltask.size",
            int(round(tez_container_size_bytes / 3)),
        )
        putHiveSitePropertyAttribute(
            "hive.auto.convert.join.noconditionaltask.size",
            "maximum",
            tez_container_size_bytes,
        )

        # CBO
        if (
            "hive-site" in services["configurations"]
            and "hive.cbo.enable"
            in services["configurations"]["hive-site"]["properties"]
        ):
            hive_cbo_enable = services["configurations"]["hive-site"]["properties"][
                "hive.cbo.enable"
            ]
        ...
        if "capacity-scheduler" in services["configurations"]:
            ...
            if "yarn.scheduler.capacity.root.queues" in capacitySchedulerProperties:
                yarn_queues = str(
                    capacitySchedulerProperties["yarn.scheduler.capacity.root.queues"]
                )
            elif (
        ...
        while len(toProcessQueues) > 0:
            ...
            if queueKey in capacitySchedulerProperties:
                ...
                queueName = queue.split(
                    ...
                    -1
                ]  # Fully qualified queue name does not work, we should use only leaf name
                leafQueueNames.add(queueName)
        leafQueues = [
            {"label": str(queueName) + " queue", "value": queueName}
            for queueName in leafQueueNames
        ]
        leafQueues = sorted(leafQueues, key=lambda q: q["value"])
        putHiveSitePropertyAttribute(
            "hive.server2.tez.default.queues", "entries", leafQueues
        )
        putHiveSiteProperty(
        ...
        # Security
        if (
            ("configurations" not in services)
            or ("hive-env" not in services["configurations"])
            or ("properties" not in services["configurations"]["hive-env"])
            or (
                "hive_security_authorization"
                not in services["configurations"]["hive-env"]["properties"]
            )
            or str(
        ...
        if (
            ...
            and "hive-env" in services["configurations"]
            ...
            rangerEnvHiveAuthProperty = services["configurations"]["hive-env"][
                "properties"
            ]["hive_security_authorization"]
        ...
        if (
            ...
            auth_manager_values = [
                x for x in auth_manager_values if x != sqlstdauth_class
            ]
            pass
        putHiveSiteProperty(
            "hive.security.metastore.authorization.manager",
        ...
        if (
            ...
            putHiveServerPropertyAttribute(
        ...
        if (
            ...
            hive_server2_auth = str(
                configurations["hive-site"]["properties"]["hive.server2.authentication"]
            ).lower()

        if hive_server2_auth == "ldap":
            putHiveSiteProperty("hive.server2.authentication.ldap.url", "")
            putHiveSitePropertyAttribute(
                "hive.server2.authentication.ldap.url", "delete", "false"
            )
        else:
            if (
                (
                ...
                or ("hive-site" not in services["configurations"])
                or (
                    "hive-site" in services["configurations"]
                    and "hive.server2.authentication.ldap.url"
                    in services["configurations"]["hive-site"]["properties"]
                )
            ):
                putHiveSitePropertyAttribute(
        ...
        if hive_server2_auth == "kerberos":
            if (
                "hive-site" in services["configurations"]
                and "hive.server2.authentication.kerberos.keytab"
                not in services["configurations"]["hive-site"]["properties"]
            ):
                putHiveSiteProperty("hive.server2.authentication.kerberos.keytab", "")
            if (
                "hive-site" in services["configurations"]
                and "hive.server2.authentication.kerberos.principal"
                not in services["configurations"]["hive-site"]["properties"]
            ):
                putHiveSiteProperty(
            ...
            "KERBEROS" not in servicesList
        ):  # Since "hive_server2_auth" cannot be relied on within the default, empty recommendations request
            if (
                (
        ...
        if hive_server2_auth == "pam":
            ...
            if (
                ...
                or (
                    "hive-site" in services["configurations"]
                    and "hive.server2.authentication.pam.services"
                    in services["configurations"]["hive-site"]["properties"]
                )
            ):
                putHiveSitePropertyAttribute(
        ...
        if hive_server2_auth == "custom":
            ...
            if (
                ...
                or (
                    "hive-site" in services["configurations"]
                    and "hive.server2.custom.authentication.class"
                    in services["configurations"]["hive-site"]["properties"]
                )
            ):
                putHiveSitePropertyAttribute(
        ...
        if hive_server_hosts is not None and len(hive_server_hosts):
            ...
            putHiveEnvProperty(
                ...
                max(512, int(hs_host_ram * hm_heapsize_multiplier)),
            )
            putHiveEnvProperty(
                "hive.heapsize", max(512, int(hs_host_ram * hs_heapsize_multiplier))
            )
            putHiveEnvPropertyAttribute(
                "hive.metastore.heapsize", "maximum", max(1024, hs_host_ram)
            )
            putHiveEnvPropertyAttribute(
                "hive.heapsize", "maximum", max(1024, hs_host_ram)
            )

        # if hive using sqla db, then we should add DataNucleus property
        sqla_db_used = (
            "hive-env" in services["configurations"]
            and "hive_database" in services["configurations"]["hive-env"]["properties"]
            and services["configurations"]["hive-env"]["properties"]["hive_database"]
        ...
        if (
            "hive-atlas-application.properties" in services["configurations"]
            and "enable.external.atlas.for.hive"
            in services["configurations"]["hive-atlas-application.properties"][
                "properties"
            ]
        ):
            enable_external_atlas_for_hive = (
                services["configurations"]["hive-atlas-application.properties"][
                    "properties"
                ]["enable.external.atlas.for.hive"].lower()
                == "true"
            )
        ...
        if (
            "hive-env" in configurations
            and "hive.atlas.hook" in configurations["hive-env"]["properties"]
        ):
            enable_atlas_hook = (
            ...
            "hive-env" in services["configurations"]
            and "hive.atlas.hook"
            in services["configurations"]["hive-env"]["properties"]
        ):
            enable_atlas_hook = (
        ...
        atlas_server_host_info = self.getHostWithComponent(
            "ATLAS", "ATLAS_SERVER", services, hosts
        )
        if is_atlas_present_in_cluster and atlas_server_host_info:
            ...
            if "application-properties" in services["configurations"]:
                if (
                    "atlas.enableTLS"
                    in services["configurations"]["application-properties"][
                        "properties"
                    ]
                ):
                    tls_enabled = services["configurations"]["application-properties"][
                        "properties"
                    ]["atlas.enableTLS"]
                if (
                    ...
                    metadata_port = services["configurations"][
                        "application-properties"
                    ]["properties"]["atlas.server.http.port"]
                if tls_enabled.lower() == "true":
                    ...
                    if (
                        ...
                        metadata_port = services["configurations"][
                            "application-properties"
                        ]["properties"]["atlas.server.https.port"]
                    else:
                        metadata_port = atlas_server_default_https_port
            putHiveSiteProperty(
                "atlas.rest.address",
                "{0}://{1}:{2}".format(scheme, atlas_rest_host, metadata_port),
            )
        else:
        ...
        hsi_hosts = self.getHostsForComponent(
            services, "HIVE", "HIVE_SERVER_INTERACTIVE"
        )
        hsi_properties = self.getServicesSiteProperties(
            services, self.HIVE_INTERACTIVE_SITE
        )
        ...
        if len(hsi_hosts) > 0:
            ...
            if hsi_properties and "hive.llap.daemon.queue.name" in hsi_properties:
                ...
                hsi_conf_properties = self.getSiteProperties(
                    configurations, self.HIVE_INTERACTIVE_SITE
                )

                hive_tez_default_queue = hsi_properties["hive.llap.daemon.queue.name"]
                if (
                    hsi_conf_properties
                    and "hive.llap.daemon.queue.name" in hsi_conf_properties
                ):
                    hive_tez_default_queue = hsi_conf_properties[
                        "hive.llap.daemon.queue.name"
                    ]

                if hive_tez_default_queue:
                    putHiveInteractiveSiteProperty(
                        "hive.server2.tez.default.queues", hive_tez_default_queue
                    )
                    self.logger.debug(
                        "Updated 'hive.server2.tez.default.queues' config : '{0}'".format(
        ...
        if hsi_properties and "hive.llap.zk.sm.connectionString" in hsi_properties:
            ...
            if zookeeper_host_port:
                putHiveInteractiveSiteProperty(
                    "hive.llap.zk.sm.connectionString", zookeeper_host_port
                )

        hive_user = "hive"
        if (
            "hive-env" in services["configurations"]
            and "hive_user" in services["configurations"]["hive-env"]["properties"]
        ):
            hive_user = services["configurations"]["hive-env"]["properties"][
                "hive_user"
            ]
        ...
        if (
            "hive-env" in configurations
            and "hive_security_authorization"
            in configurations["hive-env"]["properties"]
        ):
        ...
        if (
            ...
            in services["configurations"]["ranger-hive-plugin-properties"]["properties"]
        ):
        ...
        if "hive-atlas-application.properties" in services["configurations"]:
            security_enabled = HiveServiceAdvisor.isKerberosEnabled(
            ...
            if (
                ...
                enable_atlas_hook = (
                    configurations["hive-env"]["properties"]["hive.atlas.hook"].lower()
                    == "true"
                )
            elif (
                "hive-env" in services["configurations"]
            ...
            if security_enabled and enable_atlas_hook:
                putHiveAtlasHookProperty(
                    "atlas.jaas.ticketBased-KafkaClient.loginModuleControlFlag",
                    "required",
                )
                putHiveAtlasHookProperty(
                    "atlas.jaas.ticketBased-KafkaClient.loginModuleName",
                ...
                putHiveAtlasHookPropertyAttribute(
                    "atlas.jaas.ticketBased-KafkaClient.option.useTicketCache",
                    "delete",
                    "true",
                )
        ...
        beeline_jdbc_url_default = (
            "llap"
            if (hive_server_interactive_hosts and not hive_server_hosts)
            else "container"
        )
        putHiveEnvProperty("beeline_jdbc_url_default", beeline_jdbc_url_default)

    def druid_host(
        self, component_name, config_type, services, hosts, default_host=None
    ):
    ...
    def setLlapDaemonQueuePropAttributes(self, services, configurations):
        ...
        if "capacity-scheduler" in configurations:
            ...
            if "capacity-scheduler" in cap_sched_props_as_dict:
                cap_sched_props_as_str = configurations["capacity-scheduler"][
                    "properties"
                ]["capacity-scheduler"]
            ...
            if not capacity_scheduler_properties:
                if (
                    isinstance(cap_sched_props_as_dict, dict)
                    and len(cap_sched_props_as_dict) > 1
                ):
                    ...
                    self.logger.info(
                        "Read configurations['capacity-scheduler']['properties'] is : {0}".format(
                            cap_sched_props_as_dict
                        )
                    )
        else:
            self.logger.info(
        ...
        if leafQueueNames:
            leafQueues = [
                {"label": str(queueName), "value": queueName}
                for queueName in leafQueueNames
            ]
            leafQueues = sorted(leafQueues, key=lambda q: q["value"])
            putHiveInteractiveSitePropertyAttribute(
            ...
            self.logger.info(
                "'hive.llap.daemon.queue.name' config Property Attributes set to : {0}".format(
                    leafQueues
                )
            )
        else:
...
class HiveValidator(service_advisor.ServiceAdvisor):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.validators = [
            ("hive-site", self.validateHiveConfigurationsFromHDP30),
            ("hive-env", self.validateHiveConfigurationsEnvFromHDP30),
            ("hiveserver2-site", self.validateHiveServer2ConfigurationsFromHDP30),
            (
                "hive-interactive-env",
                self.validateHiveInteractiveEnvConfigurationsFromHDP30,
            ),
            (
                "hive-interactive-site",
                self.validateHiveInteractiveSiteConfigurationsFromHDP30,
            ),
        ]

    def validateHiveConfigurationsFromHDP30(
        ...
        validationItems = [
            {
                ...
                "item": self.validatorLessThenDefaultValue(
                    properties, recommendedDefaults, "hive.tez.container.size"
                ),
            },
            {
        ...
        if yarn_site:
            yarnSchedulerMaximumAllocationMb = self.to_number(
                yarn_site["yarn.scheduler.maximum-allocation-mb"]
            )
            hiveTezContainerSize = self.to_number(properties["hive.tez.container.size"])
            if (
                hiveTezContainerSize is not None
                and yarnSchedulerMaximumAllocationMb is not None
                and hiveTezContainerSize > yarnSchedulerMaximumAllocationMb
            ):
                validationItems.append(
                    {
                        "config-name": "hive.tez.container.size",
                        "item": self.getWarnItem(
                            "hive.tez.container.size is greater than the maximum container size specified in yarn.scheduler.maximum-allocation-mb"
                        ),
                    }
                )
        ...
        if (
            authentication_property in properties
            and properties[authentication_property].lower() == "ldap"
            and not (
                ldap_baseDN_property in properties or ldap_domain_property in properties
            )
        ):
            validationItems.append(
                {
                    "config-name": authentication_property,
                    "item": self.getWarnItem(
                        "According to LDAP value for "
                        + authentication_property
                        + ", you should add "
                        + ldap_domain_property
                        + " property, if you are using AD, if not, then "
        ...
        if (
            hive_enforce_bucketing in properties
            and properties[hive_enforce_bucketing].lower() == "false"
        ):
            validationItems.append(
                {
                    "config-name": hive_enforce_bucketing,
                    "item": self.getWarnItem(
                        "Set "
                        + hive_enforce_bucketing
                        + " to true otherwise there is a potential of data corruption!"
                    ),
                }
            )

        sqla_db_used = (
            "hive_database" in hive_env
            and hive_env["hive_database"] == "Existing SQL Anywhere Database"
        )
        ...
        if sqla_db_used:
            if not prop_name in hive_site:
                validationItems.append(
                    {
                        ...
                        "item": self.getWarnItem(
                            ...
                            " {0} needs to be added with value {1}".format(
                ...
                prop_name in hive_site
                and hive_site[prop_name]
                != "org.datanucleus.store.rdbms.adapter.SQLAnywhereAdapter"
            ):
                validationItems.append(
                    {
    ...
    def validateHiveConfigurationsEnvFromHDP30(
        self, properties, recommendedDefaults, configurations, services, hosts
    ):
        validationItems = []
        ...
        if (
            "hive_security_authorization" in hive_env
            and str(hive_env["hive_security_authorization"]).lower() == "none"
            and str(hiveserver2_site["hive.security.authorization.enabled"]).lower()
            == "true"
        ):
            authorization_item = self.getErrorItem(
                "hive_security_authorization should not be None "
                "if hive.security.authorization.enabled is set"
            )
            validationItems.append(
                {
                    "config-name": "hive_security_authorization",
                    "item": authorization_item,
                }
            )
        if (
            "hive_security_authorization" in hive_env
            and str(hive_env["hive_security_authorization"]).lower() == "ranger"
        ):
            ...
            if "RANGER" in servicesList:
                ...
                if (
                    not ranger_env
                    or not "ranger-hive-plugin-enabled" in ranger_env
                    or ranger_env["ranger-hive-plugin-enabled"].lower() != "yes"
                ):
                    validationItems.append(
                        {
                            "config-name": "hive_security_authorization",
        ...
        if (
            ...
            and "LDAP" == hive_site["hive.server2.authentication"]
        ):
            if (
                "alert_ldap_username" not in hive_env
                or hive_env["alert_ldap_username"] == ""
            ):
                validationItems.append(
                    {
                        "config-name": "alert_ldap_username",
                        "item": self.getWarnItem(
            ...
            if (
                "alert_ldap_password" not in hive_env
                or hive_env["alert_ldap_password"] == ""
            ):
                validationItems.append(
                    {
                        "config-name": "alert_ldap_password",
                        "item": self.getWarnItem(
        ...
        if beeline_jdbc_url_default not in ["container", "llap"]:
            validationItems.append(
                {
                    "config-name": "beeline_jdbc_url_default",
                    "item": self.getWarnItem(
        ...
        if (
            ...
            and not hive_server_hosts
            and hive_server_interactive_hosts
        ):
            validationItems.append(
                {
                    "config-name": "beeline_jdbc_url_default",
    ...
    def validateHiveServer2ConfigurationsFromHDP30(
        self, properties, recommendedDefaults, configurations, services, hosts
    ):
        ...
        ranger_plugin_properties = self.getSiteProperties(
            configurations, "ranger-hive-plugin-properties"
        )
        hive_env_properties = self.getSiteProperties(configurations, "hive-env")
        ranger_plugin_enabled = (
            "hive_security_authorization" in hive_env_properties
                    ...
                    validationItems.append(
                        {
                            ...
                            "item": self.getWarnItem(
                                ...
                                " {0} under hiveserver2-site needs to be set to {1}".format(
                                    prop_name, prop_val
                                )
                            ),
                        }
        ...
        validationProblems = self.toConfigurationValidationProblems(
            validationItems, "hiveserver2-site"
        )
        return validationProblems

    def validateHiveInteractiveEnvConfigurationsFromHDP30(
        self, properties, recommendedDefaults, configurations, services, hosts
    ):
        hive_site_env_properties = self.getSiteProperties(
            configurations, "hive-interactive-env"
        )
        ...
        if len(hsi_hosts) > 0:
            ...
            if "enable_hive_interactive" not in hive_site_env_properties or (
                "enable_hive_interactive" in hive_site_env_properties
                and hive_site_env_properties["enable_hive_interactive"].lower()
                != "true"
            ):
                validationItems.append(
                    {
                        "config-name": "enable_hive_interactive",
        ...
        if (
            ...
            and "yarn.resourcemanager.scheduler.monitor.enable" in yarn_site_properties
        ):
            scheduler_monitor_enabled = yarn_site_properties[
                "yarn.resourcemanager.scheduler.monitor.enable"
            ]
            if (
                scheduler_monitor_enabled.lower() == "false"
                and hive_site_env_properties
                and "enable_hive_interactive" in hive_site_env_properties
                and hive_site_env_properties["enable_hive_interactive"].lower()
                == "true"
            ):
                validationItems.append(
        ...
        validationProblems = self.toConfigurationValidationProblems(
            validationItems, "hive-interactive-env"
        )
        return validationProblems

    def validateHiveInteractiveSiteConfigurationsFromHDP30(
        self, properties, recommendedDefaults, configurations, services, hosts
    ):
        """
        ...
        if hsi_site:
            if (
                "hive.llap.daemon.queue.name" in hsi_site
                and hsi_site["hive.llap.daemon.queue.name"]
            ):
                llap_queue_name = hsi_site["hive.llap.daemon.queue.name"]
                llap_queue_cap = self.__getSelectedQueueTotalCap(
                    capacity_scheduler_properties, llap_queue_name, total_cluster_cap
                )

                if llap_queue_cap:
                    llap_queue_cap_perc = float(
                        llap_queue_cap * 100 / total_cluster_cap
                    )
                    min_reqd_queue_cap_perc = (
                        self.min_queue_perc_reqd_for_llap_and_hive_app(
                    ...
                    if llap_queue_cap_perc < min_reqd_queue_cap_perc:
                        errMsg1 = (
                            "Selected queue '{0}' capacity ({1}%) is less than minimum required capacity ({2}%) for LLAP "
                            "app to run".format(
                                llap_queue_name,
                                llap_queue_cap_perc,
                                min_reqd_queue_cap_perc,
                        ...
                        validationItems.append(
                            {
                                "config-name": "hive.llap.daemon.queue.name",
                                "item": self.getErrorItem(errMsg1),
                            }
                        )
                else:
                    self.logger.error(
                ...
                llap_selected_queue_state = self.__getQueueStateFromCapacityScheduler(
                    capacity_scheduler_properties, llap_queue_name
                )
                if llap_selected_queue_state:
                    if llap_selected_queue_state == "STOPPED":
                        errMsg2 = "Selected queue '{0}' current state is : '{1}'. It is required to be in 'RUNNING' state for LLAP to run".format(
                            llap_queue_name, llap_selected_queue_state
                        )
                        validationItems.append(
                            {
                                "config-name": "hive.llap.daemon.queue.name",
                                "item": self.getErrorItem(errMsg2),
                            }
                        )
                else:
                    self.logger.error(
            ...
            if (
                "hive.server2.enable.doAs" in hsi_site
                and hsi_site["hive.server2.enable.doAs"] == "true"
            ):
                validationItems.append(
                    {
                        "config-name": "hive.server2.enable.doAs",
                        "item": self.getErrorItem(
                            "Value should be set to 'false' for Hive2."
                        ),
                    }
                )
            ...
            if (
                llap_queue_cap
                and "hive.server2.tez.sessions.per.default.queue" in hsi_site
            ):
                num_tez_sessions = hsi_site[
                ...
                if num_tez_sessions:
                    ...
                    llap_selected_queue_cap_remaining = llap_queue_cap - (
                        normalized_tez_am_container_size * num_tez_sessions
                    )
                    if llap_selected_queue_cap_remaining <= llap_queue_cap / 2:
                        errMsg3 = (
                            " Reducing the 'Maximum Total Concurrent Queries' (value: {0}) is advisable as it is consuming more than 50% of "
                            "'{1}' queue for LLAP.".format(
                                num_tez_sessions, llap_queue_name
                            )
                        )
                        validationItems.append(
                            {
                                "config-name": "hive.server2.tez.sessions.per.default.queue",
                                "item": self.getWarnItem(errMsg3),
                            }
                        )

            if int(hsi_site["hive.llap.io.memory.size"]) > int(
                hsi_site["hive.llap.daemon.yarn.container.mb"]
            ):
                errorMessage = "In-Memory Cache per Daemon (value: {0}) may not be more then Memory per Daemon (value: {1})".format(
                    hsi_site["hive.llap.io.memory.size"],
                    hsi_site["hive.llap.daemon.yarn.container.mb"],
                )
                validationItems.append(
                    {
                        "config-name": "hive.llap.io.memory.size",
                        "item": self.getErrorItem(errorMessage),
                    }
                )
        ...
        if (
            llap_queue_name
            and llap_queue_cap_perc
            and llap_queue_name == self.AMBARI_MANAGED_LLAP_QUEUE_NAME
        ):
            curr_selected_queue_for_llap_cap = (
                float(llap_queue_cap_perc) / 100 * total_cluster_cap
            )
            available_cap_in_cluster = (
                total_cluster_cap - curr_selected_queue_for_llap_cap
            )
            if available_cap_in_cluster < MIN_ASSUMED_CAP_REQUIRED_FOR_SERVICE_CHECKS:
                errMsg4 = (
                    "Capacity used by '{0}' queue is '{1}'. Service checks may not run as remaining available capacity "
                    "({2}) in cluster is less than 512 MB.".format(
                ...
                validationItems.append(
                    {
                        "config-name": "hive.llap.daemon.queue.name",
                        "item": self.getWarnItem(errMsg4),
                    }
                )

        validationProblems = self.toConfigurationValidationProblems(
            validationItems, "hive-interactive-site"
        )
        return validationProblems
    ...
    def get_yarn_nm_mem_in_mb(self, services, configurations):
        ...
        if (
            not (
                "changed-configurations" in services
        ...
        if yarn_nm_mem_in_mb <= 0.0:
            self.logger.warning(
                "'yarn.nodemanager.resource.memory-mb' current value : {0}. Expected value : > 0".format(
                    yarn_nm_mem_in_mb
                )
            )
    ...
    def __getQueueCapacityKeyFromCapacityScheduler(
        ...
        for key in cap_sched_keys:
            ...
            ) and key.startswith("yarn.scheduler.capacity.root"):
    ...
    def __getQueueAmFractionFromCapacityScheduler(
        ...
        for key in cap_sched_keys:
            if key.endswith(
                ...
                self.logger.info(
                    "AM percent key got for '{0}' queue is : '{1}'".format(
        ...
        if llap_selected_queue_am_percent_key is None:
            self.logger.info(
                "Returning default AM percent value : '0.1' for queue : {0}".format(
                    llap_daemon_selected_queue_name
                )
            )
    ...
    def __getSelectedQueueTotalCap(
        ...
        """
        self.logger.info(
            "Entered __getSelectedQueueTotalCap fn() with llap_daemon_selected_queue_name= '{0}'.".format(
                llap_daemon_selected_queue_name
            )
        )
                ...
                if queues_list:
                    for queue in queues_list:
                        ...
                        queue_cap_perc = float(
                            capacity_scheduler_properties.get(queue_cap_key)
                        )
                        available_capacity = queue_cap_perc / 100 * available_capacity
                        self.logger.info(
                            "Total capacity available for queue {0} is : {1}".format(
    ...
    def min_queue_perc_reqd_for_llap_and_hive_app(
        self, services, hosts, configurations
    ):
        """
        ...
        normalized_val = self._normalizeUp(
            hive_tez_container_size, yarn_min_container_size
    ...
    def get_yarn_min_container_size(self, services, configurations):
        ...
        if (
            ...
            self.logger.info(
                "DBG: 'yarn.scheduler.minimum-allocation-mb' read from output as : {0}".format(
        ...
        # Check if "yarn.scheduler.minimum-allocation-mb" is input in services array.
            ...
            yarn_site_properties
            and yarn_min_allocation_property in yarn_site_properties
        ):
            yarn_min_container_size = yarn_site_properties[yarn_min_allocation_property]
            self.logger.info(
                "DBG: 'yarn.scheduler.minimum-allocation-mb' read from services as : {0}".format(
                    yarn_min_container_size
                )
            )
        ...
        if not yarn_min_container_size:
            self.logger.error(
                "{0} was not found in the configuration".format(
                    yarn_min_allocation_property
                )
            )
    ...
    def calculate_tez_am_container_size(
        ...
        if is_cluster_create_opr or enable_hive_interactive_1st_invocation:
            ...
            self.logger.info(
                "DBG: Calculated and returning 'tez_am_resource_memory_mb' as : {0}".format(
                    calculated_tez_am_resource_memory_mb
                )
            )
            return float(calculated_tez_am_resource_memory_mb)
        else:
            self.logger.info(
                "DBG: Returning 'tez_am_resource_memory_mb' as : {0}".format(
    ...
    def get_tez_am_resource_memory_mb(self, services):
        ...
        if (
            ...
            tez_am_resource_memory_mb = services["configurations"][
                "tez-interactive-site"
            ]["properties"]["tez.am.resource.memory.mb"]
