...
def wait_for_safemode_off(
    ...
    Logger.info(
        "Waiting up to {0} minutes for the NameNode to leave Safemode...".format(
            sleep_minutes
        )
    )
...
def namenode(
    ...
    if action == "configure":
        ...
        Logger.info(
            "Called service {0} with upgrade_type: {1}".format(
        ...
        if ensure_safemode_off:
            if params.rolling_restart and params.rolling_restart_safemode_exit_timeout:
                calculated_retries = (
                    int(params.rolling_restart_safemode_exit_timeout) / 30
                )
...
def namenode(
    ...
    if action == "configure":
        ...
        if not os.path.exists(namenode_format_marker):
            hadoop_cmd = "cmd /C %s" % (
                os.path.join(params.hadoop_home, "bin", "hadoop.cmd")
            )
...
def decommission():
    ...
    if params.dfs_ha_enabled:
        ...
        nn_refresh_cmd = format(
            "cmd /c hdfs dfsadmin -fs {namenode_address} -refreshNodes"
        )
...
def bootstrap_standby_namenode(params, use_path=False):
    ...
    try:
        ...
        Logger.error(
            "Bootstrap standby namenode threw an exception. Reason %s" % (str(ex))
        )
    if bootstrapped:
        for mark_dir in mark_dirs:
            Directory(mark_dir, create_parents=True)
...
def is_namenode_bootstrapped(params):
    ...
    for mark_dir in mark_dirs:
        if os.path.isdir(mark_dir):
            marked = True
            Logger.info(
                format("{mark_dir} exists. Standby Namenode already bootstrapped")
            )
            break
