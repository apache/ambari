...
class HdfsLine:
    ...
    PROGRESS_END_PATTERN = re.compile(
        "(The cluster is balanced. Exiting...|The cluster is balanced. Exiting...)"
    )
    ...
    def parseProgressLog(self, line, m):
        ...
        if m:
            ...
            self.bytesAlreadyMoved = self.parseMemory(
                m.group("memory_1"), m.group("mult_1")
            )
            self.bytesLeftToMove = self.parseMemory(
                m.group("memory_2"), m.group("mult_2")
            )
            self.bytesBeingMoved = self.parseMemory(
                m.group("memory_3"), m.group("mult_3")
            )
    ...
    def parseMemory(self, memorySize, multiplier_type):
        try:
            ...
            raise AmbariException(
                "Failed to memory value [%s %s]" % (memorySize, multiplier_type)
            )
    ...
    def __str__(self):
        return (
            "[ date=%s,iteration=%d, bytesAlreadyMoved=%d, bytesLeftToMove=%d, bytesBeingMoved=%d]"
            % (
                self.date,
                self.iteration,
                self.bytesAlreadyMoved,
                self.bytesLeftToMove,
                self.bytesBeingMoved,
            )
        )
