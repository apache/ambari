...
class HdfsServiceCheckDefault(HdfsServiceCheck):
    ...
    """
        ...
        # )
        params.HdfsResource(
            dir, type="directory", action="create_on_execute", mode=0o777
        )
        params.HdfsResource(
            tmp_file,
            type="file",
        ...
        if params.has_journalnode_hosts:
            if params.security_enabled:
                for host in params.journalnode_hosts:
                    ...
                    if not response:
                        Logger.error(
                            "Cannot access WEB UI on: {0}. Error : {1}", uri, errmsg
                        )
                        return 1
            else:
...
class HdfsServiceCheckWindows(HdfsServiceCheck):
    def service_check(self, env):
        ...
        create_file_cmd = "%s fs -put %s %s" % (
            hadoop_cmd,
            os.path.join(params.hadoop_conf_dir, "core-site.xml"),
            tmp_file,
        )
        ...
        Execute(
            create_dir_cmd, user=params.hdfs_user, logoutput=True, ignore_failures=True
        )
