...
class NameNode(Script):
    ...
    def stop(self, env, upgrade_type=None):
        ...
        namenode(
            action="stop", hdfs_binary=hdfs_binary, upgrade_type=upgrade_type, env=env
        )
...
class NameNodeDefault(NameNode):
    ...
    def finalize_non_rolling_upgrade(self, env):
        hfds_binary = self.get_hdfs_binary()
        namenode_upgrade.finalize_upgrade(
            constants.UPGRADE_TYPE_NON_ROLLING, hfds_binary
        )
    ...
    def rebalancehdfs(self, env):
        ...
        def calculateCompletePercent(first, current):
            ...
            try:
                ...
                Logger.warning(
                    "Division by zero. Bytes Left To Move = {0}. Return 1.0".format(
                        first.bytesLeftToMove
                    )
                )
                return 1.0
    ...
    def rebalancehdfs(self, env):
        ...
        for line in stdout.split("\n"):
            ...
            if pl:
                ...
                _print(
                    "[balancer] %s %s" % (str(datetime.now()), "Process is finished")
                )
                self.put_structured_out({"completePercent": 1})
                break
