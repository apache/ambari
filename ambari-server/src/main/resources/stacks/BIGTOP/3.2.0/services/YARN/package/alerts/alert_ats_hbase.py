...
MESSAGE_WITH_STATE_AND_INSTANCES = (
    "The application reported a '{0}' state in {1:.3f}s. [Live: {2}, Desired: {3}]"
)
...
def execute(configurations={}, parameters={}, host_name=None):
    ...
    try:
        ...
        if USE_EXTERNAL_HBASE_KEY in configurations:
            use_external_hbase = (
                str(configurations[USE_EXTERNAL_HBASE_KEY]).upper() == "TRUE"
            )
        ...
        if ATS_HBASE_SYSTEM_SERVICE_LAUNCH_KEY in configurations:
            is_hbase_system_service_launch = (
                str(configurations[ATS_HBASE_SYSTEM_SERVICE_LAUNCH_KEY]).upper()
                == "TRUE"
            )
        ...
        if not is_hbase_system_service_launch:
            ...
            if SECURITY_ENABLED_KEY in configurations:
                security_enabled = (
                    str(configurations[SECURITY_ENABLED_KEY]).upper() == "TRUE"
                )
