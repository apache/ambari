...
def initiate_safe_zkfc_failover():
    ...
    if unknown_namenodes:
        for unknown_namenode in unknown_namenodes:
            Logger.info(
                "NameNode HA state for {0} is unknown".format(unknown_namenode[0])
            )
...
def service(
    ...
    if action != "stop":
        ...
        if create_log_dir:
            if name == "nfs3":
                Directory(
                    log_dir, mode=0o775, owner=params.root_user, group=params.user_group
                )
            else:
                Directory(
                    log_dir, owner=user, group=params.user_group, create_parents=True
                )
...
def get_jmx_data(
    ...
    Logger.info(
        "Retrieve modeler: %s, metric: %s from JMX endpoint %s"
