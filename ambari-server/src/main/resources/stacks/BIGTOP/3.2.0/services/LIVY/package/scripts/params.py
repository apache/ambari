...
component_directory = Script.get_component_from_role(
  SERVER_ROLE_DIRECTORY_MAP, "SPARK_CLIENT"
)
...
yarn_application_classpath = config["configurations"]["yarn-site"][
...
spark_history_store_path = default(
  "/configurations/spark-defaults/spark.history.store.path", "/var/lib/spark/shs_db"
)

spark_warehouse_dir = default(
  "/configurations/hive-site/hive.metastore.warehouse.dir",
  "/warehouse/tablespace/managed/hive",
)
whs_dir_protocol = urlparse(spark_warehouse_dir).scheme
default_metastore_catalog = config["configurations"]["spark-hive-site-override"][
...
spark_hadoop_lib_native = format(
  "{hadoop_home}/lib/native:{hadoop_home}/lib/native/Linux-amd64-64"
)
...
spark_metrics_properties = config["configurations"]["spark-metrics-properties"][
  "content"
]
...
spark_kerberos_keytab = config["configurations"]["spark-defaults"][
  "spark.history.kerberos.keytab"
]
spark_kerberos_principal = config["configurations"]["spark-defaults"][
...
# security settings
if security_enabled:
  spnego_principal = config["configurations"]["spark-defaults"][
    "history.server.spnego.kerberos.principal"
  ]
  spnego_principal = spnego_principal.replace("_HOST", fqdn)
  spnego_keytab = config["configurations"]["spark-defaults"][
    "history.server.spnego.keytab.file"
  ]
  ...
  if is_hive_installed:
    spark_hive_properties.update(
      {
        "hive.metastore.sasl.enabled": str(
          config["configurations"]["hive-site"]["hive.metastore.sasl.enabled"]
        ).lower(),
        "hive.metastore.kerberos.keytab.file": config["configurations"]["hive-site"][
          "hive.metastore.kerberos.keytab.file"
        ],
        "hive.server2.authentication.spnego.principal": config["configurations"][
          "hive-site"
        ]["hive.server2.authentication.spnego.principal"],
        "hive.server2.authentication.spnego.keytab": config["configurations"][
          "hive-site"
        ]["hive.server2.authentication.spnego.keytab"],
        "hive.metastore.kerberos.principal": config["configurations"]["hive-site"][
          "hive.metastore.kerberos.principal"
        ],
        "hive.server2.authentication.kerberos.principal": config["configurations"][
          "hive-site"
        ]["hive.server2.authentication.kerberos.principal"],
        "hive.server2.authentication.kerberos.keytab": config["configurations"][
          "hive-site"
        ]["hive.server2.authentication.kerberos.keytab"],
        "hive.server2.authentication": config["configurations"]["hive-site"][
          "hive.server2.authentication"
        ],
      }
    )

    hive_kerberos_keytab = config["configurations"]["spark-hive-site-override"][
      "hive.server2.authentication.kerberos.keytab"
    ]
    default_hive_kerberos_principal = config["configurations"][
      "spark-hive-site-override"
    ]["hive.server2.authentication.kerberos.principal"]
    hive_kerberos_principal = default_hive_kerberos_principal.replace("_HOST", fqdn)

spark_transport_mode = config["configurations"]["spark-hive-site-override"][
...
if spark_transport_mode.lower() == "binary":
  ...
  spark_thrift_port = int(
    config["configurations"]["spark-hive-site-override"][
      "hive.server2.thrift.http.port"
    ]
  )
  spark_thrift_ssl_enabled = default(
    "configurations/spark-hive-site-override/hive.server2.use.SSL", False
  )
  spark_thrift_endpoint = default(
    "configurations/spark-hive-site-override/hive.server2.http.endpoint", "cliservice"
  )
...
if has_spark_thriftserver and "spark-defaults" in config["configurations"]:
  ...
  spark_thrift_cmd_opts_properties = config["configurations"]["spark-env"][
    "spark_thrift_cmd_opts"
  ]

  if (
    "spark-thrift-fairscheduler" in config["configurations"]
    and "fairscheduler_content"
    in config["configurations"]["spark-thrift-fairscheduler"]
  ):
    spark_thrift_fairscheduler_content = config["configurations"][
...
if is_hive_installed:
  ...
  spark_hive_properties.update(
    {
      "hive.metastore.client.socket.timeout": config["configurations"]["hive-site"][
        "hive.metastore.client.socket.timeout"
      ]
    }
  )

  spark_hive_site_override_properties = config["configurations"][
    "spark-hive-site-override"
  ].copy()
...
if stack_version_formatted and "livy-env" in config["configurations"]:
  livy_component_directory = Script.get_component_from_role(
    SERVER_ROLE_DIRECTORY_MAP, "LIVY_SERVER"
  )
  ...
  livy_recovery_dir = default(
    "/configurations/livy-conf/livy.server.recovery.state-store.url", "/livy-recovery"
  )
  ...
  livy_spark_blacklist_properties = config["configurations"]["livy-spark-blacklist"][
    "content"
  ]

  if "livy.server.kerberos.keytab" in config["configurations"]["livy-conf"]:
    livy_kerberos_keytab = config["configurations"]["livy-conf"][
      "livy.server.kerberos.keytab"
    ]
  else:
    livy_kerberos_keytab = config["configurations"]["livy-conf"][
  ...
  if "livy.server.kerberos.principal" in config["configurations"]["livy-conf"]:
    livy_kerberos_principal = config["configurations"]["livy-conf"][
  ...
  livy_http_scheme = (
    "https" if "livy.keystore" in config["configurations"]["livy-conf"] else "http"
  )
