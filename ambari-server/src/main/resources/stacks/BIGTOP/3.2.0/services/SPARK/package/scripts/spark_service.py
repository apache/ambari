...
def spark_service(name, upgrade_type=None, action=None):
    ...
    if action == "start":
        effective_version = (
            ...
            else params.stack_version_formatted
        )
        ...
        if name == "jobhistoryserver":
            ...
            historyserver_no_op_test = (
                as_sudo(["test", "-f", params.spark_history_server_pid_file])
                + " && "
                + as_sudo(["pgrep", "-F", params.spark_history_server_pid_file])
            )
            try:
                Execute(
            ...
            thriftserver_no_op_test = (
                as_sudo(["test", "-f", params.spark_thrift_server_pid_file])
                + " && "
                + as_sudo(["pgrep", "-F", params.spark_thrift_server_pid_file])
            )
            try:
                Execute(
                    format(
            ...
            if not hive_connection_created:
                raise ComponentIsNotRunning(
                    "Something goes wrong, STS connection was not created but STS process still alive. "
