...
class HDFSRecommender(service_advisor.ServiceAdvisor):
  ...
  def recommendConfigurationsFromHDP206(
    ...
    self.logger.info(
      f"Class: {self.__class__.__name__}, Method: {inspect.stack()[0][3]}. Total Available Ram: {str(totalAvailableRam)}"
    )
    ...
    if (
      ...
      and configurations["hdfs-site"]["properties"]["dfs.datanode.data.dir"] is not None
    ):
  ...
  def recommendConfigurationsFromHDP22(
    ...
    if (
      ...
      and "keyserver_host" in services["configurations"]["hadoop-env"]["properties"]
      and "keyserver_port" in services["configurations"]["hadoop-env"]["properties"]
    ):
      keyserverHostsString = services["configurations"]["hadoop-env"]["properties"][
        "keyserver_host"
      ]
      keyserverPortString = services["configurations"]["hadoop-env"]["properties"][
        "keyserver_port"
      ]
    ...
    if rangerKMSServerHosts is not None and len(rangerKMSServerHosts) > 0:
      ...
      if (
        ...
        keyserverPortString = services["configurations"]["kms-env"]["properties"][
          "kms_port"
        ]
  ...
  def _is_kerberos_enabled(self, config):
    ...
    return config and (
      (
        "hdfs-site" in config
        and "hadoop.security.authentication" in config["hdfs-site"]["properties"]
  ...
  def validatorOneDataDirPerPartition(
    ...
    if len(warnings) > 0:
      return self.getWarnItem(
        f"cluster-env/one_dir_per_partition is enabled but there are multiple data directories on the same mount. Affected hosts: {', '.join(sorted(warnings))}"
      )
