...
class HDFSServiceAdvisor(service_advisor.ServiceAdvisor):
    ...
    def modifyComponentLayoutSchemes(self):
        ...
        self.componentLayoutSchemes.update(
            {"NAMENODE": {"else": 0}, "SECONDARY_NAMENODE": {"else": 1}}
        )
...
class HDFSRecommender(service_advisor.ServiceAdvisor):
    ...
    def recommendConfigurationsFromHDP206(
        ...
        self.logger.info(
            "Class: %s, Method: %s. Total Available Ram: %s"
        ...
        if (
            nameServices is None
            and hdfsSiteProperties
        ...
        self.logger.info(
            "Class: %s, Method: %s. Updating HDFS mount properties."
            % (self.__class__.__name__, inspect.stack()[0][3])
        )
        self.updateMountProperties(
            "hdfs-site", hdfs_mount_properties, configurations, services, hosts
        )

        dataDirs = []
        if (
            configurations
            and "hdfs-site" in configurations
            and "dfs.datanode.data.dir" in configurations["hdfs-site"]["properties"]
            and configurations["hdfs-site"]["properties"]["dfs.datanode.data.dir"]
            is not None
        ):
            ...
            hdfsSiteProperties
            and "dfs.datanode.data.dir" in hdfsSiteProperties
            and hdfsSiteProperties["dfs.datanode.data.dir"] is not None
        ):
            dataDirs = hdfsSiteProperties["dfs.datanode.data.dir"].split(",")

        self.logger.info(
            "Class: %s, Method: %s. HDFS Data Dirs: %s"
        ...
        for host in hosts["items"]:
            ...
            if (not reservedSizeRecommendation) or (
                maxFreeVolumeSizeForHost
                and maxFreeVolumeSizeForHost < reservedSizeRecommendation
            ):
                reservedSizeRecommendation = maxFreeVolumeSizeForHost

        self.logger.info(
            "Class: %s, Method: %s. HDFS Datanode recommended reserved size: %d"
            % (
        ...
        if reservedSizeRecommendation:
            reservedSizeRecommendation = max(
                reservedSizeRecommendation * 1024 / 8, 1073741824
            )  # At least 1Gb is reserved
            putHDFSSiteProperty(
                "dfs.datanode.du.reserved", int(reservedSizeRecommendation)
            )  # Bytes
    ...
    def recommendConfigurationsFromHDP22(
        ...
        if (
            ...
            in services["configurations"]["hdfs-site"]["properties"]
        ):
            dataDirsCount = len(
                str(
            ...
            dataDirsCount = len(
                str(
                    configurations["hdfs-site"]["properties"]["dfs.datanode.data.dir"]
                ).split(",")
            )
        if dataDirsCount <= 2:
        ...
        putHdfsSiteProperty(
            "dfs.datanode.failed.volumes.tolerated", failedVolumesTolerated
        )
        ...
        putHdfsSiteProperty(
            ...
            "0.999" if len(namenodeHosts) > 1 else "1.000",
        )

        putHdfsEnvProperty = self.putProperty(configurations, "hadoop-env", services)
        putHdfsEnvPropertyAttribute = self.putPropertyAttribute(
            configurations, "hadoop-env"
        )

        putHdfsEnvProperty(
            "namenode_heapsize", max(int(clusterData["totalAvailableRam"] / 2), 1024)
        )
        ...
        if namenodeHosts is not None and len(namenodeHosts) > 0:
                ...
                masters_at_host = max(
                    len(
                        self.getHostComponentsByCategories(
                            namenodeHosts[0]["Hosts"]["host_name"],
                            ["MASTER"],
                            services,
                            hosts,
                        )
                    ...
                    len(
                        self.getHostComponentsByCategories(
                            namenodeHosts[1]["Hosts"]["host_name"],
                            ["MASTER"],
                            services,
                            hosts,
                        )
            ...
            putHdfsEnvPropertyAttribute(
                "namenode_heapsize", "maximum", max(nn_max_heapsize, 1024)
            )
        ...
        if datanodeHosts is not None and len(datanodeHosts) > 0:
            ...
            datanodeFilesM = (
                len(datanodeHosts) * dataDirsCount / 10
            )  # in millions, # of files = # of disks * 100'000
            nn_memory_configs = [
            ...
            if (
                nn_heapsize_limit is not None
                and nn_memory_config["nn_heap"] <= nn_heapsize_limit
            ):
                putHdfsEnvProperty("namenode_heapsize", nn_memory_config["nn_heap"])

            putHdfsEnvPropertyAttribute(
                "dtnode_heapsize", "maximum", int(min_datanode_ram_kb / 1024)
            )

        nn_heapsize = int(
            configurations["hadoop-env"]["properties"]["namenode_heapsize"]
        )
        ...
        putHdfsSitePropertyAttribute(
            "dfs.datanode.failed.volumes.tolerated", "maximum", dataDirsCount
        )
        ...
        if (
            ...
            and "keyserver_host"
            in services["configurations"]["hadoop-env"]["properties"]
            and "keyserver_port"
            in services["configurations"]["hadoop-env"]["properties"]
        ):
        ...
        if rangerKMSServerHosts is not None and len(rangerKMSServerHosts) > 0:
            ...
            if (
                "kms-env" in services["configurations"]
                and "kms_port" in services["configurations"]["kms-env"]["properties"]
            ):
        ...
        if (
            ...
            and "ranger-hdfs-plugin-enabled"
            ...
            rangerEnvHdfsPluginProperty = services["configurations"]["ranger-env"][
                "properties"
            ]["ranger-hdfs-plugin-enabled"]
            putHdfsRangerPluginProperty(
    ...
    def recommendConfigurationsFromHDP23(
        ...
        if ("ranger-hdfs-plugin-properties" in services["configurations"]) and (
            ...
            if (
                ...
                rangerPluginEnabled = configurations["ranger-hdfs-plugin-properties"][
                    "properties"
                ]["ranger-hdfs-plugin-enabled"]
            elif (
                "ranger-hdfs-plugin-properties" in services["configurations"]
                and "ranger-hdfs-plugin-enabled"
                in services["configurations"]["ranger-hdfs-plugin-properties"][
                    "properties"
                ]
            ):
                rangerPluginEnabled = services["configurations"][
    ...
    def recommendConfigurationsFromHDP26(
        ...
        if (
            "hadoop-env" in services["configurations"]
            and "hdfs_user" in services["configurations"]["hadoop-env"]["properties"]
        ):
        ...
        if (
            ...
            ranger_hdfs_plugin_enabled = (
                services["configurations"]["ranger-hdfs-plugin-properties"][
                    "properties"
                ]["ranger-hdfs-plugin-enabled"].lower()
                == "Yes".lower()
            )
        else:
    ...
    def recommendConfigurationsForSSO(
        ...
        if ambari_sso_details and ambari_sso_details.is_managing_services():
            putHdfsSiteProperty = self.putProperty(
                configurations, "hdfs-site", services
            )
    ...
    def _is_kerberos_enabled(self, config):
        ...
        return config and (
            (
                ...
                in config["hdfs-site"]["properties"]
                and config["hdfs-site"]["properties"]["hadoop.security.authentication"]
                == "kerberos"
            )
            or (
                "core-site" in config
...
class HDFSValidator(service_advisor.ServiceAdvisor):
    ...
    def validateHDFSConfigurationsFromHDP206(
        ...
        validationItems = [
            {
                "config-name": "dfs.datanode.du.reserved",
                "item": self.validatorLessThenDefaultValue(
                    properties, recommendedDefaults, "dfs.datanode.du.reserved"
                ),
            },
            {
                "config-name": "dfs.datanode.data.dir",
                "item": self.validatorOneDataDirPerPartition(
                    properties, "dfs.datanode.data.dir", services, hosts, clusterEnv
                ),
            },
        ]
        return self.toConfigurationValidationProblems(validationItems, "hdfs-site")

    def validatorOneDataDirPerPartition(
        self, properties, propertyName, services, hosts, clusterEnv
    ):
    ...
    def validateHDFSCoreSiteFromHDP206(
        ...
        validationItems.extend(
            self.getHadoopProxyUsersValidationItems(
                properties, services, hosts, configurations
            )
        )
        validationItems.extend(
            self.getAmbariProxyUsersForHDFSValidationItems(properties, services)
        )
    ...
    def getLZOSupportValidationItems(self, properties, services):
        ...
        if "HDFS" in services_list:
            ...
            self.validatePropertyToLZOCodec(
                "io.compression.codec.lzo.class", properties, lzo_allowed, validations
            )
        return validations

    def validatePropertyToLZOCodec(
        self, property_name, properties, lzo_allowed, validations
    ):
        """
    ...
    def validateHDFSConfigurationsFromHDP22(
        ...
        dfs_encrypt_data_transfer = (
            "dfs.encrypt.data.transfer"  # Hadoop Wire encryption
        )
        ...
        VALID_TRANSFER_PROTECTION_VALUES = [
            TRANSFER_PROTECTION_AUTHENTICATION,
            TRANSFER_PROTECTION_INTEGRITY,
            TRANSFER_PROTECTION_PRIVACY,
            TRANSFER_PROTECTION_AUTHENTICATION_AND_PRIVACY,
        ]
        ...
        if ("RANGER" in servicesList) and (
            ranger_plugin_enabled.lower() == "Yes".lower()
        ):
            if (
                ...
                validationItems.append(
                    {
                        "config-name": "dfs.permissions.enabled",
        ...
        if (
            not wire_encryption_enabled  # If wire encryption is enabled at Hadoop, it disables all our checks
            and "hadoop.security.authentication" in core_site
            and core_site["hadoop.security.authentication"] == "kerberos"
            and "hadoop.security.authorization" in core_site
            and core_site["hadoop.security.authorization"] == "true"
        ):
            # security is enabled
            ...
            if dfs_http_policy_value == HTTPS_ONLY:
                if (
                    not privileged_dfs_dn_port
                    and data_transfer_protection_value is None
                ):
                    ...
                    message = (
                        "You have set up datanode to use non-secure rpc port. "
                ...
                any_nonprivileged_ports_are_in_use = (
                    not privileged_dfs_dn_port or not privileged_dfs_http_port
                )
                if any_nonprivileged_ports_are_in_use:
                    important_properties = [dfs_datanode_address, datanode_http_address]
                    message = (
                        "You have set up datanode to use some non-secure ports. "
                        "In a secure cluster:"
            ...
            for prop in address_properties_with_warnings:
                validationItems.append(
                    {"config-name": prop, "item": self.getWarnItem(message)}
                )
            ...
            if data_transfer_protection_value is not None:
                if (
                    ...
                    not in VALID_TRANSFER_PROTECTION_VALUES
                ):
                    validationItems.append(
                        {
                            "config-name": data_transfer_protection,
                            "item": self.getWarnItem(
                                "Invalid property value: {0}. Valid values are {1}.".format(
                                    data_transfer_protection_value,
                                    VALID_TRANSFER_PROTECTION_VALUES,
    ...
    def validateRangerAuthorizerFromHDP23(
        ...
        if ("RANGER" in servicesList) and (ranger_plugin_enabled.lower() == "yes"):
            try:
                ...
                validationItems.append(
                    {
                        "config-name": "dfs.namenode.inode.attributes.provider.class",
    ...
    def getDataNodeHosts(self, services, hosts):
        ...
        if len(hosts["items"]) > 0:
            dataNodeHosts = self.getHostsWithComponent(
                "HDFS", "DATANODE", services, hosts
            )
