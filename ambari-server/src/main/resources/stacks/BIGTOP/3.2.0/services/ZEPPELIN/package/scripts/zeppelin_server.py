...
class ZeppelinServer(Script):
  ...
  def configure(self, env):
    ...
    generate_logfeeder_input_config(
      "zeppelin", Template("input.config-zeppelin.json.j2", extra_imports=[default])
    )
  ...
  def start(self, env, upgrade_type=None):
    ...
    if (
      ...
      and params.config["configurations"]["zeppelin-site"]["zeppelin.notebook.storage"]
  ...
  def reset_interpreter_settings(self, upgrade_type):
    ...
    for setting_key in interpreter_json_template.keys():
      if setting_key not in interpreter_settings:
        interpreter_settings[setting_key] = interpreter_json_template[setting_key]
      else:
    ...
    for interpreter_setting in interpreter_settings:
      ...
      if interpreter["group"] == "livy":
        ...
        if (
          ...
          self.storePropertyToInterpreter(
            interpreter, "spark.yarn.keytab", "string", params.zeppelin_kerberos_keytab
          )
        else:
          self.storePropertyToInterpreter(
    ...
    for setting_key in list(interpreter_settings.keys()):
      ...
      if (
        ...
        if "spark-env" in params.config["configurations"]:
          ...
          self.storePropertyToInterpreter(
            interpreter, "SPARK_HOME", "string", params.spark_home
          )
        else:
          del interpreter_settings[setting_key]
