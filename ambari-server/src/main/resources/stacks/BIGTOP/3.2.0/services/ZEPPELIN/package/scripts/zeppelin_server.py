...
class ZeppelinServer(Script):
    ...
    def check_and_copy_notebook_in_hdfs(self, params):
        ...
        if notebook_dir.startswith("/") or "://" in notebook_dir:
            ...
            notebook_directory = (
                "/user/" + format("{zeppelin_user}") + "/" + notebook_dir
            )

        if not self.is_directory_exists_in_HDFS(
            notebook_directory, params.zeppelin_user
        ):
            params.HdfsResource(
    ...
    def start(self, env, upgrade_type=None):
        ...
        if (
            ...
            in params.config["configurations"]["zeppelin-site"]
        ...
        pidfile = glob.glob(
            os.path.join(
                ...
                "zeppelin-" + params.zeppelin_user + "*.pid",
            )
        )[0]
    ...
    def reset_interpreter_settings(self, upgrade_type):
        ...
        interpreter_json_template = json.loads(interpreter_json_template.template)[
            "interpreterSettings"
        ]
        ...
        if upgrade_type is not None:
            ...
            for key in current_interpreters_keys:
                ...
                if (
                    interpreter_data["name"] == "sh"
                    and interpreter_data["group"] == "sh"
                ):
    ...
    def get_zeppelin_conf_FS_directory(self, params):
        ...
        if not (
            hdfs_interpreter_config.startswith("/") or "://" in hdfs_interpreter_config
        ):
            hdfs_interpreter_config = (
                "/user/" + format("{zeppelin_user}") + "/" + hdfs_interpreter_config
            )
        ...
        if params.conf_stored_in_hdfs:
            ...
            if self.is_file_exists_in_HDFS(zeppelin_conf_fs, params.zeppelin_user):
                ...
                if params.security_enabled:
                    ...
                    kinit_if_needed = ""
                interpreter_config = os.path.join(
                    params.zeppelin_conf_dir, "interpreter.json"
                )
                shell.call(
                    format(
        ...
        for interpreter_setting in interpreter_settings:
            ...
            if interpreter["group"] == "livy":
                if (
                    ...
                    and params.zeppelin_kerberos_principal
                ...
                if (
                    ...
                    self.storePropertyToInterpreter(
                        interpreter, "zeppelin.jdbc.auth.type", "string", "KERBEROS"
                    )
                    self.storePropertyToInterpreter(
                        interpreter,
                    ...
                    if (
                        ...
                        and params.zookeeper_znode_parent
                        not in interpreter["properties"]["phoenix.url"]["value"]
                    ):
                        self.storePropertyToInterpreter(
                            interpreter,
                            "phoenix.url",
                    ...
                    self.storePropertyToInterpreter(
                        interpreter, "zeppelin.jdbc.auth.type", "string", "SIMPLE"
                    )
                    self.storePropertyToInterpreter(
                ...
                if interpreter and interpreter.strip():
                    ...
                    if len(splitted_line) > 1:
                        property_groups_list = (
                            splitted_line[1].replace(")", "").strip().split(",")
                        )
        ...
        for setting_key in interpreter_settings.keys():
            ...
            if (
                ...
                if (
                    ...
                    and "hive-server" not in jdbc_property_groups
                ):
                    self.storePropertyToInterpreter(
                        interpreter,
                        "hive.driver",
                    ...
                    self.storePropertyToInterpreter(
                        interpreter, "hive.user", "string", "hive"
                    )
                    self.storePropertyToInterpreter(
                ...
                if (
                    ...
                    self.storePropertyToInterpreter(
                        interpreter,
                        hive_interactive_properties_key + ".user",
                        "string",
                        "hive",
                    )
                    self.storePropertyToInterpreter(
                ...
                if (
                    ...
                    and "spark2" not in jdbc_property_groups
                ):
                    self.storePropertyToInterpreter(
                        interpreter,
                        "spark2.driver",
                    ...
                    self.storePropertyToInterpreter(
                        interpreter, "spark2.user", "string", "hive"
                    )
                    self.storePropertyToInterpreter(
                ...
                if (
                    ...
                    if (
                        "phoenix.phoenix.query.numberFormat"
                        not in interpreter["properties"]
                    ):
                        self.storePropertyToInterpreter(
                            interpreter,
    ...
    def storePropertyToInterpreter(
        self, interpreter, property_name, property_type, property_value, mode="set"
    ):
        if (
            property_name in interpreter["properties"]
            and "value" in interpreter["properties"][property_name]
        ):
            if mode == "set":
