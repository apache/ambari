...
class YARNRecommender(service_advisor.ServiceAdvisor):
    ...
    def recommendYARNConfigurationsFromHDP206(
        ...
        if "TEZ" in servicesList:
            ...
            putYarnProperty(
                "yarn.timeline-service.http-authentication.proxyuser.{0}.hosts".format(
                    ambari_user
                ),
                ambariHostName,
            )
            putYarnProperty(
                "yarn.timeline-service.http-authentication.proxyuser.{0}.groups".format(
                    ambari_user
                ),
                "*",
            )
    ...
    def recommendYARNConfigurationsFromHDP22(
        ...
        if nodeManagerHost is not None:
            ...
            cpuLimit = max(
                ...
                int(
                    floor(
                        nodeManagerHost["Hosts"]["cpu_count"]
                ...
                if not received_as_key_value_pair:
                    for prop, val in capacity_scheduler_properties.items():
                        if prop == "yarn.scheduler.capacity.resource-calculator":
                            ...
                            and ".resource-calculator" not in prop
                        ):
                            updated_cap_sched_configs_str = (
                                updated_cap_sched_configs_str + prop + "=" + val + "\n"
                            )
                    putCapScheProperty(
                        "capacity-scheduler", updated_cap_sched_configs_str
                    )
                    self.logger.info(
                    ...
                    self.logger.info(
                        "Updated 'capacity-scheduler' configs as a dictionary."
                    )
            else:
    ...
    def recommendYARNConfigurationsFromHDP23(
        ...
        if (
            ...
            ]["properties"]["ranger-yarn-plugin-enabled"]
        ...
        if rangerPluginEnabled and (rangerPluginEnabled.lower() == "Yes".lower()):
            ...
            putYarnSitePropertyAttributes(
                "yarn.authorization-provider", "delete", "true"
            )

    def recommendYARNConfigurationsFromHDP25(
        self, configurations, clusterData, services, hosts
    ):
        hsi_env_poperties = self.getServicesSiteProperties(
            services, "hive-interactive-env"
        )
    ...
    def recommendYARNConfigurationsFromHDP26(
        ...
        putYarnSiteProperty = self.putProperty(configurations, "yarn-site", services)
        putYarnEnvProperty = self.putProperty(configurations, "yarn-env", services)
        putResTypsProperty = self.putProperty(
            configurations, "resource-types", services
        )
        putCapScheProperty = self.putProperty(
            configurations, "capacity-scheduler", services
        )
        ...
        if (
            ...
            scheduler_monitor_enabled = services["configurations"]["yarn-site"][
                "properties"
        ...
        total_preemption_per_round = str(
            round(max(float(1) / len(hosts["items"]), 0.1), 2)
        )
        putYarnSiteProperty(
        ...
        if (
            ...
            ranger_yarn_plugin_enabled = (
                services["configurations"]["ranger-yarn-plugin-properties"][
                    "properties"
                ]["ranger-yarn-plugin-enabled"].lower()
                == "Yes".lower()
            )
        else:
        ...
        if (
            ...
            if (
                ...
                if (
                    ...
                    webapp_address = services["configurations"]["yarn-site"][
                        "properties"
                    ]["yarn.timeline-service.webapp.address"]
                    webservice_url = (
        ...
        if changed_configs_has_ats_cache_size:
            ...
            if host_mem is not None:
                yarn_timeline_app_cache_size = (
                    self.calculate_yarn_apptimelineserver_cache_size(host_mem)
                )
                putYarnSiteProperty(
                    "yarn.timeline-service.entity-group-fs-store.app-cache-size",
                    yarn_timeline_app_cache_size,
                ...
                self.logger.info(
                    ...
                    "using 'host_mem' = {1}".format(
                        yarn_timeline_app_cache_size, host_mem
                    )
                )
            else:
                self.logger.info(
        ...
        if yarn_timeline_app_cache_size is not None:
            ...
            putYarnEnvProperty(
                "apptimelineserver_heapsize", ats_heapsize
            )  # Value in MB
            self.logger.info(
                "Updated YARN config 'apptimelineserver_heapsize' as : {0}, ".format(
                    ats_heapsize
                )
            )
        ...
        if (
            ...
            restyps_list = (
                yarn_restyps.split(",")
                if len(yarn_restyps) > 1
                else yarn_restyps.split()
            )
            self.logger.info("new what is yarn_restyps: '{0}'.".format(restyps_list))

        if (
            "yarn-env" in services["configurations"]
            and "yarn_cgroups_enabled"
        ...
        if "container-executor" in services["configurations"]:
            ...
            if (
                ...
                docker_module_enabled = services["configurations"][
                    "container-executor"
                ]["properties"]["docker_module_enabled"]
            ...
            if (
                ...
                self.logger.info(
                    "new what is docker_allowed_devices: '{0}'.".format(allow_dev_list)
                )
            ...
            if (
                ...
                docker_allow_vol_drive = services["configurations"][
                    "container-executor"
                ]["properties"]["docker_allowed_volume-drivers"]
                allow_vol_drive_list = (
                ...
                self.logger.info(
                    "new what is docker_allowed_volume-drivers: '{0}'.".format(
                        allow_vol_drive_list
                    )
                )
            ...
            if (
                ...
                docker_allow_romounts = services["configurations"][
                    "container-executor"
                ]["properties"]["docker_allowed_ro-mounts"]
                allow_romounts_list = (
                ...
                self.logger.info(
                    "new what is docker.allowed.ro-mounts: '{0}'.".format(
                        allow_romounts_list
                    )
                )
            ...
            if (
                ...
                cg_root = services["configurations"]["container-executor"][
                    "properties"
                ]["cgroup_root"]
                cg_root_list = (
                    cg_root.split(",") if len(cg_root) > 1 else cg_root.split()
                )
            ...
            if (
                ...
                yn_hirch = services["configurations"]["container-executor"][
                    "properties"
                ]["yarn_hierarchy"]
                yn_hirch_list = (
                    yn_hirch.split(",") if len(yn_hirch) > 1 else yn_hirch.split()
                )
        ...
        if "yarn-site" in services["configurations"]:
            if (
                ...
                self.logger.info(
                    "new what is yarn.nodemanager.linux-container-executor.cgroups.hierarchy: '{0}'.".format(
                        lce_cgrp_hirch_list
                    )
                )
            ...
            if (
                ...
                lce_cgrp_mtp_list = (
                    lce_cgrp_mtp.split(",")
                    if len(lce_cgrp_mtp) > 1
                    else lce_cgrp_mtp.split()
                )
                self.logger.info(
                    "new what is yarn.nodemanager.linux-container-executor.cgroups.mount-path: '{0}'.".format(
                        lce_cgrp_mtp_list
                    )
                )
            ...
            if (
                ...
                self.logger.info(
                    "new what is yarn.nodemanager.resource-plugins.gpu.allowed-gpu-devices: '{0}'.".format(
                        rp_gpu_agd_list
                    )
                )
            ...
            if (
                ...
                self.logger.info(
                    "new what is yarn.nodemanager.resource-plugins.gpu.docker-plugin: '{0}'.".format(
                        rp_gpu_dp_list
                    )
                )
            ...
            if (
                ...
                rp_gpu_dp_nv1_ep = services["configurations"]["yarn-site"][
                    "properties"
                ][
                    "yarn.nodemanager.resource-plugins.gpu.docker-plugin.nvidiadocker-v1.endpoint"
                ]
                ...
                self.logger.info(
                    "new what is yarn.nodemanager.resource-plugins.gpu.docker-plugin.nvidiadocker-v1.endpoint: '{0}'.".format(
                        rp_gpu_dp_nv1_ep_list
                    )
                )
        ...
        if gpu_module_enabled and gpu_module_enabled.lower() == "true":
            ...
            if docker_module_enabled and docker_module_enabled.lower() == "true":
                ...
                if "regex:^/dev/nvidia.*$" in allow_dev_list:
                    self.logger.info(
                        "gpu related devices already in docker.allowed.devices."
                    )
                else:
                ...
                if "nvidia-docker" in allow_vol_drive_list:
                    self.logger.info(
                        "nvidia-docker already in docker_allowed_volume-drivers."
                    )
                else:
                    allow_vol_drive_list.append("nvidia-docker")
                    docker_allow_vol_drive = ",".join(
                        str(x) for x in allow_vol_drive_list
                    )
                    putCanExecProperty(
                        "docker_allowed_volume-drivers", docker_allow_vol_drive
                    )

                # add nvidia_driver_<version> if it is absent in docker allowed ro-mounts
                if "regex:^nvidia_driver_.*$" in allow_romounts_list:
                    self.logger.info(
                        "nvidia_driver_<version> already in allow_romounts_list."
                    )
                else:
                    allow_romounts_list.append("regex:^nvidia_driver_.*$")
                    docker_allow_romounts = ",".join(
                        str(x) for x in allow_romounts_list
                    )
                    putCanExecProperty(
                        "docker_allowed_ro-mounts", docker_allow_romounts
                    )
    ...
    def recommendYARNConfigurationsFromHDP30(
        ...
        if (
            ...
            and "yarn.scheduler.capacity.root.acl_administer_queue"
            in cap_sched_properties
        ):
            ...
            if not (
                "*" in acl_administer_queue_items
                or hive_user in acl_administer_queue_items
            ):
                if not received_as_key_value_pair:
                    updated_cap_sched_configs_str = ""
        ...
        if (
            ...
            if not (
                "*" in acl_administer_queue_items
                or sprak_user in acl_administer_queue_items
            ):
                if not received_as_key_value_pair:
                    updated_cap_sched_configs_str = ""
                    for prop, val in cap_sched_properties.items():
        ...
        if (
            ...
            putYarnHBaseEnv = self.putProperty(
                configurations, "yarn-hbase-env", services
            )
    ...
    def recommendConfigurationsForSSO(
        self, configurations, clusterData, services, hosts
    ):
        ambari_configuration = self.get_ambari_configuration(services)
        ambari_sso_details = (
            ambari_configuration.get_ambari_sso_details()
            if ambari_configuration
            else None
        )

        if ambari_sso_details and ambari_sso_details.is_managing_services():
            putYarnSiteProperty = self.putProperty(
                configurations, "yarn-site", services
            )
    ...
    def _is_kerberos_enabled(self, config):
        ...
        return config and (
            (
                "yarn-site" in config
                and "hadoop.security.authentication"
                in config["yarn-site"]["properties"]
            ...
            or (
                "core-site" in config
                and "hadoop.security.authentication"
                in config["core-site"]["properties"]
  ...
  """
    ...
    def calculate_yarn_apptimelineserver_heapsize(
        ...
        if host_mem < 4096:
            ...
            ats_heapsize = int(
                min(
                    math.floor(host_mem / 2),
                    int(yarn_timeline_app_cache_size) * 500 + 3072,
                )
            )
        return ats_heapsize
  ...
  """
    ...
    def calculate_yarn_apptimelineserver_cache_size(self, host_mem):
        ...
        self.logger.info(
            "Calculated and returning 'yarn_timeline_app_cache_size' : {0}".format(
                yarn_timeline_app_cache_size
            )
        )
  ...
  """
    ...
    def read_yarn_apptimelineserver_cache_size(self, services):
        ...
        if (
            yarn_site_in_services
            and yarn_ats_app_cache_size_config in yarn_site_in_services
        ):
            yarn_ats_app_cache_size = yarn_site_in_services[
                yarn_ats_app_cache_size_config
            ]
            self.logger.info(
                "'yarn.scheduler.minimum-allocation-mb' read from services as : {0}".format(
                    yarn_ats_app_cache_size
                )
            )

        if not yarn_ats_app_cache_size:
            self.logger.error(
                "'{0}' was not found in the services".format(
                    yarn_ats_app_cache_size_config
                )
            )
    ...
    def update_timeline_reader_address(self, configurations, services, property_name):
        ...
        if yarn_site and property_name in yarn_site:
            timeline_hosts = self.getHostsForComponent(
            ...
            if old_address and old_address.count(":") <= 1 and len(timeline_hosts) == 1:
                ...
                if old_address != new_address:
                    ...
                    self.logger.info(
                        "Updated YARN config {0} to {1}".format(
                            property_name, new_address
                        )
                    )
    ...
    def updateLlapConfigs(self, configurations, services, hosts, llap_queue_name):
        ...
        self.logger.info(
            "Is cluster create operation ? = {0}".format(is_cluster_create_opr)
        )
        ...
        if capacity_scheduler_properties:
            ...
            if len(leafQueueNames) == 0:
                self.logger.error(
                    "Queue(s) couldn't be retrieved from capacity-scheduler."
                )
                return
            ...
            if (
                ...
                if len(leafQueueNames) == 1 or (
                    ...
                    putHiveInteractiveSiteProperty(
                        "hive.llap.daemon.queue.name", llap_queue_name
                    )
                    putHiveInteractiveSiteProperty(
            ...
            self.logger.info(
                "DBG: llap_named_queue_selected_in_curr_invocation = {0}".format(
            ...
            if (
                ...
                putHiveInteractiveEnvPropertyAttribute(
                    "num_llap_nodes", "read_only", "false"
                )
            ...
            if not llap_named_queue_selected_in_curr_invocation:  # We would be creating the 'llap' queue later. Thus, cap-sched doesn't have
                ...
                if llap_daemon_selected_queue_name:
                    llap_selected_queue_state = (
                        self.__getQueueStateFromCapacityScheduler(
                            capacity_scheduler_properties,
                            llap_daemon_selected_queue_name,
                        )
                    )
                    if (
                        llap_selected_queue_state is None
                        or llap_selected_queue_state == "STOPPED"
                    ):
                        self.logger.error(
        ...
        if "changed-configurations" in services.keys():
            config_names_to_be_checked = set(
                ["num_llap_nodes", "enable_hive_interactive"]
            )
        ...
        if (
            ...
            self.logger.info(
                "DBG: LLAP parameters not modified. Not adjusting LLAP configs."
            )
            self.logger.info(
                "DBG: Current 'changed-configuration' received is : {0}".format(
        ...
        min_nodes_required = int(
            ceil(min_memory_required / yarn_nm_mem_in_mb_normalized)
        )
        self.logger.info(
        ...
        self.logger.info(
            "DBG: selected_queue_is_ambari_managed_llap = {0}".format(
                selected_queue_is_ambari_managed_llap
            )
        )
        if not selected_queue_is_ambari_managed_llap:
            ...
            self.logger.info(
                "Setting config 'num_llap_nodes' as : {0}".format(
                    num_llap_nodes_requested
                )
            )
            ...
            hive_tez_am_cap_available = (
                queue_am_fraction_perc * total_llap_mem_normalized
            )
            self.logger.info(
            ...
            self.logger.info(
                "DBG: Calculated '{0}' queue capacity percent = {1}.".format(
            ...
            if llap_named_queue_cap_fraction > 100:
                self.logger.warning(
                    "Calculated '{0}' queue size = {1}. Cannot be > 100.".format(
                        llap_queue_name, llap_named_queue_cap_fraction
                    )
                )
                self.recommendDefaultLlapConfiguration(configurations, services, hosts)
            ...
            self.checkAndManageLlapQueue(
                ...
                hosts,
                llap_queue_name,
                llap_named_queue_cap_fraction,
            )
            hive_tez_am_cap_available = total_llap_mem_normalized
            self.logger.info(
                "DBG: hive_tez_am_cap_available : {0}".format(hive_tez_am_cap_available)
            )
        ...
        if llap_mem_for_tezAm_and_daemons < 2 * yarn_min_container_size:
            self.logger.warning(
                "Not enough capacity available on the cluster to run LLAP"
            )
            self.recommendDefaultLlapConfiguration(configurations, services, hosts)
            return
        ...
        if not llap_concurrency_in_changed_configs:
            if max_executors_per_node <= 0:
                self.logger.warning(
                    "Calculated 'max_executors_per_node' = {0}. Expected value >= 1.".format(
            ...
            if (
                llap_concurrency * normalized_tez_am_container_size
                > hive_tez_am_cap_available
            ):
                ...
                if llap_concurrency <= 0:
                    self.logger.warning(
                        "DBG: Calculated 'LLAP Concurrent Queries' = {0}. Expected value >= 1.".format(
                            llap_concurrency
                        )
                    )
                    self.recommendDefaultLlapConfiguration(
            ...
            if "hive.server2.tez.sessions.per.default.queue" in hsi_site:
                llap_concurrency = int(
                    hsi_site["hive.server2.tez.sessions.per.default.queue"]
                )
                if llap_concurrency <= 0:
                    self.logger.warning(
                        "'hive.server2.tez.sessions.per.default.queue' current value : {0}. Expected value : >= 1".format(
                            llap_concurrency
                        )
                    )
                    self.recommendDefaultLlapConfiguration(
                    ...
                    return
                self.logger.info(
                    "DBG: Read 'llap_concurrency' : {0}".format(llap_concurrency)
                )
            else:
                llap_concurrency = 1
                self.logger.warning(
        ...
        if (
            num_llap_nodes == num_llap_nodes_requested
            and num_llap_nodes == node_manager_cnt
        ):
        ...
        max_single_node_mem_available_for_daemon = self._normalizeDown(
            yarn_nm_mem_in_mb_normalized - min_mem_reserved_per_node,
            yarn_min_container_size,
        )
        if (
        ...
        if num_executors_per_node_max < 1:
            self.logger.warning(
                "Calculated 'Max. Executors per Node' = {0}. Expected values >= 1.".format(
        ...
        if num_executors_per_node <= 0:
            self.logger.warning(
                "Calculated 'Number of Executors Per Node' = {0}. Expected value >= 1".format(
        ...
        total_mem_for_executors_per_node = (
            num_executors_per_node * mem_per_thread_for_llap
        )
        ...
        self.logger.info(
            "DBG: Calculated llap_app_heap_size : {0}, using following : total_mem_for_executors : {1}".format(
                llap_xmx, total_mem_for_executors_per_node
            )
        )
        ...
        if (
            ...
            self.logger.info(
                ...
                "{2}".format(
                    hive_server_interactive_heapsize, llap_concurrency, host_mem
                )
            )
        ...
        if is_cluster_create_opr or changed_configs_has_enable_hive_int:
            ...
            self.logger.info(
                "DBG: Setting 'tez.am.resource.memory.mb' config value as : {0}".format(
                    normalized_tez_am_container_size
                )
            )
        ...
        if num_llap_nodes != num_llap_nodes_requested:
            self.logger.info(
                "DBG: User requested num_llap_nodes : {0}, but used/adjusted value for calculations is : {1}".format(
                    num_llap_nodes_requested, num_llap_nodes
                )
            )
        else:
            self.logger.info(
                "DBG: Used num_llap_nodes for calculations : {0}".format(
                    num_llap_nodes_requested
                )
            )
        ...
        if (
            ...
            putHiveInteractiveEnvProperty(
                "num_llap_nodes_for_llap_daemons", num_llap_nodes
            )
            self.logger.info(
                "DBG: Setting config 'num_llap_nodes_for_llap_daemons' as : {0}".format(
                    num_llap_nodes
                )
            )
        ...
        if is_cluster_create_opr or changed_configs_has_enable_hive_int:
            ...
            putHiveInteractiveSiteProperty(
                "hive.tez.container.size", mem_per_thread_for_llap
            )
            self.logger.info(
                "DBG: Setting 'hive.tez.container.size' config value as : {0}".format(
                    mem_per_thread_for_llap
                )
            )
        ...
        if (
            ...
            if (
                services["configurations"]["tez-site"]["properties"][
        ...
        self.logger.info(
            "DBG: Putting num_executors_per_node as {0}".format(num_executors_per_node)
        )
        putHiveInteractiveSiteProperty(
        ...
        putHiveInteractiveSitePropertyAttribute(
            "hive.llap.io.memory.size", "minimum", 0
        )
        putHiveInteractiveSitePropertyAttribute(
    ...
    def get_num_llap_nodes(self, services, configurations):
        ...
        hsi_env_properties = self.getSiteProperties(
            configurations, "hive-interactive-env"
        )
        num_llap_nodes = 0
        ...
        if hsi_env_properties and "num_llap_nodes" in hsi_env_properties:
            ...
            self.logger.error(
                "Couldn't retrieve Hive Server 'num_llap_nodes' config. Setting value to {0}".format(
                    num_llap_nodes
                )
            )
    ...
    def calculate_mem_per_thread_for_llap(
        ...
        if is_cluster_create_opr or enable_hive_interactive_1st_invocation:
            if nm_mem_per_node_normalized <= 1024:
                calculated_hive_tez_container_size = min(
                    512, nm_mem_per_node_normalized
                )
            ...
            self.logger.info(
                "DBG: Calculated and returning 'hive_tez_container_size' : {0}".format(
                    calculated_hive_tez_container_size
                )
            )
            return calculated_hive_tez_container_size
        else:
            self.logger.info(
                "DBG: Returning 'hive_tez_container_size' : {0}".format(
    ...
    def get_llap_headroom_space(self, services, configurations):
        ...
        if (
            ...
            and "llap_headroom_space"
            in configurations["hive-interactive-env"]["properties"]
        ):
            ...
            self.logger.info(
                "'llap_headroom_space' read from configurations as : {0}".format(
                    llap_headroom_space
                )
            )
        ...
        if llap_headroom_space is None:
            ...
            if (
                ...
                self.logger.info(
                    "'llap_headroom_space' read from services as : {0}".format(
                        llap_headroom_space
                    )
                )
                ...
                or (
                    ...
                    and (
                        (
                            ...
                            and enabled_hive_int_in_changed_configs
                        )
                        or (
                    ...
                    for prop, val in list(capacity_scheduler_properties.items()):
                        if llap_queue_name not in prop:
                            if prop == "yarn.scheduler.capacity.root.queues":
                                ...
                                prop == "yarn.scheduler.capacity.root.default.capacity"
                            ):
                                updated_cap_sched_configs_str = (
                                    updated_cap_sched_configs_str
                                    + prop
...
yarn.scheduler.capacity.root.{0}.maximum-am-resource-percent=1""".format(
                ...
                if updated_cap_sched_configs_str or updated_cap_sched_configs_as_dict:
                    if len(leafQueueNames) == 1:  # 'llap' queue didn't exist before
                        ...
                        self.logger.info(
                            "Adjusted YARN Queue : '{0}'. Current capacity : {1}%. State: RUNNING.".format(
                                llap_queue_name, llap_queue_cap_perc
                            )
                        )
                        self.logger.info(
                            "Adjusted 'default' queue capacity to : {0}%".format(
                                adjusted_default_queue_cap
                            )
                        )
                ...
                self.logger.debug(
                    "Not creating/adjusting {0} queue. Current YARN queues : {1}".format(
                        llap_queue_name, list(leafQueueNames)
                    )
                )
        else:
        ...
        if capacity_scheduler_properties:
            ...
            if (
                ...
                if (
                    ...
                    self.logger.error(
                        "{0} queue 'state' property not present in capacity scheduler. Skipping adjusting queues.".format(
                            llap_queue_name
                        )
                    )
                    return
                if currLlapQueueState == "RUNNING":
                    ...
                    for prop, val in capacity_scheduler_properties.items():
                        ...
                        if llap_queue_name not in prop:
                            ...
                            if (
                                ...
                                updated_llap_queue_configs = (
                                    updated_llap_queue_configs + prop + "=0\n"
                                )
                            elif (
                                prop
                                == "yarn.scheduler.capacity.root."
                    ...
                    self.logger.debug(
                        "{0} queue state is : {1}. Skipping adjusting queues.".format(
                            llap_queue_name, currLlapQueueState
                        )
                    )
                    return
                ...
                self.logger.debug(
                    "Not removing '{0}' queue as number of Queues not equal to 2. Current YARN queues : {1}".format(
                        llap_queue_name, list(leafQueueNames)
                    )
                )
        else:
...
class MAPREDUCE2Recommender(YARNRecommender):
    ...
    def recommendMapReduce2ConfigurationsFromHDP206(
        ...
        putMapredProperty(
            "yarn.app.mapreduce.am.resource.mb", int(clusterData["amMemory"])
        )
        putMapredProperty(
        ...
        putMapredProperty(
            "mapreduce.reduce.memory.mb", int(clusterData["reduceMemory"])
        )
        putMapredProperty(
        ...
        putMapredProperty(
            ...
            min(int(round(0.4 * clusterData["mapMemory"])), 1024),
        )
    ...
    def recommendMapReduce2ConfigurationsFromHDP22(
        ...
        putMapredProperty(
            ...
            min(
                ...
                max(
                    ...
                    min(
                        2 * int(clusterData["ramPerContainer"]), int(nodemanagerMinRam)
                    ),
                ),
            ),
        )
        ...
        mapredMapXmx = int(
            0.8
            * int(
                configurations["mapred-site"]["properties"]["mapreduce.map.memory.mb"]
            )
        )
        putMapredProperty(
            "mapreduce.map.java.opts",
    ...
    def recommendConfigurationsForSSO(
        ...
        if ambari_sso_details and ambari_sso_details.is_managing_services():
            putMapRedSiteProperty = self.putProperty(
                configurations, "mapred-site", services
            )
    ...
    def _is_kerberos_enabled(self, config):
        ...
        return config and (
            (
                ...
                and "hadoop.security.authentication"
                in config["mapred-site"]["properties"]
                and config["mapred-site"]["properties"][
                    "hadoop.security.authentication"
                ]
                == "kerberos"
            )
...
class MAPREDUCE2Validator(service_advisor.ServiceAdvisor):
    ...
    def validateMapReduce2SiteConfigurationsFromHDP22(
                ...
                validationItems.append(
                    {
                        "config-name": "mapreduce.map.java.opts",
                        "item": self.getWarnItem(
                            "mapreduce.map.java.opts Xmx should be less than mapreduce.map.memory.mb ({0})".format(
        ...
        if "mapreduce.reduce.java.opts" in properties and self.checkXmxValueFormat(
            ...
            mapreduceReduceMemoryMb = self.to_number(
                properties["mapreduce.reduce.memory.mb"]
            )
                ...
                validationItems.append(
                    {
                        "config-name": "mapreduce.reduce.java.opts",
                        "item": self.getWarnItem(
                            "mapreduce.reduce.java.opts Xmx should be less than mapreduce.reduce.memory.mb ({0})".format(
                ...
                validationItems.append(
                    {
                        "config-name": "yarn.app.mapreduce.am.command-opts",
                        "item": self.getWarnItem(
                            "yarn.app.mapreduce.am.command-opts Xmx should be less than yarn.app.mapreduce.am.resource.mb ({0})".format(
