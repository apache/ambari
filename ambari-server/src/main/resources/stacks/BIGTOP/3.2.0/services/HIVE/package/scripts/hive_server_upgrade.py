...
def deregister():
    ...
    command = format(
        "hive --config {hive_conf_dir} --service hiveserver2 --deregister "
...
def _get_hive_execute_path(stack_version_formatted):
    ...
    if formatted_stack_version and check_stack_feature(
        ...
        # hive_bin
        new_hive_bin = format("{stack_root}/{stack_version_formatted}/usr/lib/hive/bin")
        if (os.pathsep + params.hive_bin_dir) in hive_execute_path:
            hive_execute_path = hive_execute_path.replace(
                os.pathsep + params.hive_bin_dir, os.pathsep + new_hive_bin
            )
        # hadoop_bin_dir
        new_hadoop_bin = stack_select.get_hadoop_dir_for_stack_version(
            "bin", stack_version_formatted
        )
        old_hadoop_bin = params.hadoop_bin_dir
        if (
            new_hadoop_bin
            and len(new_hadoop_bin) > 0
            and (os.pathsep + old_hadoop_bin) in hive_execute_path
        ):
            hive_execute_path = hive_execute_path.replace(
...
def _get_current_hiveserver_version():
    ...
    try:
        ...
        if formatted_source_version and check_stack_feature(
            StackFeature.ROLLING_UPGRADE, formatted_source_version
        ):
            version_hive_bin = format("{stack_root}/{source_version}/usr/lib/hive/bin")
    ...
    if return_code != 0:
        raise Fail(
            "Unable to determine the current HiveServer2 version because of a non-zero return code of {0}".format(
                str(return_code)
            )
        )
    ...
    if match:
        ...
        raise Fail(
            'The extracted hiveserver2 version "{0}" does not matching any known pattern'.format(
                output
            )
        )
