...
class KafkaBroker(Script):
    ...
    def stop(self, env, upgrade_type=None):
        ...
        for i in range(max_wait):
            Logger.info(
                "Waiting for Kafka Broker stop, current pid: {0}, seconds: {1}s".format(
                    pid, i + 1
                )
            )
            try:
                ...
                Logger.info(
                    "Kafka Broker is not running, delete pid file: {0}".format(
                        params.kafka_pid_file
                    )
                )
    ...
    def disable_security(self, env):
        ...
        if not params.secure_acls:
            ...
            return
        Execute(
            "{0} --zookeeper.connect {1} --zookeeper.acl=unsecure".format(
                params.kafka_security_migrator, params.zookeeper_connect
            ),
