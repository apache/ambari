...
if security_enabled:
    ...
    if has_ats or has_atsv2:
        ...
        yarn_timelineservice_principal_name = (
            yarn_timelineservice_principal_name.replace("_HOST", hostname.lower())
        )
...
docker_privileged_containers_enabled = str(
    config["configurations"]["container-executor"][
        "docker_privileged-containers_enabled"
    ]
).lower()
...
if yarn_timeline_service_enabled:
    if is_empty(yarn_timeline_service_versions):
        if (
            yarn_timeline_service_version == "2.0"
            or yarn_timeline_service_version == "2"
        ):
...
if security_enabled and has_atsv2:
    ...
    yarn_hbase_regionserver_principal_name = config["configurations"][
        "yarn-hbase-site"
...
if hbase_within_cluster:
    if (
        "hbase_master_hosts" in master_configs
        and "hbase-site" in config["configurations"]
    ):
