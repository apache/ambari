...
stack_supports_ranger_kms_ssl = check_stack_feature(
    StackFeature.RANGER_KMS_SSL, version_for_stack_feature_checks
)
...
ranger_kms_audit_log_maxfilesize = default(
    "/configurations/kms-log4j/ranger_kms_audit_log_maxfilesize", 256
)
ranger_kms_audit_log_maxbackupindex = default(
    "/configurations/kms-log4j/ranger_kms_audit_log_maxbackupindex", 20
)
ranger_kms_log_maxfilesize = default(
    "/configurations/kms-log4j/ranger_kms_log_maxfilesize", 256
)
ranger_kms_log_maxbackupindex = default(
    "/configurations/kms-log4j/ranger_kms_log_maxbackupindex", 20
)
...
if has_ranger_admin:
    ...
    if (
        "admin-properties" in config["configurations"]
        and "policymgr_external_url" in config["configurations"]["admin-properties"]
        and policymgr_mgr_url.endswith("/")
    ):
    ...
    ambari_ranger_admin = config["configurations"]["ranger-env"][
        "ranger_admin_username"
    ]
    ambari_ranger_password = config["configurations"]["ranger-env"][
...
driver_curl_target = format("{kms_lib_path}/{jdbc_jar_name}")
previous_jdbc_jar = format("{kms_lib_path}/{previous_jdbc_jar_name}")
ews_lib_jar_path = format("{kms_lib_path}/{jdbc_jar_name}")
...
if has_ranger_admin:
    ...
    if stack_supports_ranger_audit_db:
        if xa_audit_db_flavor == "mysql":
            ...
            xa_previous_jdbc_jar_name = default(
                "/ambariLevelParams/previous_custom_mysql_jdbc_name", None
            )
    ...
    downloaded_connector_path = (
        format("{tmp_dir}/{jdbc_jar}") if stack_supports_ranger_audit_db else None
    )
    driver_source = (
        format("{jdk_location}/{jdbc_jar}") if stack_supports_ranger_audit_db else None
    )
    driver_target = (
        format("{kms_lib_path}/{jdbc_jar}") if stack_supports_ranger_audit_db else None
    )
    xa_previous_jdbc_jar = (
        format("{kms_lib_path}/{previous_jdbc_jar_name}")
        if stack_supports_ranger_audit_db
        else None
    )

repo_config_username = config["configurations"]["kms-properties"][
    "REPOSITORY_CONFIG_USERNAME"
]
repo_config_password = str(
...
ranger_kms_jdbc_connection_url = config["configurations"]["dbks-site"][
    "ranger.ks.jpa.jdbc.url"
]
ranger_kms_jdbc_driver = config["configurations"]["dbks-site"][
...
kms_plugin_password_properties = [
    "xasecure.policymgr.clientssl.keystore.password",
    "xasecure.policymgr.clientssl.truststore.password",
]
dbks_site_password_properties = [
...
ranger_kms_cred_ssl_path = config["configurations"]["ranger-kms-site"][
...
ranger_kms_ssl_passwd = config["configurations"]["ranger-kms-site"][
...
if "viewfs-mount-table" in config["configurations"]:
    ...
    if "content" in mount_table and mount_table["content"].strip():
        mount_table_xml_inclusion_file_full_path = os.path.join(
            kms_conf_dir, xml_inclusion_file_name
        )
        mount_table_content = mount_table["content"]

# Ranger KMS Service maximum heap size configurations
ranger_kms_max_heap_size = default(
    "/configurations/kms-env/ranger_kms_max_heap_size", "1g"
)
...
keysecure_login_password_alias = config["configurations"]["dbks-site"][
