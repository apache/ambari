    ...
    try:
        if security_enabled and stack_supports_ranger_kerberos:
            ...
            if admin_http_code == 200:
                ...
                result_code = "WARNING"
                label = (
                    "Ranger Admin service is not reachable, please restart the service"
                )
...
def check_ranger_login(ranger_auth_link, username, password):
    ...
    try:
        ...
        logger.exception(
            "Error during Ranger service authentication. Http status code - {0}. {1}".format(
                e.code, e.read()
            )
        )
        return e.code
...
def get_ranger_user(ranger_get_user, username, password, user):
    ...
    try:
        ...
        logger.exception(
            "Error getting user from Ranger service. Http status code - {0}. {1}".format(
                e.code, e.read()
            )
        )
        return False
