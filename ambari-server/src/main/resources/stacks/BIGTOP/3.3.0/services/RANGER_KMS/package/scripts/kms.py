  ...
  if params.has_ranger_admin:
    ...
    if len(hadoop_jar_files) != 0:
      for f in hadoop_jar_files:
        Execute((format('{java_home}/bin/jar'),'-uf', format('{kms_lib_path}/{f}'), format('{kms_home}/ews/webapp/META-INF/services/org.apache.hadoop.crypto.key.KeyProviderFactory')),
          user=params.kms_user)

        File(format('{kms_lib_path}/{f}'), owner=params.kms_user, group=params.kms_group)
...
def kms(upgrade_type=None):
  ...
  if params.has_ranger_admin:
    ...
    if params.stack_support_kms_hsm and params.enable_kms_hsm:
      do_keystore_setup(params.credential_provider_path, params.hms_partition_alias, str(params.hms_partition_passwd))
    ...
    File(format("{params.kms_conf_dir}/kms-logback.xml"),
...
def copy_jdbc_connector(kms_home):
  ...
  if params.db_flavor.lower() == 'sqla':
    ...
    Execute(('cp', '--remove-destination', params.jar_path_in_archive, os.path.join(params.kms_lib_path, params.jdbc_jar_name)),
      path=["/bin", "/usr/bin/"],
      sudo=True)
    ...
    Execute(('cp', '--remove-destination', params.downloaded_custom_connector, os.path.join(params.kms_lib_path, params.jdbc_jar_name)),
      path=["/bin", "/usr/bin/"],
      sudo=True)
  ...
  if params.db_flavor.lower() == 'sqla':
    ModifyPropertiesFile(format("{kms_home}/install.properties"),
      properties = {'SQL_CONNECTOR_JAR': format('{params.kms_lib_path}/sajdbc4.jar')},
      owner = params.kms_user,
    )
  else:
    ModifyPropertiesFile(format("{kms_home}/install.properties"),
