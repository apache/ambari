...
def password_validation(password, key):
    ...
    if password.strip() == "":
        raise Fail(
            "Blank password is not allowed for {0} property. Please enter valid password.".format(
                key
            )
        )
    if re.search("[\\\`'\"]", password):
        raise Fail(
            "{0} password contains one of the unsupported special characters like \" ' \ `".format(
                key
            )
        )
    else:
...
def setup_java_patch():
    ...
    if params.has_ranger_admin:
        kms_home = params.kms_home
        setup_java_patch = format(
            "ambari-python-wrap {kms_home}/db_setup.py -javapatch"
        )
        ...
        if len(hadoop_jar_files) != 0:
            for f in hadoop_jar_files:
                Execute(
                    (
                        format("{java_home}/bin/jar"),
                        "-uf",
                        format("{kms_lib_path}/{f}"),
                        format(
...
def kms(upgrade_type=None):
    ...
    if params.has_ranger_admin:
        ...
        generate_logfeeder_input_config(
            "ranger-kms",
            Template("input.config-ranger-kms.json.j2", extra_imports=[default]),
        )
...
def copy_jdbc_connector(kms_home):
    ...
    if params.db_flavor.lower() == "sqla":
        ...
        Execute(
            (
                ...
                os.path.join(params.kms_lib_path, params.jdbc_jar_name),
            ),
            path=["/bin", "/usr/bin/"],
            sudo=True,
        )
    ...
    if params.db_flavor.lower() == "sqla":
        ModifyPropertiesFile(
            format("{kms_home}/install.properties"),
            properties={
                "SQL_CONNECTOR_JAR": format("{params.kms_lib_path}/sajdbc4.jar")
            },
            owner=params.kms_user,
        )
    else:
...
def enable_kms_plugin():
    ...
    if params.has_ranger_admin:
        ...
        if params.stack_supports_ranger_kerberos and params.security_enabled:
            if (
                not is_empty(params.rangerkms_principal)
        ...
        if params.plugin_audit_password_property in plugin_audit_properties_copy:
            plugin_audit_properties_copy[params.plugin_audit_password_property] = (
                "crypted"
            )
...
def check_ranger_service():
    ...
    if response_code is not None and response_code == 200:
        ...
        if user_resp_code is not None and user_resp_code == 200:
            get_repo_flag = get_repo(
                policymgr_mgr_url, params.repo_name, ambari_username_password_for_ranger
            )
...
@safe_retry(
    times=5, sleep_time=8, backoff_factor=1.5, err_class=Fail, return_on_fail=False
)
...
def get_repo(url, name, usernamepassword):
    try:
        ...
        if isinstance(e, urllib.error.HTTPError):
            raise Fail(
                "Error getting {0} service. Http status code - {1}. \n {2}".format(
                    name, e.code, e.read()
                )
            )
        else:
            raise Fail(
                "Error getting {0} service. Reason - {1}.".format(name, e.reason)
            )
    except socket.timeout as e:
        raise Fail("Error creating service. Reason - {0}".format(e))
...
def check_ranger_service_support_kerberos(user, keytab, principal):
    ...
    if response_code is not None and response_code[0] == 200:
        get_repo_name_response = ranger_adm_obj.get_repository_by_name_curl(
            user, keytab, principal, params.repo_name, "kms", "true", is_keyadmin=True
        )
        if get_repo_name_response is not None:
            Logger.info(
                "KMS repository {0} exist".format(get_repo_name_response["name"])
            )
            return True
        else:
            create_repo_response = ranger_adm_obj.create_repository_curl(
                user,
                keytab,
                principal,
                params.repo_name,
...
def update_password_configs():
    ...
    ModifyPropertiesFile(
        ...
        properties={
            "db_root_password": "_",
            "db_password": "_",
            "KMS_MASTER_KEY_PASSWD": "_",
            "REPOSITORY_CONFIG_PASSWORD": "_",
        },
        owner=params.kms_user,
    )
