...
class Ranger_KMSServiceAdvisor(service_advisor.ServiceAdvisor):
    ...
    def isKerberosEnabled(services, configurations):
        ...
        if (
            ...
            in configurations["kms-site"]["properties"]
        ):
            return (
...
class RangerKMSRecommender(service_advisor.ServiceAdvisor):
    ...
    def recommendRangerKMSConfigurationsFromHDP23(
        ...
        putRangerKmsDbksProperty = self.putProperty(
            configurations, "dbks-site", services
        )
        putRangerKmsProperty = self.putProperty(
            configurations, "kms-properties", services
        )
        ...
        putRangerKmsSiteProperty = self.putProperty(
            configurations, "kms-site", services
        )
        ...
        if "kms-properties" in services["configurations"] and (
            ...
            rangerKmsDbFlavor = services["configurations"]["kms-properties"][
                "properties"
            ]["DB_FLAVOR"]

            if (
                "db_host" in services["configurations"]["kms-properties"]["properties"]
            ) and (
                "db_name" in services["configurations"]["kms-properties"]["properties"]
            ):
                rangerKmsDbHost = services["configurations"]["kms-properties"][
                    "properties"
                ]["db_host"]
                rangerKmsDbName = services["configurations"]["kms-properties"][
                    "properties"
                ]["db_name"]
                ...
                ranger_kms_db_url_dict = {
                    "MYSQL": {
                        ...
                        + self.getDBConnectionHostPort(
                            rangerKmsDbFlavor, rangerKmsDbHost
                        )
                        + "/"
                        + rangerKmsDbName,
                    },
        ...
        if (
            ...
            if kmsUserOld is not None and kmsUser != kmsUserOld:
                putCoreSitePropertyAttribute(
                    "hadoop.proxyuser.{0}.groups".format(kmsUserOld), "delete", "true"
                )
                services["forced-configurations"].append(
                    {
                        "type": "core-site",
                        "name": "hadoop.proxyuser.{0}.groups".format(kmsUserOld),
                    }
                )
                services["forced-configurations"].append(
                    {
                        "type": "core-site",
                        "name": "hadoop.proxyuser.{0}.groups".format(kmsUser),
                    }
                )
        ...
        if security_enabled:
            ...
            putRangerKmsSiteProperty(
                "hadoop.kms.proxyuser.{0}.users".format(ambari_user), "*"
            )
            putRangerKmsSiteProperty(
                "hadoop.kms.proxyuser.{0}.hosts".format(ambari_user), "*"
            )
            # adding for HTTP
            ...
            putRangerKmsSitePropertyAttribute(
                "hadoop.kms.proxyuser.{0}.hosts".format(ambari_user), "delete", "true"
            )
            putRangerKmsSitePropertyAttribute(
                "hadoop.kms.proxyuser.{0}.users".format(ambari_user), "delete", "true"
            )
    ...
    def recommendRangerKMSConfigurationsFromHDP25(
        ...
        if security_enabled:
            # recommendations for kms proxy related properties
            self.recommendKMSProxyUsers(
                configurations, services, hosts, required_services
            )
        else:
    ...
    def recommendRangerKMSConfigurationsFromHDP26(
        ...
        if (
            ...
            ranger_kms_ssl_port = services["configurations"]["ranger-kms-site"][
                "properties"
    ...
    def recommendRangerKMSConfigurationsFromHDP30(
        ...
        if (
            ...
            and (
                "DB_FLAVOR"
                in services["configurations"]["kms-properties"]["properties"]
            )
            and (
                "db_host" in services["configurations"]["kms-properties"]["properties"]
            )
            ...
            ranger_kms_db_privelege_url_dict = {
                ...
                "ORACLE": {
                    ...
                    + self.getOracleDBConnectionHostPort(
                        rangerKmsDbFlavor, rangerKmsDbHost, None
                    )
                },
                "POSTGRES": {
        ...
        if db_type is None or db_type == "":
            ...
            if colon_count == 0:
                if db_type in DB_TYPE_DEFAULT_PORT_MAP:
                    connection_string = (
                        db_host + ":" + DB_TYPE_DEFAULT_PORT_MAP[db_type]
                    )
                else:
        ...
        for index in range(len(requiredServices)):
            ...
            if service in servicesList:
                if (
                    ...
                    and property_name
                    in services["configurations"][config_type]["properties"]
                ):
                    service_user = services["configurations"][config_type][
                        "properties"
                    ][property_name]
                    service_old_user = self.getOldValue(
                        services, config_type, property_name
                    )

                    if "groups" in proxy_category:
                        putRangerKmsSiteProperty(
                            "hadoop.kms.proxyuser.{0}.groups".format(service_user), "*"
                        )
                    if "hosts" in proxy_category:
                        putRangerKmsSiteProperty(
                            "hadoop.kms.proxyuser.{0}.hosts".format(service_user), "*"
                        )
                    if "users" in proxy_category:
                        putRangerKmsSiteProperty(
                            "hadoop.kms.proxyuser.{0}.users".format(service_user), "*"
                        )

                    if (
                        service_old_user is not None
                        and service_user != service_old_user
                    ):
                        if "groups" in proxy_category:
                            putRangerKmsSitePropertyAttribute(
                                "hadoop.kms.proxyuser.{0}.groups".format(
                                    service_old_user
                                ),
                                "delete",
                                "true",
                            ...
                            services["forced-configurations"].append(
                                {
                                    "type": "kms-site",
                                    "name": "hadoop.kms.proxyuser.{0}.groups".format(
                                        service_old_user
                                    ),
                                }
                            )
                        ...
                        if "hosts" in proxy_category:
                            ...
                            services["forced-configurations"].append(
                                {
                                    "type": "kms-site",
                                    "name": "hadoop.kms.proxyuser.{0}.hosts".format(
                                        service_old_user
                                    ),
                                }
                            )
                        ...
                        if "users" in proxy_category:
                            putRangerKmsSitePropertyAttribute(
                                "hadoop.kms.proxyuser.{0}.users".format(
                                    service_old_user
                                ),
                                "delete",
                                "true",
                            ...
                            services["forced-configurations"].append(
                                {
                                    "type": "kms-site",
                                    "name": "hadoop.kms.proxyuser.{0}.users".format(
                                        service_old_user
                                    ),
                                }
                            )
