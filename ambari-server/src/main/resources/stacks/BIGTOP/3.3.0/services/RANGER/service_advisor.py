...
class RangerServiceAdvisor(service_advisor.ServiceAdvisor):
    ...
    def modifyComponentLayoutSchemes(self):
        ...
        self.componentLayoutSchemes.update(
            {"RANGER_ADMIN": {3: 0, 6: 1, 31: 2, "else": 2}}
        )
        self.componentLayoutSchemes.update(
...
class RangerRecommender(service_advisor.ServiceAdvisor):
    ...
    def recommendRangerConfigurationsFromHDP206(
        ...
        if rangerServiceVersion == "0.4.0":
            ...
            if (
                ...
                rangerUserSyncSource = services["configurations"][
                    "usersync-properties"
                ]["properties"]["SYNC_SOURCE"]
            ...
            putRangerEnvProperty = self.putProperty(
                configurations, "ranger-env", services
            )
            ...
            if include_hdfs:
                if "core-site" in services["configurations"] and (
                    ...
                    putRangerEnvProperty(
                        "xasecure.audit.destination.hdfs.dir", default_fs
                    )
                    ...
                    if component_audit_file in services["configurations"]:
                        ...
                        for item in ranger_audit_dict:
                            if (
                                ...
                                and item["configname"]
                                in services["configurations"][item["filename"]][
                                    "properties"
                                ]
                            ):
                                if (
                                ...
                                putRangerAuditProperty(
                                    item["target_configname"], rangerAuditProperty
                                )
    ...
    def recommendRangerUrlConfigurations(
        ...
        if (
            ...
            if (
                ...
                and "policymgr_external_url"
                in configurations["admin-properties"]["properties"]
            ):
        ...
        for index in range(len(requiredServices)):
            if requiredServices[index]["service_name"] in servicesList:
                ...
                if (
                    ...
                    putRangerSecurityProperty(
                        component_config_property, policymgr_external_url
                    )
    ...
    def recommendRangerConfigurationsFromHDP23(
        ...
        if isSolrCloudEnabled:
            ...
            if zookeeper_host_port:
                ranger_audit_zk_port = "{0}/{1}".format(
                    zookeeper_host_port, "ranger_audits"
                )
                putRangerAdminProperty(
                    "ranger.audit.solr.zookeepers", ranger_audit_zk_port
                )
        else:
                    ...
                    for item in ranger_audit_dict:
                        if (
                            ...
                            if (
                                ...
                                rangerAuditProperty = configurations[item["filename"]][
                                    "properties"
                                ][item["configname"]]
                            else:
                                rangerAuditProperty = services["configurations"][
        ...
        if "KNOX" in servicesList:
            ...
            if (
                ...
                and "gateway.port"
                in services["configurations"]["gateway-site"]["properties"]
            ):
        ...
        self.recommendRangerUrlConfigurations(
            configurations, services, required_services
        )
    ...
    def recommendRangerConfigurationsFromHDP25(
        ...
        if "ATLAS" in servicesList and has_ranger_tagsync:
            ...
            if (
                ...
                if (
                    application_properties
                    and "atlas.server.http.port" in application_properties
                ):
        ...
        if (
            "AMBARI_INFRA_SOLR" in servicesList
            ...
            if "infra-solr-env" in services["configurations"] and (
                "infra_solr_znode"
                in services["configurations"]["infra-solr-env"]["properties"]
            ):
                infra_solr_znode = services["configurations"]["infra-solr-env"][
                    "properties"
                ]["infra_solr_znode"]
                ranger_audit_zk_port = "{0}{1}".format(
                    zookeeper_host_port, infra_solr_znode
                )
            putRangerAdminProperty("ranger.audit.solr.zookeepers", ranger_audit_zk_port)
        elif (
            zookeeper_host_port
            and is_solr_cloud_enabled
            and is_external_solr_cloud_enabled
        ):
            ranger_audit_zk_port = "{0}/{1}".format(
                zookeeper_host_port, "ranger_audits"
            )
        ...
        for item in range(len(ranger_plugins_serviceuser)):
            if ranger_plugins_serviceuser[item]["service_name"] in servicesList:
                ...
                if (
                    ...
                    and config_name
                    in services["configurations"][file_name]["properties"]
                ):
        ...
        if "ATLAS" in servicesList:
            if "ranger-env" in services["configurations"]:
                ...
                if (
                    ...
                    xasecure_audit_destination_hdfs = configurations["ranger-env"][
                        "properties"
                    ]["xasecure.audit.destination.hdfs"]
                else:
                    xasecure_audit_destination_hdfs = services["configurations"][
                        "ranger-env"
                    ]["properties"]["xasecure.audit.destination.hdfs"]
                ...
                if (
                    ...
                    xasecure_audit_destination_solr = configurations["ranger-env"][
                        "properties"
                    ]["xasecure.audit.destination.solr"]
                else:
                    xasecure_audit_destination_solr = services["configurations"][
                        "ranger-env"
        ...
        if ambari_sso_details and ambari_sso_details.is_managing_services():
            ...
            if ambari_sso_details.should_enable_sso("RANGER"):
                ...
                putRangerAdminSiteProperty(
                    "ranger.sso.providerurl", ambari_sso_details.get_sso_provider_url()
                )
                putRangerAdminSiteProperty(
    ...
    def recommendConfigurationsForLDAP(
        ...
        ambari_configuration = self.get_ambari_configuration(services)
        ldap_properties = ambari_configuration.get_ambari_server_configuration_category(
            "ldap-configuration"
        )
        ...
        if (
            ...
            if "ambari.ldap.attributes.user.search_base" in ldap_properties:
                putRangerUgsyncSite(
                    ...
                    ldap_properties["ambari.ldap.attributes.user.search_base"],
                )
            if "ambari.ldap.attributes.group.member_attr" in ldap_properties:
                putRangerUgsyncSite(
                    "ranger.usersync.group.memberattributename",
                    ldap_properties["ambari.ldap.attributes.group.member_attr"],
                )
            if "ambari.ldap.attributes.group.name_attr" in ldap_properties:
                putRangerUgsyncSite(
                    "ranger.usersync.group.nameattribute",
                    ldap_properties["ambari.ldap.attributes.group.name_attr"],
                )
            if "ambari.ldap.attributes.group.object_class" in ldap_properties:
                putRangerUgsyncSite(
                    "ranger.usersync.group.objectclass",
                    ldap_properties["ambari.ldap.attributes.group.object_class"],
                )
            if "ambari.ldap.connectivity.bind_dn" in ldap_properties:
                putRangerUgsyncSite(
                    "ranger.usersync.ldap.binddn",
                    ldap_properties["ambari.ldap.connectivity.bind_dn"],
                )
            if "ambari.ldap.connectivity.server.host" in ldap_properties:
                ...
                ldapUrl = (
                    ldap_protocol
                    + ldap_properties["ambari.ldap.connectivity.server.host"]
                    + ":"
                    + ldap_port
                )
            ...
            if "ambari.ldap.attributes.user.object_class" in ldap_properties:
                putRangerUgsyncSite(
                    ...
                    ldap_properties["ambari.ldap.attributes.user.object_class"],
                )
            if "ambari.ldap.attributes.user.name_attr" in ldap_properties:
                putRangerUgsyncSite(
                    "ranger.usersync.ldap.user.nameattribute",
                    ldap_properties["ambari.ldap.attributes.user.name_attr"],
                )
...
class RangerValidator(service_advisor.ServiceAdvisor):
    ...
    def validateRangerAdminConfigurationsFromHDP23(
        ...
        return self.toConfigurationValidationProblems(
            validationItems, "admin-properties"
        )
    ...
    def validateRangerTagsyncConfigurationsFromHDP25(
        ...
        if "RANGER" in servicesList:
            ...
            if (
                ...
                and ranger_tagsync_properties["ranger.tagsync.source.atlas"].lower()
                == "true"
            ):
                validationItems.append(
                    {
    ...
    def validateRangerUsersyncConfigurationsFromHDP26(
        ...
        if (
            usersync_source_ldap_enabled
            and delta_sync_enabled
            and not group_sync_enabled
        ):
            validationItems.append(
                {
        ...
        return self.toConfigurationValidationProblems(
            validationItems, "ranger-ugsync-site"
        )
