...
class DefaultStackAdvisor(StackAdvisor):
      ...
      try:
        with open(path, "r") as fp:
          ...
          if hasattr(service_advisor, best_class_name):
            self.logger.info(
              f"ServiceAdvisor implementation for service {service_name} was loaded"
            )
        ...
        self.logger.exception(
          f"Failed to load or create ServiceAdvisor implementation for service {service_name}"
        )
  ...
  def get_heap_size_properties(self, services):
    ...
    default = {
      ...
      "DATANODE": [
        {"config-name": "hadoop-env", "property": "dtnode_heapsize", "default": "1024m"}
      ],
      "REGIONSERVER": [
        {
      ...
      "HIVE_SERVER": [
        {"config-name": "hive-env", "property": "hive.heapsize", "default": "1024m"}
      ],
      "HISTORYSERVER": [
        {
      ...
      "OOZIE_SERVER": [
        {"config-name": "oozie-env", "property": "oozie_heapsize", "default": "1024m"}
      ],
      "RESOURCEMANAGER": [
        {
            ...
            for host, hostComponents in hostsComponentsMap.items():
              ...
              for hostComponent in hostComponents:
                currentComponentName = (
                  None if "name" not in hostComponent else hostComponent["name"]
                )
  ...
  def validateRequiredComponentsPresent(self, services):
    ...
    for service in services["services"]:
      for component in service["components"]:
        ...
        if (
          self.isSlaveComponent(component) or self.isMasterComponent(component)
                ...
                if scope == "host":
                  ...
                  if type == "exclusive":
                    hostsWithIssues = list(
                      set(componentHosts) & set(dependentComponentHosts)
                    )
                    notMessagePart = "not"
                  else:
                    ...
                    items.append(
                      {
                        ...
                        "level": "ERROR",
                        "message": message,
                        "component-name": component["StackServiceComponents"][
                          "component_name"
                        ],
                      }
                    )
  ...
  def calculateYarnAllocationSizes(self, configurations, services, hosts):
    ...
    putYarnProperty(
      ...
      int(
        round(
          min(
            clusterSummary["containers"] * clusterSummary["ramPerContainer"],
            nodemanagerMinRam,
          )
        )
      ),
  ...
  def getConfigurationClusterSummary(
    ...
    if services:  # its never None but some unit tests pass it as None
      ...
      if (
        self.getOldValue(services, "yarn-site", "yarn.scheduler.minimum-allocation-mb")
        or "recommendConfigurations" != callContext
        or operation == DefaultStackAdvisor.RECOMMEND_ATTRIBUTE_OPERATION
        or (operation == DefaultStackAdvisor.ADD_SERVICE_OPERATION and not adding_yarn)
      ):
        self.logger.info(
    ...
    if (
      cluster["containers"] * cluster["minContainerSize"] > cluster["totalAvailableRam"]
    ):
      cluster["containers"] = int(
    ...
    self.logger.info(
      "Available memory for map - cluster[mapMemory]: " + str(cluster["mapMemory"])
    )
    self.logger.info(
  ...
  def getCallContext(self, services):
    if services:
      if DefaultStackAdvisor.ADVISOR_CONTEXT in services:
        self.logger.info(
          "call type context : " + str(services[DefaultStackAdvisor.ADVISOR_CONTEXT])
        )
    ...
    if os.path.exists(login_defs):
      with open(login_defs, "r") as f:
        ...
        uid = [
          x
          for x in uid
          if x.find(comment_tag) > x.find(uid_min_tag) or x.find(comment_tag) == -1
        ]
      ...
      if (
        ...
        for propertyName in recommendedDefaults[configName]["property_attributes"]:
          if propertyName in configurations[configName]["properties"]:
            if (
              "maximum"
              in recommendedDefaults[configName]["property_attributes"][propertyName]
              and propertyName in recommendedDefaults[configName]["properties"]
            ):
              userValue = convertToNumber(
                configurations[configName]["properties"][propertyName]
              )
              maxValue = convertToNumber(
                recommendedDefaults[configName]["property_attributes"][propertyName][
                  "maximum"
                ]
              )
                  ...
                  [
                    {
                      ...
                      "item": self.getWarnItem(
                        f"Value is greater than the recommended maximum of {maxValue} "
                      ),
                    }
                  ]
                )
                  ...
                  [
                    {
                      "config-name": propertyName,
                      "item": self.getWarnItem(
                        f"Value is less than the recommended minimum of {minValue} "
                      ),
                    }
                  ]
                )
    ...
    for configGroup in services["config-groups"]:
      ...
      for configName in configGroup["configurations"].keys():
        if configName in cgServices["configurations"]:
          ...
          cgServices["configurations"][configName] = configGroup["configurations"][
            configName
          ]
        ...
        for configElement in configurations[config].keys():
          ...
          for property, value in configurations[config][configElement].items():
            if config in configGroup["configurations"]:
              cgRecommendation["configurations"][config][configElement][property] = (
                value
              )
            else:
              cgRecommendation["dependent_configurations"][config][configElement][
  ...
  def isHostSuitableForComponent(self, host, component):
    return not (
      self.getComponentName(component) in self.getNotPreferableOnServerComponents()
  ...
  def filterResult(self, result, services):
    ...
    self.filterConfigs(
      result["recommendations"]["blueprint"]["configurations"], allRequestedProperties
    )
    if "config-groups" in services:
      ...
      if "properties" in names.keys():
        for name in names["properties"]:
          if type in requestedProperties.keys() and name in requestedProperties[type]:
            ...
            filteredConfigs[type]["properties"][name] = configs[type]["properties"][
              name
            ]
        ...
        for name in names["property_attributes"]:
          if type in requestedProperties.keys() and name in requestedProperties[type]:
            if type not in filteredConfigs.keys():
              ...
              filteredConfigs[type]["property_attributes"] = {}
            filteredConfigs[type]["property_attributes"][name] = configs[type][
              "property_attributes"
            ][name]
    configs.clear()
  ...
  def putProperty(self, config, configType, services=None):
    ...
    def appendProperty(key, value):
      ...
      if (
        ...
        config[configType]["properties"][key] = userConfigs[configType]["properties"][
          key
        ]
      else:
  ...
  def filterHostMounts(self, hosts, services):
    ...
    if (
      ...
      ignore_list = [
        x.strip() for x in cluster_env["agent_mounts_ignore_list"].strip().split(",")
      ]
  ...
  def getHostsWithComponent(self, serviceName, componentName, services, hosts):
    if (
      ...
      and serviceName
      in [service["StackServices"]["service_name"] for service in services["services"]]
    ):
      service = [
        serviceEntry
        for serviceEntry in services["services"]
      ...
      for service in services["services"]:
        components.extend(
          [
            componentEntry
            for componentEntry in service["components"]
            if componentEntry["StackServiceComponents"]["component_category"]
            in categories
    ...
    for user_name, user_properties in users.items():
      ...
      for prop in props:
        validationItems.append(
          {"config-name": prop, "item": self.validatorNotEmpty(properties, prop)}
        )
      ...
      "YARN": [
        (
          ...
          lambda services, hosts: len(
            self.getHostsWithComponent("YARN", "RESOURCEMANAGER", services, hosts)
          )
          > 1,
        )
      ],
  ...
  def _getHadoopProxyUsersForService(
    ...
    self.logger.info(
      f"Calculating Hadoop Proxy User recommendations for {serviceName} service."
    )
    servicesList = self.get_services_list(services)
    resultUsers = {}
      ...
      for user, (
        ...
        for proxyPropertyName, hostSelector in hostSelectorMap.items():
          ...
          if isinstance(hostSelector, (list, tuple)):
            ...
            self.logger.info(
              f"Host List for [service='{serviceName}'; user='{user}'; components='{','.join(hostSelector)}']: {componentHostNamesString}"
            )
  ...
  def recommendHadoopProxyUsers(self, configurations, services, hosts):
    ...
    if "HIVE" in servicesList:
      ...
      if hive_user and get_from_dict(
        ...
        services["forced-configurations"].append(
          {"type": "core-site", "name": f"hadoop.proxyuser.{hive_user}.hosts"}
        )
    ...
    for user_name, user_properties in users.items():
      ...
      self.logger.info(
        f"Updated hadoop.proxyuser.{user_name}.hosts as : {user_properties['propertyHosts']}"
      )
      if "propertyGroups" in user_properties:
        self.put_proxyuser_value(
          user_name,
      ...
      if userOldValue is not None and userOldValue != user_name:
        putCoreSitePropertyAttribute(
          f"hadoop.proxyuser.{userOldValue}.hosts", "delete", "true"
        )
        services["forced-configurations"].append(
          {"type": "core-site", "name": f"hadoop.proxyuser.{userOldValue}.hosts"}
        )
        services["forced-configurations"].append(
          {"type": "core-site", "name": f"hadoop.proxyuser.{user_name}.hosts"}
        )

        if "propertyGroups" in user_properties:
          putCoreSitePropertyAttribute(
            f"hadoop.proxyuser.{userOldValue}.groups", "delete", "true"
          )
          services["forced-configurations"].append(
            {"type": "core-site", "name": f"hadoop.proxyuser.{userOldValue}.groups"}
          )
          services["forced-configurations"].append(
            {"type": "core-site", "name": f"hadoop.proxyuser.{user_name}.groups"}
          )
  ...
  def recommendAmbariProxyUsersForHDFS(
    ...
    if "HDFS" in servicesList:
      ...
      if old_ambari_user is not None:
        putCoreSitePropertyAttribute(
          f"hadoop.proxyuser.{old_ambari_user}.hosts", "delete", "true"
        )
        putCoreSitePropertyAttribute(
          f"hadoop.proxyuser.{old_ambari_user}.groups", "delete", "true"
        )
  ...
  def validatorLessThenDefaultValue(
    ...
    if value < defaultValue:
      return self.getWarnItem(
        f"Value is less than the recommended default of {defaultValue}"
      )
    return None
  ...
  def validatorGreaterThenDefaultValue(
    ...
    if value > defaultValue:
      return self.getWarnItem(
        f"Value is greater than the recommended default of {defaultValue}"
      )
    return None
  ...
  def validatorEqualsPropertyItem(
    ...
    if value1 != value2:
      return self.getWarnItem(
        "It is recommended to set equal values " "for properties {0} and {1}".format(
  ...
  def getCapacitySchedulerProperties(self, services):
    ...
    if "capacity-scheduler" in services["configurations"]:
      if (
        ...
        cap_sched_props_as_str = services["configurations"]["capacity-scheduler"][
          "properties"
        ]["capacity-scheduler"]
    ...
    if dir:
      ...
      for mountPoint in mountPoints:
        ...
        if os.path.join(dir, "").startswith(os.path.join(mountPoint, "")):
          ...
          elif os.path.join(bestMountFound, "").count(os.path.sep) < os.path.join(
            mountPoint, ""
          ).count(os.path.sep):
            bestMountFound = mountPoint
