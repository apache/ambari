...
class DefaultStackAdvisor(StackAdvisor):
  ...
  """
    ...
    def getServiceAdvisor(self, key):
        ...
        return (
            self.serviceAdvisorsDict[key] if key in self.serviceAdvisorsDict else None
        )
    ...
    def loadServiceAdvisors(self):
        ...
        for service in self.services["services"]:
            ...
            for component in service["components"]:
                ...
                self.serviceAdvisorsDict[componentName] = self.serviceAdvisorsDict[
                    serviceName
                ]
    ...
    def instantiateServiceAdvisor(self, service):
        ...
        class_name = (
            service["StackServices"]["advisor_name"]
            if "advisor_name" in service["StackServices"]
            else None
        )
        path = (
            service["StackServices"]["advisor_path"]
            if "advisor_path" in service["StackServices"]
            else None
        )

        class_name_pattern = re.compile(
            "%s.*?ServiceAdvisor" % service_name, re.IGNORECASE
        )
            ...
            try:
                with open(path, "r") as fp:
                    ...
                    if hasattr(service_advisor, best_class_name):
                        self.logger.info(
                            "ServiceAdvisor implementation for service {0} was loaded".format(
                                service_name
                            )
                        )
                        ...
                        self.logger.error(
                            "Failed to load or create ServiceAdvisor implementation for service {0}: "
                            "Expecting class name {1} but it was not found.".format(
                                service_name, best_class_name
                            )
                        )
            except Exception as e:
                self.logger.exception(
                    "Failed to load or create ServiceAdvisor implementation for service {0}".format(
                        service_name
                    )
                )
    ...
    def createComponentLayoutRecommendations(self, services, hosts):
        ...
        for service in sortedServices:
            ...
            for component in masterComponents:
                ...
                filteredHosts = self.getFilteredHostsBasedOnDependencies(
                    services, component, hostsList, hostsComponentsMap
                )
                hostsForComponent = advisor.getHostsForMasterComponent(
                    services, hosts, component, filteredHosts
                )
        ...
        freeHosts = [
            hostName for hostName in hostsList if hostName not in utilizedHosts
        ]
        ...
        for service in sortedServices:
            ...
            for component in slaveClientComponents:
                ...
                hostsForComponent = advisor.getHostsForSlaveComponent(
                    services, hosts, component, filteredHosts, filteredFreeHosts
                )
        ...
        for service in sortedServices:
            ...
            if serviceAdvisor is not None:
                ...
                serviceAdvisor.colocateServiceWithServicesInfo(
                    hostsComponentsMap, serviceComponents, services
                )
        ...
        for key in hostsComponentsMap.keys():
            ...
            host_groups.insert(
                0, {"name": host_group_name, "components": hostsComponentsMap[key]}
            )
    ...
    def getHostsForMasterComponent(self, services, hosts, component, hostsList):
        ...
        if len(hostsList) > 1 and self.isMasterComponentWithMultipleInstances(
            component
        ):
            ...
            if hostsCount > 1:  # get first 'hostsCount' available hosts
                hostsForComponent = []
                hostIndex = 0
                while hostsCount > len(hostsForComponent) and hostIndex < len(
                    hostsList
                ):
    ...
    def getHostsForSlaveComponent(
        ...
        if self.isSlaveComponent(component):
            ...
            hostsMin, hostsMax = (
                0 if hostsMin is None else hostsMin,
                len(hostsList) if hostsMax is None else hostsMax,
            )
            if self.isComponentUsingCardinalityForLayout(componentName) and cardinality:
                if hostsMin > len(hostsForComponent):
                    hostsForComponent.extend(
                        freeHosts[0 : hostsMin - len(hostsForComponent)]
                    )
    ...
    def getServicesSortedByDependencies(self, services):
        ...
        for service in services["services"]:
            self.sortServicesByDependencies(
                services, service, processedServices, sortedServices
            )

        return sortedServices

    def sortServicesByDependencies(
        self, services, service, processedServices, sortedServices
    ):
        """
        Sorts the services based on their dependencies.  This is limited to non-conditional host scope dependencies.
        Services with no dependencies will go first.  Services with dependencies will go after the services they are dependent on.
        ...
        for component in components:
            dependencies = (
                [] if "dependencies" not in component else component["dependencies"]
            )
            for dependency in dependencies:
                # accounts only for dependencies that are not conditional
                conditionsPresent = (
                    "conditions" in dependency["Dependencies"]
                    and dependency["Dependencies"]["conditions"]
                )
                scope = (
                    "cluster"
                    if "scope" not in dependency["Dependencies"]
                ...
                if not conditionsPresent and scope == "host":
                    ...
                    self.sortServicesByDependencies(
                        services, requiredService, processedServices, sortedServices
                    )

        sortedServices.append(service)

    def getFilteredHostsBasedOnDependencies(
        self, services, component, hostsList, hostsComponentsMap
    ):
        """
        ...
        for dependency in dependencies:
            ...
            if not conditionsPresent:
                componentName = component["StackServiceComponents"]["component_name"]
                requiredComponentName = dependency["Dependencies"]["component_name"]
                requiredComponent = self.getRequiredComponent(
                    services, requiredComponentName
                )
                        ...
                        for host, hostComponents in hostsComponentsMap.items():
                            ...
                            for hostComponent in hostComponents:
                                currentComponentName = (
                                    None
                                    if "name" not in hostComponent
                                    else hostComponent["name"]
                                )
    ...
    def getComponentLayoutValidations(self, services, hosts):
        ...
        for service in services["services"]:
            ...
            if serviceAdvisor is not None:
                items.extend(
                    serviceAdvisor.getServiceComponentLayoutValidations(services, hosts)
                )
    ...
    def validateRequiredComponentsPresent(self, services):
        ...
        for service in services["services"]:
            for component in service["components"]:
                ...
                if (
                    self.isSlaveComponent(component)
                    or self.isMasterComponent(component)
                ) and component["StackServiceComponents"]["hostnames"]:
                    for dependency in component["dependencies"]:
                        ...
                        if not conditionsPresent:
                            ...
                            componentDisplayName = component["StackServiceComponents"][
                                "display_name"
                            ]
                            ...
                            dependentComponentHosts = (
                                dependentComponent["hostnames"]
                                if dependentComponent is not None
                                else []
                            )
                            ...
                            if (dependentComponent is None) or (
                                ...
                                type = (
                                    "inclusive"
                                    if "type" not in dependency["Dependencies"]
                                    else dependency["Dependencies"]["type"]
                                )
                                if scope == "host":
                                    componentHosts = component[
                                        "StackServiceComponents"
                                    ]["hostnames"]
                                    hostsWithIssues = []
                                    notMessagePart = ""
                                    if type == "exclusive":
                                        hostsWithIssues = list(
                                            set(componentHosts)
                                        ...
                                        for componentHost in componentHosts:
                                            if (
                                                componentHost
                                                not in dependentComponentHosts
                                            ):
                                    ...
                                    message = "{0} requires {1} to be present in the cluster.".format(
                                        componentDisplayName,
                                        dependentComponentDisplayName,
                                    )
                                    items.append(
    ...
    def calculateYarnAllocationSizes(self, configurations, services, hosts):
        # initialize data
        servicesList, componentsList = self.get_service_and_component_lists(
            services["services"]
        )
        putYarnProperty = self.putProperty(configurations, "yarn-site", services)
        putYarnPropertyAttribute = self.putPropertyAttribute(
        ...
        if "referenceNodeManagerHost" in clusterSummary:
            nodemanagerMinRam = min(
                clusterSummary["referenceNodeManagerHost"]["total_mem"] / 1024,
                nodemanagerMinRam,
            )
        ...
        putYarnProperty(
            ...
            int(
                round(
                    min(
                        clusterSummary["containers"]
                        * clusterSummary["ramPerContainer"],
                        nodemanagerMinRam,
                    )
                )
            ),
        ...
        putYarnProperty(
            ...
            int(clusterSummary["yarnMinContainerSize"]),
        )
        putYarnProperty(
        ...
        nodeManagerHost = self.getHostWithComponent(
    ...
    def getConfigurationClusterSummary(
        ...
        if len(hosts["items"]) > 0:
            nodeManagerHosts = self.getHostsWithComponent(
                "YARN", "NODEMANAGER", services, hosts
            )
        ...
        self.logger.info(
            "Memory for YARN apps - cluster[totalAvailableRam]: "
        ...
        operation = self.getUserOperationContext(
            services, DefaultStackAdvisor.OPERATION
        )
        ...
        if services:  # its never None but some unit tests pass it as None
            ...
            if (
                self.getOldValue(
                    services, "yarn-site", "yarn.scheduler.minimum-allocation-mb"
                )
                or "recommendConfigurations" != callContext
                or operation == DefaultStackAdvisor.RECOMMEND_ATTRIBUTE_OPERATION
                or (
                    operation == DefaultStackAdvisor.ADD_SERVICE_OPERATION
                    and not adding_yarn
                )
            ):
                self.logger.info(
                    "Full context: callContext = "
                    + str(callContext)
                    + " and operation = "
                    + str(operation)
                    + " and adding YARN = "
                ...
                if (
                    ...
                    self.logger.info(
                        "Using user provided yarn.scheduler.minimum-allocation-mb = "
                        + str(
                            services["configurations"]["yarn-site"]["properties"][
                                "yarn.scheduler.minimum-allocation-mb"
                            ]
                    ...
                    cluster["yarnMinContainerSize"] = int(
                        services["configurations"]["yarn-site"]["properties"][
                            "yarn.scheduler.minimum-allocation-mb"
                        ]
                    )
                    self.logger.info(
                        "Minimum ram per container due to user input - cluster[yarnMinContainerSize]: "
                    ...
                    if cluster["yarnMinContainerSize"] > cluster["totalAvailableRam"]:
                        ...
                        self.logger.info(
                            "Minimum ram per container after checking against limit - cluster[yarnMinContainerSize]: "
                            + str(cluster["yarnMinContainerSize"])
                        )
                        pass
                    cluster["minContainerSize"] = cluster[
                        "yarnMinContainerSize"
        ...
        cluster["containers"] = int(
            round(
                max(
                    3,
                    min(
                        core_multiplier * cluster["cpu"],
                        min(
                            ceil(1.8 * cluster["disk"]),
        ...
        self.logger.info(
            "Containers per node - cluster[containers]: " + str(cluster["containers"])
        )

        if (
            cluster["containers"] * cluster["minContainerSize"]
            > cluster["totalAvailableRam"]
        ):
            cluster["containers"] = int(
                ceil(cluster["totalAvailableRam"] // cluster["minContainerSize"])
            )
            self.logger.info(
                "Modified number of containers based on provided value for yarn.scheduler.minimum-allocation-mb"
            )
            pass

        cluster["ramPerContainer"] = int(
            abs(cluster["totalAvailableRam"] // cluster["containers"])
        )
        cluster["yarnMinContainerSize"] = min(
            suggestedMinContainerRam, cluster["ramPerContainer"]
        )
        self.logger.info(
            "Ram per containers before normalization - cluster[ramPerContainer]: "
        ...
        if cluster["ramPerContainer"] > cluster["yarnMinContainerSize"]:
            cluster["ramPerContainer"] = (
                int(cluster["ramPerContainer"] // cluster["yarnMinContainerSize"])
        ...
        self.logger.info(
            "Min container size - cluster[yarnMinContainerSize]: "
            + str(cluster["yarnMinContainerSize"])
        )
        self.logger.info(
            "Available memory for map - cluster[mapMemory]: "
            + str(cluster["mapMemory"])
        )
        self.logger.info(
            "Available memory for reduce - cluster[reduceMemory]: "
            + str(cluster["reduceMemory"])
        )
        self.logger.info(
            "Available memory for am - cluster[amMemory]: " + str(cluster["amMemory"])
        )
    ...
    def getCallContext(self, services):
        if services:
            if DefaultStackAdvisor.ADVISOR_CONTEXT in services:
                ...
                return services[DefaultStackAdvisor.ADVISOR_CONTEXT][
                    DefaultStackAdvisor.CALL_TYPE
                ]
        return ""
    ...
    def isServiceBeingAdded(self, services, serviceName):
        if services:
            if "user-context" in list(services.keys()):
                ...
                if (
                    DefaultStackAdvisor.OPERATION in userContext
                    and "AddService" == userContext[DefaultStackAdvisor.OPERATION]
                    and DefaultStackAdvisor.OPERATION_DETAILS in userContext
                ):
        ...
        if os.path.exists(login_defs):
            with open(login_defs, "r") as f:
                ...
                uid = [
                    x
                    for x in uid
                    if x.find(comment_tag) > x.find(uid_min_tag)
                    or x.find(comment_tag) == -1
                ]
        ...
        for validationProblem in validationProblems:
            ...
            if validationItem is not None:
                problem = {
                    ...
                    "config-name": validationProblem["config-name"],
                }
    ...
    def validateMinMax(self, items, recommendedDefaults, configurations):
        ...
        for configName in configurations:
            validationItems = []
            if (
                configName in recommendedDefaults
                and "property_attributes" in recommendedDefaults[configName]
            ):
                for propertyName in recommendedDefaults[configName][
                    "property_attributes"
                ]:
                    if propertyName in configurations[configName]["properties"]:
                        if (
                            "maximum"
                            in recommendedDefaults[configName]["property_attributes"][
                                propertyName
                            ]
                            and propertyName
                            in recommendedDefaults[configName]["properties"]
                        ):
                            userValue = convertToNumber(
                                configurations[configName]["properties"][propertyName]
                            )
                            maxValue = convertToNumber(
                                    ...
                                    [
                                        {
                                            ...
                                            "item": self.getWarnItem(
                                                "Value is greater than the recommended maximum of {0} ".format(
                                                    maxValue
                                                )
                                            ),
                                        }
                                    ...
                                    [
                                        {
                                            "config-name": propertyName,
                                            "item": self.getWarnItem(
                                                "Value is less than the recommended minimum of {0} ".format(
                                                    minValue
                                                )
                                            ),
                                        }
            ...
            items.extend(
                self.toConfigurationValidationProblems(validationItems, configName)
            )
        pass
    ...
    def getConfigurationsValidationItems(self, services, hosts):
        ...
        recommendedDefaults = recommendations["recommendations"]["blueprint"][
            "configurations"
        ]
        ...
        for service in services["services"]:
            items.extend(
                self.getConfigurationsValidationItemsForService(
                    configurations, recommendedDefaults, service, services, hosts
                )
            )
    ...
    def validateListOfConfigUsingMethod(
        self, configurations, recommendedDefaults, services, hosts, validators
    ):
        """
    ...
    def validateConfigurationsForSite(
        self, configurations, recommendedDefaults, services, hosts, siteName, method
    ):
        """
        ...
        if siteName in recommendedDefaults:
            ...
            if siteProperties is not None:
                ...
                self.logger.info(
                    "SiteName: %s, method: %s" % (siteName, method.__name__)
                )
                self.logger.info("Recommendations: %s" % str(siteRecommendations))
                return method(
                    siteProperties, siteRecommendations, configurations, services, hosts
                )
        return []

    def getConfigurationsValidationItemsForService(
        self, configurations, recommendedDefaults, service, services, hosts
    ):
        items = []
        serviceName = service["StackServices"]["service_name"]
        ...
        if validator is not None:
            for siteName, method in validator.items():
                resultItems = self.validateConfigurationsForSite(
                    configurations,
                    recommendedDefaults,
                    services,
                    hosts,
        ...
        if serviceAdvisor is not None:
            items.extend(
                serviceAdvisor.getServiceConfigurationsValidationItems(
    ...
    def recommendConfigGroupsConfigurations(
        self, recommendations, services, components, hosts, servicesList
    ):
        ...
        for configGroup in services["config-groups"]:
            ...
            for configName in configGroup["configurations"].keys():
                if configName in cgServices["configurations"]:
                    ...
                    cgServices["configurations"][configName] = configGroup[
                        "configurations"
                    ][configName]
            ...
            cgHosts = {
                "items": [
                    ...
                    if host["Hosts"]["host_name"] in configGroup["hosts"]
                ]
            }

            # Override clusterSummary
            cgClusterSummary = self.getConfigurationClusterSummary(
                servicesList, cgHosts, components, cgServices
            )
            ...
            for config in configurations.keys():
                ...
                for configElement in configurations[config].keys():
                    ...
                    for property, value in configurations[config][
                        configElement
                    ].items():
    ...
    def recommendConfigurations(self, services, hosts):
        ...
        if "config-groups" in services:
            self.recommendConfigGroupsConfigurations(
                recommendations, services, componentsList, hosts, servicesList
            )
        else:
            ...
            for service in services["services"]:
                ...
                if calculation is not None:
                    ...
                    if serviceAdvisor is not None:
                        serviceAdvisors.append(serviceAdvisor)
            for serviceAdvisor in serviceAdvisors:
                serviceAdvisor.getServiceConfigurationRecommendationsForSSO(
                    configurations, clusterSummary, services, hosts
                )
            ...
            for service in services["services"]:
                ...
                if calculation is not None:
                    ...
                    if serviceAdvisor is not None:
                        serviceAdvisors.append(serviceAdvisor)
            for serviceAdvisor in serviceAdvisors:
                serviceAdvisor.getServiceConfigurationRecommendationsForLDAP(
                    configurations, clusterSummary, services, hosts
                )
            ...
            for service in services["services"]:
                ...
                if calculation is not None:
                    ...
                    if serviceAdvisor is not None:
                        serviceAdvisors.append(serviceAdvisor)
            for serviceAdvisor in serviceAdvisors:
                serviceAdvisor.getServiceConfigurationRecommendationsForKerberos(
                    configurations, clusterSummary, services, hosts
                )
    ...
    def is_secured_cluster(self, services):
        ...
        return (
            services
            and "cluster-env" in services["configurations"]
            and "security_enabled"
            in services["configurations"]["cluster-env"]["properties"]
            and services["configurations"]["cluster-env"]["properties"][
                "security_enabled"
            ].lower()
            == "true"
        )
        ...
        if include_zookeeper:
            ...
            if include_port:
                ...
                for i in range(len(zookeeper_hosts)):
                    zookeeper_host_port_arr.append(
                        zookeeper_hosts[i] + ":" + zookeeper_port
                    )
            else:
                for i in range(len(zookeeper_hosts)):
    ...
    def getZKPort(self, services):
        ...
        if "zoo.cfg" in services["configurations"] and (
            ...
            zookeeper_port = services["configurations"]["zoo.cfg"]["properties"][
                "clientPort"
            ]
        ...
        if len(hostsList) != 1:
            ...
            if scheme is not None:
                hostIndex = next(
                    (
                        index
                        for key, index in scheme.items()
                        if isinstance(key, int) and len(hostsList) < key
                    ),
                    scheme["else"],
                )
            else:
    ...
    def isHostSuitableForComponent(self, host, component):
        return not (
            ...
            in self.getNotPreferableOnServerComponents()
            and self.isLocalHost(host)
        )
    ...
    def filterResult(self, result, services):
        ...
        self.filterConfigs(
            result["recommendations"]["blueprint"]["configurations"],
            allRequestedProperties,
        )
        if "config-groups" in services:
            for configGroup in result["recommendations"]["config-groups"]:
                self.filterConfigs(
                    configGroup["configurations"], allRequestedProperties
                )
                self.filterConfigs(
                    configGroup["dependent_configurations"], allRequestedProperties
                )
        return result
        ...
        for type, names in configs.items():
            if "properties" in names.keys():
                for name in names["properties"]:
                    if (
                        type in requestedProperties.keys()
                        and name in requestedProperties[type]
                    ):
                        if type not in filteredConfigs.keys():
                            filteredConfigs[type] = {"properties": {}}
                        filteredConfigs[type]["properties"][name] = configs[type][
                            "properties"
                        ][name]
                ...
                for name in names["property_attributes"]:
                    if (
                        ...
                        if type not in filteredConfigs.keys():
                            ...
                            filteredConfigs[type]["property_attributes"] = {}
                        filteredConfigs[type]["property_attributes"][name] = configs[
                            type
    ...
    def getAffectedConfigs(self, services):
        ...
        changedConfigs = [
            {"type": entry["type"], "name": entry["name"]} for entry in changedConfigs
        ]
        allDependencies = []
            ...
            for config in allDependencies:
                ...
                if property in dependencies or property in changedConfigs:
                    for dependedConfig in config["dependencies"]:
                        dependency = {
                            "name": dependedConfig["StackConfigurationDependency"][
                                "dependency_name"
                            ],
                            "type": dependedConfig["StackConfigurationDependency"][
                                "dependency_type"
                            ],
                        }
        ...
        if (
            "forced-configurations" in services
            and services["forced-configurations"] is not None
        ):
    ...
    def putProperty(self, config, configType, services=None):
        ...
        def appendProperty(key, value):
            ...
            if (
                not self.isPropertyRequested(configType, key, changedConfigs)
                and configType in userConfigs
                and key in userConfigs[configType]["properties"]
            ):
                config[configType]["properties"][key] = userConfigs[configType][
                    "properties"
                ][key]
            else:
    ...
    def __isPropertyInChangedConfigs(self, configType, propertyName, changedConfigs):
        for changedConfig in changedConfigs:
            if (
                changedConfig["type"] == configType
                and changedConfig["name"] == propertyName
            ):
                return True
        return False
    ...
    def isPropertyRequested(self, configType, propertyName, changedConfigs):
        ...
        if self.allRequestedProperties:
            return (
                configType in self.allRequestedProperties
                and propertyName in self.allRequestedProperties[configType]
            )
        else:
            return not self.__isPropertyInChangedConfigs(
                configType, propertyName, changedConfigs
            )
    ...
    def filterHostMounts(self, hosts, services):
        ...
        if (
            cluster_env
            and "agent_mounts_ignore_list" in cluster_env
            and cluster_env["agent_mounts_ignore_list"].strip()
        ):
            ignore_list = [
                x.strip()
                for x in cluster_env["agent_mounts_ignore_list"].strip().split(",")
            ]
    ...
    def getMountPathVariations(
        self, initial_value, component_name, services, hosts, banned_mounts=[]
    ):
        """
        ...
        for mount in mounts:
            new_mount = (
                initial_value
                if mount == "/"
                else os.path.join(mount + sep, initial_value.lstrip(sep))
            )
    ...
    def getMountPathVariation(
        ...
        try:
            return [
                self.getMountPathVariations(
                    initial_value, component_name, services, hosts, banned_mounts
                )[0]
            ]
        except IndexError:
            return []

    def updateMountProperties(
        self,
        siteConfig,
        propertyDefinitions,
        configurations,
        ...
        for prop_item in propertyDefinitions:
            ...
            if props is None or name not in props:
                if rc_type == "multi":
                    recommendation = self.getMountPathVariations(
                        default_value, component, services, hosts, banned_mounts
                    )
                else:
                    recommendation = self.getMountPathVariation(
                        default_value, component, services, hosts, banned_mounts
                    )
    ...
    def getHostNamesWithComponent(self, serviceName, componentName, services):
        ...
        if services is not None and serviceName in [
            ...
            service = [
                serviceEntry
                for serviceEntry in services["services"]
                if serviceEntry["StackServices"]["service_name"] == serviceName
            ][0]
            components = [
                componentEntry
                for componentEntry in service["components"]
                if componentEntry["StackServiceComponents"]["component_name"]
                == componentName
            ]
            if (
                len(components) > 0
                and len(components[0]["StackServiceComponents"]["hostnames"]) > 0
            ):
                componentHostnames = components[0]["StackServiceComponents"][
                    "hostnames"
                ]
        ...
        if (
            ...
            if (
                ...
                componentHosts = [
                    host
                    for host in hosts["items"]
                    if host["Hosts"]["host_name"] in componentHostnames
                ]
                return componentHosts
        return []

    def getHostWithComponent(self, serviceName, componentName, services, hosts):
        componentHosts = self.getHostsWithComponent(
            serviceName, componentName, services, hosts
        )
            ...
            for service in services["services"]:
                components.extend(
                    [
                        componentEntry
                        for componentEntry in service["components"]
                        if componentEntry["StackServiceComponents"][
                            "component_category"
                        ]
                        in categories
                        and hostname
                        in componentEntry["StackServiceComponents"]["hostnames"]
                    ]
                )
        return components
    ...
    def get_service_component_meta(self, service, component, services):
        ...
        service_meta = [
            item
            for item in services["services"]
            if item[__stack_services]["service_name"] == service
        ]
        ...
        component_meta = [
            item
            for item in service_meta["components"]
            if item[__stack_service_components]["component_name"] == component
        ]
    ...
    # region HDFS
    def getHadoopProxyUsersValidationItems(
        self, properties, services, hosts, configurations
    ):
        validationItems = []
        ...
        for user_name, user_properties in users.items():
            ...
            for prop in props:
                validationItems.append(
                    {
                        "config-name": prop,
                        "item": self.validatorNotEmpty(properties, prop),
                    }
                )
    ...
    def getHadoopProxyUsers(self, services, hosts, configurations):
        ...
        for (
            serviceName,
            serviceUserComponents,
        ) in self.getServiceHadoopProxyUsersConfigurationDict().items():
            users.update(
                self._getHadoopProxyUsersForService(
                    serviceName, serviceUserComponents, services, hosts, configurations
                )
            )
        ...
        return {
            ...
            "OOZIE": [
                (
                    "oozie-env",
                    "oozie_user",
                    {HOSTS_PROPERTY: ["OOZIE_SERVER"], GROUPS_PROPERTY: ALL_WILDCARD},
                )
            ],
            "HIVE": [
                (
                    ...
                    "hive_user",
                    {
                        HOSTS_PROPERTY: ["HIVE_SERVER", "HIVE_SERVER_INTERACTIVE"],
                ...
                (
                    "hive-env",
                    "webhcat_user",
                    {HOSTS_PROPERTY: ["WEBHCAT_SERVER"], GROUPS_PROPERTY: ALL_WILDCARD},
                ),
            ],
            "YARN": [
                (
                    "yarn-env",
                    "yarn_user",
                    {HOSTS_PROPERTY: ["RESOURCEMANAGER"]},
                    lambda services, hosts: len(
                        self.getHostsWithComponent(
                            "YARN", "RESOURCEMANAGER", services, hosts
                        )
                    )
                    > 1,
                )
    ...
    def _getHadoopProxyUsersForService(
        self, serviceName, serviceUserComponents, services, hosts, configurations
    ):
        self.logger.info(
            "Calculating Hadoop Proxy User recommendations for {0} service.".format(
                serviceName
            )
        )
        servicesList = self.get_services_list(services)
        ...
        if serviceName in servicesList:
            ...
            for values in serviceUserComponents:
                ...
                user = get_from_dict(
                    services,
                    ("configurations", userNameConfig, "properties", userNameProperty),
                    None,
                )
                if user:
            ...
            for user, (
                ...
                for proxyPropertyName, hostSelector in hostSelectorMap.items():
                    ...
                    if isinstance(hostSelector, (list, tuple)):
                        ...
                        self.logger.info(
                            "Host List for [service='{0}'; user='{1}'; components='{2}']: {3}".format(
                                serviceName,
                                user,
                                ",".join(hostSelector),
                                componentHostNamesString,
    ...
    def recommendHadoopProxyUsers(self, configurations, services, hosts):
        ...
        if "HIVE" in servicesList:
            hive_user = get_from_dict(
                services,
                ("configurations", "hive-env", "properties", "hive_user"),
                default_value=None,
            )
            if hive_user and get_from_dict(
                users, (hive_user, "propertyHosts"), default_value=None
            ):
                services["forced-configurations"].append(
                    {
                        "type": "core-site",
                        "name": "hadoop.proxyuser.{0}.hosts".format(hive_user),
                    }
                )
        ...
        for user_name, user_properties in users.items():
            ...
            self.logger.info(
                "Updated hadoop.proxyuser.{0}.hosts as : {1}".format(
                    user_name, user_properties["propertyHosts"]
                )
            )
            if "propertyGroups" in user_properties:
                self.put_proxyuser_value(
            ...
            userOldValue = self.getOldValue(
                services, user_properties["config"], user_properties["propertyName"]
            )
            if userOldValue is not None and userOldValue != user_name:
                putCoreSitePropertyAttribute(
                    "hadoop.proxyuser.{0}.hosts".format(userOldValue), "delete", "true"
                )
                services["forced-configurations"].append(
                    {
                        "type": "core-site",
                        "name": "hadoop.proxyuser.{0}.hosts".format(userOldValue),
                    }
                )
                services["forced-configurations"].append(
                    {
                        "type": "core-site",
                        "name": "hadoop.proxyuser.{0}.hosts".format(user_name),
                    }
                )

                if "propertyGroups" in user_properties:
                    putCoreSitePropertyAttribute(
                        "hadoop.proxyuser.{0}.groups".format(userOldValue),
                        "delete",
                        "true",
                    )
                    services["forced-configurations"].append(
        ...
        self.recommendAmbariProxyUsersForHDFS(
            services,
            configurations,
            servicesList,
            putCoreSiteProperty,
    ...
    def recommendAmbariProxyUsersForHDFS(
        self,
        services,
        configurations,
        servicesList,
        ...
        if "HDFS" in servicesList:
            ...
            if old_ambari_user is not None:
                putCoreSitePropertyAttribute(
                    "hadoop.proxyuser.{0}.hosts".format(old_ambari_user),
                    "delete",
                    "true",
                )
                putCoreSitePropertyAttribute(
                    "hadoop.proxyuser.{0}.groups".format(old_ambari_user),
                    "delete",
                    "true",
                )
    ...
    def getAmbariUser(self, services):
        ...
        if (
            ...
            ambari_user = services["configurations"]["cluster-env"]["properties"][
                "ambari_principal_name"
            ]
    ...
    def put_proxyuser_value(
        ...
        is_wildcard_value, current_value = self.get_data_for_proxyuser(
            user_name, services, configurations, is_groups
        )
        result_value = "*"
    ...
    def get_data_for_proxyuser(self, user_name, services, configurations, groups=False):
        ...
        if property_name in coreSite:
            ...
            if property_value == "*":
                ...
                property_value, replacement_map = self.preserve_special_values(
                    property_value
                )
                ...
                if "core-site" in configurations:
                    if property_name in configurations["core-site"]["properties"]:
                        ...
                        result_values = result_values.union(
                            [v.strip() for v in additional_value.split(",")]
                        )
            ...
            if "changed-configurations" in services.keys():
                ...
                for changedConfig in changedConfigs:
                    if (
                        changedConfig["type"] == configType
                        and changedConfig["name"] == propertyName
                        and "old_value" in changedConfig
                    ):
    ...
    def validateXmxValue(self, properties, recommendedDefaults, propertyName):
        ...
        if defaultValue is None:
            return self.getErrorItem(
                "Config's default value can't be null or undefined"
            )
        if not self.checkXmxValueFormat(value) and self.checkXmxValueFormat(
            defaultValue
        ):
        ...
        if valueInt < defaultValueInt:
            return self.getWarnItem(
                "Value is less than the recommended default of -Xmx" + defaultValueXmx
            )
        return None

    def validatorLessThenDefaultValue(
        self, properties, recommendedDefaults, propertyName
    ):
    ...
    def validatorGreaterThenDefaultValue(
        self, properties, recommendedDefaults, propertyName
    ):
        if propertyName not in recommendedDefaults:
            # If a property name exists in say hbase-env and hbase-site (which is allowed), then it will exist in the
        ...
        if value > defaultValue:
            return self.getWarnItem(
                "Value is greater than the recommended default of {0}".format(
                    defaultValue
                )
            )
        return None

    def validatorEqualsPropertyItem(
        self, properties1, propertyName1, properties2, propertyName2, emptyAllowed=False
    ):
        ...
        if value1 != value2:
            return self.getWarnItem(
                "It is recommended to set equal values "
                "for properties {0} and {1}".format(propertyName1, propertyName2)
            )

        return None

    def validatorEqualsToRecommendedItem(
        self, properties, recommendedDefaults, propertyName
    ):
        if not propertyName in properties:
        ...
        if not propertyName in recommendedDefaults:
            return self.getErrorItem(
                "Value should be recommended for %s" % propertyName
            )
        recommendedValue = recommendedDefaults.get(propertyName)
        if value != recommendedValue:
            return self.getWarnItem(
                "It is recommended to set value {0} " "for property {1}".format(
                    recommendedValue, propertyName
                )
            )
        return None
    ...
    def validatorNotRootFs(
        self, properties, recommendedDefaults, propertyName, hostInfo
    ):
        if not propertyName in properties:
            return self.getErrorItem("Value should be set")
        dir = properties[propertyName]
        if not dir.startswith("file://") or dir == recommendedDefaults.get(
            propertyName
        ):
            return None
        ...
        if (
            "/" == mountPoint
            and self.getPreferredMountPoints(hostInfo)[0] != mountPoint
        ):
            return self.getWarnItem(
                "It is not recommended to use root partition for {0}".format(
                    propertyName
                )
            )
    ...
    def validatorEnoughDiskSpace(
        ...
        mountPoints = {}
        for mountPoint in hostInfo["disk_info"]:
            mountPoints[mountPoint["mountpoint"]] = self.to_number(
                mountPoint["available"]
            )
        mountPoint = DefaultStackAdvisor.getMountPointForDir(
            dir, list(mountPoints.keys())
        )

        if not mountPoints:
            return self.getErrorItem(
                "No disk info found on host %s" % hostInfo["host_name"]
            )

        if mountPoint is None:
            return self.getErrorItem(
                "No mount point in directory %s. Mount points: %s"
                % (dir, ", ".join(list(mountPoints.keys())))
            )
    ...
    def validatorYarnQueue(
        self, properties, recommendedDefaults, propertyName, services
    ):
        ...
        if len(leaf_queue_names) == 0:
            ...
            return self.getErrorItem(
                "Queue does not exist or correspond to an existing YARN leaf queue"
            )
    ...
    def recommendYarnQueue(self, services, catalog_name=None, queue_property=None):
        ...
        if services and "configurations" in services:
            configurations = services["configurations"]
            if (
                catalog_name in configurations
                and queue_property in configurations[catalog_name]["properties"]
            ):
                old_queue_name = configurations[catalog_name]["properties"][
                    queue_property
                ]

            capacity_scheduler_properties, _ = self.getCapacitySchedulerProperties(
                services
            )
            leaf_queues = sorted(
            ...
            if leaf_queues and (
                old_queue_name is None or old_queue_name not in leaf_queues
            ):
    ...
    def isConfigPropertiesChanged(
        self, services, config_type, config_names, all_exists=True
    ):
        """
    ...
    def getCapacitySchedulerProperties(self, services):
        ...
        if "capacity-scheduler" in services["configurations"]:
            if (
                "capacity-scheduler"
                in services["configurations"]["capacity-scheduler"]["properties"]
            ):
                cap_sched_props_as_str = services["configurations"][
                    "capacity-scheduler"
                ]["properties"]["capacity-scheduler"]
                if cap_sched_props_as_str:
                    cap_sched_props_as_str = str(cap_sched_props_as_str).split("\n")
                    if (
                        len(cap_sched_props_as_str) > 0
                        and cap_sched_props_as_str[0] != "null"
                    ):
                    ...
                    self.logger.info(
                        "'capacity-scheduler' configs not passed-in as single '\\n' string in "
            ...
            if not capacity_scheduler_properties:
                ...
                capacity_scheduler_properties = services["configurations"][
                    "capacity-scheduler"
                ]["properties"]
                self.logger.info(
                    "'capacity-scheduler' configs is passed-in as a dictionary. "
        ...
        self.logger.info(
            "Retrieved 'capacity-scheduler' received as dictionary : '{0}'. configs : {1}".format(
                received_as_key_value_pair, capacity_scheduler_properties.items()
            )
        )
    ...
    def getAllYarnLeafQueues(self, capacitySchedulerProperties):
        ...
        if "yarn.scheduler.capacity.root.queues" in config_list:
            yarn_queues = capacitySchedulerProperties.get(
        ...
        if dir:
            ...
            for mountPoint in mountPoints:
                ...
                if os.path.join(dir, "").startswith(os.path.join(mountPoint, "")):
                    ...
                    elif os.path.join(bestMountFound, "").count(
                        os.path.sep
    ...
    def validateMinMemorySetting(self, properties, defaultValue, propertyName):
        ...
        try:
            ...
            if valueInt < defaultValueInt:
                return self.getWarnItem(
                    "Value is less than the minimum recommended default of -Xmx"
                    + str(defaultValue)
                )
        except:
            return None
