...
class AdvisorAdapter(object):
  """
  Proxy class forwards all override methods to either stackAdvisor(legacy) or mpackAdvisor.

  """

  def __init__(self):
    self.stackAdvisor = None
    self.mpackAdvisor = None
  ...
  def registerStackAdvisor(self, stackAdvisor):
    self.stackAdvisor = stackAdvisor
    self.mpackAdvisor = None
    self.initialize_logger('StackAdvisor')

  def registerMpackAdvisor(self, mpackAdvisor):
    self.mpackAdvisor = mpackAdvisor
    self.stackAdvisor = None
    self.initialize_logger('MpackAdvisor')

  def recommendComponentLayout(self, services, hosts):
    if self.stackAdvisor and not self.mpackAdvisor:
      return self.stackAdvisor.recommendComponentLayout(services, hosts)
    elif self.mpackAdvisor and not self.stackAdvisor:
      return self.mpackAdvisor.recommendComponentLayout(services, hosts)

    pass

  def validateComponentLayout(self, services, hosts):
    if self.stackAdvisor and not self.mpackAdvisor:
      return self.stackAdvisor.validateComponentLayout(services, hosts)
    elif self.mpackAdvisor and not self.stackAdvisor:
      return self.mpackAdvisor.validateComponentLayout(services, hosts)

    pass

  def recommendConfigurations(self, services, hosts):
    if self.stackAdvisor and not self.mpackAdvisor:
      return self.stackAdvisor.recommendConfigurations(services, hosts)
    elif self.mpackAdvisor and not self.stackAdvisor:
      return self.mpackAdvisor.recommendConfigurations(services, hosts)

    pass

  def recommendConfigurationsForSSO(self, services, hosts):
    if self.stackAdvisor and not self.mpackAdvisor:
      return self.stackAdvisor.recommendConfigurationsForSSO(services, hosts)
    elif self.mpackAdvisor and not self.stackAdvisor:
      return self.mpackAdvisor.recommendConfigurationsForSSO(services, hosts)

    pass

  def validateConfigurations(self, services, hosts):
    if self.stackAdvisor and not self.mpackAdvisor:
      return self.stackAdvisor.validateConfigurations(services, hosts)
    elif self.mpackAdvisor and not self.stackAdvisor:
      return self.mpackAdvisor.validateConfigurations(services, hosts)

    pass

  def initialize_logger(self, name='StackAdvisor', logging_level=logging.INFO, format='%(asctime)s %(levelname)s %(name)s %(funcName)s: - %(message)s'):
  ...
  def getServiceComponentLayoutValidations(self, services, hosts):
    ...
    if self.stackAdvisor and not self.mpackAdvisor:
      return self.stackAdvisor.getServiceComponentLayoutValidations(services, hosts)
    elif self.mpackAdvisor and not self.stackAdvisor:
      return self.mpackAdvisor.getServiceComponentLayoutValidations(services, hosts)

  def get_heap_size_properties(self, services):

    if self.stackAdvisor and not self.mpackAdvisor:
      return self.stackAdvisor.get_heap_size_properties(services)
    elif self.mpackAdvisor and not self.stackAdvisor:
      return self.mpackAdvisor.get_heap_size_properties(services)
