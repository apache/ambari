...
def setup_kms_db(stack_version=None):
  ...
  if params.has_ranger_admin:
    ...
    ModifyPropertiesFile(format("/usr/iop/current/ranger-kms/install.properties"),
  ...
  if response_code is not None and response_code == 200:
    ...
    if user_resp_code is not None and user_resp_code == 200:
      ...
      if not get_repo_flag:
        create_repo_flag = create_repo(policymgr_mgr_url, json.dumps(params.kms_ranger_plugin_repo), ambari_username_password_for_ranger)
        if create_repo_flag:
          return True
        else:
          return False
