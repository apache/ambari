...
hadoop_mapred2_jar_location = format("/usr/iop/current/hadoop-mapreduce")
mapred_bin = format("/usr/iop/current/hadoop-mapreduce/sbin")
hadoop_yarn_home = format("/usr/iop/current/hadoop-yarn")
yarn_bin = format("/usr/iop/current/hadoop-yarn/sbin")
yarn_container_bin = format("/usr/iop/current/hadoop-yarn/bin")
...
yarn_web_address_property_name = 'yarn.resourcemanager.webapp.address' if not yarn_https_on else 'yarn.resourcemanager.webapp.https.address'
if len(rm_hosts) > 1:
  rm_ids = config['configurations']['yarn-site']['yarn.resourcemanager.ha.rm-ids'].split(',')
  yarn_web_address_property_name = yarn_web_address_property_name + '.' + rm_ids[0]

yarn_rm_address = config['configurations']['yarn-site'][yarn_web_address_property_name]
rm_active_port = yarn_rm_address.split(':')[-1]
...
if len(rm_hosts) > 1:
  ...
  rm_webui_address = format("{rm_host}:{rm_active_port},{additional_rm_host}:{rm_active_port}")
  rm_webui_https_address = format("{rm_host}:{rm_active_port},{additional_rm_host}:{rm_active_port}")
else:
  rm_webui_address = format("{rm_host}:{rm_active_port}")
  rm_webui_https_address = format("{rm_host}:{rm_active_port}")
