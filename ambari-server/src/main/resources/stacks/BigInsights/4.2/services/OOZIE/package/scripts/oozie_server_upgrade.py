...
def prepare_libext_directory():
  ...
  target_version_needs_compression_libraries = compare_versions(
    format_hdp_stack_version(params.version), '4.0.0.0') >= 0
  ...
  # iop-select set hadoop-client has not run yet, therefore we cannot use
...
def upgrade_oozie_database_and_sharelib():
  ...
  upgrade_stack = iop_select._get_upgrade_stack()
