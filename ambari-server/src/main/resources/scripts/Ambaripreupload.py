...
with Environment() as env:
  ...
  def get_stack_version():
    if not options.hdp_version:
      ...
      with open(tmpfile.name, "r+") as file:
        get_stack_version_cmd = (
          f"/usr/bin/hdp-select status hadoop-mapreduce-historyserver > {tmpfile.name}"
        )
      ...
      if not stack_version:
        Logger.error(
          f"Could not parse HDP version from output of hdp-select: {str(out)}"
        )
        return 1
    else:
  ...
  def putSQLDriverToOozieShared():
    params.HdfsResource(
      hdfs_path_prefix + f"/user/oozie/share/lib/sqoop/{sql_driver_filename}",
      owner="hdfs",
      type="file",
      action=["create_on_execute"],
      mode=0o644,
    ...
    if os.path.exists(spark_client_dir):
      try:
        ...
        if not os.path.exists(format("{oozie_shared_lib}/lib/spark")):
          Execute(("mkdir", format("{oozie_shared_lib}/lib/spark")), sudo=True)
        ...
        Execute(
          format("cp -P {spark_client_dir}/lib/*.jar {oozie_shared_lib}/lib/spark")
        )
        ...
        Execute(
          ("chmod", "-R", "0755", format("{oozie_shared_lib}/lib/spark")), sudo=True
        )
