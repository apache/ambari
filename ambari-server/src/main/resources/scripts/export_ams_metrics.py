...
class Utils:
    ...
    def get_data_from_url(collector_uri):
        ...
        if connection:
            try:
                ...
                logger.warn(
                    "Error parsing json data returned from URI: %s" % collector_uri
                )
                logger.debug(str(e))
    ...
    def get_epoch(input):
        if len(input) == 13:
            ...
            return int(
                time.mktime(
                    datetime.datetime.strptime(input, "%Y-%m-%dT%H:%M:%SZ").timetuple()
                )
                * 1000
            )
        else:
            ...
            for metrics_dir in AmsMetricsProcessor.get_metrics_dirs(Params.INPUT_DIR):
                for dir_item in os.listdir(metrics_dir):
                    dir_item_path = os.path.join(
                        Params.INPUT_DIR, metrics_dir, dir_item
                    )
                    if dir_item == "configs":
    ...
    def get(self):
        ...
        if "hostname" in args and args["hostname"] != "":
            ...
            for host_name in host_names:
                if (
                    metric_name
                    in self.ams_metrics_processor.metrics_for_hosts[host_name]
                ):
                    if (
                        len(
            ...
            for metric in self.ams_metrics_processor.metrics_for_hosts:
                if metric_name == metric:
                    metric_dict = self.ams_metrics_processor.metrics_for_hosts[
                        metric_name
                    ]
                    break
            ...
            for host in self.ams_metrics_processor.metrics_for_hosts:
                for metric in self.ams_metrics_processor.metrics_for_hosts[host]:
                    if (
                        metric_name == metric
                        and len(
                            self.ams_metrics_processor.metrics_for_hosts[host][
                                metric_name
                            ]["metrics"]
                        )
                        > 0
...
def main():
    ...
    time_suffix = "{0}-{1}-{2}-{3}-{4}-{5}".format(
    ...
    parser.add_option(
        ...
        default=logfile,
        metavar="FILE",
        help="Log file. [default: %s]" % logfile,
    )
    ...
    if Params.ACTION == "export":
        ...
        Params.OUT_DIR = (
            ...
            if options.output_dir == output_dir
                ...
                options.output_dir, "ambari_metrics_export_" + time_suffix
            )
        )
