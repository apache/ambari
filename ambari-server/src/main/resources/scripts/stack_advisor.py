...
def dumpJson(json_object, dump_file):
    try:
        ...
        traceback.print_exc()
        raise StackAdvisorException(
            "Error writing to file {0} : {1}".format(dump_file, str(err))
        )
    ...
    for version in reversed(versions):
        try:
            ...
            if os.path.isfile(path):
                ...
                print(
                    "StackAdvisor implementation for stack {0}, version {1} was loaded".format(
                        stackName, version
                    )
                )
            ...
            traceback.print_exc()
            print(
                "StackAdvisor implementation for stack {0}, version {1} was not found".format(
                    stackName, version
                )
            )
