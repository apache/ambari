...
def get_server_info(silent=False):
    if not silent:
        ...
        protocol = get_validated_string_input(
...
class AmbariBlueprint:
    ...
    def importBlueprint(self, blueprintLocation, hostsLocation, clusterName):
        ...
        if not blueprintInfo:
            raise Exception(
                "Cannot read blueprint info from blueprint at %s" % blueprintLocation
            )

        blueprint_name = blueprintInfo.get("blueprint_name")
        if not blueprint_name:
            raise Exception(
                "blueprint_name required inside Blueprints %s" % blueprintInfo
            )
        ...
        if isDefaultJson and INLINE_ARGUMENTS:
            ...
            expectedMasterCount = self.parseDefaultJsonData(
                blueprint_json, slaves.split(",")
            )
            if expectedMasterCount != len(masters.split(",")):
                logger.info(
                    "Mismatch in cardinality. Inferring host assignment for masters..."
                )
            pass
        ...
        if retCode == "201":
            ...
            logger.error(
                "Unable to create blueprint from location %s" % blueprintLocation
            )
            sys.exit(1)
        pass
    ...
    def buildHostAssignments(
        ...
        hosts = '{{"fqdn":"{0}"}},'
        logger.debug(
            "Blueprint: {0}, Masters: {1}, Slaves: {2}".format(
                blueprintName, masters, slaves
            )
        )
        mastersUsed = 0
        slavesUsed = 0
        ...
        for hostGroup in hostGroups:
            if hostGroup.get("name").find("master") != -1:
                ...
                masterHostsGroup = hostGroupHosts.format(
                    hostGroup.get("name"), masterHosts
                )
            ...
            if hostGroup.get("name").find("slave") != -1:
                ...
                slaveHostsGroup = hostGroupHosts.format(
                    hostGroup.get("name"), slaveHosts
                )
            ...
            if hostGroup.get("name").find("gateway") != -1:
                ...
                gatewayHostGroup = hostGroupHosts.format(
                    hostGroup.get("name"), gatewayHosts
                )
        ...
        hostGroupsJson = (
            hostGroupsJson.rstrip(",")
            if hostGroupsJson.endswith(",")
            else hostGroupsJson
        )
    ...
    def parseHostData(self):
        ...
        if masters and slaves:
            ...
            raise Exception(
                "Master and Slave assignments required for a multi-node cluster."
            )
                ...
                if hostGroupName:
                    if hostGroupName.find("slave") != -1:
                        ...
                        if cardinality and cardinality.find(SLAVES_REPLACE_EXPR) != -1:
                            json_data["host_groups"]["slave"]["cardinality"] = len(
                                slaves
                            )
    ...
    def exportBlueprint(self, clusterName, exportFilePath):
        ...
        if resp:
            ...
            logger.error(
                "Unable to perform export operation on cluster, %s" % clusterName
            )
...
def main():
    ...
    if options.silent:
        if options.blueprint is None:
            raise Exception(
                "Destination file path required. '-f' option not " "provided."
            )
        elif (
            options.action == "import"
            and options.hosts is None
            and options.arguments is None
        ):
            raise Exception(
    ...
    if options.action == "import":
        ambariBlueprint.importBlueprint(
            options.blueprint, options.hosts, options.cluster
        )
