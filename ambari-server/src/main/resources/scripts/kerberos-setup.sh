...
########################
getEnvironmentCMD () {
  os=`ambari-python-wrap -c 'import sys; sys.path.append("/usr/lib/ambari-server/lib/"); from ambari_commons import OSCheck; print(OSCheck.get_os_family())'`
  version=`ambari-python-wrap -c 'import sys; sys.path.append("/usr/lib/ambari-server/lib/"); from ambari_commons import OSCheck; print(OSCheck.get_os_major_version())'`
  os=$os$version;
  case $os in
  'ubuntu12' )
    pkgmgr='apt-get'
