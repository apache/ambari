    ...
    def read_data_to_map(self, path):
        ...
        for properties in root.getiterator("property"):
            ...
            if name != None:
                ...
                logger.warn(
                    "No name is found for one of the properties in {0}, ignoring it".format(
                        path
                    )
                )
                continue
            ...
            if value != None:
                ...
                logger.warn(
                    'No value is found for "{0}" in {1}, using empty string for it'.format(
                        name_text, path
                    )
                )
                value_text = ""
        ...
        logger.debug(
            "Following configurations found in {0}:\n{1}".format(path, configurations)
        )
...
class ConfigMerge:
    ...
    def __init__(
        self, config_files_map=None, left_file_paths=None, right_file_paths=None
    ):
    ...
    @staticmethod
    def get_all_supported_files_grouped_by_name(
        extensions=SUPPORTED_EXTENSIONS, directory=INPUT_DIR
    ):
        filePaths = {}
            ...
            for file in fileList:
                ...
                if ext in extensions:
                    ...
                    if (
                        ...
                        logger.warn(
                            "File {0} is not configurable by Ambari. Skipping...".format(
                                file_path
                            )
                        )
                        continue
                    ...
                    if ConfigMerge.UNKNOWN_FILES_MAPPING_FILE:
                        for (
                            path_regex,
                            name,
                        ) in ConfigMerge.CONTENT_UNKNOWN_FILES_MAPPING_FILE.items():
                            match = re.match(
                                path_regex,
                                os.path.relpath(file_path, ConfigMerge.INPUT_DIR),
                            )
                            if match:
                                config_name = name
                                break
                    ...
                    if not config_name:
                        if file in ConfigMerge.NOT_MAPPED_FILES:
                            if ConfigMerge.UNKNOWN_FILES_MAPPING_FILE:
                                logger.error(
                                    "File {0} doesn't match any regex from {1}".format(
                                        file_path,
                                        ConfigMerge.UNKNOWN_FILES_MAPPING_FILE,
                                    )
                                )
                            else:
                                logger.error(
                                    "Cannot map {0} to Ambari config type. Please use -u option to specify config mapping for this file. \n"
                                    "For more information use --help option for script".format(
                                        file_path
                                    )
                                )
                            continue
                    ...
                    filePaths[config_name].append(
                        (file_path, ConfigMerge.PARSER_BY_EXTENSIONS[ext])
                    )
        return filePaths
    ...
    def merge_configurations(filepath_to_configurations):
        ...
        for path, configurations in filepath_to_configurations.items():
            for configuration_name, value in configurations.items():
                ...
                logger.debug(
                    "Iterating over '{0}' with value '{1}' in file '{2}'".format(
                        configuration_name, value, path
                    )
                )
                property_name_to_value_to_filepaths[configuration_name][value].append(
                    path
                )
    ...
    def format_for_blueprint(configurations, attributes):
        ...
        for configuration_type, configuration_properties in configurations.items():
            ...
            if is_content:
                ...
                all_configs[-1][configuration_type] = {
            ...
            for (
                configuration_type_attributes,
                properties_attributes,
            ) in attributes.items():
                if (
                    properties_attributes
                    and configuration_type == configuration_type_attributes
                ):
                    all_configs[-1][configuration_type].update(
                        {"properties_attributes": {"final": properties_attributes}}
                    )
    ...
    def format_conflicts_output(property_name_to_value_to_filepaths):
        ...
        for (
            property_name,
            value_to_filepaths,
        ) in property_name_to_value_to_filepaths.items():
    ...
    def perform_merge(self):
        ...
        for filename, paths_and_parsers in self.config_files_map.items():
            ...
            # configs merge
            merged_configurations, property_name_to_value_to_filepaths = (
                ConfigMerge.merge_configurations(filepath_to_configurations)
            )

            # properties attributes merge
            merged_attributes, property_name_to_attribute_to_filepaths = (
                ConfigMerge.merge_configurations(filepath_to_property_attributes)
            )

            configuration_conflicts_output = ConfigMerge.format_conflicts_output(
                property_name_to_value_to_filepaths
            )
            attribute_conflicts_output = ConfigMerge.format_conflicts_output(
                property_name_to_attribute_to_filepaths
            )
            ...
            if configuration_conflicts_output:
                ...
                logger.warn(
                    "You have configurations conflicts for {0}. Please check {1}".format(
                        configuration_type, conflict_filename
                    )
                )
            ...
            if attribute_conflicts_output:
                ...
                logger.warn(
                    "You have property attribute conflicts for {0}. Please check {1}".format(
                        configuration_type, conflict_filename
                    )
                )
                with open(conflict_filename, "w") as fp:
        ...
        with open(result_json_file, "w") as outfile:
            outfile.write(
                json.dumps(
                    ConfigMerge.format_for_blueprint(
                        result_configurations, result_property_attributes
                    ),
                    sort_keys=True,
                    indent=4,
                    separators=(",", ": "),
    ...
    def perform_diff(self):
        ...
        for match_config in matches_configs:
            ...
            ) = ConfigMerge.configuration_diff(
                self.left_file_paths[match_config], self.right_file_paths[match_config]
            )

        file_conflicts = ConfigMerge.get_missing_files(
            self.right_file_paths, matches_configs, ConfigMerge.LEFT_INPUT_DIR
        ) + ConfigMerge.get_missing_files(
            self.left_file_paths, matches_configs, ConfigMerge.RIGHT_INPUT_DIR
        )

        configuration_diff_output = None
        configuration_diff_output = ConfigMerge.format_diff_output(
            file_conflicts, configurations_conflicts, attributes_conflicts
        )
        ...
        if configuration_diff_output and configuration_diff_output != "":
            ...
            logger.warn(
                "You have file diff conflicts. Please check {0}".format(
                    conflict_filename
                )
            )
            with open(conflict_filename, "w") as fp:
    ...
    def configuration_diff(left, right):
        ...
        right_configurations, right_attributes = right_parser.read_data_to_map(
            right_path
        )
        ...
        matches_configs, properties_conflicts = ConfigMerge.get_conflicts_and_matches(
            left_configurations, right_configurations, left_path, right_path
        )
        properties_conflicts += ConfigMerge.get_missing_properties(
            left_configurations, matches_configs, right_path
        ) + ConfigMerge.get_missing_properties(
            right_configurations, matches_configs, left_path
        )

        if left_attributes and right_attributes:
            matches_attributes, attributes_conflicts = (
                ConfigMerge.get_conflicts_and_matches(
                    left_attributes, right_attributes, left_path, right_path
                )
            )
            attributes_conflicts += ConfigMerge.get_missing_attributes(
                left_attributes, matches_attributes, right_path
            ) + ConfigMerge.get_missing_attributes(
                right_attributes, matches_attributes, left_path
            )
    ...
    def get_missing_attributes(attributes, matches, file_path):
        ...
        for key, value in attributes.items():
            if not key in matches:
                conflicts.append(
                    {key: "Final attribute is missing in {0} file".format(file_path)}
                )
        return conflicts
    ...
    def get_missing_properties(configurations, matches, file_path):
        ...
        for key, value in configurations.items():
            if not key in matches:
                conflicts.append(
                    {key: "Property is missing in {0} file".format(file_path)}
                )
        return conflicts
    ...
    def get_missing_files(config_file_paths, matches, input_dir):
        ...
        for file_name in config_file_paths:
            if file_name not in matches:
                conflicts.append(
                    {
                        file_name: "Configurations file is missing for {0} directory".format(
                            input_dir
                        )
                    }
                )
...
def main():
    ...
    if options.unknown_files_mapping_file and os.path.exists(
        options.unknown_files_mapping_file
    ):
        ...
        logger.warning(
            "Config mapping file was not found at {0}. "
            "Please provide it at the given path or provide a different path to it using -u option.".format(
                options.unknown_files_mapping_file
            )
        )
    if options.action == "merge":
        ConfigMerge.INPUT_DIR = options.inputDir
        file_paths = ConfigMerge.get_all_supported_files_grouped_by_name(
            directory=ConfigMerge.INPUT_DIR
        )
        logger.info("Writing logs into '{0}' file".format(logegr_file_name))
        logger.debug(
            "Following configuration files found:\n{0}".format(file_paths.items())
        )
        ...
        if options.leftInputDir and os.path.isdir(options.leftInputDir):
            ...
            logger.error(
                'Directory "{0}" doesn\'t exist. Use option "-h" for details'.format(
                    options.leftInputDir
                )
            )
            return -1
        ...
        left_file_paths = ConfigMerge.get_all_supported_files_grouped_by_name(
            directory=ConfigMerge.LEFT_INPUT_DIR
        )
        logger.debug(
            "Following configuration files found:\n{0} for left directory".format(
                left_file_paths.items()
            )
        )
        right_file_paths = ConfigMerge.get_all_supported_files_grouped_by_name(
            directory=ConfigMerge.RIGHT_INPUT_DIR
        )
        logger.debug(
            "Following configuration files found:\n{0} for right directory".format(
                right_file_paths.items()
            )
        )
        config_merge = ConfigMerge(
            left_file_paths=left_file_paths, right_file_paths=right_file_paths
        )
        return config_merge.perform_diff()

    else:
        logger.error(
            'Action "{0}" doesn\'t supports by script. Use option "-h" for details'.format(
                options.action
            )
        )
        return -1
