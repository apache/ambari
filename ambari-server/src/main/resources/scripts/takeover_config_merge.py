...
class ConfigMerge:
        ...
        if ext in extensions:
          ...
          if ConfigMerge.UNKNOWN_FILES_MAPPING_FILE:
            for (
              ...
              match = re.match(
                path_regex, os.path.relpath(file_path, ConfigMerge.INPUT_DIR)
              )
              if match:
                config_name = name
                break
          ...
          if not config_name:
            if file in ConfigMerge.NOT_MAPPED_FILES:
              if ConfigMerge.UNKNOWN_FILES_MAPPING_FILE:
                logger.error(
                  f"File {file_path} doesn't match any regex from {ConfigMerge.UNKNOWN_FILES_MAPPING_FILE}"
                )
              else:
                logger.error(
  ...
  def merge_configurations(filepath_to_configurations):
    ...
    for path, configurations in filepath_to_configurations.items():
      for configuration_name, value in configurations.items():
        ...
        logger.debug(
          f"Iterating over '{configuration_name}' with value '{value}' in file '{path}'"
        )
  ...
  def format_for_blueprint(configurations, attributes):
    ...
    for configuration_type, configuration_properties in configurations.items():
      ...
      for configuration_type_attributes, properties_attributes in attributes.items():
        if (
          properties_attributes and configuration_type == configuration_type_attributes
        ):
  ...
  def perform_diff(self):
    ...
    for match_config in matches_configs:
      configurations_conflicts[match_config], attributes_conflicts[match_config] = (
  ...
  def configuration_diff(left, right):
    ...
    if left_attributes and right_attributes:
      matches_attributes, attributes_conflicts = ConfigMerge.get_conflicts_and_matches(
      ...
      for right_key, right_value in right_items.items():
        if left_key == right_key:
          ...
          if left_value != right_value:
            conflicts.append(
              {right_key: [{left_path: left_value}, {right_path: right_value}]}
            )
  ...
  def get_missing_files(config_file_paths, matches, input_dir):
    ...
    for file_name in config_file_paths:
      if file_name not in matches:
        conflicts.append(
          {file_name: f"Configurations file is missing for {input_dir} directory"}
        )
    return conflicts
...
def main():
  ...
  if options.action == "merge":
    ...
    if options.leftInputDir and os.path.isdir(options.leftInputDir):
      ...
      logger.error(
        f'Directory "{options.leftInputDir}" doesn\'t exist. Use option "-h" for details'
      )
      return -1
    ...
    if options.rightInputDir and os.path.isdir(options.rightInputDir):
      ...
      logger.error(
        f'Directory "{options.rightInputDir}" doesn\'t exist. Use option "-h" for details'
      )
      return -1
    ...
    logger.error(
      f'Action "{options.action}" doesn\'t supports by script. Use option "-h" for details'
    )
    return -1
