...
def api_accessor(host, login, password, protocol, port, unsafe=None):
    def do_request(api_url, request_type=GET_REQUEST_TYPE, request_body=None):
        try:
            ...
            admin_auth = (
                base64.encodebytes(("%s:%s" % (login, password)).encode())
                .decode()
                .replace("\n", "")
            )
...
def get_config_tag(cluster, config_type, accessor):
    ...
    try:
        ...
        raise Exception(
            '"{0}" not found in server response. Response:\n{1}'.format(
...
def update_from_file(config_file):
    def update(cluster, config_type, accessor):
        ...
        try:
            ...
            raise Exception(
                'File "{0}" should be in the following JSON format ("properties_attributes" is optional):\n{1}'.format(
                    config_file, FILE_FORMAT
                )
            )
...
def delete_properties(cluster, config_type, args, accessor, version_note):
    ...
    update_config(
        cluster,
        config_type,
        delete_specific_property(config_name),
        accessor,
        version_note,
    )
    return 0
...
def main():
    ...
    if not options.credentials_file and (not options.user or not options.password):
        parser.error(
            "You should use option (-e) to set file with Ambari user credentials OR use (-u) username and (-p) password"
        )
    ...
    if options.credentials_file:
        if os.path.isfile(options.credentials_file):
            try:
                with open(options.credentials_file) as credentials_file:
                    ...
                    if len(login_lines) == 2:
                        ...
                        logger.error(
                            "Incorrect content of {0} file. File should contain Ambari username and password separated by new line.".format(
                ...
                logger.error(
                    "You don't have permissions to {0} file".format(
                        options.credentials_file
                    )
                )
                return -1
            ...
            logger.error(
                "File {0} doesn't exist or you don't have permissions.".format(
                    options.credentials_file
                )
            )
            return -1
    ...
    if action == SET_ACTION:
        ...
        return delete_properties(
            cluster, config_type, action_args, accessor, version_note
        )
    else:
        logger.error(
            'Action "{0}" is not supported. Supported actions: "get", "set", "delete".'.format(
                action
            )
        )
        return -1
