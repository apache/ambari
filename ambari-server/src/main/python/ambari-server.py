...
def init_action_parser(action, parser):
    ...
    add_parser_options(
        ...
        help="Comma separated list of resources to purge ({0}). By default ({1}) will be purged.".format(
            purge_resources, default_purge_resources
        ),
        dest="purge_list",
        parser=parser,
        optional_for_actions=[INSTALL_MPACK_ACTION],
...
def init_ldap_setup_parser_options(parser):
    ...
    parser.add_option(
        "--ldap-ssl",
        default=None,
        help="Use SSL [true/false] for LDAP",
        dest="ldap_ssl",
    )
    parser.add_option(
        "--ldap-type",
        default=None,
        help="Specify ldap type [{}] for offering defaults for missing options.".format(
            "/".join(LDAP_TYPES)
        ),
        dest="ldap_type",
    )
    ...
    try:
        ...
        if options.warnings:
            ...
            options.exit_message = (
                "Ambari Server '%s' completed with warnings." % action
            )
            pass
...
def mainBody():
    ...
    if options.verbose:
        ...
        try:
            ...
            print_error_msg(
                "Unexpected {0}: {1}".format((e).__class__.__name__, str(e))
