...
class SCP:
    ...
    def run(self):
        ...
        self.host_log.write(
            "host=" + self.host + ", exitcode=" + str(scpstat.returncode)
        )
        self.host_log.write(
...
class SSH:
    ...
    def run(self):
        ...
        self.host_log.write(
            "host=" + self.host + ", exitcode=" + str(sshstat.returncode)
        )
        self.host_log.write(
            "Command end time " + datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        )
...
class Bootstrap(threading.Thread):
    ...
    def try_to_execute(self, action):
        last_retcode = {
            "exitstatus": 177,
            "log": "Try to execute '{0}'".format(str(action)),
...
class BootstrapWindows(Bootstrap):
    ...
    def createTargetDir(self):
        ...
        psr = PSR(
            ...
            self.host,
            self.host_log,
            params="{0} {1}".format(self.host, self.getTempFolder()),
        )
        retcode = psr.run()
        self.host_log.write("\n")
        return retcode
    ...
    def unzippingBootstrapArchive(self):
        ...
        psr = PSR(
            ...
            self.host,
            self.host_log,
            params="{0} {1} {2}".format(self.host, zipFile, self.getTempFolder()),
        )
        result = psr.run()
        self.host_log.write("\n")
        return result
    ...
    def copyBootstrapArchive(self):
        ...
        psr = PSR(
            ...
            self.host,
            self.host_log,
            params="{0} {1} {2}".format(self.host, fileToCopy, target),
        )
        result = psr.run()
        self.host_log.write("\n")
        return result
    ...
    def configureChocolatey(self):
        ...
        psr = PSR(
            ...
            self.host,
            self.host_log,
            params="{0} {1}".format(self.host, tmpConfig),
        )
        result = psr.run()
        self.host_log.write("\n")
        return result
    ...
    def runSetupAgent(self):
        ...
        command = os.path.join(
            self.shared_state.script_dir, self.RUN_REMOTING_SCRIPT_NAME
        )
        psr = PSR(
            command,
            self.host,
            self.host_log,
            params='{0} "{1}"'.format(self.host, self.getRunSetupCommand(self.host)),
        )
        retcode = psr.run()
        self.host_log.write("\n")
        return retcode
...
class BootstrapDefault(Bootstrap):
    ...
    def getRepoDir(self):
        if OSCheck.is_redhat_family():
            ...
            raise Exception(
                "Unsupported OS family '{0}'".format(OSCheck.get_os_family())
            )
    ...
    def copyCommonFunctions(self):
        ...
        self.host_log.write("\n")
        result["exitstatus"] = (
            0  # ignore not copied *.pyc files with permission denied.
        )
        return result

    def getMoveRepoFileWithPasswordCommand(self, targetDir):
        return (
            "{sudo} -S mv ".format(sudo=AMBARI_SUDO)
    ...
    def getMoveRepoFileWithoutPasswordCommand(self, targetDir):
        return (
            "{sudo} mv ".format(sudo=AMBARI_SUDO)
            + str(self.getRemoteName(self.AMBARI_REPO_FILENAME))
            + " "
            + os.path.join(str(targetDir), self.AMBARI_REPO_FILENAME)
        )
    ...
    def copyNeededFiles(self):
        ...
        if os.path.exists(fileToCopy):
            ...
            self.host_log.write(
                "Ambari repo file not found: {0}".format(self.getRepoFile())
            )
            retcode = -1
            pass
    ...
    def getRunSetupWithPasswordCommand(self, expected_hostname):
        ...
        return (
            "{sudo} -S python3 ".format(sudo=AMBARI_SUDO)
            + str(setupFile)
            + " "
    ...
    def getRunSetupWithoutPasswordCommand(self, expected_hostname):
        ...
        port = self.getAmbariPort()
        return (
            "{sudo} python3 ".format(sudo=AMBARI_SUDO)
            + str(setupFile)
            + " "
            + str(expected_hostname)
            + " "
    ...
    def run(self):
        ...
        if self.hasPassword():
            action_queue.extend(
                [self.copyPasswordFile, self.changePasswordFileModeOnHost]
            )
        action_queue.extend(
            [
            ...
            for bootstrap in running_list:
                if bootstrap.getStatus()["return_code"] is not None:
                    ...
                    if elapsedtime > HOST_BOOTSTRAP_TIMEOUT:
                        # bootstrap timed out
                        logging.warn(
                            "Bootstrap at host {0} timed out and will be "
