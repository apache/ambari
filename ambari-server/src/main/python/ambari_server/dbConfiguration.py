...
class DBMSConfig(object):
    def __init__(self, options, properties, storage_type):
        ...
        self.JDBC_DRIVER_INSTALL_MSG = (
            "Before starting Ambari Server, you must install the JDBC driver."
        )
    ...
    def ensure_jdbc_driver_installed(self, properties):
        ...
        if default_driver_path and os.path.isfile(default_driver_path):
            ambari_should_use_existing_default_jdbc = get_YN_input(
                "Should ambari use existing default jdbc {0} [y/n] (y)? ".format(
                    default_driver_path
                ),
                True,
            )
        ...
        if path_to_custom_jdbc_driver and os.path.isfile(path_to_custom_jdbc_driver):
                ...
                if not path_to_custom_jdbc_driver == os.path.join(
                    ...
                    if os.path.isfile(
                        ...
                        replace_jdbc_in_share_dir = get_YN_input(
                            "You already have file {0} in /usr/share/java/. Should it be replaced? [y/n] (y)? ".format(
                                custom_jdbc_name
                            ),
                            True,
                        )
                        if replace_jdbc_in_share_dir:
                            try:
                                ...
                                err = (
                                    "ERROR: Could not remove jdbc file. %s"
                                    % os.path.join(
                                        configDefaults.JAVA_SHARE_PATH, custom_jdbc_name
                                    )
                                )
                    ...
                    shutil.copy(
                        path_to_custom_jdbc_driver, configDefaults.JAVA_SHARE_PATH
                    )
                    print(
            ...
            properties.process_pair(
                JDBC_DRIVER_PATH_PROPERTY, path_to_custom_jdbc_driver
            )
            update_properties(properties)
            return True
        else:
    ...
    def _read_password_from_properties(properties, options):
        ...
        password_file = get_value_from_properties(
            properties, JDBC_PASSWORD_PROPERTY, ""
        )
        if password_file:
    ...
    def _init_member_with_default(options, attr_name, default_val):
        ...
        val = (
            options_val
            if options_val is not None and options_val != ""
            else default_val
        )
        return val
...
class DBMSConfigFactoryLinux(DBMSConfigFactory):
    def __init__(self):
        ...
        self.DBMS_LIST = [
            ...
            DBMSDesc(
                self.DBMS_KEYS_LIST[6], STORAGE_TYPE_REMOTE, "BDB", "", createBDBConfig
            ),
        ]
    ...
    def select_dbms(self, options):
        try:
            ...
            db_name = (
                get_value_from_properties(
                    get_ambari_properties(), JDBC_DATABASE_PROPERTY, ""
                )
                .strip()
                .lower()
            )
    ...
    def get_default_dbms_name(self):
        ...
        default_dbms_name = (
            get_value_from_properties(properties, DEFAULT_DBMS_PROPERTY, "")
            .strip()
            .lower()
        )
        ...
        try:
            ...
            raise FatalException(
                ...
                "Invalid database selection: {0} {1}".format(
                    getattr(options, "persistence_type", ""),
                    getattr(options, "dbms", ""),
                ),
            )
...
def ensure_dbms_is_running(options, properties, scmStatus=None):
    ...
    if result == -1:
        raise FatalException(
            -1, "JDBC driver is not installed. Run ambari-server setup and try again."
        )
