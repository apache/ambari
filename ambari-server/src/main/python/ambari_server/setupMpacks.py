...
def _get_mpack_name_version(mpack_path):
  ...
  if not mpack_metadata:
    raise FatalException(
      -1, f"Malformed management pack {mpack_path}. Metadata file missing!"
    )
          ...
          for file in sorted(os.listdir(src_stack_version_dir)):
            if file == SERVICES_DIRNAME:
              ...
              for file in sorted(os.listdir(src_stack_services_dir)):
                create_symlink(
                  src_stack_services_dir, dest_stack_services_dir, file, options.force
                )
              ...
              create_symlink(
                src_stack_version_dir, dest_stack_version_dir, file, options.force
              )
...
def create_dashboard_symlinks(
  ...
  if os.path.exists(src_metrics_file):
    ...
    if os.path.exists(os.path.join(dest_metrics_dir, service_metrics_filename)):
      ...
      create_symlink(
        src_metrics_dir, dest_metrics_dir, service_metrics_filename, options.force
      )
...
def process_service_definitions_artifact(artifact, artifact_source_dir, options):
  ...
  for file in sorted(os.listdir(artifact_source_dir)):
    ...
    for file in sorted(os.listdir(src_service_definitions_dir)):
      create_symlink(
        src_service_definitions_dir, dest_service_definitions_dir, file, options.force
      )
        ...
        if (
          ...
          for applicable_stack in applicable_stacks:
            ...
            if os.path.exists(dest_stack_path) and os.path.exists(
              ...
              create_dashboard_symlinks(
                source_service_version_path, service_name, dashboard_location, options
              )
    ...
    for dir in staged_mpack_dirs:
      ...
      if os.path.isdir(staged_mpack_dir):
        ...
        if not staged_mpack_metadata:
          print_error_msg(
            f"Skipping malformed management pack in directory {staged_mpack_dir}."
          )
          continue
        ...
        if mpack_name == staged_mpack_name and (
          ...
          or compare_versions(staged_mpack_version, max_mpack_version, format=True) < 0
        ):
...
def _uninstall_mpack(mpack_name, mpack_version):
    ...
    for dir in staged_mpack_dirs:
      ...
      if os.path.isdir(staged_mpack_dir):
        ...
        if (
          ...
          and compare_versions(staged_mpack_version, mpack_version, format=True) == 0
        ):
          print_info_msg(
            f"Removing management pack staging location {staged_mpack_dir}"
          )
  ...
  if not found:
    ...
    print_info_msg(
      f"Management pack {mpack_name}-{mpack_version} successfully uninstalled!"
    )
...
def _install_mpack(options, replay_mode=False, is_upgrade=False):
  ...
  if os.path.exists(mpack_staging_dir):
    if options.force:
      print_info_msg(
        f"Force removing previously installed management pack from {mpack_staging_dir}"
      )
  ...
  print_info_msg(
    f"Management pack {mpack_name}-{mpack_version} successfully installed! Please restart ambari-server."
  )
  return (
    mpack_metadata,
    mpack_name,
...
def install_mpack(options, replay_mode=False):
  ...
  if replay_mode:
    options.force = True
  (mpack_metadata, mpack_name, mpack_version, mpack_staging_dir, mpack_archive_path) = (
