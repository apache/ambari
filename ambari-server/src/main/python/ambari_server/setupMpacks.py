...
from ambari_server.serverConfiguration import (
    ...
    get_ambari_version,
    get_stack_location,
    get_common_services_location,
    get_mpacks_staging_location,
...
from ambari_server.setupActions import (
    INSTALL_MPACK_ACTION,
    UNINSTALL_MPACK_ACTION,
...
from resource_management.libraries.functions.tar_archive import (
    untar_archive,
    get_archive_root_dir,
)
from resource_management.libraries.functions.version import compare_versions


MPACK_INSTALL_CHECKER_CMD = (
    "{0} -cp {1} "
...
def download_mpack(mpack_path):
    ...
    print_info_msg(
        "Download management pack to temp location {0}".format(tmp_archive_path)
    )
...
def expand_mpack(archive_path):
    ...
    if not os.path.exists(tmp_root_dir):
        ...
        raise FatalException(
            -1, "Malformed management pack. Failed to expand management pack!"
        )
    return tmp_root_dir
...
def _get_mpack_name_version(mpack_path):
    ...
    if not mpack_metadata:
        raise FatalException(
            -1,
            "Malformed management pack {0}. Metadata file missing!".format(mpack_path),
        )
...
def run_mpack_install_checker(options, mpack_stacks):
    ...
    command = MPACK_INSTALL_CHECKER_CMD.format(
        jdk_path, class_path, ",".join(mpack_stacks)
    )
...
def validate_purge(options, purge_list, mpack_dir, mpack_metadata, replay_mode=False):
    ...
    if STACK_DEFINITIONS_RESOURCE_NAME in purge_list:
        ...
        if not mpack_stacks:
            ...
            err = (
                "The management pack you are attempting to install does not contain {0}. Since this management pack "
    ...
    if not replay_mode:
        purge_resources = set(
            (v) for k, v in RESOURCE_FRIENDLY_NAMES.items() if k in purge_list
        )
        answer = "yes" if options.silent else "no"
        warn_msg = (
            "CAUTION: You have specified the --purge option with --purge-list={0}. "
...
def purge_stacks_and_mpacks(purge_list, replay_mode=False):
    ...
    if EXTENSION_DEFINITIONS_RESOURCE_NAME in purge_list and os.path.exists(
        extension_location
    ):
    ...
    if SERVICE_DEFINITIONS_RESOURCE_NAME in purge_list and os.path.exists(
        ...
        print_info_msg(
            "Purging service definitions location: " + service_definitions_location
        )
        sudo.rmtree(service_definitions_location)

    # Don't purge mpacks staging directory in replay mode
    if (
        MPACKS_RESOURCE_NAME in purge_list
        and not replay_mode
        and os.path.exists(mpacks_staging_location)
    ):
                ...
                if os.path.isfile(os.path.join(src_stack_dir, file)):
                    ...
                    for file in sorted(os.listdir(src_stack_version_dir)):
                        if file == SERVICES_DIRNAME:
                            src_stack_services_dir = os.path.join(
                                src_stack_version_dir, file
                            )
                            dest_stack_services_dir = os.path.join(
                                dest_stack_version_dir, file
                            )
                            ...
                            for file in sorted(os.listdir(src_stack_services_dir)):
                                create_symlink(
                                    src_stack_services_dir,
                                    dest_stack_services_dir,
                                    file,
                                    options.force,
                                )
                                src_service_dir = os.path.join(
                                    src_stack_services_dir, file
                                )
                                create_dashboard_symlinks(
                                    src_service_dir, file, dashboard_location, options
                                )
                        else:
                            create_symlink(
                                src_stack_version_dir,
                                dest_stack_version_dir,
                                file,
                                options.force,
                            )


def create_dashboard_symlinks(
    src_service_dir, service_name, dashboard_location, options
):
    """
    ...
    src_grafana_dashboards_dir = os.path.join(
        src_dashboards_dir, GRAFANA_DASHBOARDS_DIRNAME
    )
    src_metrics_dir = os.path.join(src_dashboards_dir, SERVICE_METRICS_DIRNAME)
    if os.path.exists(src_grafana_dashboards_dir):
        dest_grafana_dashboards_dir = os.path.join(
            dashboard_location, GRAFANA_DASHBOARDS_DIRNAME
        )
        dest_service_dashboards_link = os.path.join(
            dest_grafana_dashboards_dir, service_name
        )
        if os.path.exists(dest_service_dashboards_link):
            message = "Grafana dashboards already exist for service {0}.".format(
                service_name
            )
            print_warning_msg(message)
        else:
            create_symlink_using_path(
                src_grafana_dashboards_dir, dest_service_dashboards_link, options.force
            )
    ...
    if os.path.exists(src_metrics_file):
        ...
        if os.path.exists(os.path.join(dest_metrics_dir, service_metrics_filename)):
            message = "Service metrics already exist for service {0}.".format(
                service_name
            )
            print_warning_msg(message)
        else:
    ...
    for file in sorted(os.listdir(artifact_source_dir)):
        if os.path.isfile(os.path.join(artifact_source_dir, file)):
            ...
            for file in sorted(os.listdir(src_extension_dir)):
                create_symlink(
                    src_extension_dir, dest_extension_dir, file, options.force
                )
                src_extension_version_dir = os.path.join(src_extension_dir, file)
                src_services_dir = os.path.join(
                    src_extension_version_dir, SERVICES_DIRNAME
                )
...
def process_service_definitions_artifact(artifact, artifact_source_dir, options):
    ...
    for file in sorted(os.listdir(artifact_source_dir)):
        ...
        for file in sorted(os.listdir(src_service_definitions_dir)):
            create_symlink(
                src_service_definitions_dir,
                dest_service_definitions_dir,
                file,
                options.force,
            )
            src_service_version_dir = os.path.join(src_service_definitions_dir, file)
            create_dashboard_symlinks(
                src_service_version_dir, service_name, dashboard_location, options
            )
...
def process_stack_addon_service_definitions_artifact(
    ...
    if not service_versions_map:
        msg = (
            "Must provide service versions map for "
    ...
    for service_name in sorted(os.listdir(artifact_source_dir)):
        ...
        for service_version in sorted(os.listdir(source_service_path)):
            source_service_version_path = os.path.join(
                source_service_path, service_version
            )
            for service_version_entry in service_versions_map:
                if (
                    service_name == service_version_entry.service_name
                    and service_version == service_version_entry.service_version
                ):
        ...
        for dir in staged_mpack_dirs:
            ...
            if os.path.isdir(staged_mpack_dir):
                ...
                if not staged_mpack_metadata:
                    print_error_msg(
                        "Skipping malformed management pack in directory {0}.".format(
                            staged_mpack_dir
                        )
                    )
                    continue
                ...
                if mpack_name == staged_mpack_name and (
                    not max_mpack_version
                    or compare_versions(
                        staged_mpack_version, max_mpack_version, format=True
                    )
                    < 0
                ):
...
def _uninstall_mpack(mpack_name, mpack_version):
    ...
    """
    print_info_msg(
        "Uninstalling management pack {0}-{1}".format(mpack_name, mpack_version)
    )
    # Get ambari mpack properties
    (
        stack_location,
    ...
    if os.path.exists(mpacks_staging_location) and os.path.isdir(
        ...
        for dir in staged_mpack_dirs:
            ...
            if os.path.isdir(staged_mpack_dir):
                ...
                if not staged_mpack_metadata:
                    print_error_msg(
                        "Skipping malformed management pack {0}-{1}. Metadata file missing!".format(
                ...
                staged_mpack_version = staged_mpack_metadata.version
                if (
                    mpack_name == staged_mpack_name
                    and compare_versions(
                        staged_mpack_version, mpack_version, format=True
                    )
                    == 0
                ):
                    print_info_msg(
    ...
    if not found:
        print_error_msg(
            "Management pack {0}-{1} is not installed!".format(
...
def validate_mpack_prerequisites(mpack_metadata):
    ...
    if "min_ambari_version" in mpack_prerequisites:
        ...
        if (
            compare_versions(min_ambari_version, current_ambari_version, format=True)
            > 0
        ):
            print_error_msg(
                "Prerequisite failure! Current Ambari Version = {0}, "
                "Min Ambari Version = {1}".format(
                    current_ambari_version, min_ambari_version
                )
            )
            fail = True
    if "max_ambari_version" in mpack_prerequisites:
        ...
        if (
            compare_versions(max_ambari_version, current_ambari_version, format=True)
            < 0
        ):
            print_error_msg(
                "Prerequisite failure! Current Ambari Version = {0}, "
                "Max Ambari Version = {1}".format(
                    current_ambari_version, max_ambari_version
                )
            )
    ...
    if fail:
        raise FatalException(
            -1,
            "Prerequisites for management pack {0}-{1} failed!".format(
...
def _install_mpack(options, replay_mode=False, is_upgrade=False):
    ...
    if not is_upgrade and options.purge and options.purge_list:
        ...
        validate_purge(
            options, purge_resources, tmp_root_dir, mpack_metadata, replay_mode
        )
    ...
    if not os.path.exists(dashboard_location):
        ...
        sudo.makedir(
            os.path.join(dashboard_location, GRAFANA_DASHBOARDS_DIRNAME), 0o755
        )
    ...
    mpack_archive_path = os.path.join(
        mpacks_cache_location, os.path.basename(tmp_archive_path)
    )
    ...
    if os.path.exists(mpack_staging_dir):
        if options.force:
            print_info_msg(
                "Force removing previously installed management pack from {0}".format(
                    mpack_staging_dir
                )
            )
    ...
    for artifact in mpack_metadata.artifacts:
        ...
        print_info_msg(
            "Processing artifact {0} of type {1} in {2}".format(
        ...
        if artifact.type == STACK_DEFINITIONS_ARTIFACT_NAME:
            ...
            process_extension_definitions_artifact(
                artifact, artifact_source_dir, options
            )
            ...
            print_info_msg(
                "Unknown artifact {0} of type {1}".format(artifact_name, artifact_type)
            )
    ...
    if ambari_user:
        ...
        for pack in adjust_ownership_list:
            ...
            recursive = pack[3]
            logger.info(
                "Setting file permissions: {0} {1} {2} {3}".format(
                    file, mod, user, recursive
                )
            )
    ...
    print_info_msg(
        "Management pack {0}-{1} successfully installed! Please restart ambari-server.".format(
            mpack_name, mpack_version
        )
    )
    return (
        ...
        for hook in hooks:
            if hook_name == hook.name:
                ...
                if os.path.exists(hook_script):
                    print_info_msg(
                        "Executing {0} hook script : {1}".format(hook_name, hook_script)
                    )
                    if hook.type == PYTHON_HOOK_TYPE:
                        ...
                        raise FatalException(
                            -1,
                            "Malformed management pack. Unknown hook type for {0} hook script".format(
                                hook_name
                            ),
                        )
                    (returncode, stdoutdata, stderrdata) = run_os_command(command)
                    if returncode != 0:
                        msg = "Failed to execute {0} hook. Failed with error code {0}".format(
                            hook_name, returncode
                        )
                        print_error_msg(msg)
                    ...
                    raise FatalException(
                        -1,
                        "Malformed management pack. Missing {0} hook script {1}".format(
...
def add_replay_log(
    mpack_command, mpack_archive_path, purge, purge_list, force, verbose
):
    """
    ...
    log = {
        "mpack_command": mpack_command,
        "mpack_path": mpack_archive_path,
        "purge": purge,
...
def upgrade_mpack(options, replay_mode=False):
    ...
    (
        ...
        mpack_staging_dir,
        mpack_archive_path,
    ) = _install_mpack(options, replay_mode, is_upgrade=True)
    ...
    print_info_msg(
        "Management pack {0}-{1} successfully upgraded!".format(
            mpack_name, mpack_version
        )
    )
    if not replay_mode:
        ...
        with open(replay_log_file, "r") as f:
            for replay_log in f:
                ...
                if "purge_list" not in replay_options:
                    replay_options.purge_list = ",".join(
                ...
                if replay_options.mpack_command == INSTALL_MPACK_ACTION:
                    ...
                    error_msg = (
                        "Invalid mpack command {0} in mpack replay log {1}!".format(
                            replay_options.mpack_command, replay_log_file
                        )
                    )
                    print_error_msg(error_msg)
                    raise FatalException(-1, error_msg)
