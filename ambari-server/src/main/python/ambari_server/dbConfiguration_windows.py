...
from ambari_commons.str_utils import (
    cbool,
...
class MSSQLConfig(DBMSConfig):
    ...
    def _prompt_db_properties(self):
        if self.must_set_database_options:
            ...
            hostname_prompt = (
                "SQL Server host and instance for the {0} database: ({1}) ".format(
            ...
            if str(auth_option) == "1":
                ...
                user_prompt = (
                    "SQL Server user name for the {0} database: ({1}) ".format(
    ...
    def _install_jdbc_driver(self, properties, files_list):
        driver_path = get_value_from_properties(
            properties, JDBC_DRIVER_PATH_PROPERTY, None
        )
        if driver_path is None or driver_path == "":
        ...
        if (
            ...
            if 0 != ret:
                raise FatalException(
                    -1, "Error starting SQL Server Browser: " + string(ret)
                )
        pass
    ...
    def _build_sql_server_connection_string(self):
        databaseUrl = "jdbc:sqlserver://{0}".format(
            ensure_double_backslashes(self.database_host)
        )
        if self.database_port is not None and self.database_port != "":
    ...
    def _execute_db_script(databaseHost, databaseScript, minReportedSeverityLevel=10):
        dbCmd = "sqlcmd -S {0} -b -V {1} -i {2}".format(
            databaseHost, minReportedSeverityLevel, databaseScript
        )
        retCode, outData, errData = run_os_command(["cmd", "/C", dbCmd])
        if not retCode == 0:
            err = "Running database create script failed. Error output: {0} Output: {1} Exiting.".format(
                errData, outData
            )
