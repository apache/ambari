...
def load_stack_values(version, filename):
    ...
    for ostag in root:
        ...
        for repotag in ostag:
            ...
            if (
                reponametag is not None
                and repoidtag is not None
                and baseurltag is not None
            ):
...
def change_objects_owner(args):
    ...
    properties = (
        Properties()
    )  # Dummy, args contains the dbms name and parameters already
...
def run_schema_upgrade(args):
    ...
    print_info_msg(
        "Return code from schema upgrade command, retcode = {0}".format(str(retcode)),
        True,
    )
    if stdout:
...
def check_gpl_license_approved(upgrade_response):
    ...
    while not write_gpl_license_accepted(
        text=LZO_ENABLED_GPL_TEXT
    ) and not get_YN_input(INSTALLED_LZO_WITHOUT_GPL_TEXT, False):
        pass
...
def upgrade(args):
    ...
    print_info_msg(
        "Updating Ambari Server properties in {0} ...".format(AMBARI_PROPERTIES_FILE),
        True,
    )
    ...
    print_info_msg(
        "Updating Ambari Server properties in {0} ...".format(AMBARI_ENV_FILE), True
    )
    ...
    if not retcode == 0:
        print_error_msg(
            "Ambari server upgrade failed. Please look at {0}, for more details.".format(
    ...
    for db_name in CUSTOM_JDBC_DB_NAMES:
        ...
        if os.path.lexists(os.path.join(resources_dir, symlink_name)):
            ...
            properties.process_pair(
                ...
                default_connectors_map[db_name],
            )
...
def set_current(options):
    ...
    sys.stdout.write(
        "\nCurrent version successfully updated to "
        + finalize_options.desired_repo_version
    )
...
def find_and_copy_custom_services(
    ...
    managed_services.extend(
        ["NAGIOS", "GANGLIA", "MAPREDUCE", "WEBHCAT", "AMBARI_INFRA"]
    )
    ...
    if stack_backup_dirs:
        ...
        for backup_service in backup_services:
            ...
            current_base_service_dir = regex.sub(
                base_service_dir, backup_base_service_dir
            )
