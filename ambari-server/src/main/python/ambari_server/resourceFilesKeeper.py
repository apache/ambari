...
class ResourceFilesKeeper:
    ...
    def _update_resources_subdir_archive(self, subdir):
        ...
        self.dbg_out(
            "Updating archive for {0} dir at {1}...".format(subdir, archive_root)
        )
    ...
    def update_directory_archives(self):
        ...
        # archive stacks
        self.dbg_out(
            "Updating archives for stack dirs at {0}...".format(self.stacks_root)
        )
        ...
        common_services_root = os.path.join(
            self.resources_dir, self.COMMON_SERVICES_DIR
        )
        self.dbg_out(
            "Updating archives for common services dirs at {0}...".format(
                common_services_root
            )
        )
        ...
        self.dbg_out(
            "Updating archives for extensions dirs at {0}...".format(extensions_root)
        )
    ...
    def update_directory_archive(self, directory):
        ...
        if cur_hash != saved_hash:
            ...
            if skip_empty_directory and (
                ...
                self.dbg_out(
                    "Empty directory. Skipping generation of hash file for {0}".format(
                        directory
                    )
                )
            else:
    ...
    def count_hash_sum(self, directory):
        ...
        try:
            ...
            raise KeeperException(
                "Can not calculate directory " "hash: {0}".format(str(err))
            )
    ...
    def read_hash_sum(self, directory):
        ...
        if os.path.isfile(hash_file):
            try:
                ...
                raise KeeperException(
                    "Can not read file {0} : {1}".format(hash_file, str(err))
                )
        else:
            return None
        ...
        try:
            if skip_if_empty:
                if not os.path.exists(directory) or not os.listdir(directory):
                    self.dbg_out(
                        "Empty directory. Skipping archive creation for {0}".format(
                            directory
                        )
                    )
                    return
            ...
            for root, dirs, files in os.walk(directory):
                for filename in files:
                    ...
                    if not self.is_ignored(filename):
                        ...
                        self.dbg_out(
                            "zipping %s as %s" % (os.path.join(root, filename), arcname)
                        )
