...
def get_ambari_admin_username_password_pair(options):
    ...
    admin_login = (
        ...
        and options.ambari_admin_username is not None
        else get_validated_string_input(
            "Enter Ambari Admin login: ", None, None, None, False, False
        )
    )
    admin_password = (
        options.ambari_admin_password
        if hasattr(options, "ambari_admin_password")
        and options.ambari_admin_password is not None
        else get_validated_string_input(
            "Enter Ambari Admin password: ", None, None, None, True, False
        )
    )
...
def get_json_via_rest_api(properties, admin_login, admin_password, entry_point):
    ...
    admin_auth = (
        base64.encodebytes(("%s:%s" % (admin_login, admin_password)).encode())
        .decode()
        .replace("\n", "")
    )
...
def perform_changes_via_rest_api(
    ...
    with closing(
        ...
        if response_status_code not in (200, 201):
            err = (
                "Error while performing changes via Ambari REST API. Http status code - "
...
def get_eligible_services(
    ...
    if json_data and "items" in json_data:
        ...
        if len(services) > 0:
            print_info_msg(
                "Found %s enabled services: %s"
                % (service_qualifier, ", ".join(services))
            )
        else:
