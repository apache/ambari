...
class LinuxDBMSConfig(DBMSConfig):
    def __init__(self, options, properties, storage_type):
        ...
        self.jdbc_connection_pool_max_idle_time = (
            DBMSConfig._init_member_with_prop_default(
                options,
                "jdbc_connection_pool_max_idle_time",
                properties,
    ...
    def _reset_remote_database(self):
        client_usage_cmd_drop = self._get_remote_script_line(
            self.drop_tables_script_file
        )
    ...
    def _install_jdbc_driver(self, properties, files_list):
        if type(files_list) is not int:
            ...
            if db_default_driver_path in files_list:
                os.symlink(
                    os.path.join(resources_dir, self.driver_file_name), jdbc_symlink
                )
        else:
            if files_list == -1:
                return False
    ...
    @staticmethod
    def _configure_database_password(
        showDefault=True, defaultPassword=DEFAULT_PASSWORD
    ):
    ...
    def _find_jdbc_driver(jdbc_pattern):
        drivers = []
        drivers.extend(
            glob.glob(configDefaults.JAVA_SHARE_PATH + os.sep + jdbc_pattern)
        )
        if drivers:
            return drivers
        return -1
    ...
    def _store_remote_properties(self, properties, options):
        ...
        self._store_password_property(
            properties, JDBC_RCA_PASSWORD_FILE_PROPERTY, options
        )
    ...
    def _store_connection_pool_properties(self, properties):
        properties.process_pair(
            JDBC_CONNECTION_POOL_TYPE, self.jdbc_connection_pool_type
        )
...
class PGConfig(LinuxDBMSConfig):
    ...
    if (
        ...
        PG_START_CMD = AMBARI_SUDO_BINARY + " %s start %s" % (
            SERVICE_CMD,
            PG_SERVICE_NAME,
        )
        PG_RESTART_CMD = AMBARI_SUDO_BINARY + " %s restart %s" % (
            SERVICE_CMD,
            PG_SERVICE_NAME,
        )
        PG_HBA_RELOAD_CMD = AMBARI_SUDO_BINARY + " %s reload %s" % (
            SERVICE_CMD,
            PG_SERVICE_NAME,
        )
    else:
            ...
            if OSCheck.is_suse_family() and not is_service_exist(PG_SERVICE_NAME):
                ...
                if versioned_script_paths:
                    versioned_script_path_tps = [
                        (
                            re.search(r"postgresql-([0-9]+\.?[0-9]*)", path).group(1),
                            path,
                        )
                        for path in versioned_script_paths
                    ]
                    versioned_script_path_tps.sort(
                        key=lambda t: float(t[0]), reverse=True
                    )
                    for versioned_script_path_tp in versioned_script_path_tps:
                        pgsql_service_file_name = (
                            "postgresql-%s" % versioned_script_path_tp[0]
                        )
                        if is_service_exist(pgsql_service_file_name):
                            ...
                            PG_HBA_DIR = (
                                "/var/lib/pgsql/%s/data" % versioned_script_path_tp[0]
                            )
                            break

        PG_ST_CMD = "%s %s status" % (SERVICE_CMD, PG_SERVICE_NAME)

        PG_START_CMD = AMBARI_SUDO_BINARY + " %s %s start" % (
            SERVICE_CMD,
            PG_SERVICE_NAME,
        )
        PG_RESTART_CMD = AMBARI_SUDO_BINARY + " %s %s restart" % (
            SERVICE_CMD,
            PG_SERVICE_NAME,
        )
        PG_HBA_RELOAD_CMD = AMBARI_SUDO_BINARY + " %s %s reload" % (
            SERVICE_CMD,
            PG_SERVICE_NAME,
        )
    ...
    def ensure_dbms_is_running(self, options, properties, scmStatus=None):
        if self._is_local_database():
            if is_root():
                ...
                if not retcode == 0:
                    err = "Unable to start PostgreSQL server. Status {0}. {1}. Exiting".format(
                        pg_status, err
                    )
                    raise FatalException(retcode, err)
            else:
    ...
    def _reset_local_database(self):
        ...
        okToRun = get_YN_input(
            "Confirm server reset [yes/no]({0})? ".format(default), get_silent()
        )
        if not okToRun:
    ...
    def _is_jdbc_user_changed(database_username):
        ...
        previos_user = get_value_from_properties(
            properties, JDBC_USER_NAME_PROPERTY, ""
        )
    ...
    def _check_postgre_up():
        ...
        if pg_status == PGConfig.PG_STATUS_RUNNING:
            ...
            try:
                ...
                print_info_msg(
                    "Waiting for postgres to start at port {0}...".format(PG_PORT)
                )
                wait_for_port_opened(
                    "127.0.0.1",
                    PG_PORT,
                    PG_PORT_CHECK_TRIES_COUNT,
    ...
    def _configure_postgresql_conf():
        ...
        postgresql_conf_file_out = re.sub(
            "#+listen_addresses.*?(#|$)",
            listenAddress,
            postgresql_conf_file_in.decode(),
        )
        ...
        for tbl in tables:
            ...
            if owner != self.database_username:
                ...
                if retcode != 0 or "ALTER TABLE" not in stdout:
                    print_error_msg(
                        "Failed to change owner of table:{0} to user:{1}".format(
                            tbl, owner
                        )
                    )
                    return False
...
class SQLAConfig(LinuxDBMSConfig):
    ...
    def __init__(self, options, properties, storage_type):
        ...
        self.database_url_pattern_alt = (
            "jdbc:sqlanywhere:eng={0};dbf={1};host={2};port={3}"
        )
    ...
    def _extract_client_tarball(self, properties):
        ...
        cmd = SQLAConfig.EXTRACT_CMD.format(
            files[0], get_resources_location(properties)
        )
