...
SETUP_LDAP_CONFIG_URL = (
    "services/AMBARI/components/AMBARI_SERVER/configurations/ldap-configuration"
)
...
LDAP_DISABLE_ENDPOINT_IDENTIFICATION = (
    "ambari.ldap.advanced.disable_endpoint_identification"
)
...
def adjust_directory_permissions(ambari_user):
    ...
    print_info_msg(
        "Cleaning bootstrap directory ({0}) contents...".format(bootstrap_dir)
    )
    ...
    if not os.path.exists(bootstrap_dir):
        try:
            ...
            print_warning_msg(
                "Failed recreating the bootstrap directory: {0}".format(str(ex))
            )
            pass
    else:
        print_warning_msg(
    ...
    if os.path.exists(credStoreFile):
        configDefaults.NR_ADJUST_OWNERSHIP_LIST.append(
            (
                credStoreFile,
                configDefaults.CREDENTIALS_STORE_FILE_PERMISSIONS,
                "{0}",
                False,
            )
        )
    ...
    if ambari_repo_file:
        if os.path.exists(ambari_repo_file):
            ...
            configDefaults.NR_ADJUST_OWNERSHIP_LIST.append(
                (ambari_repo_file, "644", ambari_repo_file_owner, False)
            )
...
def get_ldap_properties_from_db(properties, admin_login, admin_password):
    ...
    admin_auth = (
        base64.b64encode("{}:{}".format(admin_login, admin_password).encode())
        .decode()
        .replace("\n", "")
    )
        ...
        try:
            with closing(
                ...
                if response_status_code != 200:
                    ...
                    err = (
                        "Error while fetching LDAP configuration. Http status code - "
...
def is_ldap_enabled(properties, admin_login, admin_password):
    ldap_enabled = get_ldap_property_from_db(
        properties, admin_login, admin_password, AMBARI_LDAP_AUTH_ENABLED
    )
...
def sync_ldap(options):
    ...
    if ldap_sync_options.ldap_sync_all:
        ...
        if ldap_sync_options.ldap_sync_users is not None:
            new_specs = [
                {"principal_type": "users", "sync_type": "specific", "names": ""}
            ]
            get_ldap_event_spec_names(
                ldap_sync_options.ldap_sync_users, specs, new_specs
            )
        if ldap_sync_options.ldap_sync_groups is not None:
            new_specs = [
                {"principal_type": "groups", "sync_type": "specific", "names": ""}
            ]
            get_ldap_event_spec_names(
                ldap_sync_options.ldap_sync_groups, specs, new_specs
            )
    ...
    while request_in_progress:
        ...
        try:
            response = urllib.request.urlopen(
                request, context=get_ssl_context(properties)
            )
        except Exception as e:
            request_in_progress = False
...
def setup_sensitive_data_encryption(options):
    if not is_root():
        warn = (
            "ambari-server encrypt-passwords is run as "
    ...
    db_sql_auth = (
        False
        if db_windows_auth_prop and db_windows_auth_prop.lower() == "true"
        else True
    )
    ...
    if isSecure:
        ...
        decrypt = get_YN_input(
            "Do you want to decrypt passwords managed by Ambari? [y/n] (n): ", False
        )
        if not decrypt:
            resetKey = get_YN_input(
                "Do you want to reset Master Key? [y/n] (n): ", False
            )
...
def encrypt_sensitive_data(
    ...
    if db_password and not is_alias_string(db_password):
        ...
        if retCode != 0:
            ...
            propertyMap[JDBC_PASSWORD_PROPERTY] = get_alias_string(
                JDBC_RCA_PASSWORD_ALIAS
            )
...
def setup_master_key(masterKeyFile, options, properties, resetKey):
    ...
    if persist:
        ...
        try:
            ...
            print_info_msg(
                "Deleting master key file at location: " + str(masterKeyFile)
            )
        except Exception as e:
...
def printManualDecryptionWarning(db_password, db_sql_auth, ts_password):
    ...
    print(
        "- Edit "
        + os.path.join(get_conf_dir(), AMBARI_PROPERTIES_FILE)
        + " and set "
        + SECURITY_IS_ENCRYPTION_ENABLED
        + " = "
        + "false."
...
def setup_ambari_krb5_jaas(options):
    ...
    if os.path.exists(jaas_conf_file):
        print(
            "Setting up Ambari kerberos JAAS configuration to access "
        ...
        raise NonFatalException(
            "No jaas config file found at location: " + jaas_conf_file
        )
...
class LdapPropTemplate:
    def __init__(
        ...
        self.default_value = (
            LdapDefault(stored_value) if stored_value else i_prop_default
        )
    ...
    def get_default_value(self, ldap_type):
        return (
            self.default_value.get_default_value(ldap_type)
...
def init_ldap_properties_list_reqd(properties, options):
    ...
    ldap_properties = [
        LdapPropTemplate(
            ...
            LdapDefaultMap(
                {
                    LDAP_IPA: "ipa.ambari.apache.org",
                    LDAP_GENERIC: "ldap.ambari.apache.org",
                }
            ),
        ),
        LdapPropTemplate(
        ...
        LdapPropTemplate(
            ...
            False,
            LdapDefaultMap(
                {LDAP_AD: "sAMAccountName", LDAP_IPA: "uid", LDAP_GENERIC: "uid"}
            ),
        ),
        LdapPropTemplate(
            properties,
...
def update_ldap_configuration(
    ...
    request_data = {
        "Configuration": {"category": "ldap-configuration", "properties": {}}
    }
...
def query_ldap_type(ldap_type_option):
    return get_validated_string_input(
        "Please select the type of LDAP you want to use [{}]({}):".format(
            "/".join(LDAP_TYPES), LDAP_GENERIC
        ),
        LDAP_GENERIC,
        REGEX_LDAP_TYPE,
        ...
        if ldap_prop.prop_name == LDAP_ANONYMOUS_BIND:
            ...
            if ldaps:
                disable_endpoint_identification = (
                    get_validated_string_input(
                        "Disable endpoint identification during SSL handshake [true/false] ({0}): ".format(
                ...
                if custom_trust_store:
                    ts_type = (
                        get_validated_string_input(
                            "TrustStore type [jks/jceks/pkcs12] {0}:".format(
                                get_prompt_default(ssl_truststore_type_default)
                            ),
                    ...
                    while True:
                        ...
                        if os.path.exists(ts_path):
                            ...
                            hasAnswer = (
                                options.trust_store_path is not None
                                and options.trust_store_path
                            )
    ...
    for property in ldap_property_list_reqd:
        if property.prop_name in ldap_property_value_map:
            print(
                "%s %s"
                % (
                    property.get_prompt_text(ldap_type),
    ...
    for property in ldap_property_list_opt:
        if property in ldap_property_values_in_ambari_properties:
            if property not in ldap_property_list_passwords:
                print(
                    "%s: %s"
                    % (property, ldap_property_values_in_ambari_properties[property])
                )
            else:
                print("%s: %s" % (property, BLIND_PASSWORD))
...
def ensure_can_start_under_current_user(ambari_user):
    ...
    if ambari_user is None:
        err = (
            "Unable to detect a system user for Ambari Server.\n" + SETUP_OR_UPGRADE_MSG
        )
        raise FatalException(1, err)
    if current_user != ambari_user and not is_root():
        err = (
            'Unable to start Ambari Server as user {0}. Please either run "ambari-server start" '
            'command as root, as sudo or as user "{1}"'.format(
                current_user, ambari_user
            )
        )
        raise FatalException(1, err)
    return current_user
...
def setup_pam(options):
    ...
    if get_value_from_properties(properties, CLIENT_SECURITY, "") == "ldap":
        query = (
            "LDAP is currently configured, do you wish to use PAM instead [y/n] (n)? "
        )
...
def migrate_ldap_pam(args):
    ...
    confirm = get_YN_input(
        ...
        "you have made a backup of the Ambari Server database [y/n] (y)? " % db_title,
        True,
    )
...
def populate_ambari_requires_ldap(options, properties):
    if options.ldap_enabled_ambari is None:
        enabled = get_boolean_from_dictionary(
            properties, AMBARI_LDAP_AUTH_ENABLED, False
        )
        enabled = get_YN_input(
            "Use LDAP authentication for Ambari [y/n] ({0})? ".format(
                "y" if enabled else "n"
            ),
            enabled,
        )
    ...
    if options.ldap_enabled_services is None:
        if options.ldap_manage_services is None:
            ...
            manage_services = get_YN_input(
                "Manage LDAP configurations for eligible services [y/n] ({0})? ".format(
            ...
            print(
                "Manage LDAP configurations for eligible services [y/n] ({0})? {1}".format(
        ...
        if manage_services:
            ...
            configure_for_all_services = get_YN_input(
                " Manage LDAP for all services [y/n] ({0})? ".format(
                    "y" if all else "n"
                ),
                all,
            )
                ...
                if cluster_name:
                    ...
                    if eligible_services and len(eligible_services) > 0:
                        ...
                        for service in eligible_services:
                            ...
                            question = "   Manage LDAP for {0} [y/n] ({1})? ".format(
        ...
        services = (
            options.ldap_enabled_services.upper()
            if options.ldap_enabled_services
            else ""
        )
