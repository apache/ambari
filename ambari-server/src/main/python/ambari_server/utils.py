            ...
            try:
                ...
                if pattern in arg:
                    found_pids += [
                        {
                            "pid": pid,
                            "exe": get_symlink_path(
                                os.path.join(PROC_DIR, pid, PROC_EXEC)
                            ),
...
def get_postgre_hba_dir(OS_FAMILY):
    ...
    if OSCheck.is_ubuntu_family():
        ...
        if not os.path.isfile(get_pg_hba_init_files()):
            ...
            os.symlink(
                glob.glob(get_pg_hba_init_files() + "*")[0], get_pg_hba_init_files()
            )
...
def update_latest_in_repoinfos_for_stacks(
    ...
    for stack_name in os.walk(stacks_root).next()[1]:
        if predicate(stack_name):
            for stack_version in os.walk(os.path.join(stacks_root, stack_name)).next()[
                1
            ]:
