    ...
    def perform_backup(self):
        ...
        try:
            ...
            zipf = zipfile.ZipFile(
                self.zip_folder_path + self.zipname, "w", allowZip64=True
            )
...
def validate_folders(folders):
    ...
    for folder in folders:
        if not os.path.isdir(folder):
            sys.exit(
                "Error while validating folders. Folder " + folder + " does not exist."
            )
...
def retrieve_path_and_zipname(archive_absolute_path):
    ...
    try:
        ...
        if elements is not None and len(elements) > 0:
            ...
            target["path"] = archive_absolute_path.replace(
...
def main(argv=None):
    ...
    backup_restore = BackupRestore(
        AMBARI_FILESYSTEM_STATE, ambari_backup_zip_filename, zip_file_path
    )
