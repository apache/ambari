...
def validate_options(options):
    ...
    if options.sso_enabled == "true":
        ...
        if options.sso_provider_url and not re.match(
            REGEX_URL, options.sso_provider_url
        ):
            errors.append("Invalid --sso-provider-url")
        if options.sso_enabled_ambari and not re.match(
            REGEX_TRUE_FALSE, options.sso_enabled_ambari
        ):
            errors.append("--sso-enabled-ambari should be to either 'true' or 'false'")
        if options.sso_manage_services and not re.match(
            REGEX_TRUE_FALSE, options.sso_manage_services
        ):
...
def populate_sso_provider_url(options, properties):
    if not options.sso_provider_url:
        provider_url = get_value_from_dictionary(
            properties, SSO_PROVIDER_URL, SSO_PROVIDER_URL_DEFAULT
        )
        provider_url = get_validated_string_input(
            "Provider URL ({0}): ".format(provider_url),
            provider_url,
            REGEX_URL,
...
def populate_sso_public_cert(options, properties):
    if not options.sso_public_cert_file:
        ...
        if get_cert:
            ...
            properties[SSO_CERTIFICATE] = (
                ensure_complete_cert(cert_string) if cert_string else ""
            )
    else:
...
def populate_jwt_cookie_name(options, properties):
    if not options.sso_jwt_cookie_name and (
        not options.sso_provider_url or not options.sso_public_cert_file
    ):
        cookie_name = get_value_from_dictionary(
            properties, JWT_COOKIE_NAME, JWT_COOKIE_NAME_DEFAULT
        )
        cookie_name = get_validated_string_input(
            "JWT Cookie name ({0}): ".format(cookie_name),
            cookie_name,
            REGEX_ANYTHING,
            "Invalid cookie name",
            False,
        ...
        cookie_name = (
            options.sso_jwt_cookie_name
            if options.sso_jwt_cookie_name
            else JWT_COOKIE_NAME_DEFAULT
        )
...
def populate_jwt_audiences(options, properties):
    if options.sso_jwt_audience_list is None and (
        not options.sso_provider_url or not options.sso_public_cert_file
    ):
        audiences = get_value_from_dictionary(
            properties, JWT_AUDIENCES, JWT_AUDIENCES_DEFAULT
        )
        audiences = get_validated_string_input(
            "JWT audiences list (comma-separated), empty for any ({0}): ".format(
                audiences
            ),
            audiences,
            REGEX_ANYTHING,
        ...
        audiences = (
            options.sso_jwt_audience_list
            if options.sso_jwt_audience_list
            else JWT_AUDIENCES_DEFAULT
        )
...
def populate_ambari_requires_sso(options, properties):
    if options.sso_enabled_ambari is None:
        enabled = get_boolean_from_dictionary(
            properties, AMBARI_SSO_AUTH_ENABLED, False
        )
        enabled = get_YN_input(
            "Use SSO for Ambari [y/n] ({0})? ".format("y" if enabled else "n"), enabled
        )
    else:
...
def populate_service_management(
    options, properties, ambari_properties, admin_login, admin_password
):
    if not options.sso_enabled_services:
        if not options.sso_manage_services:
            manage_services = get_boolean_from_dictionary(
                properties, SSO_MANAGE_SERVICES, False
            )
            manage_services = get_YN_input(
                "Manage SSO configurations for eligible services [y/n] ({0})? ".format(
                    "y" if manage_services else "n"
                ),
                manage_services,
            )
        else:
            ...
            if not options.sso_provider_url:
                stored_manage_services = get_boolean_from_dictionary(
                    properties, SSO_MANAGE_SERVICES, False
                )
                print(
                    "Manage SSO configurations for eligible services [y/n] ({0})? {1}".format(
                        "y" if stored_manage_services else "n",
                        "y" if manage_services else "n",
                    )
                )
        ...
        if manage_services:
            enabled_services = (
                get_value_from_dictionary(properties, SSO_ENABLED_SERVICES, "")
                .upper()
                .split(",")
            )
            ...
            configure_for_all_services = get_YN_input(
                " Use SSO for all services [y/n] ({0})? ".format("y" if all else "n"),
                all,
            )
            if configure_for_all_services:
                ...
                if cluster_name:
                    eligible_services = get_eligible_services(
                        ...
                        admin_password,
                        cluster_name,
                        FETCH_SERVICES_FOR_SSO_ENTRYPOINT,
                        "SSO",
                    )
                    ...
                    if eligible_services and len(eligible_services) > 0:
                        ...
                        for service in eligible_services:
                            ...
                            question = "   Use SSO for {0} [y/n] ({1})? ".format(
                                service, "y" if enabled else "n"
                            )
        ...
        services = (
            options.sso_enabled_services.upper() if options.sso_enabled_services else ""
        )
...
def get_sso_properties(properties, admin_login, admin_password):
    ...
    try:
        response_code, json_data = get_json_via_rest_api(
            properties, admin_login, admin_password, SSO_CONFIG_API_ENTRYPOINT
        )
    ...
    if (
        json_data
        and "Configuration" in json_data
        and "properties" in json_data["Configuration"]
    ):
...
def update_sso_conf(
    ambari_properties, sso_configuration_properties, admin_login, admin_password
):
    request_data = {
        "Configuration": {"category": "sso-configuration", "properties": {}}
    }
        ...
        if not options.sso_enabled:
            ...
            if sso_status == "enabled":
                enable_sso = not get_YN_input(
                    "Do you want to disable SSO authentication [y/n] (n)? ", False
                )
            elif get_YN_input(
                "Do you want to configure SSO authentication [y/n] (y)? ", True
            ):
                enable_sso = True
            else:
                return False
