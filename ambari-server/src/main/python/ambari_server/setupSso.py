...
FETCH_SERVICES_FOR_SSO_ENTRYPOINT = (
  "clusters/%s/services?ServiceInfo/sso_integration_supported=true&fields=ServiceInfo/*"
)
...
def populate_jwt_audiences(options, properties):
  if options.sso_jwt_audience_list is None and (
    ...
    audiences = get_validated_string_input(
      f"JWT audiences list (comma-separated), empty for any ({audiences}): ",
      audiences,
      REGEX_ANYTHING,
      "Invalid value",
      False,
...
def populate_ambari_requires_sso(options, properties):
  if options.sso_enabled_ambari is None:
    ...
    enabled = get_YN_input(
      f"Use SSO for Ambari [y/n] ({'y' if enabled else 'n'})? ", enabled
    )
  else:
  ...
  if not options.sso_enabled_services:
    if not options.sso_manage_services:
      ...
      manage_services = get_YN_input(
        f"Manage SSO configurations for eligible services [y/n] ({'y' if manage_services else 'n'})? ",
        manage_services,
      )
    else:
      ...
      if not options.sso_provider_url:
        ...
        print(
          "Manage SSO configurations for eligible services [y/n] ({0})? {1}".format(
            "y" if stored_manage_services else "n", "y" if manage_services else "n"
          )
        )
    ...
    if manage_services:
      ...
      configure_for_all_services = get_YN_input(
        f" Use SSO for all services [y/n] ({'y' if all else 'n'})? ", all
      )
...
def remove_sso_conf(ambari_properties, admin_login, admin_password):
  perform_changes_via_rest_api(
    ambari_properties, admin_login, admin_password, SSO_CONFIG_API_ENTRYPOINT, "DELETE"
  )
...
def update_sso_conf(
  ...
  request_data = {"Configuration": {"category": "sso-configuration", "properties": {}}}
