    ...
    def _create_custom_user(self):
        ...
        if get_silent():
            ...
            password = get_validated_string_input(
                "Enter password for user {0}:".format(user),
                "",
                None,
                "Password",
                True,
        ...
        if status == UserHelper.ACTION_FAILED:
            print_warning_msg(
                "Can't add SeServiceLogonRight to user {0}. Failed with message {1}".format(
                    user, message
                )
            )
            return UserHelper.ACTION_FAILED
        ...
        if status == UserHelper.ACTION_FAILED:
            print_warning_msg(
                "Can't add SeBatchLogonRight to user {0}. Failed with message {1}".format(
                    user, message
                )
            )
            return UserHelper.ACTION_FAILED
...
class AmbariUserChecksLinux(AmbariUserChecks):
    ...
    def _create_custom_user(self):
        ...
        if retcode == 9:  # 9 = username already in use
            print_info_msg(
                "User {0} already exists, " "skipping user creation".format(user)
            )
...
class JDKSetup(object):
    ...
    def download_and_install_jdk(self, args, properties, ambariOnly=False):
        ...
        if args.java_home:
            ...
            if not validate_jdk(args.java_home):
                err = (
                    "Path to java home "
                    + args.java_home
                    + " or java binary file does not exists"
                )
                raise FatalException(1, err)

            print_warning_msg(
                "JAVA_HOME " + args.java_home + " must be valid on ALL hosts"
            )
            ...
            if (
                ...
                print_warning_msg(
                    "JAVA_HOME "
                    + args.stack_java_home
                    + " (Stack) must be valid on ALL hosts"
                )
                print_warning_msg(jcePolicyWarn)
        ...
        if java_home_var:
            ...
            if ambariOnly:
                message = (
                    "Do you want to change Oracle JDK for Ambari Server [y/n] (n)? "
                )
        ...
        if os.path.exists(dest_file):
            ...
            print(
                "NOTE: If you have already downloaded the file, please verify if the name is exactly same as {0}.".format(
                    jdk_cfg.dest_file
                )
            )
            print("Exiting...")
    ...
    def unpack_jce_policy(jdk_path, resources_dir, jce_packed_file):
        ...
        jce_zip_path = os.path.abspath(
            os.path.join(str(resources_dir), jce_packed_file)
        )
    ...
    def _download_jce_policy(
        ...
        if not os.path.exists(dest_file):
            if properties[JCE_DOWNLOAD_SUPPORTED_PROPERTY].upper() == "FALSE":
                ...
                sys.exit(1)
            print(
                "Downloading JCE Policy archive from " + jcpol_url + " to " + dest_file
            )
            try:
    ...
    def _ensure_java_home_env_var_is_set(self, java_home_dir):
        if JAVA_HOME not in os.environ or os.environ[JAVA_HOME] != java_home_dir:
            ...
            retcode, out, err = run_os_command(
                "SETX {0} {1} /M".format(JAVA_HOME, java_home_dir_unesc)
            )
            if retcode != 0:
...
class JDKSetupLinux(JDKSetup):
    ...
    def adjust_jce_permissions(self, jdk_path):
        ...
        cmds.append(
            self.SET_JCE_FILE_MODE.format(
                jdk_path, configDefaults.JDK_SECURITY_DIR, "*"
            )
        )
        cmds.append(
            self.SET_JCE_JAR_MODE.format(
                jdk_path, configDefaults.JDK_SECURITY_DIR, "*.jar"
            )
        )
        ...
        if process.returncode != 0:
            print_warning_msg(
                "Failed to change jce permissions. {0}\n{1}".format(
...
def download_and_install_jdk(options):
    ...
    ambari_java_version_valid = check_ambari_java_version_is_valid(
        get_JAVA_HOME(), jdkSetup.JAVA_BIN, 8, properties
    )
...
def setup_jdbc(args):
    ...
    if args.jdbc_db not in JDBC_DB_OPTION_VALUES:
        err = "Unsupported database name {0}. Please see help for more information.".format(
            args.jdbc_db
        )
        raise FatalException(1, err)
...
def _cache_jdbc_driver(args):
    ...
    if custom_db_jdbc_property_value:
        ...
        if os.path.lexists(symlink_name):
            properties.process_pair(
                "previous." + custom_db_jdbc_property_name,
...
def prompt_db_properties(options):
    ...
    if not factory.force_dbms_setup():
        ...
        if options.must_set_database_options:
            ok = get_YN_input(
                "Enter advanced database configuration [y/n] (n)? ", False
            )
    else:
        ok = True
...
def expand_jce_zip_file(jce_zip_path, jdk_security_path):
    ...
    if unziped_jce_path:
        ...
        dir_to_delete = os.path.join(
            jdk_security_path, unziped_jce_path.split(os.sep)[0]
        )
...
def setup(options):
    ...
    if json_url:
        print(
            "Ambari repo file contains latest json url {0}, updating stacks repoinfos with it...".format(
                json_url
            )
        )
        properties = get_ambari_properties()
...
def check_ambari_java_version_is_valid(java_home, java_bin, min_version, properties):
    ...
    try:
        ...
        if process.returncode != 0:
            err = (
                "Checking JDK version command returned with exit code %s"
                % process.returncode
            )
            ...
            if actual_jdk_version < min_version:
                print(
                    "Minimum JDK version is {0} for Ambari. Setup JDK again only for Ambari Server.".format(
                        min_version
                    )
                )
                ...
                print(
                    "Minimum JDK version is {0} for Ambari. Skipping to setup different JDK for Ambari Server.".format(
                        min_version
                    )
                )
