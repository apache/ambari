    ...
    if not truststore_path:
        ...
        while not truststore_path:
            truststore_path = get_validated_string_input(
                "Path to TrustStore file {0}:".format(
                    get_prompt_default(SSL_TRUSTSTORE_PATH_DEFAULT)
                ),
    ...
    if truststore_password:
        ...
        if truststore_password:
            ...
            properties.process_pair(
                SSL_TRUSTSTORE_PASSWORD_PROPERTY, encrypted_password
            )
...
def import_cert_and_key(security_server_keys_dir, options):
    ...
    if retcode == 0:
        keystoreFilePath = os.path.join(
            security_server_keys_dir, SSL_KEYSTORE_FILE_NAME
        )
        keystoreFilePathTmp = os.path.join(
            tempfile.gettempdir(), SSL_KEYSTORE_FILE_NAME
        )
        passFilePath = os.path.join(
            security_server_keys_dir, SSL_KEY_PASSWORD_FILE_NAME
        )
        passFilePathTmp = os.path.join(
            tempfile.gettempdir(), SSL_KEY_PASSWORD_FILE_NAME
        )
    ...
    if retcode == 0:
        ...
        retcode, out, err = run_os_command(
            VALIDATE_KEYSTORE_CMD.format(
                keystoreFilePath, passwordFilePath, passinFilePath
            )
        )
...
def setup_https(options):
    if not is_root():
        warn = (
            "ambari-server setup-https is run as "
            ...
            if import_cert:
                import_cert_confirm = (
                    ...
                    if options.import_cert_path is not None
                        ...
                        "Do you want to import a certificate [y/n] (y)? ", True
                    )
                )
                if import_cert_confirm:
                    aliasOption = (
                        options.import_cert_alias
                        if options.import_cert_alias is not None
                        and options.import_cert_alias
                        else None
                    )
                    alias = (
                        aliasOption
