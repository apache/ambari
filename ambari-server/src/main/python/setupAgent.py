...
def run_setup(argv=None):
  ...
  if (
    projectVersion == "null"
    or projectVersion == "{ambariVersion}"
    or projectVersion == ""
  ):
  ...
  if (
    ...
    if not isAgentPackageAlreadyInstalled(availableProjectVersion):
      ...
      if os.path.exists(ambari_repo_file):
        ...
        return {
          "exitstatus": 2,
          "log": f"Ambari repo file not found: {ambari_repo_file}",
        }
        pass
    ...
    if (
      ...
      return {
        "exitstatus": 1,
        "log": "Desired version (" + projectVersion + ") of ambari-agent package"
        " is not available."
      ...
      logmessage = (
        "Desired version ("
        + projectVersion
        + ") of ambari-agent package is not available."
      )
      ambari_repo_file = get_ambari_repo_file_full_name()
      if not os.path.exists(ambari_repo_file):
        logmessage = (
          logmessage + " " + f"Ambari repo file not found: {ambari_repo_file}"
        )
