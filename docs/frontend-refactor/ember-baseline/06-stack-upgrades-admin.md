...
| VER-LIST-007 | Handles OUT_OF_SYNC hosts by confirming the affected host list, obtaining the KDC session when needed, and bulk reinstalling or removing only components whose server state is `INSTALL_FAILED` | Classic performs no client-side component-cardinality or minimum-instance validation in this flow; both mutations are constrained by `HostRoles/host_name.in(...)&HostRoles/state=INSTALL_FAILED` | `common.host_components.update`, `host.host_component.delete_components` | `app/controllers/main/admin/stack_and_upgrade_controller.js#removeOutOfSyncComponents`, `#reinstallOutOfSyncComponents`; controller tests |
...
| UPG-START-003 | Sets slave component failure and service check failure tolerance | The shared Upgrade Options template displays both controls for the upgrade flow generally; Classic does not gate either control by the selected upgrade method and warns about the risk of skipping | Included in the `admin.upgrade.start` payload | `app/templates/main/admin/stack_upgrade/upgrade_options.hbs`; upgrade options view |
