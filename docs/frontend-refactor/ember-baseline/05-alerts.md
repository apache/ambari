...
| ALERT-DEF-006 | Contains an unreachable controller method for deleting a definition; this is `BROKEN/DEAD_CODE`, not a user-facing feature | `deleteAlertDefinition` and its AJAX definition exist, but no inspected Classic template, view, route, menu, or action invokes the method. React must not add deletion UI solely to match this dead code | `alerts.delete_alert_definition` is defined but has no reachable UI call site | `app/controllers/main/alerts/definition_details_controller.js#deleteAlertDefinition`; inspected details template/view and Alerts action menu |
...
| --- | --- | --- | --- |
| ALERT-CREATE-001 | Step 1 Choose Alert Type | Port and Web enter functional-looking configuration paths. Metric parameters are `OUT_OF_SCOPE`. Raw has no renderer case and is `BROKEN/PLACEHOLDER`; Script and Aggregate enter Step 2 but their inspected data paths are broken as described in `ALERT-CREATE-002` | None |
| ALERT-CREATE-002 | Step 2 Define Alert and Thresholds | Port and Web render the common name/service/component/scope fields and their type-specific source fields. Script dereferences `content.parameters` even though the wizard content only contains `selectedType`. Aggregate renders no common definition fields and Step 2 continuation dereferences the absent definition name. Script and Aggregate are therefore `BROKEN` in inspected Classic source. A React repair may be documented as `IMPROVED_BEYOND_CLASSIC`, but not strict parity | Stack/definitions load |
...
| ID | Function and behavior | Preconditions/boundaries | Backend requests | Primary evidence |
| --- | --- | --- | --- | --- |
| ALERT-GROUP-001 | Lists groups, definitions, and notification targets by service | The Manage Groups action is visible; the inspected Classic group contract has no description field | `alerts.load_alert_groups`, `alerts.notifications` | manage alert groups controller/template |
...
| ALERT-GROUP-003 | Renames or copies a group | Definitions in the default group cannot be modified as for an ordinary group; the inspected Classic group contract has no description field | `alert_groups.update` or create for a copy | manage alert groups controller |
