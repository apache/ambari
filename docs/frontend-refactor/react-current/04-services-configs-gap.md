...
| Add Service | Cancel invalidates pending persistence, awaits cleanup in every wizard step, releases ownership, and reloads the stored return route or the Services index without losing the active `/latest` deployment prefix; the Services index converges to the first installed service Summary like Ember. Completion clears persisted state without changing cluster provisioning state. | Focused cancellation, route, and hash-reload tests plus live Step 1 cancellation pass; second-user and partial deploy remain live-only. |
