...
| 11 | 静态、focused matrix、容器、RPM 产物和 deploy 验证入口 | 已完成且有代码证据 | 四套 runner、Ruff/metadata/artifact audit、源码 provenance、RPM/SBOM 门禁和 deploy enrollment/runtime import checks 均已通过；最终 `ambari-26643-final-v4` RPM 构建完成，Agent/Server Python runner 分别 629/629、1391/1391，deploy 全量 438/438，最终三节点部署 13 个 phase 全部 `completed` 且状态 `applied`。本次交付只构建和验证 Linux x86_64 RPM，不构建 DEB 或容器镜像 |
...
- 最终完整构建和 RPM/SBOM 审计：已完成。`deploy/.ambari-build/rocky8-x86_64/build-state/runs/ambari-26643-final-v4/manifest.json` 显示 Agent RPM SHA-256 `83a343c86b3a7ba50af00389f242bec28ceb3eea7479be61a9a174b6d267a0a6`、Server RPM SHA-256 `fa6a8fb57e9b43bb6105bb6835c791f9c70c418fb33434f3e1d0061b59341248`；构建输入由 `downloads/ambari-source.json` 绑定到提交 `75586efb11887792eaca4686d932933e982557ba`，归档 SHA-256 为 `e7270fc424e409899d3371382e599bff10546efb1c8f4e05e863fb72886c54a`。本次不构建 DEB 或容器镜像。
- 最终部署：已完成。`minimal8g-admin-v3` 的 deployment state 为 `applied`，13 个 phase 全部成功；HDFS、YARN、HBase、Hive、MapReduce2、Tez、ZooKeeper 服务状态与期望状态一致，三台节点使用 Python 3.9.25，Java 17 基础服务与 Hive Java 8 兼容运行均已现场核验。
