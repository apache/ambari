...
class Multiplier:
    ...
    def parse_configs(self):
        ...
        if not os.path.exists(self.CONFIG_FILE):
            print(
                "Did not find Agent Multiplier config file: %s" % str(self.CONFIG_FILE)
            )
            sys.exit(-1)
    ...
    def validate(self):
        ...
        if not os.path.isfile(self.source_config_file):
            errors.append(
                "Ambari Agent config file does not exist at %s"
    ...
    def change_config(self, config_file, config_dict):
        ...
        if len(configs_found) < len(config_dict.keys()):
            ...
            print(
                "ERROR: Did not find all required configs. Missing: %s"
        ...
        for line in lines:
            if line.startswith("127.0.0.1"):
                new_change = (
                    ...
                    + " ".join([x.host_name for x in self.hosts])
                    + "\n"
                )
                if line != new_change:
                    line = new_change
    ...
    def aggregate_status(self):
        ...
        for host in self.hosts:
            pid_file = os.path.join(
                self.home_dir, host.host_name, self.pid_file.lstrip("/")
            )
