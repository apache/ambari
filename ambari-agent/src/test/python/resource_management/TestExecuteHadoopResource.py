...
class TestExecuteHadoopResource(TestCase):
    ...
    def test_run_default_args(self, execute_mock):
        ...
        with Environment() as env:
            ...
            self.assertEqual(
                execute_mock.call_args[0][0].command, "hadoop --config conf_dir command"
            )
            self.assertEqual(
                execute_mock.call_args[0][0].arguments,
                {
                    "logoutput": True,
                    "tries": 1,
                    "user": "user",
    ...
    def test_run_command_list(self, execute_mock):
        ...
        with Environment("/") as env:
            ...
            self.assertEqual(
                execute_mock.call_args_list[0][0][0].command,
                "hadoop --config conf_dir command1",
            )
            self.assertEqual(
                execute_mock.call_args_list[1][0][0].command,
                "hadoop --config conf_dir command2",
            )
            self.assertEqual(
                execute_mock.call_args_list[0][0][0].arguments,
                {
                    "logoutput": None,
                    "tries": 1,
                    "user": "user",
            ...
            self.assertEqual(
                execute_mock.call_args_list[1][0][0].arguments,
                {
                    "logoutput": None,
                    "tries": 1,
                    "user": "user",
