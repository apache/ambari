...
class DummyTemplate(object):
    def __init__(self, name, extra_imports=[], **kwargs):
        self._template = InlineTemplate(
            DummyTemplate._inline_text, extra_imports, **kwargs
        )
...
class TestRepositoryResource(TestCase):
    ...
    def test_create_repo_redhat(
        ...
        with Environment("/") as env:
            with patch.object(
                ...
                self.assertEqual(
                    file_mock.call_args[0][0], "/etc/yum.repos.d/Repository.repo"
                )
    ...
    @patch(
        "ambari_commons.os_utils.current_user",
    ...
    def test_create_repo_ubuntu_repo_exists(
        ...
        self.assertEqual(
            template_content, b"deb http://download.base_url.org/rpm/ a b c"
        )
