...
class TestHostInfo:  # (TestCase):
    ...
    def test_checkFolders(self, path_mock):
        ...
        results = []
        existingUsers = [
            {"name": "a1", "homeDir": os.path.join("home", "a1")},
            {"name": "b1", "homeDir": os.path.join("home", "b1")},
        ]
        hostInfo.checkFolders(
            [os.path.join("etc", "conf"), os.path.join("var", "lib"), "home"],
            ["a1", "b1"],
            ["c", "d"],
            existingUsers,
            results,
    ...
    @patch.object(
        OSCheck,
        "os_distribution",
        new=MagicMock(return_value=("redhat", "11", "Final")),
    )
    ...
    def test_checkReverseLookup(
        self, gethostname_mock, gethostbyname_mock, getfqdn_mock
    ):
    ...
    @patch("os.path.isfile")
    @patch("builtins.open")
    def test_transparent_huge_page(
        self, open_mock, os_path_isfile_mock, get_os_family_mock
    ):
    ...
    @patch("os.path.isfile")
    @patch("builtins.open")
    def test_transparent_huge_page_debian(
        self, open_mock, os_path_isfile_mock, get_os_family_mock
    ):
        context_manager_mock = MagicMock()
        open_mock.return_value = context_manager_mock
