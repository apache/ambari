...
class TestPythonExecutor(TestCase):
    ...
    def test_watchdog_1(self, kill_process_with_children_mock):
        ...
        kill_process_with_children_mock.side_effect = (
            lambda pid: subproc_mock.terminate()
        )
    ...
    def test_back_up_log_file_if_exists(self, rename_mock, isfile_mock):
        ...
        self.assertEqual(isfile_mock.called, True)
        self.assertEqual(
            rename_mock.call_args_list[0][0][0],
            "/var/lib/ambari-agent/data/output-13.txt",
        )
        self.assertEqual(
            rename_mock.call_args_list[0][0][1],
