...
class TestHostCleanup(TestCase):
    ...
    def test_read_host_check_file_with_content(self, os_listdir_mock):
        ...
        self.assertTrue(
            "/etc/oozie/conf.dist" in propMap["alternatives"]["target_list"]
        )
    ...
    @patch.object(HostCleanup.HostCleanup, "get_files_in_dir")
    @patch.object(OSCheck, "get_os_type")
    def test_find_repo_files_for_repos(
        self, get_os_type_method, get_files_in_dir_method
    ):
        out = io.StringIO()
        sys.stdout = out
