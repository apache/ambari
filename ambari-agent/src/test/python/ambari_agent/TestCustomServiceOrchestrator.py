...
class TestCustomServiceOrchestrator:  # (TestCase):
    ...
    def test_dump_command_to_json_with_retry(
        self, FileCache_mock, unlink_mock, isfile_mock, hostname_mock
    ):
    ...
    def test_cancel_command(
        ...
        self.assertFalse(
            command["taskId"] in list(orchestrator.commands_in_progress.keys())
        )
    ...
    def test_resolve_hook_script_path(self, FileCache_mock, isfile_mock):
        ...
        orchestrator = CustomServiceOrchestrator(initializer_module)
        # Testing None param
        res1 = orchestrator.resolve_hook_script_path(
            None, "prefix", "command", "script_type"
        )
        self.assertEqual(res1, None)
        # Testing existing hook script
        isfile_mock.return_value = True
        res2 = orchestrator.resolve_hook_script_path(
            "hooks_dir", "prefix", "command", "script_type"
        )
        self.assertEqual(
            res2,
            (
        ...
        res3 = orchestrator.resolve_hook_script_path(
            "hooks_dir", "prefix", "command", "script_type"
        )
