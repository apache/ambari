...
class ActionQueue(threading.Thread):
    ...
    def cancel(self, commands):
        for command in commands:
            logger.info(
                "Canceling command with taskId = {tid}".format(
    ...
    def run(self):
        while not self.stop_event.is_set():
            try:
                    ...
                    if self.parallel_execution == 0:
                        ...
                        while not self.stop_event.is_set():
                            ...
                            if (
                                ...
                                retry_able = (
                                    command["commandParams"]["command_retry_enabled"]
                                    == "true"
                                )
                            if retry_able:
                                logger.info(
                                    "Kicking off a thread for the command, id={} taskId={}".format(
                                        command["commandId"], command["taskId"]
                                    )
                                )
                                t = threading.Thread(
                ...
                logger.exception(
                    "ActionQueue thread failed with exception. Re-running it"
                )
    ...
    def process_command(self, command):
        ...
        try:
            ...
            logger.exception(
                "Exception while processing {0} command".format(command_type)
            )

    def tasks_in_progress_or_pending(self):
        return (
            not self.commandQueue.empty() or self.recovery_manager.has_active_command()
        )
    ...
    def execute_command(self, command):
        ...
        while retry_duration >= 0:
            ...
            if status != CommandStatus.completed and retry_able and retry_duration > 0:
                ...
                logger.info(
                    "Retrying command with taskId = {cid} after a wait of {delay}".format(
        ...
        logger.info(
            "Command with taskId = {cid} completed successfully!".format(cid=taskId)
            if status == CommandStatus.completed
            else "Command with taskId = {cid} failed after {attempts} tries".format(
                cid=taskId, attempts=num_attempts
            )
        )
        ...
        if "structuredOut" in command_result:
            role_result["structuredOut"] = str(
                json.dumps(command_result["structuredOut"])
            )
        else:
    ...
    def on_background_command_complete_callback(self, process_condensed_result, handle):
        ...
        status = (
            CommandStatus.completed if handle.exitCode == 0 else CommandStatus.failed
        )
        ...
        role_result.update(
            {
                ...
                "structuredOut": str(
                    json.dumps(process_condensed_result["structuredOut"])
                )
