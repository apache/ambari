...
class CustomServiceOrchestrator(object):
    ...
    def cancel_command(self, task_id, reason):
        with self.commands_in_progress_lock:
            if task_id in self.commands_in_progress.keys():
                ...
                logger.warn(
                    "Unable to find process associated with taskId = %s" % task_id
                )
            ...
            for config_type, password_properties in commandJson["serviceLevelParams"][
                ...
                if config_type in commandJson["configurations"]:
                    ...
                    for key_name, value_name in password_properties.items():
                        if key_name == value_name:
                            ...
                            if len(keyname_keyconfig) > 1:
                                if (
                                    keyname_keyconfig[1]
                                    not in commandJson["configurations"]
                                ):
                                    continue
                    ...
                    if len(credentials) > 0:
                        ...
                        logger.info(
                            "Identifying config {0} for CS: ".format(config_type)
                        )
    ...
    def generateJceks(self, commandJson):
        ...
        logger.info(
            "Generating the JCEKS file: roleCommand={0} and taskId = {1}".format(
                roleCommand, task_id
            )
        )
        ...
        for config_type, credentials in list(configtype_credentials.items()):
            ...
            if "role" in commandJson and commandJson["role"]:
                ...
                file_path = os.path.join(
                    self.getProviderDirectory(roleName), "{0}.jceks".format(config_type)
                )
            else:
                file_path = os.path.join(
    ...
    def runCommand(
        ...
        try:
            ...
            try:
                log_out_files = (
                    self.config.get("logging", "log_out_files", default=None)
                    is not None
                )
            ...
            if cluster_id != "-1" and cluster_id != "null" and not is_status_command:
                self.commands_for_component_in_progress[cluster_id][
                    command["role"]
                ] += 1
                ...
                if "serviceName" in command:
                    service_component_name = (
                        command["serviceName"] + "/" + command["role"]
                    )
            ...
            for py_file, current_base_dir in filtered_py_file_list:
                log_info_on_failure = (
                    command_name not in self.DONT_DEBUG_FAILURES_FOR_COMMANDS
                )
                script_params = [
                    command_name,
                    json_path,
            ...
            message = "Caught an exception while executing custom service command: {0}: {1}; {2}".format(
                exc_type, exc_obj, e
            )
    ...
    def generate_command(self, command_header):
        ...
        command = Utils.update_nested(
            Utils.get_mutable_copy(command_dict), command_header
        )
    ...
    def dump_command_to_json(self, command, retry=False, is_status_command=False):
        ...
        if is_status_command:
            # make sure status commands that run in parallel don't use the same files
            file_path = os.path.join(
                self.tmp_dir, "status_command_{0}.json".format(uuid.uuid4())
            )
        else:
            task_id = command["taskId"]
            ...
            if command_type == AgentCommand.auto_execution:
                file_path = os.path.join(
                    self.tmp_dir, "auto_command-{0}.json".format(task_id)
                )
        ...
        if os.path.exists(command_json_path):
            ...
            if remove_command_file:
                try:
                    ...
                    logger.error(
                        "Failed to remove %s due to error: %s",
                        command_json_path,
                        str(e),
                    )
