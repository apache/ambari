...
agent_piddir = (
    os.environ["AMBARI_PID_DIR"]
    if "AMBARI_PID_DIR" in os.environ
    else "/var/run/ambari-agent"
)
...
def update_log_level(config):
    ...
    log_cfg_file = os.path.join(
        os.path.dirname(AmbariConfig.AmbariConfig.getConfigFile(home_dir)),
        "logging.conf",
    )
...
def update_open_files_ulimit(config):
    ...
    if open_files_ulimit >= soft_limit:
        try:
            ...
            logger.error(
                "Unable to set open files ulimit to {0}: {1}".format(
                    open_files_ulimit, str(err)
                )
            )
...
def perform_prestart_checks(expected_hostname):
    ...
    # check if ambari prefix exists
        ...
        msg = "Ambari prefix dir %s does not exists, can't continue" % config.get(
            "agent", "prefix"
        )
        logger.error(msg)
        print(msg)
...
def init_loggers(options):
    ...
    setup_logging(
        alerts_logger, AmbariConfig.AmbariConfig.getAlertsLogFile(), logging_level
    )
    setup_logging(
        alerts_logger_2, AmbariConfig.AmbariConfig.getAlertsLogFile(), logging_level
    )
    setup_logging(
        alerts_logger_global,
...
def main(options, initializer_module, heartbeat_stop_callback=None):
    ...
    try:
        ...
        logger.warning(
            "Cannot set locale for ambari-agent. Please check your systemwide locale settings. Failed due to: {0}.".format(
                str(ex)
            )
        )
    ...
    while not connected and not stopped and not initializer_module.stop_event.is_set():
        for server_hostname in server_hostnames:
            ...
            try:
                ...
                logger.info(
                    "Connecting to Ambari server at %s (%s)", server_url, server_ip
                )
            except socket.error:
                logger.warn(
