...
def update_log_level(config):
  ...
  log_cfg_file = os.path.join(
    os.path.dirname(AmbariConfig.AmbariConfig.getConfigFile(home_dir)), "logging.conf"
  )
...
def init_loggers(options):
  ...
  setup_logging(
    alerts_logger_global, AmbariConfig.AmbariConfig.getAlertsLogFile(), logging_level
  )
  setup_logging(
...
def main(options, initializer_module, heartbeat_stop_callback=None):
  ...
  try:
    ...
    logger.warning(
      f"Cannot set locale for ambari-agent. Please check your systemwide locale settings. Failed due to: {str(ex)}."
    )
