...
class HeartbeatThread(threading.Thread):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                response = self.blocking_request(
                    heartbeat_body, Constants.HEARTBEAT_ENDPOINT
                )
                ...
                if isinstance(ex, (ConnectionIsAlreadyClosed)):
                    ...
                    logger.info(
                        'Connection error "{0}". Re-running the registration'.format(
                            str(ex)
                        )
                    )
                else:
                    logger.exception(
                        "Exception in HeartbeatThread. Re-running the registration"
                    )
        ...
        for endpoint, cache, listener, subscribe_to in self.post_registration_requests:
            try:
                ...
                try:
                    ...
                    logger.exception(
                        "Exception while handing response to request at {0} {1}".format(
                            endpoint, response
                        )
                    )
                    raise
    ...
    def handle_registration_response(self, response):
        ...
        if exitstatus != 0:
            ...
            if "log" in response.keys():
                error_message = "Registration failed due to: {0}".format(
                    response["log"]
                )
            else:
    ...
    def blocking_request(
        ...
        def presend_hook(correlation_id):
            if log_handler:
                self.server_responses_listener.logging_handlers[correlation_id] = (
                    log_handler
                )
        ...
        try:
            ...
            logger.warn(
                "Connection failed while trying to connect to {0}".format(destination)
            )
            raise
        ...
        try:
            ...
            raise Exception(
                "{0} seconds timeout expired waiting for response from server at {1} to message from {2}".format(
                    timeout, Constants.SERVER_RESPONSES_TOPIC, destination
                )
            )
