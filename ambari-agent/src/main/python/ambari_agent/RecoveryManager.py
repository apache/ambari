...
from ambari_agent.models.commands import (
    CommandStatus,
    RoleCommand,
    CustomCommand,
    AgentCommand,
    ...
    * Generate START command
    ...
    def __init__(
        ...
        if len(self.initializer_module.configurations_cache):
            self.cluster_id = list(self.initializer_module.configurations_cache.keys())[
                0
            ]
        ...
        if len(self.initializer_module.host_level_params_cache):
            self.cluster_id = list(
                self.initializer_module.host_level_params_cache.keys()
            )[0]
    ...
    def requires_recovery(self, component):
        ...
        if self.auto_start_only or self.auto_install_start:
            ...
            if (
                status["current"] == status["desired"]
                and status["stale_config"] is False
            ):
                return False
            ...
            try:
                for component in self.actions.keys():
                    ...
                    recovery_state = {
                        ...
                        "limitReached": self.max_lifetime_count
                        <= action["lifetimeCount"],
                    }
            ...
            if (
                ...
                if self.auto_start_only:
                    if (
                        ...
                        and status["current"] == self.INSTALLED
                    ):
                    ...
                    if status["desired"] != status["current"]:
                        if (
                            ...
                            status["desired"] == self.INSTALLED
                            and status["current"] == self.STARTED
                        ):
        ...
        if action_counter["lifetimeCount"] < self.max_lifetime_count:
            if action_counter["count"] < self.max_count:
                if seconds_since_last_attempt > self.retry_gap_in_sec:
                    ...
                    logger.info(
                        "Not running recovery command due to retry_gap = {0} (seconds)".format(
                            self.retry_gap_in_sec
                        )
                    )
            else:
    ...
    def update_recovery_config(self, dictionary):
        if dictionary and "recoveryConfig" in dictionary:
            if logger.isEnabledFor(logging.INFO):
                logger.info(
                    "RecoverConfig = %s", pprint.pformat(dictionary["recoveryConfig"])
                )
        ...
        if (
            ...
            if "recovery_retry_interval" in config:
                retry_gap = self._read_int_(
                    config, "recovery_retry_interval", retry_gap
                )
    ...
    def update_config(
        ...
        if window_in_min <= 0:
            logger.warn(
                "Recovery disabled: window_in_min must be a non-negative number"
            )
            return
        ...
        if retry_gap >= window_in_min:
            logger.warn(
                "Recovery disabled: retry_gap must be smaller than window_in_min"
            )
            return
