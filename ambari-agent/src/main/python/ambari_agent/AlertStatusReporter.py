...
class AlertStatusReporter(threading.Thread):
        ...
        while not self.stop_event.is_set():
            try:
                if self.initializer_module.is_registered:
                    ...
                    alerts_to_send = (
                        self.get_changed_alerts(alerts)
                        if self.send_alert_changes_only
                        else alerts
                    )
    ...
    def save_results(self, alerts):
        ...
        for alert in alerts:
            ...
            self.reported_alerts[cluster_id][alert_name] = [
                alert[field] for field in self.FIELDS_CHANGED_RESEND_ALERT
            ]
    ...
    def get_changed_alerts(self, alerts):
        ...
        for alert in alerts:
            ...
            alert_definitions = [
                definition
                for definition in self.alert_definitions_cache[cluster_id][
                    "alertDefinitions"
                ]
                if definition["name"] == alert_name
            ]
            if alert_definitions:
                alert_definition = alert_definitions[0]
                definition_tolerance_enabled = alert_definition[
                    "repeat_tolerance_enabled"
                ]
                if definition_tolerance_enabled:
                    alert_tolerance = int(alert_definition["repeat_tolerance"])
                else:
                    alert_tolerance = int(
                        self.initializer_module.configurations_cache[cluster_id][
                            "configurations"
                ...
                if [
                    ...
                    self.alert_repeats[cluster_id][alert_name] < alert_tolerance
                    and alert_state != "OK"
                ):
                    changed_alerts.append(alert)
            else:
