...
class AmsAlert(MetricAlert):
    ...
    def _collect(self):
        ...
        if isinstance(self.metric_info, AmsMetric):
            ...
            if not raw_data_points and http_code not in [200, 307]:
                ...
                raise Exception(
                    "[Alert][{0}] Unable to extract JSON from HTTP response".format(
                        self.get_name()
                    )
                )
            else:
                ...
                if logger.isEnabledFor(logging.DEBUG):
                    logger.debug(
                        "[Alert][{0}] Computed result = {1}".format(
    ...
    def _load_metric(self, ssl, host, port, ams_metric):
        ...
        try:
            ...
            if logger.isEnabledFor(logging.DEBUG):
                logger.exception(
                    "[Alert][{0}] Unable to retrieve metrics from AMS: {1}".format(
        ...
        """.format(encoded_get_metrics_parameters, data)
            ...
            if conn is not None:
                try:
                    ...
                    logger.debug(
                        "[Alert][{0}] Unable to close URL connection to {1}".format(
                            self.get_name(), url
                        )
                    )
        json_is_valid = True
        ...
        if self.custom_value_module is not None:
            ...
            if self.minimum_value:
                data_points = [
                    ...
                    if data_point > self.minimum_value
                ]
        return data_points
