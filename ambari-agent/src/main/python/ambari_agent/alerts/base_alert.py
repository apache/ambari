...
class BaseAlert(object):
    ...
    def set_helpers(
        self, collector, cluster_configuration_cache, configuration_builder
    ):
        """
    ...
    def collect(self):
        ...
        try:
            ...
            if (
                ...
                res_base_text = self.alert_source_meta["reporting"][reporting_state][
                    "text"
                ]
        ...
        try:
            ...
            try:
                ...
                logger.warn(
                    "[Alert][{0}] - {1}".format(self.get_name(), str(value_error))
                )
            ...
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(
                    "[Alert][{0}] text = {1}".format(self.get_name(), data["text"])
                )
        except Exception as exception:
            logger.exception(
                "[Alert][{0}] - The alert's data is not properly formatted".format(
                    self.get_name()
                )
            )
            ...
            data["text"] = "There is a problem with the alert definition: {0}".format(
                str(exception)
            )
        finally:
    ...
    def _get_uri_from_structure(self, alert_uri_lookup_keys):
        ...
        if (
            ...
            alert_uri = self._get_uri_from_ha_structure(
                alert_uri_lookup_keys, configurations
            )
    ...
    def _get_uri_from_ha_structure(self, alert_uri_lookup_keys, configurations):
        ...
        logger.debug(
            "[Alert][{0}] HA URI structure detected in definition, attempting to lookup dynamic HA properties".format(
                self.get_name()
            )
        )
        ...
        is_ssl_enabled = self._check_uri_ssl_property(
            alert_uri_lookup_keys, configurations
        )
        if is_ssl_enabled:
