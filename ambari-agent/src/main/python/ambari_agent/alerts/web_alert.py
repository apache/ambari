...
class WebAlert(BaseAlert):
    ...
    def _collect(self):
        ...
        if status_code == 0:
            return (
                self.RESULT_CRITICAL,
                [status_code, url, time_seconds, error_message],
            )
    ...
    def _make_web_request(self, url):
        ...
        try:
            ...
            if logger.isEnabledFor(logging.DEBUG):
                logger.exception(
                    "[Alert][{0}] Unable to make a web request.".format(self.get_name())
                )
        ...
        try:
            ...
            if response is not None:
                try:
                    ...
                    if logger.isEnabledFor(logging.DEBUG):
                        logger.exception(
                            "[Alert][{0}] Unable to close socket connection".format(
                                self.get_name()
                            )
                        )
