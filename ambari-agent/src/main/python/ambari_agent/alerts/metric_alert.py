...
class MetricAlert(BaseAlert):
    ...
    def _collect(self):
        ...
        if isinstance(self.metric_info, JmxMetric):
            ...
            if not jmx_property_values and http_code in [200, 307]:
                ...
                value_list.append(
                    "HTTP {0} response (metrics unavailable)".format(str(http_code))
                )
            elif not jmx_property_values and http_code not in [200, 307]:
                raise Exception(
                    "[Alert][{0}] Unable to extract JSON from JMX response".format(
                        self.get_name()
                    )
                )
            else:
                ...
                if logger.isEnabledFor(logging.DEBUG):
                    logger.debug(
                        "[Alert][{0}] Resolved values = {1}".format(
    ...
    def _load_jmx(self, ssl, host, port, jmx_metric):
        ...
        security_enabled = (
            str(
                self._get_configuration_value(configurations, SECURITY_ENABLED_KEY)
            ).upper()
            == "TRUE"
        )
        ...
        for jmx_property_key, jmx_property_value in jmx_metric.property_map.items():
            ...
            try:
                ...
                if logger.isEnabledFor(logging.DEBUG):
                    logger.exception(
                        "[Alert][{0}] Unable to make a web request: {1}".format(
