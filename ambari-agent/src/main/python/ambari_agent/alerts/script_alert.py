    ...
    def _collect(self):
        ...
        if cmd_module is not None:
            ...
            try:
                ...
                logger.debug(
                    "The script {0} does not have a get_tokens() function".format(
                        str(cmd_module)
                    )
                )
            ...
            if matchObj:
                ...
                with Environment(
                    ...
                    result = cmd_module.execute(
                        configurations, self.parameters, self.host_name
                    )
            else:
                result = cmd_module.execute(
                    configurations, self.parameters, self.host_name
            ...
            return (
                self.RESULT_UNKNOWN,
                ["Unable to execute script {0}".format(self.path)],
            )

    def _load_source(self):
        if (
            self.path is None
            and self.stack_path is None
            and self.host_scripts_dir is None
        ):
        ...
        if (
            not os.path.exists(self.path_to_script)
            and self.common_services_dir is not None
        ):
        ...
        if (
            not os.path.exists(self.path_to_script)
            and self.host_scripts_dir is not None
        ):
