...
class AlertSchedulerHandler:
    ...
    def reschedule(self):
        ...
        for scheduled_job in scheduled_jobs:
            ...
            if uuid_valid is False:
                jobs_removed += 1
                logger.info(
                    "[AlertScheduler] Unscheduling {0}".format(scheduled_job.name)
                )
    ...
    def __load_definitions(self):
        ...
        for cluster_id, command_json in self.alert_definitions_cache.items():
            ...
            publicHostName = (
                ""
                if not "publicHostName" in command_json
                else command_json["publicHostName"]
            )
            ...
            if clusterName != "" and clusterHash is not None:
                logger.info(
                    "[AlertScheduler] Caching cluster {0} with alert hash {1}".format(
                        clusterName, clusterHash
                    )
                )
    ...
    def __json_to_callable(
        self, clusterName, hostName, publicHostName, json_definition
    ):
        """
    ...
    def execute_alert(self, execution_commands):
        ...
        for execution_command in execution_commands:
            try:
                ...
                hostName = (
                    ""
                    if not "hostName" in execution_command
                    else execution_command["hostName"]
                )
                publicHostName = (
                    ""
