...
class Hardware:
    ...
    def osdisks(self):
        ...
        if self.config:
            ignore_mount_value = self.config.get(
                "agent", "ignore_mount_points", default=""
            )
            blacklisted_mount_points = [
                item.strip()
                for item in ignore_mount_value.split(",")
                if len(item.strip()) != 0
            ]
      ...
      """
            if (
                mount["device"] not in self.IGNORE_DEVICES
                and mount["mountpoint"].strip()[1:].split("/")[0]
                not in self.IGNORE_ROOT_MOUNTS
                and self._chk_writable_mount(mount["mountpoint"])
                and not path_isfile(mount["mountpoint"])
                and not self._is_mount_blacklisted(
                    blacklisted_mount_points, mount["mountpoint"]
                )
            ):
        ...
        if len(ignored_mounts) > 0:
            ...
            logger.info(
                "Some mount points were ignored: {0}".format(", ".join(ignore_list))
            )
    ...
    def _chk_writable_mount(self, mount_point):
        if os.geteuid() == 0:
            ...
            try:
                ...
                logger.exception(
                    "Exception happened while checking mount {0}".format(mount_point)
                )
                return False
            except Fail:
                logger.exception(
    ...
    def osdisks(self):
        mounts = []
        runner = shellRunner()
        command_result = runner.runPowershell(
            script_block=Hardware.WINDOWS_GET_DRIVES_CMD
        )
