...
def get_erase_cmd():
    if OSCheck.is_redhat_family():
        ...
        raise Exception(
            "Unsupported OS family '{0}', cannot remove package. ".format(
    ...
    def read_host_check_file(self, config_file_path):
        ...
        try:
            if config.has_option(PACKAGE_SECTION, PACKAGE_KEY):
                propertyMap[PACKAGE_SECTION] = config.get(
                    PACKAGE_SECTION, PACKAGE_KEY
                ).split(",")
        except:
        ...
        for folder in file_map:
            if isinstance(file_map[folder], list):  # here is list of file masks/files
                for mask in file_map[folder]:
                    remList += self.get_files_in_dir(
                        "%s/%s" % (cache_root, folder), mask
                    )
            elif isinstance(file_map[folder], dict):  # here described sub-folder
                remList += self.do_clear_cache(
                    "%s/%s" % (cache_root, folder), file_map[folder]
                )
                            ...
                            if alternates:
                                for entry in alternates:
                                    if entry:
                                        ...
                                        logger.debug(
                                            "Erasing alternative named: "
                                            + alt_name
                                            + ", "
                                            "path: " + alt_path
                                        )

                                        command = ALT_ERASE_CMD.format(
                                            alt_name, alt_path
                                        )
                    ...
                    if identifier in line:
                        ...
                        if len(tokens) > 1:
                            ...
                            if pid and pid not in pidList:
                                logger.info(
                                    "Adding pid: " + str(pid) + " for " + identifier
                                )
                                pidList.append(pid)
                    ...
                    with open(filePath, "r") as file:
                        ...
                        while content != "":
                            for repoName in repoNameList:
                                if (
                                    content.find(repoName) == 0
                                    and filePath not in repoFiles
                                ):
                        ...
                        if os.path.exists(path):
                            if os.path.isdir(path):
                                try:
                                    ...
                                    logger.warn(
                                        "Failed to remove dir {0} , error: {1}".format(
                            ...
                            logger.info(
                                "Deleting broken symbolic link {0}".format(path)
                            )
                            self.do_erase_files_silent([path])
                        else:
            ...
            for path in pathList:
                if path and (os.path.exists(path) or os.path.islink(path)):
                    try:
                        ...
                        logger.warn(
                            "Failed to delete file: {0}, error: {1}".format(
                                path, str(sys.exc_info()[0])
                            )
                        )
                else:
...
def main():
    ...
    if not os.path.exists(hostcheckfileca):
        if options.silent:
            print(
                "Host Check results not found. There is no {0}. Running host checks.".format(
                    hostcheckfileca
                )
            )
            h.run_check_hosts()
            ...
            run_check_hosts_input = get_YN_input(
                "Host Check results not found. There is no {0}. Do you want to run host checks [y/n] (y)".format(
                    hostcheckfileca
                ),
                True,
            )
