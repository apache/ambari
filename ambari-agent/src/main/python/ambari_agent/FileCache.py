...
class FileCache:
    ...
    def get_service_base_dir(self, command):
        ...
        return self.provide_directory(
            self.cache_dir, service_subpath, self.get_server_url_prefix(command)
        )
    ...
    def get_hook_base_dir(self, command):
        ...
        try:
            ...
            return None
        return self.provide_directory(
            self.cache_dir, hooks_path, self.get_server_url_prefix(command)
        )
    ...
    def get_custom_resources_subdir(self, command):
        ...
        return self.provide_directory(
            self.cache_dir, custom_dir, self.get_server_url_prefix(command)
        )
    ...
    def provide_directory(self, cache_path, subdirectory, server_url_prefix):
        ...
        try:
            if full_path not in self.uptodate_paths:
                logger.debug(
                    "Checking if update is available for " "directory {0}".format(
                        full_path
                    )
                )
    ...
    def build_download_url(self, server_url_prefix, directory, filename):
        ...
        """
        return "{0}/{1}/{2}".format(
            server_url_prefix, urllib.request.pathname2url(directory), filename
        )
    ...
    def fetch_url(self, url):
        ...
        try:
            ...
            logger.debug(
                "Connected with {0} with code {1}".format(u.geturl(), u.getcode())
            )
            ...
            raise CachingException(
                "Can not download file from url {0} : {1}".format(url, str(err))
            )
    ...
    def write_hash_sum(self, directory, new_hash):
        ...
        try:
            ...
            raise CachingException(
                "Can not write to file {0} : {1}".format(hash_file, str(err))
            )
    ...
    def invalidate_directory(self, directory):
        ...
        try:
          ...
          """
            ...
            raise CachingException(
                "Can not invalidate cache directory {0}: {1}", directory, str(err)
            )
    ...
    def unpack_archive(self, mem_buffer, target_directory):
        ...
        try:
            ...
            raise CachingException(
                "Can not unpack zip file to directory {0} : {1}".format(
