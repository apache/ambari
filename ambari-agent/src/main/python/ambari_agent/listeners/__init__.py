  ...
  """
    ...
    def dequeue_unprocessed_events(self):
        while not self.unprocessed_messages_queue.empty():
            ...
            if payload:
                logger.info(
                    "Processing event from unprocessed queue {0} {1}".format(
                        payload[0], payload[1]
                    )
                )
                ...
                try:
                    ...
                    logger.exception(
                        "Exception while handing event from {0} {1} {2}".format(
                            destination, headers, message
                        )
                    )
    ...
    def on_message(self, headers, message):
        ...
        if destination.rstrip("/") == self.get_handled_path().rstrip("/"):
            try:
                ...
                logger.exception(
                    "Received from server event is not a valid message json. Message is:\n{0}".format(
                        message
                    )
                )
                self.report_status_to_sender(headers, message, ex)
            ...
            if destination != Constants.ENCRYPTION_KEY_TOPIC:
                logger.info(
                    "Event from server at {0}{1}".format(
                        destination,
    ...
    def report_status_to_sender(self, headers, message, ex=None):
        ...
        try:
            connection.send(
                message=confirmation_of_received,
