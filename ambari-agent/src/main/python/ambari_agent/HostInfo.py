...
class HostInfoLinux(HostInfo):
    ...
    REDHAT7_SERVICE_STATUS_CMD = "%s status %s" % (
        REDHAT7_SERVICE_CMD,
    ...
    def etcAlternativesConf(self, projects, etcResults):
        ...
        files = [
            f for f in os.listdir("/etc/alternatives") if re.match(projectRegex, f)
        ]
        for conf in files:
            result = {}
...
class HostInfoWindows(HostInfo):
    ...
    def javaProcs(self, list):
        try:
            ...
            if code == 0:
                ...
                for i in [
                    index
                    for index in range(0, len(splitted_output))
                    if (index % 3) == 0
                ]:
    ...
    def getServiceStatus(self, serivce_name):
        ...
        code, out, err = run_powershell_script(
            self.SERVICE_STATUS_CMD.format(serivce_name)
        )
        return out, err, code
