...
class Facter(object):
    ...
    def getMacAddress(self):
        ...
        if uuid.getnode() == mac:
            mac = ":".join(
                "%02X" % ((mac >> 8 * i) & 0xFF) for i in reversed(list(range(6)))
            )
        else:
            mac = "UNKNOWN"
        return mac
    ...
    def replaceFacterInfoWithSystemResources(self, systemResources, facterInfo):
        ...
        for key in facterInfo:
            facterInfo[key] = self.getSystemResourceIfExists(
                systemResources, key, facterInfo[key]
            )
        return facterInfo
    ...
    def getSystemResourceOverrides(self):
        ...
        if self.config.has_option("agent", "system_resource_overrides"):
            systemResourceDir = self.config.get(
                "agent", "system_resource_overrides", ""
            ).strip()
            if systemResourceDir:
                if os.path.isdir(systemResourceDir) and os.path.exists(
                    systemResourceDir
                ):
                    try:
                        ...
                        log.warn(
                            "Cannot read values from json files in %s. it won't be used for gathering system resources."
                            % systemResourceDir
                        )
                else:
                    log.info(
                        "Directory: '%s' does not exist - it won't be used for gathering system resources."
                        % systemResourceDir
                    )
            else:
                log.info(
                    "'system_resource_dir' is not set - it won't be used for gathering system resources."
                )
...
class FacterLinux(Facter):
    ...
    def setDataIfConfigShortOutput():
        try:
            return_code, stdout, stderr = run_os_command(
                FacterLinux.GET_IFCONFIG_SHORT_CMD
            )
            return stdout
        except OSError:
    ...
    def getNetmask(self):
        ...
        for ifname in self.getInterfaces().split(","):
            if ifname.strip():
                ip_address_by_ifname = self.get_ip_address_by_ifname(
                    ifname.replace("'", "").encode()
                )
    ...
    def get_ip_address_by_ifname(self, ifname):
        ...
        try:
            ip_address_by_ifname = socket.inet_ntoa(
                fcntl.ioctl(
                    s.fileno(),
                    0x8915,  # SIOCGIFADDR
    ...
    # Return interfaces
    def getInterfaces(self):
        result = self.return_first_words_from_list(
            self.DATA_IFCONFIG_SHORT_OUTPUT.splitlines()[1:]
        )
        ...
        log.warn(
            "Can't get a network interfaces list from {0}".format(
    ...
    def getUptimeSeconds(self):
        try:
            return int(
                self.data_return_first(
                    FacterLinux.DIGITS_REGEXP, self.DATA_UPTIME_OUTPUT
                )
            )
        except ValueError:
            log.warn(
                "Can't get an uptime value from {0}".format(self.DATA_UPTIME_OUTPUT)
            )
            return 0
    ...
    def getMemoryFree(self):
        ...
        try:
            return int(
                self.data_return_first(
                    FacterLinux.FREEMEM_REGEXP, self.DATA_MEMINFO_OUTPUT
                )
            )
        except ValueError:
            log.warn(
                "Can't get free memory size from {0}".format(self.DATA_MEMINFO_OUTPUT)
            )
            return 0
    ...
    def getMemoryTotal(self):
        try:
            return int(
                self.data_return_first(
                    FacterLinux.TOTALMEM_REGEXP, self.DATA_MEMINFO_OUTPUT
                )
            )
        except ValueError:
            log.warn(
                "Can't get total memory size from {0}".format(self.DATA_MEMINFO_OUTPUT)
            )
            return 0
    ...
    def getSwapFree(self):
        ...
        try:
            return int(
                self.data_return_first(
                    FacterLinux.SWAPFREE_REGEXP, self.DATA_MEMINFO_OUTPUT
                )
            )
        except ValueError:
            log.warn(
                "Can't get free swap memory size from {0}".format(
    ...
    def getSwapSize(self):
        ...
        try:
            return int(
                self.data_return_first(
                    FacterLinux.SWAPTOTAL_REGEXP, self.DATA_MEMINFO_OUTPUT
                )
            )
        except ValueError:
            log.warn(
                "Can't get total swap memory size from {0}".format(
                    self.DATA_MEMINFO_OUTPUT
                )
            )
            return 0
