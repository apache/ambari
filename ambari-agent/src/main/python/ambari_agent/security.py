    ...
    def establish_connection(self, conn):
        ...
        try:
            ...
            if isinstance(ex, socket_error):
                logger.warn(
                    "Could not connect to {0}. {1}".format(self.connection_url, str(ex))
                )
        ...
        **keyword_headers,
        ...
        logger.info(
            "Event to server at {0} (correlation_id={1}): {2}".format(
                destination, correlation_id, logged_message
            )
        )
...
class CachedHTTPSConnection:
    ...
    def connect(self):
        if not self.connected:
            self.httpsconn = VerifiedHTTPSConnection(
                self.server, self.port, self.config
            )
    ...
    def request(self, req):
        ...
        try:
            ...
            logger.debug(
                "Error in sending/receving data from the server "
