        ...
        while not self.stop_event.is_set():
            try:
                ...
                for cluster_id in self.topology_cache.get_cluster_ids():
                    ...
                    current_host_id = self.topology_cache.get_current_host_id(
                        cluster_id
                    )
                    ...
                    for component_dict in cluster_components:
                        for command_name in status_commands_to_run:
                            ...
                            if self.customServiceOrchestrator.commandsRunningForComponent(
                                ...
                                self.logger.info(
                                    "Skipping status command for {0}. Since command for it is running".format(
                                        component_name
                                    )
                                )
                                continue
                ...
                self.logger.exception(
                    "Exception in ComponentStatusExecutor. Re-running it"
                )
            ...
            for cluster_report in cluster_reports:
                for discarded_report in reports_to_discard:
                    if Utils.are_dicts_equal(
                        ...
                        self.logger.info(
                            "Discarding outdated status {0} before sending".format(
                                cluster_report
                            )
                        )
                        break
    ...
    def check_component_status(
        ...
        if (
            ...
            != self.reported_component_status[cluster_id][
                "{0}/{1}".format(service_name, component_name)
            ][command_name]
        ):
            logging.info(
                "Status for {0} has changed to {1}".format(component_name, status)
            )
