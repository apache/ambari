...
class DataCleaner(threading.Thread):
    ...
    FILE_NAME_PATTERN = (
        AUTO_COMMAND_FILE_NAMES_PATTERN + "|" + COMMAND_FILE_NAMES_PATTERN
    )
    ...
    def __init__(self, config):
        ...
        if (
            self.file_max_age is None or self.file_max_age < 86400
        ):  # keep for at least 24h
            logger.warn(
        ...
        self.cleanup_max_size_MB = config.get(
            "agent", "data_cleanup_max_size_MB", 10000
        )
