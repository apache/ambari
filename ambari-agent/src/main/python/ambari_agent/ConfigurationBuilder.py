    ...
    def get_configuration(
        ...
        if cluster_id:
            if (
                ...
                raise Exception(
                    "Command requires configs with timestamp={0} but configs on agent have timestamp={1}".format(
                        configurations_timestamp, self.configurations_cache.timestamp
                    )
                )
            ...
            command_dict = {
                ...
                "clusterHostInfo": self.topology_cache.get_cluster_host_info(
                    cluster_id
                ),
                "localComponents": self.topology_cache.get_cluster_local_components(
                    cluster_id
                ),
                ...
                "agentLevelParams": {
                    "hostname": self.topology_cache.get_current_host_info(cluster_id)[
                        "hostName"
                    ]
                },
