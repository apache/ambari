...
public class DaemonWatcher extends PidFile {
  ...
  public synchronized static DaemonWatcher createInstance(String name, int port) {
    if(instance == null) {
      instance = new DaemonWatcher(name, port);
  ...
  private DaemonWatcher(String name, int port) {
    super(name, port);
  }
